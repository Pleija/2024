...
namespace Puerts
{
    ...
    public class PuertsFilter : UnityEditor.Editor
    {
        ...
        private static BindingMode Filter(MemberInfo memberInfo) =>
            FilterObsolete(memberInfo) || FilterByBlackList(memberInfo) || FilterByName(memberInfo)
                ? BindingMode.DontBinding : BindingMode.FastBinding;
        //适用于puerts_1.3.0版本以下
        //static bool Filter(MemberInfo memberInfo) => FilterObsolete(memberInfo) || FilterByBlackList(memberInfo);

        private static bool FilterObsolete(MemberInfo memberInfo)
        {
            var obsolete =
                memberInfo.GetCustomAttributes(typeof(ObsoleteAttribute), true).FirstOrDefault() as
                    ObsoleteAttribute;
            if(obsolete != null) return obsolete.IsError; //过滤Obsolete-Error属性时
            return false;
        }

        private static bool FilterByBlackList(MemberInfo memberInfo)
        {
            var declaringTypeName = memberInfo.DeclaringType.FullName.Replace("+", ".");
            ...
            if(blacklist.TryGetValue(declaringTypeName, out methodOrProp) &&
               (methodOrProp.TryGetValue(memberInfo.Name, out paramtersList) ||
                   (memberInfo is ConstructorInfo &&
                       methodOrProp.TryGetValue(memberInfo.DeclaringType.Name,
                           out paramtersList)))) {
                //如果是字段声明, 直接返回
                if(!(memberInfo is MethodInfo || memberInfo is ConstructorInfo))
                    return true;
                var paramters = memberInfo is MethodInfo ? ((MethodInfo)memberInfo).GetParameters()
                    : ((ConstructorInfo)memberInfo).GetParameters();
                var paramterNames =
                    (from pInfo in paramters select GetFriendlyName(pInfo.ParameterType)).ToArray();
            ...
            //过滤扩展方法
            if(memberInfo is MethodInfo && ((MethodInfo)memberInfo).IsStatic &&
               memberInfo.IsDefined(typeof(ExtensionAttribute))) {
                var mParamters = ((MethodInfo)memberInfo).GetParameters();
                var mParamterNames =
                    (from pInfo in mParamters select GetFriendlyName(pInfo.ParameterType))
                    .ToArray();
        ...
        private static bool IsMatch(List<string[]> paramtersList, string[] mParamters)
        {
            foreach(var paramters in paramtersList) {
                //*(通配) / 屏蔽所有此名称的方法
                // if(paramters.Length == 1 && paramters[0] == "*")
                //     return true;
                if(paramters.Length > 0 && paramters[0] == "**")
                    return true;

                //对比方法参数
                if(paramters.Length == mParamters.Length) {
                    var exclude = true;
                    for(var i = 0; i < paramters.Length && exclude; i++)
                        if(paramters[i] != mParamters[i] && paramters[i] != "*")
                            exclude = false;
                    if(exclude)
                        return true;
                }
        ...
        private static Dictionary<string, Dictionary<string, List<string[]>>> _blacklist;

        private static Dictionary<string, Dictionary<string, List<string[]>>> blacklist {
            get {
                if(_blacklist == null) {
                    ...
                    foreach(var memberInfo in BlackList) {
                        ...
                        var fullname = _memberInfo[0];
                        var methodName = _memberInfo[1];
                        Dictionary<string, List<string[]>> methodOrProp;
