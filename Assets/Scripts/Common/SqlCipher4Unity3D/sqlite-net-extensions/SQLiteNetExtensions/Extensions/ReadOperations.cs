...
namespace SqlCipher4Unity3D.sqlite_net_extensions.SQLiteNetExtensions.Extensions
{
    ...
    #region Private methods
        static void GetChildrenRecursive(this SQLiteConnection conn, object element, bool onlyCascadeChildren,
        ...
        static void GetChildRecursive(this SQLiteConnection conn, object element,
            PropertyInfo relationshipProperty, bool recursive, ObjectCache objectCache)
        {
            var relationshipAttribute = relationshipProperty.GetAttribute<RelationshipAttribute>();
            if (relationshipAttribute is OneToOneAttribute) {
                conn.GetOneToOneChildren(new List<object> { element }, relationshipProperty, recursive,
                    objectCache);
            }
            ...
            PropertyInfo relationshipProperty, bool recursive, ObjectCache objectCache)
        {
            ...
            var hasInverseForeignKey =
                currentEntityPrimaryKeyProperty != null && otherEntityForeignKeyProperty != null;
            ...
            foreach (var element in elements) {
                ...
                if (hasForeignKey) {
                    ...
                    value = recursive
                        ? (T) ReplaceWithCacheObjectIfPossible(value, otherEntityPrimaryKeyProperty,
                            objectCache, out isLoadedFromCache)
                        : value;
                }
                            ...
                            foreach (var keyElement in keyElements) {
                                ...
                                if (value != null && recursive) {
                                    SaveObjectToCache(value,
                                        otherEntityPrimaryKeyProperty.GetValue(value, null), objectCache);
        ...
        static void AddPrimaryKeyToDictionary<T>(object key, T element,
        ...
        static void GetChildrenRecursiveBatched<T>(this SQLiteConnection conn, IList<T> elements,
            ...
            PropertyInfo relationshipProperty, bool recursive, ObjectCache objectCache)
        {
            ...
            if (primaryKeyValue != null) {
                ...
                var query = string.Format("select * from [{0}] where [{1}] in ({2})",
                    entityType.GetTableName(), otherEntityPrimaryKeyProperty.GetColumnName(), keysQuery);
