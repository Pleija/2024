...
public partial class GameController : SystemBase
{
    ...
    private readonly Dictionary<Entity, GameObject>
        objects = new Dictionary<Entity, GameObject>(32);
    ...
    public void Initial()
    {
        if(Inited || !Manager.Instance) return;
        Inited = true;
        UnityEngine.Physics.autoSimulation = false;
        matPropBlock = new MaterialPropertyBlock();

        // setup physics parameters
        World.GetOrCreateSystem<FixedStepSimulationSystemGroup>().Timestep =
            (float)(sfloat.One / (sfloat)60.0f);
        Entity physicsStep = EntityManager.CreateEntity(typeof(PhysicsStep));
        PhysicsStep physicsStepParams = PhysicsStep.Default;
        physicsStepParams.SolverStabilizationHeuristicSettings =
    ...
    protected override void OnUpdate()
    {
        ...
        if(!Inited) Initial();
        Entities.ForEach(
            (ref Entity e, ref Translation t, ref Rotation r, ref PhysicsVelocity _vel) => {
    ...
    private Color RandomColor()
    {
        ...
        return Color.HSVToRGB((float)rand.SFloatInclusive(sfloat.Zero, sfloat.One),
            (float)rand.SFloatInclusive(sfloat.Zero, sfloat.One),
            (float)rand.SFloatInclusive((sfloat)0.5f, sfloat.One));
    }

    private
        Dictionary<(sfloat radius, UnityS.Physics.Material material),
            BlobAssetReference<UnityS.Physics.Collider>> ballColliders =
    ...
    {
        if(!ballColliders.TryGetValue((radius, material),
               out BlobAssetReference<UnityS.Physics.Collider> collider)) {
            collider = UnityS.Physics.SphereCollider.Create(new SphereGeometry {
                Center = float3.zero,
                Radius = radius
        ...
        renderer.material = (Manager.Instance.defaultMaterial);
    ...
    private
        Dictionary<(float3 size, UnityS.Physics.Material material),
            BlobAssetReference<UnityS.Physics.Collider>> boxColliders =
            new Dictionary<(float3 size, UnityS.Physics.Material material),
                BlobAssetReference<UnityS.Physics.Collider>>();

    public void CreateBox(float3 position, float3 size, quaternion rotation,
        UnityS.Physics.Material material, PhysicsParams physicsParams)
    {
        if(!Inited) Initial();

        if(!boxColliders.TryGetValue((size, material),
               out BlobAssetReference<UnityS.Physics.Collider> collider)) {
            collider = UnityS.Physics.BoxCollider.Create(new BoxGeometry {
                Center = float3.zero,
                Size = size,
    ...
    public Entity CreateEntity(float3 position, quaternion rotation,
        BlobAssetReference<UnityS.Physics.Collider> collider, PhysicsParams physicsParams)
    {
        ...
        BlobAssetReference<UnityS.Physics.Collider> collider, PhysicsParams physicsParams)
    {
        ...
        if(physicsParams.isDynamic) {
            UnityS.Physics.Collider*
                colliderPtr = (UnityS.Physics.Collider*)collider.GetUnsafePtr();
...
partial class PhysicsController : SystemBase
{
    ...
    private void ShootBox(sfloat mass, float3 size, float3 startingPos, float3 startingVelocity)
    {
        ...
        physicsParams.startingLinearVelocity = startingVelocity;
        physicsParams.mass = mass;
        GameController.Instance.CreateBox(startingPos, size, quaternion.identity, material,
            physicsParams);
    }
    ...
    protected override void OnUpdate()
    {
       if(Manager.Instance == null) return;
        ...
        if(frame >= 60 && frame % 10 == 0 && frame < 500) {
            ShootBox((sfloat)0.25f, (sfloat)7.0f,
                new float3((sfloat)50.0f, (sfloat)20.0f, (sfloat)(-50.0f)),
                new float3((sfloat)(-100.0f), (sfloat)1.0f, (sfloat)100.0f));
        }

        if(frame >= 100 && frame % 30 == 0 && frame < 900) {
            ShootBall((sfloat)0.25f, (sfloat)5.0f,
                new float3((sfloat)0.0f, (sfloat)200.0f, (sfloat)(0.0f)), float3.zero);
        }

        if(frame == 720) {
            ShootBall((sfloat)20.0f, (sfloat)12.0f,
                new float3((sfloat)50.0f, (sfloat)20.0f, (sfloat)(-50.0f)),
                new float3((sfloat)(-100.0f), (sfloat)1.0f, (sfloat)100.0f));
        }
        ++frame;
