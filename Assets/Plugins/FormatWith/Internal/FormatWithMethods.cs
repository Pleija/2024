...
namespace FormatWith.Internal
{
    ...
    {
        public static string FormatWith(string formatString,
            ...
            char closeBraceChar = '}')
        {
            return FormatWith(formatString,
                (key, format) =>
                    new ReplacementResult(replacements.TryGetValue(key, out var value), value),
        ...
        public static string FormatWith(string formatString, object replacementObject,
            MissingKeyBehaviour missingKeyBehaviour = MissingKeyBehaviour.ThrowException,
            object fallbackReplacementValue = null, char openBraceChar = '{',
            char closeBraceChar = '}')
        {
            ...
            return FormatWith(formatString,
                (key, format) => FromReplacementObject(key, replacementObject), missingKeyBehaviour,
            ...
            char closeBraceChar = '}')
        {
            ...
            return FormatHelpers.ProcessTokens(tokens, handler, missingKeyBehaviour,
                fallbackReplacementValue, formatString.Length * 2);
        }

        public static FormattableString FormattableWith(string formatString,
            IDictionary<string, string> replacements,
            MissingKeyBehaviour missingKeyBehaviour = MissingKeyBehaviour.ThrowException,
            string fallbackReplacementValue = null, char openBraceChar = '{',
            char closeBraceChar = '}')
        ...
        {
            return FormattableWith(formatString,
                key => FromReplacementObject(key, replacementObject), missingKeyBehaviour,
                fallbackReplacementValue, openBraceChar, closeBraceChar);
        }
            ...
            char closeBraceChar = '}')
        {
            // get the parameters from the format string
            var tokens = FormatHelpers.Tokenize(formatString, openBraceChar, closeBraceChar);
            return FormatHelpers.ProcessTokensIntoFormattableString(tokens, handler,
                missingKeyBehaviour, fallbackReplacementValue, formatString.Length * 2);
        }

        /// <summary>
        ///     Gets an <see cref="IEnumerable{String}">IEnumerable&lt;string&gt;</see> that will return all
        ///     format parameters used within the format string.
        /// </summary>
        ...
        /// <returns></returns>
        public static IEnumerable<string> GetFormatParameters(string formatString,
            char openBraceChar = '{', char closeBraceChar = '}')
        {
            return FormatHelpers.Tokenize(formatString, openBraceChar, closeBraceChar)
                .Where(t => t.TokenType == TokenType.Parameter).Select(pt => pt.Value);
        }
        ...
        private static ReplacementResult FromReplacementObject(string key, object replacementObject)
        {
            ...
            if(members.Length == 1) {
                var propertyInfo =
                    replacementObject.GetType().GetProperty(key, propertyBindingFlags);
                ...
                var currentObject = replacementObject;

                foreach(var member in members) {
                    var propertyInfo = currentObject.GetType()
                        .GetProperty(member, propertyBindingFlags);
                    if(propertyInfo == null)
                        return new ReplacementResult(false, null);
                    else
