...
namespace FormatWith.Internal
{
    ...
    internal static class FormatHelpers
    {
        ...
        /// </param>
        /// <param name="outputLengthHint">
        ///     Provides a hint to the underlying string builder to help reduce
        ///     buffer reallocations.
        /// </param>
        /// <returns>The processed result of joining the tokens with the replacement dictionary.</returns>
        public static string ProcessTokens(IEnumerable<FormatToken> tokens,
            Func<string, string, ReplacementResult> handler,
            MissingKeyBehaviour missingKeyBehaviour, object fallbackReplacementValue,
            int outputLengthHint)
        {
            ...
            foreach(var thisToken in tokens)
                if(thisToken.TokenType == TokenType.Text) {
                    ...
                    resultBuilder.Append(thisToken.SourceString, thisToken.StartIndex,
                        thisToken.Length);
                }
                    ...
                    var replacementResult = handler(tokenKey, format);
                    ...
                    if(replacementResult.Success) {
                        ...
                        if(string.IsNullOrWhiteSpace(format))
                            ...
                            resultBuilder.AppendFormat("{0:" + format + "}",
        ...
        public static FormattableString ProcessTokensIntoFormattableString(
            IEnumerable<FormatToken> tokens, Func<string, ReplacementResult> handler,
            MissingKeyBehaviour missingKeyBehaviour, object fallbackReplacementValue,
            int outputLengthHint)
        {
            ...
            foreach(var thisToken in tokens)
                if(thisToken.TokenType == TokenType.Text) {
                    ...
                    resultBuilder.AppendWithEscapedBrackets(thisToken.SourceString,
                        thisToken.StartIndex, thisToken.Length);
                }
                else if(thisToken.TokenType == TokenType.Parameter) {
                    // token is a parameter token
                    // perform parameter logic now.
                    ...
                    // append the replacement for this parameter
                    var replacementResult = handler(tokenKey);

                    string IndexAndFormat()
                    {
                        if(string.IsNullOrWhiteSpace(format)) return "{" + placeholderIndex + "}";
            ...
            // return the resultant string
            return FormattableStringFactory.Create(resultBuilder.ToString(),
                replacementParams.ToArray());
        }
        ...
        public static IEnumerable<FormatToken> Tokenize(string formatString,
            char openBraceChar = '{', char closeBraceChar = '}')
        {
                ...
                if(!insideBraces) {
                    ...
                    if(formatString[index] == openBraceChar) {
                        ...
                        if(index < formatString.Length - 1 &&
                           formatString[index + 1] == openBraceChar) {
                            // ESCAPED OPEN BRACE
                            ...
                            yield return new FormatToken(TokenType.Text, formatString,
                                currentTokenStart, index - currentTokenStart + 1);
                            // skip over braces
                            index += 2;
                            ...
                            if(currentTokenStart < index)
                                yield return new FormatToken(TokenType.Text, formatString,
                                    currentTokenStart, index - currentTokenStart);
                        ...
                        if(index < formatString.Length - 1 &&
                           formatString[index + 1] == closeBraceChar) {
                            ...
                            throw new FormatException(
                                $"Unexpected closing brace at position {index}");
                        }
                    }
                    else {
                    ...
                    if(formatString[index] == openBraceChar) {
                        ...
                        if(index < formatString.Length - 1 &&
                           formatString[index + 1] == openBraceChar)
                            ...
                            throw new FormatException(
                                $"Unexpected opening brace inside a parameter at position {index}");
                    }
                    else if(formatString[index] == closeBraceChar) {
                        // END OF PARAMETER
                        ...
                        yield return new FormatToken(TokenType.Parameter, formatString,
                            currentTokenStart, index - currentTokenStart + 1);
            ...
            if(insideBraces) {
                throw new FormatException(
                    $"The format string ended before the parameter was closed. Position {index}");
            }
            else {
                // outside braces. Add on any remaining text at the end of the format string
                if(currentTokenStart < index)
                    yield return new FormatToken(TokenType.Text, formatString, currentTokenStart,
