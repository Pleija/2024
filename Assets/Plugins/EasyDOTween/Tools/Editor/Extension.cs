...
#pragma warning disable CS0618
                value = UnityEditor.EditorGUI.ObjectField(position, info.Name, (Object)value,
                    info.ParameterType);
#pragma warning restore CS0618
            else if(info.ParameterType == typeof(bool))
                value = UnityEditor.EditorGUI.ToggleLeft(position, info.Name,
                ...
                value = UnityEditor.EditorGUI.FloatField(position, info.Name,
                    (float)Convert.ToDouble(value));
            else if(info.ParameterType == typeof(double))
                value = UnityEditor.EditorGUI.DoubleField(position, info.Name,
                    Convert.ToDouble(value));
            else if(info.ParameterType == typeof(string))
                value = UnityEditor.EditorGUI.TextField(position, info.Name,
                    value == null ? string.Empty : (string)value);
            else if(info.ParameterType == typeof(Vector3))
                value = UnityEditor.EditorGUI.Vector3Field(position, info.Name,
                    (Vector3?)value ?? Vector3.zero);
            else if(info.ParameterType == typeof(Color))
                value = UnityEditor.EditorGUI.ColorField(position, info.Name,
        ...
        public static object ReflectionGetTarget(this SerializedProperty sp)
        {
            ...
            object ret = null;
            var propertyPath = sp.propertyPath.Split('.');
