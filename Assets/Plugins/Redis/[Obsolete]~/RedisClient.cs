...
public class RedisClient : MonoBehaviour
{
    ...
    public enum CLIENT_TYPE { READWRITER = 0, PUBLISHER, SUBSCRIBER }
...
#region Pub/Sub
    ...
    public void TestAlive()
    {
        ...
        if(isConnect) {
            Set("redis:test", "ok");
            Debug.Log($"check redis: test =>{Get("redis:test")}");
            return;
        }
        Debug.Log($"redis not connected");
    }
    ...
    public void Connect()
    {
        try {
            ...
            redis.Configuration.Password = password;
            ...
            if(redis == null)
                return;
            Debug.Log(string.Format("RedisClient :: connected to [ {0} : {1} ]", host,
                port.ToString()));
            isConnect = true;
        }
        catch(Exception err) {
            Debug.LogError($"RedisClient :: {err}");
        }
    }
...
#region Pub/Sub
    public void Subscribe(params string[] channels)
    {
        if(cliantAs == CLIENT_TYPE.SUBSCRIBER) {
            ...
            Debug.LogWarning(
                "RedisClient :: Subscribe can only client type [ CLIENT_TYPE.SUBSCRIBER ]");
    }
    ...
    public void Publish(string channel, string message)
    {
        if(cliantAs == CLIENT_TYPE.PUBLISHER) {
            ...
            Debug.LogWarning(
                "RedisClient :: Publish can only client type [ CLIENT_TYPE.PUBLISHER ]");
    }
    ...
    private void messageReceived(string channel, string message)
    {
        ...
        Debug.Log(string.Format("RedisClient :: channel is [ {0} ] / message is [ {1} ]", channel,
            message));
    }
#endregion
}
