...
namespace Favourites.Editor
{
    ...
    public static class FavouritesEd
    {
        ...
        static Dictionary<Scene, FavouritesContainer> cache = null;
        static bool loaded;
        ...
        static FavouritesEd() {
            ...
            EditorApplication.playModeStateChanged += change => {
                if (change == PlayModeStateChange.EnteredEditMode) UpdateSceneCache(true);
            };
        ...
        public static void UpdateSceneCache(bool force) {
            ...
            foreach (FavouritesContainer c in objs)
                if (!cache.ContainsKey(c.gameObject.scene))
                    cache.Add(c.gameObject.scene, c);
