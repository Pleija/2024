...
namespace Favourites.Editor
{
    public class TreeViewItem<T> : TreeViewItem where T : TreeElement
    {
        ...
        public TreeViewItem(int id, int depth, string displayName, Texture2D icon, T data) : base(id, depth,
            displayName) {
            this.icon = icon;
            Data = data;
        }
    ...
    public class TreeViewWithTreeModel<T> : TreeView where T : TreeElement
    {
        ...
        public TreeViewWithTreeModel(TreeViewState state) : base(state) { }
        ...
        protected override TreeViewItem BuildRoot() {
            ...
            return new TreeViewItem<T>(m_TreeModel.Root.ID, depthForHiddenRoot, m_TreeModel.Root.Name,
                m_TreeModel.Root.Icon, m_TreeModel.Root);
        }

        protected override IList<TreeViewItem> BuildRows(TreeViewItem root) {
            if (m_TreeModel.Root == null) Debug.LogError("tree model root is null. did you call SetData()?");
            ...
            if (!string.IsNullOrEmpty(searchString)) {
                ...
                if (m_TreeModel.Root.HasChildren) AddChildrenRecursive(m_TreeModel.Root, 0, m_Rows);
            }
