...
namespace BitBenderGames
{
    ...
    public class MobileTouchCameraEditor: CustomInspector
    {
        public void OnSceneGUI()
        {
            ...
            Vector3 pBottomLeft =
                mobileTouchCamera.UnprojectVector2(new Vector2(boundaryMin.x, boundaryMin.y), offset);

            Vector3 pBottomRight =
                mobileTouchCamera.UnprojectVector2(new Vector2(boundaryMax.x, boundaryMin.y), offset);

            Vector3 pTopLeft =
                mobileTouchCamera.UnprojectVector2(new Vector2(boundaryMin.x, boundaryMax.y), offset);

            Vector3 pTopRight =
                mobileTouchCamera.UnprojectVector2(new Vector2(boundaryMax.x, boundaryMax.y), offset);
        ...
        #region min/max handles
            var fmh_49_63_638003527255564440 = Quaternion.identity; pBottomLeft = Handles.FreeMoveHandle(pBottomLeft,Quaternion.identity,  handleSize, Vector3.one,
                Handles.SphereHandleCap);

            var fmh_52_59_638003527255587250 = Quaternion.identity; pTopRight = Handles.FreeMoveHandle(pTopRight, Quaternion.identity, handleSize, Vector3.one,
                Handles.SphereHandleCap);
        ...
        #region min/max handles that need to be remapped
            var fmh_60_76_638003527255588780 = Quaternion.identity; Vector3 pBottomRightNew = Handles.FreeMoveHandle(pBottomRight, Quaternion.identity, handleSize,
                Vector3.one, Handles.SphereHandleCap);

            var fmh_63_68_638003527255589810 = Quaternion.identity; Vector3 pTopLeftNew = Handles.FreeMoveHandle(pTopLeft, Quaternion.identity, handleSize,
                Vector3.one, Handles.SphereHandleCap);
        ...
        #region one way handles //Disabled because it doesn't draw correctly in perspective scene mode.
            ...
            boundaryMax.x = DrawOneWayHandle(mobileTouchCamera, handleSize,
                    new Vector2(boundaryMax.x, 0.5f * (boundaryMax.y + boundaryMin.y)), offset)
                .x;

            boundaryMax.y = DrawOneWayHandle(mobileTouchCamera, handleSize,
                    new Vector2(0.5f * (boundaryMax.x + boundaryMin.x), boundaryMax.y), offset)
                .y;

            boundaryMin.x = DrawOneWayHandle(mobileTouchCamera, handleSize,
                    new Vector2(boundaryMin.x, 0.5f * (boundaryMax.y + boundaryMin.y)), offset)
                .x;

            boundaryMin.y = DrawOneWayHandle(mobileTouchCamera, handleSize,
                    new Vector2(0.5f * (boundaryMax.x + boundaryMin.x), boundaryMin.y), offset)
                .y;
        #endregion
        ...
        private Vector3 DrawOneWayHandle(MobileTouchCamera mobileTouchCamera, float handleSize,
            Vector2 pRelative, float offset)
        {
            Vector3 point = mobileTouchCamera.UnprojectVector2(pRelative, offset);
            var fmh_112_62_638003527255596630 = Quaternion.identity; Vector3 pointNew = Handles.FreeMoveHandle(point, Quaternion.identity, handleSize, Vector3.one,
        ...
        #region boundary
            ...
            WrapWithValidationColor(() => {
                vector2BoundaryMax.y =
                    EditorGUILayout.FloatField(vector2BoundaryMax.y, GUILayout.Width(sizeFloatInput));
            }, isBoundaryYValid);
            ...
            GUILayout.FlexibleSpace();
            WrapWithValidationColor(() => {
                vector2BoundaryMin.y =
                    EditorGUILayout.FloatField(vector2BoundaryMin.y, GUILayout.Width(sizeFloatInput));
            }, isBoundaryYValid);
        ...
        #region auto scroll damp
            AutoScrollDampMode selectedDampMode =
                (AutoScrollDampMode) serializedObject.FindProperty("autoScrollDampMode").enumValueIndex;

            if (selectedDampMode == AutoScrollDampMode.DEFAULT && mobileTouchCamera.AutoScrollDamp != 300) {
                serializedObject.FindProperty("autoScrollDampMode").enumValueIndex =
                    (int) AutoScrollDampMode
                        .CUSTOM; //Set selected mode to custom in case it was set to default but the damp wasn't the default value. This may happen for users that have changed the damp and upgraded from an older version of the asset.
            ...
            if (mobileTouchCamera.EnableTilt == true) {
                DrawPropertyField("tiltAngleMin", mobileTouchCamera.TiltAngleMin >= minTiltErrorAngle,
                    mobileTouchCamera.TiltAngleMin >= minTiltWarningAngle, subSettingsInset);

                if (mobileTouchCamera.TiltAngleMin < minTiltErrorAngle) {
                    DrawErrorLine(
                        "Error: The minimum tilt angle\nmust not be lower than " +
                        minTiltErrorAngle +
                    ...
                    DrawErrorLine(
                        "Warning: The minimum tilt angle\nshould not be lower than " +
                        minTiltWarningAngle +
                        ".\nOtherwise the camera computations\nmay become instable.", Color.yellow);
                }

                DrawPropertyField("tiltAngleMax", mobileTouchCamera.TiltAngleMax <= maxTiltErrorAngle, true,
                    subSettingsInset);
            ...
            if (mobileTouchCamera.EnableZoomTilt == true) {
                DrawPropertyField("zoomTiltAngleMin", mobileTouchCamera.ZoomTiltAngleMin >= minTiltErrorAngle,
                    mobileTouchCamera.ZoomTiltAngleMin >= minTiltWarningAngle, subSettingsInset);

                DrawPropertyField("zoomTiltAngleMax", mobileTouchCamera.ZoomTiltAngleMax <= maxTiltErrorAngle,
                    true, subSettingsInset);
            }
            ...
            SerializedProperty serializedPropertyExpertMode =
                serializedObject.FindProperty("expertModeEnabled");
            ...
            if (GUI.changed) {
                ...
                AutoScrollDampMode dampModeAfterApply =
                    (AutoScrollDampMode) serializedObject.FindProperty("autoScrollDampMode").enumValueIndex;
        ...
        private void OnScrollDampModeChanged(AutoScrollDampMode dampMode)
        {
            ...
            SerializedProperty serializedScrollDampCurve =
                serializedObject.FindProperty("autoScrollDampCurve");

            switch (dampMode) {
                case AutoScrollDampMode.DEFAULT:
                    serializedScrollDamp.floatValue = 300;
                    serializedScrollDampCurve.animationCurveValue = new AnimationCurve(
                        new Keyframe(0, 1, 0, 0), new Keyframe(0.7f, 0.9f, -0.5f, -0.5f),
                ...
                case AutoScrollDampMode.SLOW_FADE_OUT:
                    ...
                    serializedScrollDampCurve.animationCurveValue =
                        new AnimationCurve(new Keyframe(0, 1, -1, -1), new Keyframe(1, 0.01f, -1, -1));
