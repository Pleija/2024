...
namespace NodeCanvas.Framework
{
    partial class Node
    {
        ...
        public bool isSelected {
            get {
                return GraphEditorUtility.activeElement == this ||
        ...
        private Texture2D icon {
            get {
                if(_icon == null) {
                    ...
                    if(_icon == null) {
                        var iconAtt = this.GetType()
                            .RTGetAttribute<ParadoxNotion.Design.IconAttribute>(true);
        ...
        static public void ShowNodeInspectorGUI(Node node)
        {
            ...
            if(!string.IsNullOrEmpty(node.customName) && GUILayout.Button("Edit...", GUILayout.Width(35))) {
                //todo edit ts file
                ...
                if(otherNode.rect.Contains(Event.current.mousePosition)) {
                    if(connection.relinkState == Connection.RelinkState.Target &&
                       otherNode != connection.targetNode) {
                        if(Node.IsNewConnectionAllowed(connection.sourceNode, otherNode,
                               connection)) {
                    ...
                    if(connection.relinkState == Connection.RelinkState.Source &&
                       otherNode != connection.sourceNode) {
                        if(Node.IsNewConnectionAllowed(otherNode, connection.targetNode,
                               connection)) {
        ...
        virtual protected void DrawNodeConnections(Rect drawCanvas, bool fullDrawPass,
            Vector2 canvasMousePos, float zoomFactor)
        {
            var e = Event.current;
            ...
            if(clickedPort != null && clickedPort.parent == this) {
                ...
                ParadoxNotion.CurveUtils.ResolveTangents(clickedPort.pos, e.mousePosition,
                    Prefs.connectionsMLT, graph.flowDirection, out tangA, out tangB);
