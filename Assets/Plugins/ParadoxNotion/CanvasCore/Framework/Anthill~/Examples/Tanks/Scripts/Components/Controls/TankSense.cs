namespace Tanks
{
	...
	public class TankSense : MonoBehaviour, ISense
	{
		...
		public void CollectConditions(AntAIAgent aAgent, AntAICondition aWorldState)
		{
			...
			{
				aWorldState.Set("Alive", true);
				aWorldState.Set("Injured", (_control.Health < _control.maxHealth));
				aWorldState.Set("Enemy Alive", true); // All time is true ;)

				aWorldState.Set("Has Gun", _control.Tower.IsHasGun);
				aWorldState.Set("Has Ammo", _control.Tower.IsHasAmmo);
				aWorldState.Set("Has Bomb", _control.Tower.IsHasBomb);

				// Reset items visibility.
				aWorldState.Set("See Gun", false);
				aWorldState.Set("See Ammo", false);
				aWorldState.Set("See Bomb", false);
				aWorldState.Set("See Repair", false);
				...
				for (int i = 0; i < SEE_ITEMS_COUNT; i++)
				{
					if (_seeItems[i] != null)
					{
						...
						{
							case ItemKind.Gun :
								aWorldState.Set("See Gun", true);
								break;
							case ItemKind.Ammo :
								aWorldState.Set("See Ammo", true);
								break;
							case ItemKind.Bomb :
								aWorldState.Set("See Bomb", true);
								break;
							case ItemKind.Repair :
								aWorldState.Set("See Repair", true);
								break;
						}
					}
				}
				...
				aWorldState.Set("See Enemy", false);
				...
				for (int i = 0; i < TANKS_COUNT; i++)
				{
					if (_seeTanks[i] != null)
					{
						...
						aWorldState.Set("See Enemy", true);
					}
				}

				// Check the enemy on line of the fire.
				aWorldState.Set("On Line of Fire", false);
				...
				if (_blackboard.HasEnemy)
				{
					...
					{
						aWorldState.Set("On Line of Fire", true);
					}
				}

				// Check the touching of enemy tanks.
				aWorldState.Set("Near Enemy", false);

				for (int i = 0; i < TANKS_COUNT; i++)
				{
					if (_touchTanks[i] != null)
					{
						aWorldState.Set("Near Enemy", true);
						break;
					}
				}
			}
