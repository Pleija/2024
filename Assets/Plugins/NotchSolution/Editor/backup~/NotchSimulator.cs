...
//			EditorApplication.update -= RespawnMockup;
		...
		private void OnGUI() {
			...
			settings.ActiveConfiguration.Orientation
				= (PreviewOrientation) EditorGUILayout.EnumPopup("Orientation",
					settings.ActiveConfiguration.Orientation);
				...
				if (screen != null) {
					...
					if (aspectDiff > 0.01f) {
						EditorGUILayout.HelpBox(
							...
							+ "The overlay mockup won't be displayed correctly.", MessageType.Warning);
					}
				}
			}
		...
		public static void ShowWindow() {
			...
			win.titleContent = new GUIContent("Notch");
		}
		...
		static void ClearAll() {
			...
			initial = false;
			path = "";
			EditorApplication.playModeStateChanged += aChange => {
				force = true;
				RespawnMockup();
				win ??= (NotchSimulator) GetWindow(typeof(NotchSimulator));
				AddOverlayInPlayMode();
				//win.OnGUI();
			};
		}

		internal static void RespawnMockup() {
			var refresh = Selection.activeGameObject != null
				&& Selection.activeGameObject.transform is RectTransform tRectTransform
				&& tRectTransform.GetComponentInParent<Canvas>() != old;
			if (refresh) {
				old = Selection.activeGameObject.GetComponentInParent<Canvas>();
			}

			//When the game view is changed, the mockup sometimes disappears or isn't scaled correctly
			if (force || refresh || !initial || path != SceneManager.GetActiveScene().path
				|| gameviewResolution != NotchSimulatorUtility.GetMainGameViewSize()) {
				force = false;
				initial = true;
		...
		private static void AddOverlayInPlayMode() {
			Debug.Log($"Initial: {Core.CurrentMethod()}".ToPink());
			UpdateAllMockups();
		}

		[RuntimeInitializeOnLoadMethod]
		static void Reload1() => UpdateAllMockups();

		[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.BeforeSceneLoad)]
		static void Reload2() => UpdateAllMockups();

		[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterAssembliesLoaded)]
		static void Reload3() => UpdateAllMockups();
		...
		internal static void UpdateAllMockups() {
		   ...
		   if(win != null) win.Repaint();			
			...
			if (enableSimulation && selectedDevice != null) {
				...
				foreach (var mockup in AllMockupCanvases) {
					mockup.UpdateMockupSprite(mockupSprite, NotchSimulatorUtility.GetGameViewOrientation(),
						enableSimulation, settings.FlipOrientation, Settings.Instance.PrefabModeOverlayColor);
				}
			}
			else {
				DestroyHiddenCanvas();
