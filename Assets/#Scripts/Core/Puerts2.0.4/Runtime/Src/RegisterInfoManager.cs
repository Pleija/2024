...
namespace Puerts
{
    ...
    {
        public enum MemberType { Constructor = 1, Method = 2, Property = 3 }
        ...
        {
            private Dictionary<Type, Func<RegisterInfo>> RegisterInfoGetters =
                new Dictionary<Type, Func<RegisterInfo>>();

            protected BindingMode _DefaultBindingMode = BindingMode.FastBinding;

            public BindingMode DefaultBindingMode {
                get => _DefaultBindingMode;
            ...
            internal bool TryGetValue(Type key, out Func<RegisterInfo> value) =>
                RegisterInfoGetters.TryGetValue(key, out value);

            internal bool Remove(Type type) => RegisterInfoGetters.Remove(type);
        }
    }
}
