                ...
                if (!isDelegate) {
                    if (ctors != null && ctors.Length > 0 && (!type.IsArray || type == typeof(System.Array))) {
                        foreach (var ctor in ctors) {
                            ...
                            var wrapData = NativeAPI.AddConstructor(typeInfo, signature, wrapper, methodInfoPointer, methodPointer, usedTypes.Count);
                    ...
                    Action<string, MethodInfo, bool, bool, bool> AddMethodToType = (string name, MethodInfo method, bool isGetter, bool isSetter, bool isExtensionMethod) => {
                        ...
                        var wrapData = NativeAPI.AddMethod(typeInfo, signature, wrapper, name, !isExtensionMethod && method.IsStatic, isExtensionMethod, isGetter, isSetter, methodInfoPointer, methodPointer, usedTypes.Count);
