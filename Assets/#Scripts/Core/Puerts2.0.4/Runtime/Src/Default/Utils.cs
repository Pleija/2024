...
namespace Puerts
{
    ...
    public static class Utils
    {
        ...
        public static MethodInfo[] GetMethodAndOverrideMethodByName(Type type, string name)
        {
            ...
            Dictionary<string, IEnumerable<Type[]>> errorMethods = type.GetMethods().Where(m => m.DeclaringType != type && IsObsoleteError(m)).GroupBy(m => m.Name).ToDictionary(i => i.Key, i => i.Cast<MethodInfo>().Select(m => m.GetParameters().Select(o => o.ParameterType).ToArray()));
            IEnumerable<Type[]> matchTypes;
            Type objType = typeof(Object);

            while (type.BaseType != null && type.BaseType != objType) {
                type = type.BaseType;
                MethodInfo[] methods = type.GetMember(name).Select(m => (MethodInfo)m).Where(m => !IsObsoleteError(m) && !IsVirtualMethod(m)).Where(m => !errorMethods.TryGetValue(m.Name, out matchTypes) || !IsMatchParameters(matchTypes, m.GetParameters().Select(o => o.ParameterType).ToArray())) //filter override method
                    .ToArray();
