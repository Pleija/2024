...
public class U2018Compatible
{
    ...
    private static BindingMode Filter(MemberInfo memberInfo)
    {
        ...
        if (memberInfo.DeclaringType.IsGenericType &&
            memberInfo.DeclaringType.GetGenericTypeDefinition() == typeof(HashSet<>)) {
            if (memberInfo.MemberType == MemberTypes.Constructor) {
                var constructorInfo = memberInfo as ConstructorInfo;
                var parameterInfos = constructorInfo.GetParameters();
        ...
        if (memberInfo.DeclaringType.ToString() == "System.Type" && memberInfo.Name == "IsSZArray")
            return BindingMode.DontBinding;
        if (memberInfo.DeclaringType.ToString() == "System.Threading.Tasks.Task" &&
            memberInfo.Name == "IsCompletedSuccessfully")
            return BindingMode.DontBinding;
