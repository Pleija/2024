...
{
    public enum EnclosedType { None, Array, List, ObservableCollection }
    ...
    public static class ReflectionExtensions
    {
        ...
        public static object GetDefault(this Type type) => type.GetTypeInfo().IsValueType
            ? Activator.CreateInstance(type) : null;

        private static PropertyInfo
            GetExplicitForeignKeyProperty(this Type type, Type destinationType) =>
        ...
        private static PropertyInfo GetConventionForeignKeyProperty(this Type type,
            ...
            PropertyInfo relationshipProperty, Type intermediateType = null, bool inverse = false)
        {
            ...
            else if (!inverse && inverseAttribute != null &&
        ...
        public static PropertyInfo GetInverseProperty(this Type elementType, PropertyInfo property)
        {
            ...
            if (attribute == null || (attribute.InverseProperty != null &&
                    attribute.InverseProperty.Equals("")))
        ...
        public static ManyToManyMetaInfo GetManyToManyMetaInfo(this Type type,
            PropertyInfo relationship)
        {
            ...
            var inverseKeyProperty =
                type.GetForeignKeyProperty(relationship, intermediateType, true);
        ...
        public static List<PropertyInfo> GetRelationshipProperties(this Type type) =>
            (from property in type.GetRuntimeProperties()
                where property.IsPublicInstance() &&
                    property.GetAttribute<RelationshipAttribute>() != null
                select property).ToList();

        public static PropertyInfo GetPrimaryKey(this Type type) =>
            (from property in type.GetRuntimeProperties()
                where property.IsPublicInstance() &&
                    property.GetAttribute<PrimaryKeyAttribute>() != null
                select property).FirstOrDefault();
        ...
        private static bool IsPublicInstance(this PropertyInfo propertyInfo) =>
            propertyInfo != null && propertyInfo.GetMethod != null &&
            !propertyInfo.GetMethod.IsStatic && propertyInfo.GetMethod.IsPublic &&
