...
#region Private methods
            ...
            , bool recursive, ISet<object> objectCache = null)
        {
            ...
            foreach (var relationshipProperty in element.GetType().GetRelationshipProperties()) {
                ...
                if (relationshipAttribute.ReadOnly || !relationshipAttribute.IsCascadeInsert) continue;
            ...
            if (recursive)
                foreach (var element in elementList) {
                    ...
                    foreach (var relationshipProperty in type.GetRelationshipProperties()) {
                        ...
                        if (!relationshipAttribute.IsCascadeDelete || relationshipAttribute.ReadOnly) continue;
                        var value = relationshipProperty.GetValue(element, null);
        ...
        private static void DeleteAllObjects(this SQLiteConnection conn, IEnumerable elements)
        {
            ...
            foreach (var groupElement in groupedElements) {
                ...
                var primaryKeyValues = (from element in groupElement select primaryKeyProperty.GetValue(element, null))
                    .ToArray();
            ...
            , PropertyInfo relationshipProperty)
        {
            ...
            if (child != null) {
                if (inverseForeignKeyProperty != null && keyValue != null)
                    inverseForeignKeyProperty.SetValue(child, keyValue, null);
                if (inverseProperty != null) inverseProperty.SetValue(child, element, null);
                if (inversePrimaryKeyProperty != null) childKey = inversePrimaryKeyProperty.GetValue(child, null);
            }
