...
using SqlCipher4Unity3D.sqlite_net_extensions.SQLiteNetExtensions.Attributes;
using SqlCipher4Unity3D.sqlite_net_extensions.SQLiteNetExtensions.Exceptions;
using SqlCipher4Unity3D.sqlite_net_extensions.SQLiteNetExtensions.Extensions.TextBlob;
using SqlCipher4Unity3D.SQLite.Attribute;

namespace SqlCipher4Unity3D.sqlite_net_extensions.SQLiteNetExtensions.Extensions
{
    public static class WriteOperations
    {
        private const int queryLimit = 990; //Make room for extra keys added by the code
        ...
        ///     stored correctly in the database. This operation will create or delete the required
        ///     intermediate
        ///     objects for ManyToMany relationships. All related objects must have a primary key assigned in
        ///     order
        ///     to work correctly. This also implies that any object with 'AutoIncrement' primary key must've
        ///     been
        ///     inserted in the database previous to this call.
        ///     This method will also update inverse relationships of objects that currently exist in the
        ///     object tree,
        ///     but it won't update inverse relationships of objects that are not reachable through this object
        ///     properties. For example, objects removed from a 'ToMany' relationship won't be updated in
        ///     memory.
        /// </summary>
        ...
        /// <summary>
        ///     Inserts the element and all the relationships that are annotated with
        ///     <c>CascadeOperation.CascadeInsert</c>
        ///     into the database. If any element already exists in the database a 'Constraint' exception will
        ///     be raised.
        ///     Elements with a primary key that it's not <c>AutoIncrement</c> will need a valid identifier
        ///     before calling
        ///     this method.
        ///     If the <c>recursive</c> flag is set to true, all the relationships annotated with
        ///     <c>CascadeOperation.CascadeInsert</c> are inserted recursively in the database. This method
        ///     will handle
        ...
        public static void InsertWithChildren(this SQLiteConnection conn, object element,
        ...
        ///     <c>CascadeOperation.CascadeInsert</c> into the database. If any element already exists in the
        ///     database
        ///     it will be replaced. Elements with <c>AutoIncrement</c> primary keys that haven't been assigned
        ///     will
        ...
        /// <summary>
        ///     Inserts all the elements and all the relationships that are annotated with
        ///     <c>CascadeOperation.CascadeInsert</c>
        ///     into the database. If any element already exists in the database a 'Constraint' exception will
        ///     be raised.
        ///     Elements with a primary key that it's not <c>AutoIncrement</c> will need a valid identifier
        ...
        /// <param name="elements">Objects to be inserted.</param>
        /// <param name="recursive">If set to <c>true</c> all the insert-cascade properties will be inserted</param>
        public static void InsertOrReplaceAllWithChildren(this SQLiteConnection conn,
            IEnumerable elements, bool recursive = false)
        {
        ...
        ///     deleted from the database recursively. Inverse relationships and closed entity loops are
        ///     handled
        ///     correctly to avoid endless loops
        /// </summary>
        /// <param name="conn">SQLite Net connection object</param>
        /// <param name="recursive">
        ///     If set to <c>true</c> all relationships marked with 'CascadeDelete' will be
        ///     deleted from the database recursively
        /// </param>
        /// <param name="objects">Objects to be deleted from the database</param>
        public static void DeleteAll(this SQLiteConnection conn, IEnumerable objects,
            bool recursive = false)
        {
        ...
        public static void DeleteAllIds<T>(this SQLiteConnection conn,
            IEnumerable<object> primaryKeyValues)
        {
            var type = typeof(T);
            var primaryKeyProperty = type.GetPrimaryKey();
            conn.DeleteAllIds(primaryKeyValues.ToArray(), type.GetTableName(),
                primaryKeyProperty.GetColumnName());
        }

#region Private methods
        private static void InsertAllWithChildrenRecursive(this SQLiteConnection conn,
            IEnumerable elements, bool replace, bool recursive, ISet<object> objectCache = null)
        {
            if (elements == null) return;
            objectCache = objectCache ?? new HashSet<object>();
            var insertedElements = conn.InsertElements(elements, replace, objectCache)
                .Cast<object>().ToList();
        ...
        private static void InsertWithChildrenRecursive(this SQLiteConnection conn, object element,
        ...
        private static void InsertChildrenRecursive(this SQLiteConnection conn, object element,
            bool replace, bool recursive, ISet<object> objectCache = null)
        {
            ...
            foreach (var relationshipProperty in element.GetType().GetRelationshipProperties()) {
                var relationshipAttribute =
                    relationshipProperty.GetAttribute<RelationshipAttribute>();
        ...
        private static void InsertValue(this SQLiteConnection conn, object value, bool replace,
            bool recursive, ISet<object> objectCache)
        {
            ...
            if (recursive) {
                if (enumerable != null)
                    conn.InsertAllWithChildrenRecursive(enumerable, replace, recursive,
                        objectCache);
                else
        ...
        private static IEnumerable InsertElements(this SQLiteConnection conn, IEnumerable elements,
            bool replace, ISet<object> objectCache)
        {
            if (elements == null) return Enumerable.Empty<object>();
            objectCache = objectCache ?? new HashSet<object>();
            var elementsToInsert = elements.Cast<object>().Except(objectCache).ToList();
            if (elementsToInsert.Count == 0) return Enumerable.Empty<object>();
            var primaryKeyProperty = elementsToInsert[0].GetType().GetPrimaryKey();
            var isAutoIncrementPrimaryKey = primaryKeyProperty != null &&
                primaryKeyProperty.GetAttribute<AutoIncrementAttribute>() != null;
        ...
        private static void InsertElement(this SQLiteConnection conn, object element, bool replace,
            PropertyInfo primaryKeyProperty, bool isAutoIncrementPrimaryKey,
            ISet<object> objectCache)
        {
            ...
            if (replace && isAutoIncrementPrimaryKey) {
                ...
                isPrimaryKeySet = primaryKeyValue != null &&
                    !primaryKeyValue.Equals(defaultPrimaryKeyValue);
            }
            var shouldReplace = replace && (!isAutoIncrementPrimaryKey || isPrimaryKeySet);
        ...
        private static void DeleteAllRecursive(this SQLiteConnection conn, IEnumerable elements,
            bool recursive, ISet<object> objectCache = null)
        {
            ...
            foreach (var element in elementList) objectCache.Add(element);
            ...
            if (recursive)
                foreach (var element in elementList) {
                    ...
                    foreach (var relationshipProperty in type.GetRelationshipProperties()) {
                        ...
                        if (!relationshipAttribute.IsCascadeDelete ||
                            relationshipAttribute.ReadOnly)
                            continue;
                        var value = relationshipProperty.GetValue(element, null);
        ...
        private static void DeleteValueRecursive(this SQLiteConnection conn, object value,
            bool recursive, ISet<object> objectCache)
        {
            if (value == null) return;
        ...
        private static void DeleteAllObjects(this SQLiteConnection conn, IEnumerable elements)
        {
            if (elements == null) return;
            ...
            foreach (var groupElement in groupedElements) {
                ...
                var primaryKeyProperty = type.GetPrimaryKey();
                Assert(primaryKeyProperty != null, type, null,
                    "Cannot delete objects without primary key");
                var primaryKeyValues =
                    (from element in groupElement select primaryKeyProperty.GetValue(element, null))
                    .ToArray();
                conn.DeleteAllIds(primaryKeyValues, type.GetTableName(),
                    primaryKeyProperty.GetColumnName());
            }
        }
        ...
        private static void RefreshForeignKeys(object element)
        {
            ...
            foreach (var relationshipProperty in type.GetRelationshipProperties()) {
                ...
                if (relationshipAttribute.ReadOnly) continue;

                if (relationshipAttribute is OneToOneAttribute ||
                    relationshipAttribute is ManyToOneAttribute) {
                    ...
                    if (foreignKeyProperty != null) {
                        ...
                        if (relationshipValue != null)
                            foreignKeyValue =
                                destinationPrimaryKeyProperty.GetValue(relationshipValue, null);
        ...
        private static void UpdateOneToManyInverseForeignKey(this SQLiteConnection conn,
            object element, PropertyInfo relationshipProperty)
        {
            var type = element.GetType();
            EnclosedType enclosedType;
            var entityType = relationshipProperty.GetEntityType(out enclosedType);
            ...
            var inverseForeignKeyProperty =
                type.GetForeignKeyProperty(relationshipProperty, inverse: true);
            Assert(enclosedType != EnclosedType.None, type, relationshipProperty,
            ...
            if (children != null)
                foreach (var child in children) {
                    ...
                    if (inverseProperty != null) inverseProperty.SetValue(child, element, null);
                }
            ...
            for (var i = 0; i < loopTo; i++) {
                ...
                var query = string.Format("update [{0}] set [{1}] = ? where [{2}] in ({3})",
                    entityType.GetTableName(), inverseForeignKeyProperty.GetColumnName(),
                    inversePrimaryKeyProperty.GetColumnName(), childrenPlaceHolders);
        ...
        private static void UpdateOneToOneInverseForeignKey(this SQLiteConnection conn,
            object element, PropertyInfo relationshipProperty)
        {
            var type = element.GetType();
            EnclosedType enclosedType;
            ...
            if (inverseForeignKeyProperty != null && inversePrimaryKeyProperty != null) {
                ...
                var deleteQuery = string.Format(
                    "update [{0}] set [{1}] = NULL where [{1}] == ? and [{2}] not in (?)",
                    entityType.GetTableName(), inverseForeignKeyProperty.GetColumnName(),
                    inversePrimaryKeyProperty.GetColumnName());
            ...
            PropertyInfo relationshipProperty)
        {
            ...
            for (var i = 0; i < loopTo; i++) {
                ...
                var childrenPlaceHolders = string.Join(",", Enumerable.Repeat("?", chunk.Count));
                var currentChildrenQuery = string.Format(
                    "select [{0}] from [{1}] where [{2}] == ? and [{0}] in ({3})",
                ...
                currentChildKeyList.AddRange(
                    from object child in conn.Query(conn.GetMapping(intermediateType),
                        currentChildrenQuery, parameters.ToArray())
                    select otherEntityForeignKeyProperty.GetValue(child, null));
            }

            // Insert missing relationships in the intermediate table
            var missingChildKeyList =
                childKeyList.Where(o => !currentChildKeyList.Contains(o)).ToList();
            ...
            for (var i = 0; i < loopTo; i++) {
                ...
                var deleteQuery = string.Format(
                    "delete from [{0}] where [{1}] == ? and [{2}] not in ({3})",
                    intermediateType.GetTableName(),
                    currentEntityForeignKeyProperty.GetColumnName(),
            ...
            string entityName, string primaryKeyName)
        {
            ...
            if (primaryKeyValues.Length <= queryLimit) {
                var placeholdersString =
                    string.Join(",", Enumerable.Repeat("?", primaryKeyValues.Length));
                var deleteQuery = string.Format("delete from [{0}] where [{1}] in ({2})",
                    entityName, primaryKeyName, placeholdersString);
        ...
        private static void Assert(bool assertion, Type type, PropertyInfo property, string message)
        {
            if (EnableRuntimeAssertions && !assertion)
                throw new IncorrectRelationshipException(type.Name,
                    property != null ? property.Name : string.Empty, message);
        }
#endregion
    }
}
