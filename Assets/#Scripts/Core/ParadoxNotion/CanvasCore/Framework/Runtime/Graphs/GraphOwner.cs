...
namespace NodeCanvas.Framework
{
    ...
    public abstract class GraphOwner : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        protected void OnDrawGizmosSelected()
        {
            if (Editor.GraphEditorUtility.activeElement != null) {
                var rootElement = Editor.GraphEditorUtility.activeElement.graph.GetFlatMetaGraph()
                    .FindReferenceElement(Editor.GraphEditorUtility.activeElement);
                if (rootElement != null)
    ...
    public abstract class GraphOwner<T> : GraphOwner where T : Graph
    {
        ...
        {
            StartBehaviour(newGraph, updateMode, null);
        }
