    ...
    public static class IGraphAssignableExtensions
    {
        ...
        public static Graph CheckInstance(this IGraphAssignable assignable)
        {
            ...
            if (assignable.instances == null)
                assignable.instances = new System.Collections.Generic.Dictionary<Graph, Graph>();
        ...
        /// <summary>
        ///     Utility to start sub graph (makes instance, writes mapping, starts graph and on stop reads
        ///     mapping)
        /// </summary>
        ...
        {
            if (!assignable.currentInstance.allowBlackboardOverrides ||
            ...
            for (var i = 0; i < assignable.variablesMap.Count; i++) {
                ...
                var targetSubVariable =
                    assignable.currentInstance.blackboard.GetVariableByID(
                        bbParam.targetSubGraphVariableID);

                if (targetSubVariable != null && targetSubVariable.isExposedPublic &&
                    !targetSubVariable.isPropertyBound) {
        ...
        public static void ValidateSubGraphAndParameters(this IGraphAssignable assignable)
        {
            if (!ParadoxNotion.Services.Threader.applicationIsPlaying)
                if (assignable.subGraph == null || !assignable.subGraph.allowBlackboardOverrides ||
                    assignable.subGraph.blackboard.variables.Count == 0)
                    assignable.variablesMap = null;
        }
        ...
        public static void ShowVariablesMappingGUI(this IGraphAssignable assignable)
        {
            ...
            foreach (var variable in subTreeVariables) {
                ...
                if (assignable.variablesMap == null)
                    assignable.variablesMap =
                        new System.Collections.Generic.List<BBMappingParameter>();
                var bbParam =
                    assignable.variablesMap.Find(x => x.targetSubGraphVariableID == variable.ID);

                if (bbParam == null) {
                    GUILayout.BeginHorizontal();
                    GUI.enabled = false;
                    EditorUtils.DrawEditorFieldDirect(new GUIContent(variable.name), variable.value,
                        variable.varType, default);
                    GUI.enabled = true;
                    var tmp = 0;
                    if (GUILayout.Button(EditorUtils.GetTempContent("▽", null, "Write (In)"),
                    ...
                    if (GUILayout.Button(EditorUtils.GetTempContent("△", null, "Read (Out)"),
                        Styles.centerLabel, GUILayout.Width(13)))
                        tmp = -1;
                ...
                Editor.BBParameterEditor.ParameterField(variable.name, bbParam);
                ...
                if (GUILayout.Button(
                    EditorUtils.GetTempContent(bbParam.canWrite ? "▼" : "▽", null, "Write (In)"),
                    Styles.centerLabel, GUILayout.Width(13))) {
                ...
                if (GUILayout.Button(
                    EditorUtils.GetTempContent(bbParam.canRead ? "▲" : "△", null, "Read (Out)"),
                    Styles.centerLabel, GUILayout.Width(13))) {
            ...
            if (assignable.variablesMap != null)
                for (var i = assignable.variablesMap.Count; i-- > 0;) {
                    var bbParam = assignable.variablesMap[i];
                    var variable =
                        assignable.subGraph.blackboard.GetVariableByID(bbParam
                            .targetSubGraphVariableID);
