...
namespace NodeCanvas.Editor
{
    public class WelcomeWindow : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            if (assetType == null)
                assetType = ReflectionTools.GetImplementationsOf(typeof(Graph))
                    .Where(x => x.IsDefined(typeof(GraphInfoAttribute), true)).LastOrDefault();
            att = assetType != null
                ? assetType.RTGetAttributesRecursive<GraphInfoAttribute>().LastOrDefault() : null;
                    ...
                    foreach (var board in boards) {
                        ...
                        if (isAll || target.ToLower() == packageName.ToLower()) {
                            if (!isAll) {
                                ...
                                var uptodate = Framework.Internal.GraphSource.FRAMEWORK_VERSION ==
                                    float.Parse(version);
                                result += uptodate
        ...
        private void OnGUI()
        {
            ...
            var headerRect = header != null ? new Rect(0, 0, header.width, header.height)
                : new Rect(0, 0, maxSize.x, 110);
            ...
            if (header != null) {
                ...
                GUI.Label(headerRect, $"\t\t<size=30><b>{packageName}</b></size>",
                    Styles.leftLabel);
            }
            var copyrightText =
                "<color=#9c9c9c><size=10><b>© 2014-2023 Paradox Notion. All rights reserved.</b></size></color>";
            ...
            if (!string.IsNullOrEmpty(webMessage)) {
                var messageRect = Rect.MinMaxRect(headerRect.xMax - 385, headerRect.yMin + 5,
                    headerRect.xMax - 5, headerRect.yMax - 5);
                GUI.color = Color.white.WithAlpha(0.5f);
                Styles.Draw(messageRect, Styles.roundedBox);
                GUI.color = Color.white;
                GUI.Label(messageRect.ExpandBy(-5), $"<color=#d2d2d2>{webMessage}</color>",
                    Styles.topLeftLabel);
            }
            ///----------------------------------------------------------------------------------------------
            ...
            ShowEntry(supportIcon,
                "<size=16><b>Support</b></size>\nJoin the online forums, get support and give feedback.",
                forumsURL);
            ShowEntry(communityIcon,
                "<size=16><b>Community</b></size>\nJoin the online Discord community.", discordUrl);
            ///----------------------------------------------------------------------------------------------
            GUILayout.FlexibleSpace();
            Prefs.hideWelcomeWindow =
                EditorGUILayout.ToggleLeft("Don't show again.", Prefs.hideWelcomeWindow);
