    ...
    public class SerializedMethodInfo : ISerializedMethodBaseInfo
    {
        ...
        void ISerializationCallbackReceiver.OnAfterDeserialize()
        {
            ...
            if (_method == null) {
                ...
                _method = methods.FirstOrDefault(m =>
                    m.Name == name && m.GetParameters().Length == parameterTypes.Length &&
                    isSerializedGeneric == m.IsGenericMethod);
                if (_method == null) _method = methods.FirstOrDefault(m => m.Name == name);

                if (_method != null && _method.IsGenericMethod) {
                    var argType = isSerializedGeneric
                        ? ReflectionTools.GetType(_genericArgumentsInfo.Split('|').First(), true)
                        : _method.GetFirstGenericParameterConstraintType();
        ...
        public MemberInfo AsMemberInfo() => _method;
