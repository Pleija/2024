...
namespace ParadoxNotion.Design
{
    ...
    partial class EditorUtils
    {
        ...
        public static List<ScriptInfo> GetScriptInfosOfType(Type baseType)
        {
            ...
            if (cachedInfos.TryGetValue(baseType, out infosResult)) return infosResult.ToList();
            infosResult = new List<ScriptInfo>();
            var subTypes = baseType.IsGenericTypeDefinition ? new Type[] { baseType }
                : ReflectionTools.GetImplementationsOf(baseType);

            foreach (var subType in subTypes) {
                if (subType.IsAbstract || subType.RTIsDefined(typeof(DoNotListAttribute), true) ||
                    subType.RTIsDefined(typeof(ObsoleteAttribute), true)) continue;
                ...
                if (nameAttribute != null) {
                    ...
                    if (isGeneric && !scriptName.EndsWith("<T>")) scriptName += " (T)";
                }
                var categoryAttribute = subType.RTGetAttribute<CategoryAttribute>(true);
                if (categoryAttribute != null) scriptCategory = categoryAttribute.category;
                ...
                if (isGeneric) {
                    var exposeAsBaseDefinition = NodeCanvas.Editor.Prefs.collapseGenericTypes ||
                        subType.RTIsDefined<ExposeAsDefinitionAttribute>(true);
            ...
            infosResult = infosResult.Where(s => s.isValid).OrderBy(s => s.originalCategory)
                .ThenBy(s => s.priority * -1).ThenBy(s => s.originalName).ToList();
