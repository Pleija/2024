...
namespace ParadoxNotion.Design
{
    ...
    partial class EditorUtils
    {
        ...
        private static readonly Dictionary<Type, bool> registeredEditorFoldouts =
        ...
        public static void Separator()
        {
            ...
            GUI.DrawTexture(
                Rect.MinMaxRect(lastRect.xMin, lastRect.yMax + 4, lastRect.xMax, lastRect.yMax + 6),
        ...
        public static void BoldSeparator()
        {
            ...
            GUILayout.Space(14);
            GUI.color = new Color(0, 0, 0, 0.3f);
            GUI.DrawTexture(new Rect(0, lastRect.yMax + 6, Screen.width, 4),
                Texture2D.whiteTexture);
            GUI.DrawTexture(new Rect(0, lastRect.yMax + 6, Screen.width, 1),
                Texture2D.whiteTexture);
            GUI.DrawTexture(new Rect(0, lastRect.yMax + 9, Screen.width, 1),
                Texture2D.whiteTexture);
            GUI.color = Color.white;
        }
        ...
        public static void EndOfInspector()
        {
            ...
            GUI.DrawTexture(new Rect(0, lastRect.yMax + 6, Screen.width, 4),
                Texture2D.whiteTexture);
            GUI.DrawTexture(new Rect(0, lastRect.yMax + 4, Screen.width, 1),
                Texture2D.whiteTexture);
            GUI.color = Color.white;
        }
        ...
        public static string SearchField(string search)
        {
            ...
            if (!string.IsNullOrEmpty(search) &&
                GUILayout.Button(string.Empty, Styles.toolbarSearchCancelButton)) {
                search = string.Empty;
        ...
        /// <summary>
        ///     Used just after a textfield with no prefix to show an italic transparent text inside when
        ///     empty
        /// </summary>
        ...
        public static LayerMask LayerMaskField(string prefix, LayerMask layerMask,
            params GUILayoutOption[] layoutOptions) => LayerMaskField(
            string.IsNullOrEmpty(prefix) ? GUIContent.none : new GUIContent(prefix), layerMask,
            layoutOptions);
            ...
            params GUILayoutOption[] layoutOptions)
        {
            LayerMask tempMask = EditorGUILayout.MaskField(content,
                UnityEditorInternal.InternalEditorUtility.LayerMaskToConcatenatedLayersMask(
                    layerMask), UnityEditorInternal.InternalEditorUtility.layers, layoutOptions);
            layerMask =
        ...
        public static IList ListEditor(GUIContent content, IList list, Type listType,
            InspectedFieldInfo info)
        {
            var optionsAtt =
                info.attributes?.FirstOrDefault(x => x is ListInspectorOptionAttribute) as
                    ListInspectorOptionAttribute;
            var argType = listType.GetEnumerableElementType();
            if (argType == null) return list;
            ...
            if (optionsAtt == null || optionsAtt.showFoldout) {
                if (!CachedFoldout(listType, content)) return list;
            }
            else {
        ...
        public static IDictionary DictionaryEditor(GUIContent content, IDictionary dict,
            Type dictType, InspectedFieldInfo info)
        {
            ...
            if (!CachedFoldout(dictType, content)) return dict;
            GUILayout.BeginVertical();
            ...
            if (GUILayout.Button("Add Element")) {
                if (!typeof(UnityObject).IsAssignableFrom(keyType)) {
                    ...
                    if (dict.Contains(newKey)) {
                        Logger.LogWarning(
                            string.Format("Key '{0}' already exists in Dictionary",
                                newKey.ToString()), "Editor");
                        return dict;
                    }
                    keys.Add(newKey);
                }
            ...
            for (var i = 0; i < keys.Count; i++)
                try {
                    ...
                    Logger.Log("Dictionary Key removed due to duplicate found", "Editor");
                }
            Separator();
            GUILayout.EndVertical();
            return dict;
        ...
        /// <summary>
        ///     An editor field where if the component is null simply shows an object field, but if its not,
        ///     shows a dropdown
        ...
        {
            if (comp == null)
                return EditorGUILayout.ObjectField(content, comp, type, true, GUIOptions) as
                    Component;
            var components = comp.GetComponents(type).ToList();
            var componentNames = components.Where(c => c != null).Select(c =>
                c.GetType().FriendlyName() + " (" + c.gameObject.name + ")").ToList();
            componentNames.Insert(0, "[NONE]");
            var index = components.IndexOf(comp);
            index = EditorGUILayout.Popup(content, index,
                componentNames.Select(n => new GUIContent(n)).ToArray(), GUIOptions);
            return index == 0 ? null : components[index];
        }

        ///<summary>A popup that is based on the string rather than the index</summary>
        public static string StringPopup(GUIContent content, string selected,
            IEnumerable<string> options, params GUILayoutOption[] GUIOptions)
        {
            ...
            index = EditorGUILayout.Popup(content, index,
                copy.Select(n => new GUIContent(n)).ToArray(), GUIOptions);
            EditorGUILayout.EndVertical();
            return index == 0 ? string.Empty : copy[index];
        }

        ///<summary>Generic Popup for selection of any element within a list</summary>
        public static T Popup<T>(T selected, IEnumerable<T> options,
            params GUILayoutOption[] GUIOptions) =>
            Popup<T>(GUIContent.none, selected, options, GUIOptions);

        ///<summary>Generic Popup for selection of any element within a list</summary>
        public static T Popup<T>(string prefix, T selected, IEnumerable<T> options,
            params GUILayoutOption[] GUIOptions) => Popup<T>(
            string.IsNullOrEmpty(prefix) ? GUIContent.none : new GUIContent(prefix), selected,
            options, GUIOptions);
            ...
            params GUILayoutOption[] GUIOptions)
        {
            ...
            var stringedOptions =
                new List<string>(listOptions.Select(o => o != null ? o.ToString() : "[NONE]"));
            ...
            index = EditorGUILayout.Popup(content, index,
                stringedOptions.Select(s => new GUIContent(s)).ToArray(), GUIOptions);
        ...
        public static void ButtonPopup<T>(string prefix, T selected, List<T> options,
            Action<T> Callback)
        {
            ButtonPopup<T>(string.IsNullOrEmpty(prefix) ? GUIContent.none : new GUIContent(prefix),
                selected, options, Callback);
        }

        ///<summary>Generic Button Popup for selection of any element within a list</summary>
        public static void ButtonPopup<T>(GUIContent content, T selected, List<T> options,
            Action<T> Callback)
        {
            ...
            if (GUILayout.Button(buttonText, (GUIStyle)"MiniPopup", GUILayout.Width(0),
        ...
        {
            ButtonTypePopup(string.IsNullOrEmpty(prefix) ? GUIContent.none : new GUIContent(prefix),
                selected, Callback);
        }
        ...
        public static void ButtonTypePopup(GUIContent content, Type selected, Action<Type> Callback)
        {
            ...
            if (GUILayout.Button(buttonText, (GUIStyle)"MiniPopup", GUILayout.Width(0),
                GUILayout.ExpandWidth(true))) {
                var menu = GetPreferedTypesSelectionMenu(typeof(object), Callback);
