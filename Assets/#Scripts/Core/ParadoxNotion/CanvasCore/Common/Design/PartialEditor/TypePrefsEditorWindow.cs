    ...
    public class TypePrefsEditorWindow : OdinEditorWindow
    {
        ...
        protected override void OnEnable()
        {
            ...
            alltypes = ReflectionTools.GetAllTypes(true)
                .Where(t => !t.IsGenericType && !t.IsGenericTypeDefinition).ToList();
        }
        ...
        protected override void OnGUI()
        {
            ...
            EditorGUILayout.HelpBox(
                ...
                TypePrefs.SYNC_FILE_NAME +
                "' it will automatically sync with the list. Useful when working with others on source control.",
                MessageType.Info);

            if (GUILayout.Button("Add New Type", EditorStyles.miniButton)) {
                GenericMenu.MenuFunction2 Selected = delegate(object o) {
                    if (o is System.Type) AddType((System.Type)o);
                ...
                menu.AddItem(new GUIContent("Classes/System/Object"), false, Selected,
                    typeof(object));

                foreach (var t in alltypes) {
                    var a = (string.IsNullOrEmpty(t.Namespace) ? "No Namespace/"
                        : t.Namespace.Replace(".", "/") + "/") + t.FriendlyName();
                    var b = string.IsNullOrEmpty(t.Namespace) ? string.Empty
                        : " (" + t.Namespace + ")";
                    ...
                    menu.AddItem(new GUIContent(category + friendlyName), typeList.Contains(t),
                        Selected, t);
            ...
            if (GUILayout.Button("Generate AOTClasses.cs and link.xml Files",
                ...
                if (EditorUtility.DisplayDialog("Generate AOT Classes",
                    "A script relevant to AOT compatibility for certain platforms will now be generated.",
                    "OK")) {
                    var path = EditorUtility.SaveFilePanelInProject("AOT Classes File",
                        "AOTClasses", "cs", "");
                    if (!string.IsNullOrEmpty(path))
                        AOTClassesGenerator.GenerateAOTClasses(path,
                            TypePrefs.GetPreferedTypesList(true).ToArray());
                }

                if (EditorUtility.DisplayDialog("Generate link.xml File",
                    "A file relevant to 'code stripping' for platforms that have code stripping enabled will now be generated.",
                    "OK")) {
                    var path =
                        EditorUtility.SaveFilePanelInProject("AOT link.xml", "link", "xml", "");
                    if (!string.IsNullOrEmpty(path))
                        AOTClassesGenerator.GenerateLinkXML(path,
                            TypePrefs.GetPreferedTypesList().ToArray());
                }
            ...
            if (GUILayout.Button("Reset Defaults", EditorStyles.miniButtonLeft))
                if (EditorUtility.DisplayDialog("Reset Preferred Types", "Are you sure?", "Yes",
                    "NO!")) {
            ...
            if (GUILayout.Button("Save Preset", EditorStyles.miniButtonMid)) {
                var path = EditorUtility.SaveFilePanelInProject("Save Types Preset",
                    "PreferredTypes", "typePrefs", "");

                if (!string.IsNullOrEmpty(path)) {
                    System.IO.File.WriteAllText(path,
                        JSONSerializer.Serialize(typeof(List<System.Type>), typeList, null, true));
                    AssetDatabase.Refresh();
                }
            }
            ...
            EditorGUILayout.HelpBox(
                syncPath != null
                    ? "List synced with file: " +
                    syncPath.Replace(Application.dataPath, ".../Assets")
                    : "No sync file found in '.../Assets/Editor Default Resources'. Types are currently saved in Unity EditorPrefs only.",
                MessageType.None);
            GUILayout.Space(5);
            ...
            for (var i = 0; i < typeList.Count; i++) {
                ...
                if (GUILayout.Button("X", GUILayout.Width(18))) RemoveType(type);
                GUILayout.EndHorizontal();
            }
        ...
        private void AddType(System.Type t)
        {
            if (!typeList.Contains(t)) {
                ...
                Save();
                ShowNotification(
                    new GUIContent(string.Format("Type '{0}' Added!", t.FriendlyName())));
                return;
            }
            ShowNotification(new GUIContent(string.Format("Type '{0}' is already in the list.",
                t.FriendlyName())));
        }
        ...
        private void RemoveType(System.Type t)
        {
            ...
            Save();
            ShowNotification(
                new GUIContent(string.Format("Type '{0}' Removed.", t.FriendlyName())));
        }
