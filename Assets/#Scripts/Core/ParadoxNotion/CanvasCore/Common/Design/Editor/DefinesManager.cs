...
namespace ParadoxNotion.Design
{
    ...
    public static class DefinesManager
    {
        ...
        public static void SetDefineActiveForTargetGroup(BuildTargetGroup target, string define,
            bool enable)
        {
            var defines = PlayerSettings.GetScriptingDefineSymbolsForGroup(target).Split(';')
                .ToList();
            if (enable == true && !defines.Contains(define)) defines.Add(define);
            if (enable == false) defines.Remove(define);
