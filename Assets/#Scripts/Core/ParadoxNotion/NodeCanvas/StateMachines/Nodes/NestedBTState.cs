...
namespace NodeCanvas.StateMachines
{
    ...
    public class NestedBTState : FSMStateNested<BehaviourTree>
    {
        ...
        public enum BTExitMode { StopAndRestart, PauseAndResume }
        ...
        protected override void OnEnter()
        {
            ...
            currentInstance.StartGraph(graph.agent, graph.blackboard.parent,
                Graph.UpdateMode.Manual, OnFinish);
            OnUpdate();
        }
        ...
        private void OnFinish(bool success)
        {
            if (status == Status.Running) {
                ...
                if (!string.IsNullOrEmpty(successEvent) && success) SendEvent(successEvent);
                if (!string.IsNullOrEmpty(failureEvent) && !success) SendEvent(failureEvent);
                Finish(success);
            }
        }
        ...
        protected override void OnExit()
        {
            if (currentInstance != null) {
                if (status == Status.Running) this.TryReadAndUnbindMappedVariables();
