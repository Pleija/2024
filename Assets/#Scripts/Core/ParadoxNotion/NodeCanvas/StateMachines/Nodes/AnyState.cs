...
namespace NodeCanvas.StateMachines
{
    ...
    public class AnyState : FSMNode, IUpdatable
    {
        ...
        {
            for (var i = 0; i < outConnections.Count; i++) (outConnections[i] as FSMConnection).DisableCondition();
        }

        void IUpdatable.Update()
        {
            if (outConnections.Count == 0) return;
            status = Status.Running;

            for (var i = 0; i < outConnections.Count; i++) {
                var connection = (FSMConnection)outConnections[i];
                var condition = connection.condition;
                if (!connection.isActive || condition == null) continue;
                if (dontRetriggerStates)
                    if (FSM.currentState == (FSMState)connection.targetNode &&
                        FSM.currentState.status == Status.Running)
                        continue;
        ...
        {
            base.OnNodeGUI();
            if (dontRetriggerStates) GUILayout.Label("<b>[NO RETRIGGER]</b>");
        }

        public override string GetConnectionInfo(int index)
        {
            if ((outConnections[index] as FSMConnection).condition == null) return "* Never Triggered *";
            return null;
        }
