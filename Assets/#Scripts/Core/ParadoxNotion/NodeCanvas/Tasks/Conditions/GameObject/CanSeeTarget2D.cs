    ...
    public class CanSeeTarget2D : ConditionTask<Transform>
    {
        ...
        protected override bool OnCheck()
        {
            ...
            if (Vector2.Distance(agent.position, t.position) <= awarnessDistance.value) {
                var hit = Physics2D.Linecast((Vector2)agent.position + offset,
                    (Vector2)t.position + offset, layerMask.value);
                if (hit.collider != t.GetComponent<Collider2D>()) return false;
                return true;
            }
            if (Vector2.Distance(agent.position, t.position) > maxDistance.value) return false;
            if (Vector2.Angle((Vector2)t.position - (Vector2)agent.position, agent.right) >
                viewAngle.value)
                return false;
            var hit2 = Physics2D.Linecast((Vector2)agent.position + offset,
                (Vector2)t.position + offset, layerMask.value);
            if (hit2.collider != t.GetComponent<Collider2D>()) return false;
            return true;
        }
        ...
        public override void OnDrawGizmosSelected()
        {
            if (agent != null) {
                ...
                Gizmos.DrawLine((Vector2)agent.position + offset,
                    (Vector2)agent.position + offset + (Vector2)agent.right * maxDistance.value);
                Gizmos.DrawWireSphere(
                    (Vector2)agent.position + offset + (Vector2)agent.right * maxDistance.value,
                    0.1f);
                Gizmos.DrawWireSphere((Vector2)agent.position, awarnessDistance.value);
                Gizmos.matrix = Matrix4x4.TRS((Vector2)agent.position + offset,
                    Quaternion.LookRotation(agent.right), Vector3.one);
