...
namespace NodeCanvas.Tasks.Actions
{
    ...
    public class Patrol : ActionTask<NavMeshAgent>
    {
        ...
        protected override void OnExecute()
        {
            ...
            if (targetList.value.Count == 1) {
                ...
                if (patrolMode.value == PatrolMode.Random) {
                    var newIndex = index;
                    while (newIndex == index) newIndex = Random.Range(0, targetList.value.Count);
                    index = newIndex;
                }
            ...
            if (targetGo == null) {
                ParadoxNotion.Services.Logger.LogWarning(
                    "List game object is null on Patrol Action Task.", LogTag.EXECUTION, this);
                EndAction(false);
                return;
            }
            ...
            if ((agent.transform.position - targetPos).magnitude <
        ...
        {
            if (lastRequest != null && agent.gameObject.activeSelf) agent.ResetPath();
            lastRequest = null;
        }
