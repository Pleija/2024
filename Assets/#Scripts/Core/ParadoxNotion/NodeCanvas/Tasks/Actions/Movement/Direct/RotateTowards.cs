...
{
    [Category("Movement/Direct"), Description("Rotate the agent towards the target per frame")]
    public class RotateTowards : ActionTask<Transform>
    {
        ...
        {
            if (Vector3.Angle(target.value.transform.position - agent.position, agent.forward) <=
