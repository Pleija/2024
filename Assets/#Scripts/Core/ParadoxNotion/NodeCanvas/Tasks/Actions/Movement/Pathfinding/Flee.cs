...
{
    [Category("Movement/Pathfinding"), Description("Flees away from the target")]
    public class Flee : ActionTask<NavMeshAgent>
    {
        ...
        protected override string info => string.Format("Flee from {0}", target);
        ...
        protected override void OnUpdate()
        {
            ...
            var fleePos = targetPos + (agent.transform.position - targetPos).normalized *
                (fledDistance.value + lookAhead.value + agent.stoppingDistance);
            if (!agent.SetDestination(fleePos)) EndAction(false);
        }
