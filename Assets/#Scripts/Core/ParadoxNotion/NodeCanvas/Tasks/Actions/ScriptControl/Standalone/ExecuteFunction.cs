...
{
    [Name("Execute Function (Desktop Only)", 10), Category("✫ Reflected/Faster Versions (Desktop Platforms Only)"),
     Description(
    ...
    public class ExecuteFunction : ActionTask, IReflectedWrapper
    {
        ...
        ISerializedReflectedInfo IReflectedWrapper.GetSerializedInfo() => functionWrapper?.GetSerializedMethod();
            ...
            get {
                ...
                if (targetMethod.ReturnType == typeof(void)) {
                    ...
                    returnInfo =
                        ...
                        variables[0].isNone ? "" : variables[0] + " = ";
        ...
        private IEnumerator InternalCoroutine(IEnumerator routine)
        {
            ...
            while (routineRunning && routine.MoveNext()) {
                if (routineRunning == false) yield break;
                yield return routine.Current;
            }
            if (routineRunning) EndAction();
        }
        ...
        protected override void OnTaskInspectorGUI()
        {
                ...
                if (agent != null) {
                    foreach (var comp in agent.GetComponents(typeof(Component))
                        ...
                        menu = EditorUtils.GetInstanceMethodSelectionMenu(comp.GetType(), typeof(object),
                            typeof(object), SetMethod, 6, false, false, menu);
                    menu.AddSeparator("/");
                }
            ...
            if (m != null) {
                ...
                if (m.ReturnType == typeof(IEnumerator)) GUILayout.Label("<b>This will execute as a Coroutine!</b>");
                ...
                if (m.ReturnType == typeof(void)) {
                    for (var i = 0; i < paramNames.Length; i++)
                        Editor.BBParameterEditor.ParameterField(paramNames[i], variables[i]);
                }
                else {
                    for (var i = 0; i < paramNames.Length; i++)
                        Editor.BBParameterEditor.ParameterField(paramNames[i], variables[i + 1]);
                    if (m.ReturnType != typeof(IEnumerator))
                        Editor.BBParameterEditor.ParameterField("Save Return Value", variables[0], true);
                }
            }
        }
