...
{
    [Name("Filter"), Category("Decorators")
     , Description(
    ...
    {
        public enum FilterMode { LimitNumberOfTimes = 0, CoolDown = 1 }
        public enum Policy { SuccessOrFailure, SuccessOnly, FailureOnly }
        ...
        [Name("Optional When Filtered")
         , Tooltip(
             "If enabled, the Filter Decorator will return an Optional status when it is filtered. Otherwise it will return Failure.")]
        ...
        protected override Status OnExecute(Component agent, IBlackboard blackboard)
        {
            ...
            switch (filterMode) {
                case FilterMode.CoolDown:
                    if (currentTime > 0) return inactiveWhenLimited ? Status.Optional : Status.Failure;
                    status = decoratedConnection.Execute(agent, blackboard);
                    if (status == Status.Success || status == Status.Failure) StartCoroutine(Cooldown());
                    break;
                case FilterMode.LimitNumberOfTimes:
                    if (executedCount >= maxCount.value) return inactiveWhenLimited ? Status.Optional : Status.Failure;
                    status = decoratedConnection.Execute(agent, blackboard);
                    if ((status == Status.Success && policy == Policy.SuccessOnly) ||
        ...
        protected override void OnNodeGUI()
        {
            if (filterMode == FilterMode.CoolDown) {
                ...
                UnityEditor.EditorGUI.ProgressBar(pRect, currentTime / coolDownTime.value
                    , currentTime > 0 ? "Cooling..." : "Cooled");
            }
            else if (filterMode == FilterMode.LimitNumberOfTimes) {
