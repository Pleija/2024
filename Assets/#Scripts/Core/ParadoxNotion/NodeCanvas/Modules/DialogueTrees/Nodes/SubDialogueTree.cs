...
namespace NodeCanvas.DialogueTrees
{
    ...
    public class SubDialogueTree : DTNodeNested<DialogueTree>, IUpdatable
    {
        ...
        {
            if (subGraph == null) return Error("No Sub Dialogue Tree assigned!");
            currentInstance = (DialogueTree)this.CheckInstance();
            this.TryWriteAndBindMappedVariables();
            TryWriteMappedActorParameters();
            currentInstance.StartGraph(
                finalActor is Component ? (Component)finalActor : finalActor.transform, bb.parent,
        ...
        private void TryWriteMappedActorParameters()
        {
            ...
            foreach (var pair in _actorParametersMap) {
                ...
                var sourceParam = DLGTree.GetParameterByID(pair.Value);
        ...
        protected override void OnNodeInspectorGUI()
        {
            ...
            if (subGraph != null) ShowActorParametersMapping();
        }
        ...
        private void ShowActorParametersMapping()
        {
            ...
            foreach (var param in subGraph.actorParameters) {
                ...
                var currentParam = DLGTree.GetParameterByID(_actorParametersMap[param.ID]);
                var newParam = EditorUtils.Popup<DialogueTree.ActorParameter>(param.name,
                    currentParam, DLGTree.actorParameters);
