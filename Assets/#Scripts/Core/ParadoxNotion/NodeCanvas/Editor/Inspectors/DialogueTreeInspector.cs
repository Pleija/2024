    ...
    public class DialogueTreeInspector : GraphInspector
    {
        ...
        public static void ShowActorParameters(DialogueTree dialogue)
        {
            ...
            if (GUILayout.Button("Add Actor Parameter")) {
                ...
                dialogue.actorParameters.Add(
                    new DialogueTree.ActorParameter("actor parameter name"));
                UndoUtility.SetDirty(dialogue);
            }
            EditorGUILayout.LabelField(DialogueTree.INSTIGATOR_NAME +
                " --> Replaced by the Actor starting the Dialogue");
            ...
            EditorUtils.ReorderableList(dialogue.actorParameters, options, (i, picked) => {
                ...
                if (dialogue.actorParameters.Where(r => r != reference).Select(r => r.name)
                ...
                var newReference = (Object)EditorGUILayout.ObjectField(reference.actor as Object,
                    typeof(Object), true);
                ...
                if (!ReferenceEquals(newReference, reference.actor)) {
                    ...
                    if (newReference == null) {
                        ...
                        if (newReference is Component)
                            newReference =
                                (newReference as Component).GetComponent(typeof(IDialogueActor));
                        if (newReference is GameObject)
                            newReference =
                                (newReference as GameObject).GetComponent(typeof(IDialogueActor));
                        if (newReference is IDialogueActor)
                            reference.actor = (IDialogueActor)newReference;
                        else
                            ParadoxNotion.Services.Logger.LogWarning(
                                "Object is not an IDialogueActor and none of the components attached is an IDialogueActor either.");
                    }
                    UndoUtility.SetDirty(dialogue);
                }
