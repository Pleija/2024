...
namespace FlowCanvas
{
    ...
    public abstract class Port
    {
        ...
        internal void EnsureCachedGUIContent()
        {
            ...
            if (IsFlowPort()) {
                ...
                if (IsDelegate()) text = string.Format("<b><color={0}>[{1}]</color></b>", hexColor, text);
                content.text = text;
                content.image = icon;
            }
        ...
        public bool IsUnitySceneObject() => typeof(Component).RTIsAssignableFrom(type) || type == typeof(GameObject);
    ...
    public abstract class ValueInput : Port
    {
        ...
        /// <summary>
        ///     Convenience method to set default port value and serialized port value, which should be the same value in any
        ///     case.
        /// </summary>
    ...
    /// <summary>
    ///     Value Input port for a known type. .value refers to either connected binded port if any, or the serialized
    ///     exposed value
    /// </summary>
    public class ValueInput<T> : ValueInput
    {
        ...
        /// <summary>
        ///     Convenience method to flag the value input as required to set which will show an error in UI (useful for
    ...
    public abstract class ValueOutput : Port
    {
        ...
        public static ValueOutput<T>
            CreateInstance<T>(FlowNode parent, string name, string ID, ValueHandler<T> getter) =>
            new ValueOutput<T>(parent, name, ID, getter);
