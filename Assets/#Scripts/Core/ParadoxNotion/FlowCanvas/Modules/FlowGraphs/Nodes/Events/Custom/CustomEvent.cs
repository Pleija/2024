...
namespace FlowCanvas.Nodes
{
    ...
    public class CustomEvent : RouterEventNode<GraphOwner>
    {
        ...
        protected override void RegisterPorts()
        {
            ...
            AddValueOutput<GraphOwner>("Receiver", () => {
                return receiver;
            });
            AddValueOutput<GraphOwner>("Sender", () => {
                return sender;
            });
        }
        ...
        private void OnCustomEvent(string eventName, ParadoxNotion.IEventData msg)
        {
            if (eventName.Equals(this.eventName.value, System.StringComparison.OrdinalIgnoreCase)) {
                ...
                Logger.Log(string.Format("Event Received from ({0}): '{1}'", receiver.name, eventName), LogTag.EVENT
                    , this);
