...
namespace FlowCanvas.Nodes
{
    ...
    [Name("Identity", 10), Category("Utility")
     , Description("Use this for organization. It returns exactly what is provided in the input."), ExposeAsDefinition]
    ...
    [Name("Cache", 9), Category("Utility"), Description("Caches the value only when the node is called.")
     , ExposeAsDefinition]
    ...
    public class RemapFloat : PureFunctionNode<float, float, float, float, float, float>
    {
        public override float Invoke(float current, float iMin, float iMax = 1f, float oMin = 0, float oMax = 100) =>
            Mathf.Lerp(oMin, oMax, Mathf.InverseLerp(iMin, iMax, current));
    }
    ...
    public class RemapVector3 : PureFunctionNode<Vector3, float, float, float, Vector3, Vector3>
    {
        public override Vector3 Invoke(float current, float iMin, float iMax, Vector3 oMin, Vector3 oMax) =>
            Vector3.Lerp(oMin, oMax, Mathf.InverseLerp(iMin, iMax, current));
    }
    ...
    [Category("Utility"), Description("Send a Local Event with 1 argument to specified graph"), ExposeAsDefinition]
    ...
    [Category("Utility"), Description("Send a Global Event with 1 argument to all graphs"), ExposeAsDefinition]
