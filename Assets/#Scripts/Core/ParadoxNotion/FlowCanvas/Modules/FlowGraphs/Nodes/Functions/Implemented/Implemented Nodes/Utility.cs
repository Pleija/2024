...
namespace FlowCanvas.Nodes
{
    [Category("Utility"),
     Description(
         "Custom cast input object to type T. Note that casts are already automatic at a connection-level. Use this only if necessary in your setup."),
     ExposeAsDefinition]
    public class Cast<T> : PureFunctionNode<T, object>
    {
        public override T Invoke(object obj) => (T)obj;
    }

    [Name("Identity", 10), Category("Utility"),
    ...
    [Name("Cache", 9), Category("Utility"),
     Description("Caches the value only when the node is called."), ExposeAsDefinition]
    ...
    [Category("Utility"),
     Description(
         "Remaps from input min/max to output min/max, by current value provided between input min/max"),
     Name("Remap To Float")]
    public class RemapFloat : PureFunctionNode<float, float, float, float, float, float>
    {
        public override float Invoke(float current, float iMin, float iMax = 1f, float oMin = 0,
            float oMax = 100) => Mathf.Lerp(oMin, oMax, Mathf.InverseLerp(iMin, iMax, current));
    }
    ...
    {
        public override Vector3 Invoke(float current, float iMin, float iMax, Vector3 oMin,
            Vector3 oMax) => Vector3.Lerp(oMin, oMax, Mathf.InverseLerp(iMin, iMax, current));
    }

    [Category("Utility"), Description("Log input value on the console")]
    public class LogValue : CallableActionNode<object>
    {
        public override void Invoke(object obj)
        {
            Logger.Log(obj, LogTag.EXECUTION, parentNode);
        }
    }

    [Category("Utility"), Description("Log text in the console")]
    public class LogText : CallableActionNode<string>
    {
        public override void Invoke(string text)
        {
            Logger.Log(text, LogTag.EXECUTION, parentNode);
        }
    }

    [Category("Utility"), Description("Send a Local Event to specified graph")]
    ...
    [Category("Utility"), Description("Send a Local Event with 1 argument to specified graph"),
     ExposeAsDefinition]
    ...
    [Category("Utility"), Description("Send a Global Event to all graphs")]
    ...
    [Category("Utility"), Description("Send a Global Event with 1 argument to all graphs"),
     ExposeAsDefinition]
