...
namespace FlowCanvas.Nodes
{
    [Category("Flow Controllers/Switchers")
     , Description(
         "Calls one output based on it's probability to be selected, in comparison to all other outputs. The probability of each output is respectively set by the same name float input.\nFor example, if all input values are the same (eg '1'), then all outputs have the same chance to be called.")
    ...
    public class SwitchProbability : FlowControlNode
    {
        ...
        protected override void RegisterPorts()
        {
            ...
            for (var i = 0; i < _portCount; i++) {
                probabilityValues.Add(AddValueInput<float>(StringUtils.GetAlphabetLetter(i), i.ToString())
                    .SetDefaultAndSerializedValue(1));
                probabilityOuts.Add(AddFlowOutput(StringUtils.GetAlphabetLetter(i), i.ToString()));
            }
            AddValueOutput<int>("Current", () => {
                return current;
            });
