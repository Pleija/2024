...
{
    [Category("Time"), Description("Wait for a certain amount of time before continueing")]
    ...
    [Category("Time"), Description("Wait for one (the next) frame")]
    ...
    [Category("Time"), Description("Wait until the physics update frame")]
    ...
    [Category("Time"), Description("Wait until the end of current frame")]
    ...
    [Category("Time"), Description("Wait Until condition value becomes true")]
    ...
    [Category("Time"), Description("Wait While condition value is true")]
    ...
    public class Buffer<T> : PureFunctionNode<T, T, int>
    {
        ...
        public override T Invoke(T value, int size = 60)
        {
            ...
            //init
            if (currentFrame - lastFrame > 1) q = new Queue<T>();
    ...
    public class DampVector3 : PureFunctionNode<Vector3, Vector3, Vector3, float>
    {
        public override Vector3 Invoke(Vector3 current, Vector3 target, float damp = 1f) =>
            Vector3.Lerp(current, target, damp * Time.deltaTime);
    }
}
