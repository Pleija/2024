...
{
    [ExposeAsDefinition, ContextDefinedInputs(typeof(Wild), typeof(string)),
        ...
        protected override void RegisterPorts()
        {
            ...
            for (var i = 0; i < stringCases.Count; i++)
                if (stringCases[i] != null)
                    cases[i] =
                        AddValueInput<T>(string.Format("Is \"{0}\"", stringCases[i].ToString()),
                            i.ToString());
            ...
            AddValueOutput<T>("Result", "Value", () => {
                ...
                if (selectorValue != null)
                    for (var i = 0; i < cases.Length; i++)
                        if (selectorValue.Equals(stringCases[i],
