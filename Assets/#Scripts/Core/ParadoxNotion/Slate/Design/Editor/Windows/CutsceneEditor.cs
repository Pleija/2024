...
namespace Slate
{
    ...
    {
        private enum EditorPlaybackState { Stoped, PlayingForwards, PlayingBackwards }
        ...
        private Color HIGHLIGHT_COLOR =>
            isProSkin ? new Color(0.65f, 0.65f, 1) : new Color(0.1f, 0.1f, 0.1f);

        private float MAGNET_SNAP_INTERVAL => viewTime * 0.01f;
        ...
        [System.NonSerialized]
        private EditorPlaybackState editorPlaybackState = EditorPlaybackState.Stoped;
        ...
        [System.NonSerialized]
        private CutsceneTrack pickedTrack;

        [System.NonSerialized]
        private CutsceneGroup pickedGroup;

        [System.NonSerialized]
        private float lastStartPlayTime;
        ...
        [System.NonSerialized]
        private bool isAboutButtonPressed;
        ...
        [System.NonSerialized]
        private float[] magnetSnapTimesCache;

        [System.NonSerialized]
        private List<GuideLine> pendingGuides;

        [System.NonSerialized]
        private System.Action postWindowsGUI;

        [System.NonSerialized]
        private CutsceneTrack copyTrack;

        [System.NonSerialized]
        private float timeInfoStart;

        [System.NonSerialized]
        private float timeInfoEnd;

        [System.NonSerialized]
        private float timeInfoInterval;

        [System.NonSerialized]
        private float timeInfoHighMod;
        ...
        public Cutscene cutscene {
            ...
            private set {
                _cutscene = value;
                if (value != null) _cutsceneID = value.GetInstanceID();
            }
        }
        ...
        //A white texture
        private static Texture2D whiteTexture => Styles.whiteTexture;

        //Is cutscene reference an asset in project?
        private bool isCutsceneAsset => cutscene != null && EditorUtility.IsPersistent(cutscene);
        ...
        private Color scruberColor => cutscene.isActive ? Color.yellow : new Color(1, 0.3f, 0.3f);
        ...
        private float TimeToPos(float time) => (time - viewTimeMin) / viewTime * centerRect.width;

        //Convert position to time
        private float PosToTime(float pos) =>
            (pos - LEFT_MARGIN) / centerRect.width * viewTime + viewTimeMin;
        ...
        private void SafeDoAction(System.Action call)
        {
        ...
        private bool IsFilteredOutBySearch(IDirectable directable, string search)
        {
        ...
        private void DrawGuideLine(float time, Color color)
        {
            if (time >= viewTimeMin && time <= viewTimeMax) {
                ...
                var guideRect = new Rect(xPos + centerRect.x - 1, centerRect.y, 2,
                    centerRect.height);
                GUI.color = color;
        ...
        private void DoPopup(System.Action call)
        {
            onDoPopup = call;
        }
        ...
        private void CacheMagnetSnapTimes(ActionClip clip = null)
        {
            ...
            foreach (var cw in clipWrappers) {
                ...
                if (clip == null ||
                    (action != clip && action.parent.parent == clip.parent.parent)) {
        ...
        {
            var window = GetWindow(typeof(CutsceneEditor)) as CutsceneEditor;
...
#pragma warning restore
...
#endif
            Tools.hidden = false;
            if (cutscene != null && !Application.isPlaying) Stop(true);
            current = null;
        }
        ...
        private void OnSelectionChange()
        {
            if (Selection.activeGameObject != null) {
                ...
                if (cut != null && cutscene != cut) InitializeAll(cut);
            }
        }

        //Before scene is saved we need to stop so that cutscene changes are reverted.
        private void OnWillSaveScene(UnityEngine.SceneManagement.Scene scene, string path)
        {
        ...
        private void InitializeAll(Cutscene newCutscene)
        {
            ...
            if (newCutscene != null) {
                ...
                multiSelection = null;
                InitClipWrappers();
                if (!Application.isPlaying) Stop(true);
            }
            willRepaint = true;
        }

        //initialize the action clip wrappers
        private void InitClipWrappers()
        {
            ...
            for (var g = 0; g < cutscene.groups.Count; g++) {
                for (var t = 0; t < cutscene.groups[g].tracks.Count; t++) {
                    for (var a = 0; a < cutscene.groups[g].tracks[t].clips.Count; a++) {
                        var id = UID(g, t, a);

                        if (clipWrappers.ContainsKey(id)) {
                            Debug.LogError(
                                "Collided UIDs. This should really not happen but it did!");
                            continue;
                        }
            ...
            if (lastTime > 0) cutscene.currentTime = lastTime;
        }
        ...
        public void Play(Cutscene.WrapMode wrapMode = Cutscene.WrapMode.Loop,
            System.Action callback = null)
        {
            ...
            if (Application.isPlaying) {
                ...
                cutscene.Play(0, length, cutscene.defaultWrapMode, callback,
                    Cutscene.PlayingDirection.Forwards);
        ...
        public void PlayReverse()
        {
            ...
            if (Application.isPlaying) {
                ...
                cutscene.Play(0, length, cutscene.defaultWrapMode, null,
                    Cutscene.PlayingDirection.Backwards);
                cutscene.currentTime = temp;
                return;
            }
        ...
        public void Stop(bool forceRewind)
        {
            ...
            cutscene.Sample(editorPlaybackState != EditorPlaybackState.Stoped && !forceRewind
                ? lastStartPlayTime : 0);
            editorPlaybackState = EditorPlaybackState.Stoped;
            willRepaint = true;
        }
        ...
        private void StepForward()
        {
            ...
            cutscene.currentTime = cutscene.GetPointerTimes()
                .FirstOrDefault(t => t > cutscene.currentTime + 0.01f);
        }
        ...
        private void StepBackward()
        {
            ...
            if (keyable != null) {
                var time =
                    keyable.animationData.GetKeyPrevious(keyable.RootTimeToLocalTimeUnclamped());
                cutscene.currentTime = time + keyable.startTime;
                return;
            }
            ...
            cutscene.currentTime = cutscene.GetPointerTimes()
                .LastOrDefault(t => t < cutscene.currentTime - 0.01f);
        }
        ...
        private void OnEditorUpdate()
        {
            ...
            if (cutscene == null || cutscene.isActive) return;
            ...
            //Nothing.
            if (editorPlaybackState == EditorPlaybackState.Stoped) return;

            //Playback.
            if (cutscene.currentTime >= length &&
                editorPlaybackState == EditorPlaybackState.PlayingForwards) {
            ...
            cutscene.currentTime += editorPlaybackState == EditorPlaybackState.PlayingForwards
                ? delta : -delta;
        }
        ...
        private void OnSceneGUI(SceneView sceneView)
        {
            ...
            if (cutscene.currentTime > 0 &&
                (cutscene.currentTime < cutscene.length || !Application.isPlaying)) {
                //view frame. Red = scrubbing, yellow = active in playmode
                var cam = sceneView.camera;
                var lineWidth = 3f;
                var top = new Rect(0, 0, cam.pixelWidth, lineWidth);
                var bottom = new Rect(0, cam.pixelHeight - lineWidth - 10, cam.pixelWidth,
                    lineWidth + 10);
        ...
        protected override void OnGUI()
        {
            ...
            if (doRecordUndo) {
                Undo.RegisterFullObjectHierarchyUndo(cutscene.groupsRoot.gameObject,
                    "Cutscene Change");
            ...
            topMiddleRect = new Rect(LEFT_MARGIN, TOOLBAR_HEIGHT,
                screenWidth - LEFT_MARGIN - RIGHT_MARGIN, TOP_MARGIN);
            leftRect = new Rect(0, TOOLBAR_HEIGHT + TOP_MARGIN, LEFT_MARGIN,
                screenHeight - TOOLBAR_HEIGHT - TOP_MARGIN + scrollPos.y);
            centerRect = new Rect(LEFT_MARGIN, TOP_MARGIN + TOOLBAR_HEIGHT,
                screenWidth - LEFT_MARGIN - RIGHT_MARGIN,
                screenHeight - TOOLBAR_HEIGHT - TOP_MARGIN + scrollPos.y);
            ...
            if (showDragDropInfo &&
                cutscene.groups.Find(g => g.GetType() == typeof(ActorGroup)) == null) {
                var label =
                    "Drag & Drop GameObjects or Prefabs in this window to create Actor Groups";
                var size = new GUIStyle("label").CalcSize(new GUIContent(label));
                var notificationRect = new Rect(0, 0, size.x, size.y);
                notificationRect.center = new Vector2(screenWidth / 2 + LEFT_MARGIN / 2,
                    screenHeight / 2 + TOP_MARGIN);
            ...
            if (willDirty) {
                ...
                foreach (var o in cutscene.GetComponentsInChildren(typeof(IDirectable), true)
                    .Cast<Object>())
            ...
            Handles.DrawLine(new Vector2(0, centerRect.y + 1),
                new Vector2(centerRect.xMax, centerRect.y + 1));
        ...
        private void DoKeyboardShortcuts()
        {
            ...
            if (e.type == EventType.KeyDown && GUIUtility.keyboardControl == 0 && !e.control &&
                !e.shift) {
                //play
        ...
        private void DrawGuides()
        {
            ...
            if (cutscene.currentTime > 0) DrawGuideLine(cutscene.currentTime, scruberColor);
            ...
            if (cutscene.isActive) {
                if (cutscene.playTimeMin > 0) DrawGuideLine(cutscene.playTimeMin, Color.red);
                if (cutscene.playTimeMax < length) DrawGuideLine(cutscene.playTimeMax, Color.red);
            }
        ...
        {
            if (cutscene.currentTime > 0) return;
            var e = Event.current;
                ...
                for (var i = 0; i < DragAndDrop.objectReferences.Length; i++) {
                    ...
                    if (o is GameObject) {
                        ...
                        if (go.GetComponent<DirectorCamera>() != null) {
                            ShowNotification(new GUIContent(
                                "The 'DIRECTOR' group is already used for the 'DirectorCamera' object"));
                            continue;
                        }

                        if (cutscene.GetAffectedActors().Contains(go)) {
                            ShowNotification(new GUIContent(
                                string.Format("GameObject '{0}' is already in the cutscene",
                                    o.name)));
                            continue;
                        }
        ...
        private void ShowToolbar()
        {
            ...
            if (GUILayout.Button(string.Format("[{0}]", cutscene.name),
                ...
                foreach (var cut in cutscenes)
                    menu.AddItem(new GUIContent(string.Format("[{0}]", cut.name)), cut == cutscene,
                        SelectCutscene, cut);
                menu.ShowAsContext();
            }
            ...
            if (GUILayout.Button("Snap: " + Prefs.snapInterval.ToString(),
                ...
                menu.AddItem(new GUIContent("24 FPS"), false, () => {
            ...
            Prefs.magnetSnapping = GUILayout.Toggle(Prefs.magnetSnapping,
                new GUIContent(Styles.magnetIcon, "Clips Magnet Snapping"),
                EditorStyles.toolbarButton);
            Prefs.rippleMode = GUILayout.Toggle(Prefs.rippleMode,
                new GUIContent(Styles.rippleIcon,
                    "Ripple moving of next clips and keyframes (Shift)"),
                EditorStyles.toolbarButton);
            Prefs.retimeMode = GUILayout.Toggle(Prefs.retimeMode,
                new GUIContent(Styles.retimeIcon, "Retiming of keyframes when scaling clips"),
                EditorStyles.toolbarButton);
            GUILayout.Space(10);
            ...
            if (!Prefs.autoKey) {
                ...
                if (hasChangedParams) {
                    ...
                    GUI.color = Color.white;
                    var b2 = GUILayout.Button(
                        string.Format("Key ({0}) Changed Parameters", changedParams.Count),
                        EditorStyles.toolbarButton);
                    GUI.backgroundColor = Color.white;
                    if (b1 || b2)
            ...
            if (GUILayout.Button(
                string.Format("SLATE Cinematic Sequencer v{0}",
                    Cutscene.VERSION_NUMBER.ToString("0.00")), EditorStyles.toolbarButton))
            ...
            isAboutButtonPressed = GUILayout.Toggle(isAboutButtonPressed, Styles.helpIcon,
                EditorStyles.toolbarButton);
            GUILayout.EndHorizontal();
            GUI.backgroundColor = Color.white;
            GUI.enabled = true;
        ...
        private void DoScrubControls()
        {
            ...
            if (e.type == EventType.MouseDown && topMiddleRect.Contains(mousePosition)) {
                ...
                if (e.button == 1 && isEndCarret && cutscene.directables != null) {
                    ...
                    menu.AddItem(new GUIContent("Set To Last Clip Time"), false, () => {
                        var lastClip = cutscene.directables.Where(d => d is ActionClip)
                            .OrderBy(d => d.endTime).LastOrDefault();
                        if (lastClip != null) length = lastClip.endTime;
                    });
                    menu.ShowAsContext();
                }
                e.Use();
            ...
            if (isMovingScrubCarret) {
                ...
                cutscene.currentTime = Mathf.Clamp(cutscene.currentTime,
                    Mathf.Max(viewTimeMin, 0) + float.Epsilon, length - float.Epsilon);
            }
            ...
            if (isMovingEndCarret) {
                ...
                length = Mathf.Clamp(length, viewTimeMin + float.Epsilon,
        ...
        private void DoZoomAndPan()
        {
            ...
            if ((e.type == EventType.ScrollWheel && Prefs.scrollWheelZooms) ||
                (e.alt && !e.shift && e.button == 1)) {
                AddCursorRect(centerRect, MouseCursor.Zoom);

                if (e.type == EventType.MouseDrag || e.type == EventType.MouseDown ||
                    e.type == EventType.MouseUp || e.type == EventType.ScrollWheel) {
        ...
        //top left controls
        private void ShowPlaybackControls(Rect topLeftRect)
        {
            var autoKeyRect = new Rect(topLeftRect.xMin + 10, topLeftRect.yMin + 4, 32, 32);
            AddCursorRect(autoKeyRect, MouseCursor.Link);
            GUI.backgroundColor = Prefs.autoKey ? Color.black.WithAlpha(0.5f)
                : Color.grey.WithAlpha(0.5f);
            ...
            if (GUI.Button(autoKeyRect, Styles.keyIcon, (GUIStyle)"box")) {
                ...
                ShowNotification(new GUIContent(
                    string.Format("AutoKey {0}", Prefs.autoKey ? "Enabled" : "Disabled"),
                    Styles.keyIcon));
            }
            ...
            if (GUILayout.Button(Styles.stepReverseIcon, (GUIStyle)"box", GUILayout.Width(20),
                GUILayout.Height(20))) {
                StepBackward();
                Event.current.Use();
            }
            ...
            var isStoped = Application.isPlaying ? cutscene.isPaused || !cutscene.isActive
                : editorPlaybackState == EditorPlaybackState.Stoped;
            ...
            if (isStoped) {
                ...
                if (GUILayout.Button(Styles.playIcon, (GUIStyle)"box", GUILayout.Width(20),
                    GUILayout.Height(20))) {
                    Play();
                    Event.current.Use();
                }
                ...
                if (GUILayout.Button(Styles.pauseIcon, (GUIStyle)"box", GUILayout.Width(44),
                    GUILayout.Height(20))) {
                    Pause();
                    Event.current.Use();
                }
            ...
            if (GUILayout.Button(Styles.stopIcon, (GUIStyle)"box", GUILayout.Width(20),
                GUILayout.Height(20))) {
                Stop(false);
                Event.current.Use();
            }
            ...
            if (GUILayout.Button(Styles.stepIcon, (GUIStyle)"box", GUILayout.Width(20),
                GUILayout.Height(20))) {
                StepForward();
                Event.current.Use();
            }
        ...
        private void ShowTimeInfo(Rect topMiddleRect)
        {
            ...
            timeInfoStart = (float)Mathf.FloorToInt(viewTimeMin / timeInfoInterval) *
                timeInfoInterval;
            ...
            GUI.BeginGroup(topMiddleRect);
            {
                ...
                if (centerRect.width / (viewTime / timeStep) > 6)
                    for (var i = timeInfoStart; i <= timeInfoEnd; i += timeStep) {
                        ...
                        var frameRect = Rect.MinMaxRect(posX - 1, TOP_MARGIN - 2, posX + 1,
                            TOP_MARGIN - 1);
                ...
                for (var i = timeInfoStart; i <= timeInfoEnd; i += timeInfoInterval) {
                    ...
                    GUI.color = isProSkin ? Color.white : Color.black;
                    var markRect =
                        Rect.MinMaxRect(posX - 2, TOP_MARGIN - 3, posX + 2, TOP_MARGIN - 1);
                    GUI.DrawTexture(markRect, whiteTexture);
                    GUI.color = Color.white;
                    var text = doFrames ? (rounded * Prefs.frameRate).ToString("0")
                    ...
                    GUI.color = rounded % timeInfoHighMod == 0 ? Color.white
                ...
                if (cutscene.currentTime > 0) {
                    var label = doFrames ? (cutscene.currentTime * Prefs.frameRate).ToString("0")
                    ...
                    GUI.backgroundColor = isProSkin ? Color.black.WithAlpha(0.4f)
        ...
        private void ShowGroupsAndTracksList(Rect leftRect)
        {
            var e = Event.current;
            ...
            if (e.type == EventType.MouseDown && e.button == 0 &&
                scaleRect.Contains(e.mousePosition)) {
            ...
            GUI.enabled = true;
            var collapseAllRect =
                Rect.MinMaxRect(leftRect.x + 5, leftRect.y + 4, 20, leftRect.y + 20 - 1);
            var searchRect = Rect.MinMaxRect(leftRect.x + 20, leftRect.y + 4, leftRect.xMax - 18,
                leftRect.y + 20 - 1);
            var searchCancelRect = Rect.MinMaxRect(searchRect.xMax, searchRect.y, leftRect.xMax - 4,
                searchRect.yMax);
            ...
            var addRect = Rect.MinMaxRect(leftRect.xMin + 10, addButtonY, leftRect.xMax - 10,
                addButtonY + 20);
            GUI.color = Color.white.WithAlpha(0.5f);
        ...
        private void ShowListGroups(Event e, ref float nextYPos)
        {
            ...
            for (var g = 0; g < cutscene.groups.Count; g++) {
                ...
                var groupRect = new Rect(4, nextYPos, leftRect.width - GROUP_RIGHT_MARGIN - 4,
                    GROUP_HEIGHT - 3);
                AddCursorRect(groupRect,
                    pickedGroup == null ? MouseCursor.Link : MouseCursor.MoveArrow);
                nextYPos += GROUP_HEIGHT;

                //highligh?
                var groupSelected = ReferenceEquals(group, CutsceneUtility.selectedObject) ||
                    group == pickedGroup;
                ...
                if (group.isLocked) {
                    var lockIconRect = new Rect(plusRect.xMin - (group.isActive ? 20 : 36),
                        groupRect.y + 1, 16, 16);
                ...
                var isVirtual = group.referenceMode ==
                    CutsceneGroup.ActorReferenceMode.UseInstanceHideOriginal;
                group.isCollapsed = !EditorGUI.Foldout(foldRect, !group.isCollapsed,
                    string.Format("<b>{0} {1}</b>", group.name,
                        isVirtual ? "(Ref)" : string.Empty));
                GUI.color = Color.white;

                //Actor Object Field
                if (group.actor == null) {
                    var oRect = Rect.MinMaxRect(groupRect.xMin + 20, groupRect.yMin + 1,
                        groupRect.xMax - 20, groupRect.yMax - 1);
                    group.actor = (GameObject)EditorGUI.ObjectField(oRect, group.actor,
                ...
                if ((e.type == EventType.ContextClick && groupRect.Contains(e.mousePosition)) ||
                    ...
                    foreach (var _info in
                        EditorTools.GetTypeMetaDerivedFrom(typeof(CutsceneTrack))) {
                        var info = _info;
                        if (info.attachableTypes == null ||
                            !info.attachableTypes.Contains(group.GetType()))
                            continue;
                        var canAdd = !info.isUnique ||
                            group.tracks.Find(track => track.GetType() == info.type) == null;
                        var finalPath = string.IsNullOrEmpty(info.category) ? info.name
                    ...
                    if (group.CanAddTrack(copyTrack))
                        menu.AddItem(new GUIContent("Paste Track"), false, () => {
                            group.DuplicateTrack(copyTrack);
                        });
                    else
                    ...
                    if (!(group is DirectorGroup)) {
                        ...
                        menu.AddItem(new GUIContent("Replace Actor"), false, () => {
                            group.actor = null;
                        });
                        ...
                        menu.AddItem(new GUIContent("Delete Group"), false, () => {
                            if (EditorUtility.DisplayDialog("Delete Group", "Are you sure?", "YES",
                                "NO!")) {
                ...
                //REORDERING
                if (e.type == EventType.MouseDown && e.button == 0 &&
                    groupRect.Contains(e.mousePosition)) {
                    CutsceneUtility.selectedObject = group;
                    if (!(group is DirectorGroup)) pickedGroup = group;
                    if (e.clickCount == 2) Selection.activeGameObject = group.actor;
                    e.Use();
                }
                ...
                if (pickedGroup != null && pickedGroup != group && !(group is DirectorGroup)) {
                    if (groupRect.Contains(e.mousePosition)) {
                        var markRect = new Rect(groupRect.x,
                            cutscene.groups.IndexOf(pickedGroup) < g ? groupRect.yMax - 2
                ...
                if (!group.isCollapsed) {
                    ...
                    GUI.color = groupSelected ? LIST_SELECTION_COLOR : GROUP_COLOR;
                    var verticalRect = Rect.MinMaxRect(groupRect.x, groupRect.yMax, groupRect.x + 3,
                        nextYPos - 2);
        ...
        //...
        private void ShowListTracks(Event e, CutsceneGroup group, ref float nextYPos)
        {
            //TRACKS
            for (var t = 0; t < group.tracks.Count; t++) {
                var track = group.tracks[t];
                var yPos = nextYPos;
                var trackRect = new Rect(10, yPos, leftRect.width - TRACK_RIGHT_MARGIN - 10,
                    track.finalHeight);
                nextYPos += track.finalHeight + TRACK_MARGINS;

                //GRAPHICS
                GUI.color = ColorUtility.Grey(isProSkin ? track.isActive ? 0.25f : 0.2f :
                ...
                if (ReferenceEquals(track, CutsceneUtility.selectedObject) ||
                    track == pickedTrack) {
                ...
                if (track.isActive && track.color != Color.white && track.color.a > 0.2f) {
                    ...
                    var colorRect = new Rect(trackRect.xMax + 1, trackRect.yMin, 2,
                        track.finalHeight);
                ...
                if (e.type == EventType.ContextClick && trackRect.Contains(e.mousePosition)) {
                    ...
                    menu.AddItem(new GUIContent("Delete Track"), false, () => {
                        if (EditorUtility.DisplayDialog("Delete Track", "Are you sure?", "YES",
                            "NO!")) {
                ...
                //REORDERING
                if (e.type == EventType.MouseDown && e.button == 0 &&
                    trackRect.Contains(e.mousePosition)) {
                ...
                if (pickedTrack != null && pickedTrack != track &&
                    ...
                    if (trackRect.Contains(e.mousePosition)) {
                        var markRect = new Rect(trackRect.x,
                            group.tracks.IndexOf(pickedTrack) < t ? trackRect.yMax - 2
                                : trackRect.y, trackRect.width, 2);
                        GUI.color = Color.grey;
                        GUI.DrawTexture(markRect, Styles.whiteTexture);
                        GUI.color = Color.white;
                    }
        ...
        private void ShowTimeLines(Rect centerRect)
        {
            ...
            //bg graphic
            var bgRect = Rect.MinMaxRect(centerRect.xMin, TOP_MARGIN + TOOLBAR_HEIGHT + scrollPos.y,
                centerRect.xMax, screenHeight - TOOLBAR_HEIGHT + scrollPos.y);
            GUI.color = Color.black.WithAlpha(0.1f);
            GUI.DrawTexture(bgRect, whiteTexture);
            GUI.color = Color.black.WithAlpha(0.03f);
            GUI.DrawTextureWithTexCoords(bgRect, Styles.stripes,
                new Rect(0, 0, bgRect.width / -7, bgRect.height / -7));
            GUI.color = Color.white;
            ...
            for (var _i = timeInfoStart; _i <= timeInfoEnd; _i += timeInfoInterval) {
                ...
                if (i % timeInfoHighMod == 0) DrawGuideLine(i, Color.black.WithAlpha(0.05f));
            }
            ...
            for (var g = 0; g < cutscene.groups.Count; g++) {
                ...
                var groupRect = Rect.MinMaxRect(Mathf.Max(TimeToPos(viewTimeMin), TimeToPos(0)),
                    nextYPos, TimeToPos(viewTimeMax), nextYPos + GROUP_HEIGHT);
                nextYPos += GROUP_HEIGHT;

                //if collapsed, just show a heat minimap of clips.
                if (group.isCollapsed) {
                    GUI.color = Color.black.WithAlpha(0.15f);
                    var collapseRect = Rect.MinMaxRect(groupRect.xMin + 2, groupRect.yMin + 2,
                        groupRect.xMax, groupRect.yMax - 4);
                    ...
                    foreach (var track in group.tracks) {
                        foreach (var clip in track.clips) {
                            ...
                            GUI.DrawTexture(
                                Rect.MinMaxRect(start + 0.5f, collapseRect.y + 2, end - 0.5f,
                                    collapseRect.yMax - 2), Styles.whiteTexture);
                        }
                    }
                    GUI.color = Color.white;
                    continue;
                ...
                for (var t = 0; t < group.tracks.Count; t++) {
                    ...
                    var trackPosRect =
                        Rect.MinMaxRect(
                            Mathf.Max(TimeToPos(viewTimeMin), TimeToPos(track.startTime)), yPos,
                            TimeToPos(viewTimeMax), yPos + track.finalHeight);
                    var trackTimeRect = Rect.MinMaxRect(Mathf.Max(viewTimeMin, track.startTime), 0,
                        viewTimeMax, 0);
                    nextYPos += track.finalHeight + TRACK_MARGINS;
                    ...
                    if (viewTimeMin < 0) //just visual clarity
                        GUI.Box(
                            Rect.MinMaxRect(TimeToPos(viewTimeMin), trackPosRect.yMin, TimeToPos(0),
                                trackPosRect.yMax), string.Empty);

                    if (track.startTime > track.parent.startTime ||
                        track.endTime < track.parent.endTime) {
                        Handles.color = Color.white;
                        GUI.color = Color.black.WithAlpha(0.2f);

                        if (track.startTime > track.parent.startTime) {
                            var tStart = TimeToPos(track.startTime);
                            var r = Rect.MinMaxRect(TimeToPos(0), yPos, tStart,
                                yPos + track.finalHeight);
                            GUI.DrawTexture(r, whiteTexture);
                            GUI.DrawTextureWithTexCoords(r, Styles.stripes,
                                new Rect(0, 0, r.width / 7, r.height / 7));
                        ...
                        if (track.endTime < track.parent.endTime) {
                            ...
                            var r = Rect.MinMaxRect(tEnd, yPos, TimeToPos(length),
                                yPos + track.finalHeight);
                            GUI.DrawTexture(r, whiteTexture);
                            GUI.DrawTextureWithTexCoords(r, Styles.stripes,
                                new Rect(0, 0, r.width / 7, r.height / 7));
                    ...
                    if (ReferenceEquals(CutsceneUtility.selectedObject, track)) {
                        GUI.color = Color.grey;
                        GUI.Box(trackPosRect.ExpandBy(0, 2), string.Empty,
                            Styles.hollowFrameHorizontalStyle);
                        GUI.color = Color.white;
                    }
                    //
                    if (track.isLocked)
                        ...
                        postWindowsGUI += () => {
                            ...
                            if (!track.isActive) {
                                ...
                                GUI.DrawTextureWithTexCoords(trackPosRect, Styles.stripes,
                                    new Rect(0, 0, trackPosRect.width / 5,
                                        trackPosRect.height / 5));
                                GUI.color = Color.white;
                            }
                            ...
                            if (track.isLocked) {
                                GUI.color = Color.black.WithAlpha(0.15f);
                                GUI.DrawTextureWithTexCoords(trackPosRect, Styles.stripes,
                                    new Rect(0, 0, trackPosRect.width / 20,
                                        trackPosRect.height / 20));
                                GUI.color = Color.white;
                            }
                            ...
                            if (isProSkin) {
                                ...
                                var size =
                                    Styles.centerLabel.CalcSize(new GUIContent(overlayLabel));
                                var bgLabelRect = new Rect(0, 0, size.x, size.y);
                                bgLabelRect.center = trackPosRect.center;
                                GUI.Label(trackPosRect, string.Format("<b>{0}</b>", overlayLabel),
                                    Styles.centerLabel);
                                GUI.color = Color.white;
                            }
                        };
                    ...
                    for (var a = 0; a < track.clips.Count; a++) {
                        ...
                        if (interactingClip != null &&
                            ReferenceEquals(interactingClip.action, action) &&
                            ...
                            if (multiSelection != null && multiSelection.Count > 1) {
                                ...
                                var boundMin = Mathf.Min(multiSelection
                                    .Select(b => b.action.startTime).ToArray());
                            ...
                            if (multiSelection == null || multiSelection.Count < 1) {
                                var preCursorClip = track.clips.LastOrDefault(x =>
                                    x != action && x.startTime < cursorTime);
                                var postCursorClip = track.clips.FirstOrDefault(x =>
                                    x != action && x.endTime > cursorTime);
                                ...
                                var postTime = postCursorClip != null ? postCursorClip.startTime
                                ...
                                if (Prefs.magnetSnapping && !e.control) {
                                    ...
                                    var snapEnd = MagnetSnapTime(xTime + action.length,
                                        magnetSnapTimesCache);

                                    if (snapStart != null && snapEnd != null) {
                                        var distStart = Mathf.Abs(snapStart.Value - xTime);
                                        var distEnd =
                                            Mathf.Abs(snapEnd.Value - (xTime + action.length));
                                        var bestTime = distEnd < distStart ? snapEnd.Value
                                            : snapStart.Value;
                                        pendingGuides.Add(new GuideLine(bestTime, Color.white));
                                        xTime = distEnd < distStart ? snapEnd.Value - action.length
                                            : snapStart.Value;
                                    }
                                    else {
                                        if (snapEnd != null) {
                                            pendingGuides.Add(new GuideLine(snapEnd.Value,
                                                Color.white));
                                            xTime = snapEnd.Value - action.length;
                                        }

                                        if (snapStart != null) {
                                            pendingGuides.Add(new GuideLine(snapStart.Value,
                                                Color.white));
                                ...
                                if (action.CanCrossBlend(preCursorClip))
                                    preTime -= Mathf.Min(action.length / 2,
                                        preCursorClip.length / 2);
                                if (action.CanCrossBlend(postCursorClip))
                                    postTime += Mathf.Min(action.length / 2,
                                        postCursorClip.length / 2);
                                ...
                                if (xTime != lastTime) {
                                    ...
                                    if (e.shift || Prefs.rippleMode)
                                        foreach (var cw in clipWrappers.Values.Where(c =>
                                            c.action.parent == action.parent &&
                                            c.action != action && c.action.startTime > lastTime))
                        ...
                        var isVisible = Rect
                            .MinMaxRect(0, scrollPos.y, centerRect.width, centerRect.height)
                            .Overlaps(clipRect);

                        if (!isSelected && !isVisible) {
                            clipWrapper.rect =
                                default; //we basicaly "nullify" the rect. Too much trouble to work with nullable rect.
                            continue;
                        }
                        ...
                        clipWrapper.rect = GUI.Window(ID, clipRect, ActionClipWindow, string.Empty,
                            GUIStyle.none);
                        ...
                        var nextPosX =
                            TimeToPos(nextClip != null ? nextClip.startTime : viewTimeMax);
                        var prevPosX = TimeToPos(previousClip != null ? previousClip.endTime
                            : viewTimeMin);
                        var extRectLeft = Rect.MinMaxRect(prevPosX, clipRect.yMin, clipRect.xMin,
                            clipRect.yMax);
                        var extRectRight = Rect.MinMaxRect(clipRect.xMax, clipRect.yMin, nextPosX,
                            clipRect.yMax);
                        action.ShowClipGUIExternal(extRectLeft, extRectRight);

                        //draw info text outside if clip is too small
                        if (clipRect.width <= 20)
                            GUI.Label(extRectRight,
                                string.Format("<size=9>{0}</size>", action.info));
                    }
                }

                //highligh selected group
                if (ReferenceEquals(CutsceneUtility.selectedObject, group)) {
                    var r = Rect.MinMaxRect(groupRect.xMin, groupRect.yMin, groupRect.xMax,
                        nextYPos);
                    GUI.color = Color.grey;
            ...
            if (viewTimeMax > length) {
                ...
                var darkRect = Rect.MinMaxRect(endPos, centerRect.yMin, centerRect.xMax,
                    centerRect.yMax);
            ...
            if (viewTimeMin < 0) {
                ...
                var darkRect = Rect.MinMaxRect(centerRect.xMin, centerRect.yMin, startPos,
                    centerRect.yMax);
                GUI.color = Color.black.WithAlpha(0.3f);
                GUI.Box(darkRect, string.Empty, (GUIStyle)"TextField");
                GUI.color = Color.white;
        ...
        //Group sections...
        private void ShowGroupSections(CutsceneGroup group, Rect rect)
        {
            var e = Event.current;
            ...
            for (var i = 0; i < sections.Count - 1; i++) {
                ...
                var markRect = new Rect(sectionRect.x + 2, sectionRect.y + 2, 2,
                    sectionRect.height - 4);
                var clickRect = new Rect(0, y, 15, sectionRect.height);
                var loopRect = Rect.MinMaxRect(Mathf.Max(sectionRect.xMax - 18, sectionRect.xMin),
                    sectionRect.yMin + 2, sectionRect.xMax - 2, sectionRect.yMax - 2);
                ...
                GUI.DrawTexture(markRect, whiteTexture);
                GUI.color =
                    (section1.color.grayscale >= 0.5 ? Color.black : Color.white).WithAlpha(0.5f);
                ...
                if (section1.exitMode == Section.ExitMode.Loop) {
                    ...
                    if (section1.loopCount > 0) {
                        var text = string.Format("<size=9>x {0}/{1}</size>",
                            Mathf.Min(section1.currentLoopIteration, section1.loopCount),
                            section1.loopCount);
                        var loopCountRect = Rect.MinMaxRect(sectionRect.xMin, sectionRect.yMin,
                            loopRect.xMin - 2, sectionRect.yMax);
                    ...
                    if (i != 0 && e.type == EventType.ContextClick && sectionsMenu != null) {
                        sectionsMenu.AddItem(new GUIContent("Edit"), false, () => {
                            DoPopup(() => {
                                ...
                                var previousSectionTime = sections.Last(s =>
                                    s.time < section1.time && s != section1).time;
                                var nextSectionTime = sections.First(s =>
                                    s.time > section1.time && s != section1).time;
                                section1.time = EditorGUILayout.Slider("Time", section1.time,
                                    previousSectionTime + 0.1f, nextSectionTime - 0.1f);
                                section1.exitMode =
                                    (Section.ExitMode)EditorGUILayout.EnumPopup("Exit Mode",
                                        section1.exitMode);
                                if (section1.exitMode == Section.ExitMode.Loop)
                                    section1.loopCount =
                                        EditorGUILayout.IntField("Loops", section1.loopCount);
                                section1.color =
                                    EditorGUILayout.ColorField("Color", section1.color);
                                section1.colorizeBackground =
                                    EditorGUILayout.Toggle("Colorize Background",
                                        section1.colorizeBackground);
                            });
                        });
                ...
                if (i != 0 && clickRect.Contains(e.mousePosition)) {
                    AddCursorRect(clickRect, MouseCursor.SlideArrow);
            ...
            if (draggedSection != null) {
                ...
                if (e.shift || e.control || Prefs.rippleMode)
                    foreach (var section in group.sections.Where(s =>
                        s != draggedSection && s.time > lastTime))
            ...
            if (sectionsMenu != null) sectionsMenu.ShowAsContext();
        }
        ...
        private void DoMultiSelection()
        {
            ...
            if (multiSelectStartPos != null) {
                ...
                if ((start - e.mousePosition).magnitude > 10) {
                    ...
                    foreach (var wrapper in clipWrappers.Values.Where(b =>
                        r.Overlaps(b.rect) && !b.action.isLocked)) {
            ...
            if (e.rawType == EventType.MouseUp) {
                if (bigEnough) {
                    multiSelection = clipWrappers.Values
                        .Where(b => r.Overlaps(b.rect) && !b.action.isLocked).ToList();
            ...
            if (multiSelection != null) {
                var boundRect = RectUtility
                    .GetBoundRect(multiSelection.Select(b => b.rect).ToArray()).ExpandBy(4);
                var leftDragRect =
                    new Rect(boundRect.xMin - 6, boundRect.yMin, 4, boundRect.height);
                var rightDragRect =
                    new Rect(boundRect.xMax + 2, boundRect.yMin, 4, boundRect.height);
                ...
                if (e.type == EventType.MouseDown && (leftDragRect.Contains(e.mousePosition) ||
                    rightDragRect.Contains(e.mousePosition))) {
                    multiSelectionScaleDirection = leftDragRect.Contains(e.mousePosition) ? -1 : 1;
                    var minTime =
                        Mathf.Min(multiSelection.Select(b => b.action.startTime).ToArray());
                ...
                if (e.type == EventType.MouseDrag && multiSelectionScaleDirection != 0) {
                    foreach (var clipWrapper in multiSelection) {
                        ...
                        var lerpMin = multiSelectionScaleDirection == -1
                            ? Mathf.Clamp(pointerTime, 0, preTimeMax) : preTimeMin;
                        var lerpMax = multiSelectionScaleDirection == 1
                            ? Mathf.Max(pointerTime, preTimeMin) : preTimeMax;
                        var normIn = Mathf.InverseLerp(preTimeMin, preTimeMax,
                            clipWrapper.preScaleStartTime);
                        clipWrapper.action.startTime = Mathf.Lerp(lerpMin, lerpMax, normIn);
                        var normOut = Mathf.InverseLerp(preTimeMin, preTimeMax,
                            clipWrapper.preScaleEndTime);
        ...
        private void ShowWelcome()
        {
            ...
            GUI.DrawTextureWithTexCoords(bgRect, Styles.stripes,
                new Rect(0, 0, bgRect.width / -7, bgRect.height / -7));
            GUI.color = Color.white;
            if (cutscene == null) isAboutButtonPressed = false;
            var label = string.Format("<size=24><b>{0}</b></size>",
                "Welcome to SLATE Cinematic Sequencer!");
            var size = new GUIStyle("label").CalcSize(new GUIContent(label));
            ...
            GUI.Label(new Rect(iconRect.x + 68, iconRect.yMax - 42, iconRect.width, 40),
                "v" + Cutscene.VERSION_NUMBER, Styles.leftLabel);
            GUI.color = Color.white;
            var boardRect = Rect.MinMaxRect(iconRect.xMax, iconRect.y + 10, titleRect.xMax,
                iconRect.yMax - 10);
            GUI.color = Color.black.WithAlpha(0.2f);
            GUI.Box(boardRect, string.Empty);
            GUI.color = Color.white;
            GUI.Label(boardRect.ExpandBy(-5), webMessage);
            var buttonsRect = Rect.MinMaxRect(titleRect.xMin, titleRect.yMax + 5, titleRect.xMax,
                screenHeight);
            ...
            if (isAboutButtonPressed && cutscene != null) {
                ...
                GUI.backgroundColor = new Color(0.8f, 0.8f, 1, 1f);
                if (GUI.Button(backRect, "Close Panel")) isAboutButtonPressed = false;
                GUI.backgroundColor = Color.white;
            }
        }

        //...
        private void FetchWebMessageBoard()
        {
                    ...
                    foreach (var board in boards) {
                        ...
                        if (isAll || target.ToLower() == "slate") {
                            if (!isAll) {
                                ...
                                result += uptodate
                                    ? "<b>You are up to date on the latest version!</b>"
                                    : string.Format(
                                        "<b>There is a new version available! ( v{0} )</b>",
                                        version);
                            }
        ...
        private void ActionClipWindow(int id)
        {
            ActionClipWrapper wrapper = null;
            if (clipWrappers.TryGetValue(id, out wrapper)) wrapper.OnClipGUI(id);
        }

        ///----------------------------------------------------------------------------------------------

        //A wrapper of an ActionClip placed in cutscene
        private class ActionClipWrapper
        {
            private const float CLIP_DOPESHEET_HEIGHT = 13f;
            ...
            private CutsceneEditor editor => current;
            ...
            public ActionClipWrapper(ActionClip action) => this.action = action;
            ...
            public void OnClipGUI(int windowID)
            {
                ...
                e = Event.current;
                overlapIn = previousClip != null
                    ? Mathf.Max(previousClip.endTime - action.startTime, 0) : 0;
                overlapOut = nextClip != null ? Mathf.Max(action.endTime - nextClip.startTime, 0)
                    : 0;
                ...
                allowScale = action.CanScale() && action.length > 0 &&
                    rect.width > SCALE_RECT_WIDTH * 2;
                dragRect =
                    new Rect(0, 0, rect.width,
                ...
                if (hasActiveParameters && action.length > 0) ShowClipDopesheet(wholeRect);
                //...
                ...
                if (e.type == EventType.MouseDown) {
                    ...
                    if (e.control && dragRect.Contains(e.mousePosition)) {
                        ...
                        if (multiSelection != null &&
                            !multiSelection.Select(cw => cw.action).Contains(action))
                            multiSelection = null;
                    }
                    if (e.clickCount == 2)
                        ...
                        Selection.activeObject =
                            action.GetType().GetProperty("actor").GetValue(action, null) as Object;
                }
                if (e.type == EventType.MouseDrag && isWaitingMouseDrag) isDragging = true;
                if (e.rawType == EventType.ContextClick) DoClipContextMenu();
                DrawBlendGraphics();
                DoEdgeControls();
                ...
                if (e.button == 0) GUI.DragWindow(dragRect);
            ...
            private void DrawBlendGraphics()
            {
                if (action.blendIn > 0) {
                    ...
                    Handles.DrawAAPolyLine(2, new Vector2(0, rect.height),
                        new Vector2(blendInPosX, 0));
                ...
                if (action.blendOut > 0 && overlapOut == 0) {
                    Handles.color = Color.black.WithAlpha(0.5f);
                    Handles.DrawAAPolyLine(2, new Vector2(blendOutPosX, 0),
                        new Vector2(rect.width, rect.height));
                    Handles.color = Color.black.WithAlpha(0.3f);
                    Handles.DrawAAConvexPolygon(new Vector3(rect.width, 0),
                        new Vector2(blendOutPosX, 0), new Vector2(rect.width, rect.height));
                }

                if (overlapIn > 0) {
                    Handles.color = Color.black;
                    Handles.DrawAAPolyLine(2, new Vector2(blendInPosX, 0),
                        new Vector2(blendInPosX, rect.height));
                }
                Handles.color = Color.white;
            }
            ...
            private void DoEdgeControls()
            {
                ...
                if (!isScalingStart && !isScalingEnd && !isControlingBlendIn &&
                    !isControlingBlendOut) {
                ...
                if (isControlingBlendIn)
                    action.blendIn = Mathf.Clamp(pointerTime - action.startTime, 0,
                        action.length - action.blendOut);
                if (isControlingBlendOut)
                    action.blendOut = Mathf.Clamp(action.endTime - pointerTime, 0,
                        action.length - action.blendIn);
                ...
                if (isScalingStart) {
                    ...
                    //magnet snap
                    if (Prefs.magnetSnapping && !e.control) {
                        var snapStart = editor.MagnetSnapTime(snapedPointerTime,
                            editor.magnetSnapTimesCache);
                ...
                if (isScalingEnd) {
                    ...
                    //magnet snap
                    if (Prefs.magnetSnapping && !e.control) {
                        var snapEnd = editor.MagnetSnapTime(snapedPointerTime,
                            editor.magnetSnapTimesCache);
                ...
                foreach (var curve in action.GetCurvesAll()) {
                    for (var i = 0; i < curve.keys.Length; i++) {
                        ...
                        if (retime) {
                            ...
                            var newTime = Mathf.LerpUnclamped(0, action.length,
                                preKey.time / preLength);
                            preKey.time = newTime;
                        }
                        if (trim) preKey.time -= action.startTime - preScaleStartTime;
            ...
            public void StretchFit()
            {
                ...
                var targetStart = previousClip != null ? previousClip.endTime
                    : action.parent.startTime;
                ...
                if (hasParameters) {
                    foreach (var curve in action.GetCurvesAll()) curve.OffsetCurveTime(delta);
                    CutsceneUtility.RefreshAllAnimationEditorsOf(action.animationData);
                }
                if (action is ISubClipContainable)
            ...
            /// <summary>
            ///     Clean keys off clip range after adding a key at 0 and length if there is any key outside
            ///     that range
            /// </summary>
            public void CleanKeysOffRange()
            {
                if (hasParameters) {
                    foreach (var param in action.animationData.animatedParameters)
                        if (param.HasAnyKey()) {
                            if (param.GetKeyPrevious(0) < 0) param.TryKeyIdentity(0);
            ...
            private void ShowClipDopesheet(Rect rect)
            {
                var dopeRect = new Rect(0, rect.height - CLIP_DOPESHEET_HEIGHT, rect.width,
                    CLIP_DOPESHEET_HEIGHT);
                GUI.color = isProSkin ? new Color(0, 0.2f, 0.2f, 0.5f)
                    : new Color(0, 0.8f, 0.8f, 0.5f);
                GUI.Box(dopeRect, string.Empty, Styles.clipBoxHorizontalStyle);
                GUI.color = Color.white;
                DopeSheetEditor.DrawDopeSheet(action.animationData, action, dopeRect, 0,
                    action.length, false);
            }

            //CONTEXT
            private void DoClipContextMenu()
            {
                var menu = new GenericMenu();
                ...
                if (allowScale) {
                    ...
                    if (action.length > 0) {
                        menu.AddItem(new GUIContent("Split At Cursor"), false, () => {
                            Split(snapedPointerTime);
                        });
                ...
                if (hasActiveParameters) {
                    ...
                    menu.AddItem(new GUIContent("Remove Animation"), false, () => {
                        if (EditorUtility.DisplayDialog("Remove Animation",
                            ...
                            "Yes", "No"))
                            editor.SafeDoAction(() => {
                                action.ResetAnimatedParameters();
                            });
                    });
                }
