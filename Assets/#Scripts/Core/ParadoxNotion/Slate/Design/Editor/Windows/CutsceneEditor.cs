...
namespace Slate
{
    public class CutsceneEditor : OdinEditorWindow
    {
        ...
        private Color HIGHLIGHT_COLOR => isProSkin ? new Color(0.65f, 0.65f, 1) : new Color(0.1f, 0.1f, 0.1f);
        ...
        public Cutscene cutscene {
            get {
                if (_cutscene == null) _cutscene = EditorUtility.InstanceIDToObject(_cutsceneID) as Cutscene;
                return _cutscene;
            }
            private set {
                _cutscene = value;
        ...
        private float PosToTime(float pos) => (pos - LEFT_MARGIN) / centerRect.width * viewTime + viewTimeMin;
        ...
        private void CacheMagnetSnapTimes(ActionClip clip = null)
        {
            ...
            if (cutscene.directorGroup != null) result.AddRange(cutscene.directorGroup.sections.Select(s => s.time));
        ...
        protected override void OnEnable()
        {
            ...
            UnityEditor.Experimental.SceneManagement.PrefabStage.prefabStageClosing += (stage) => {
                if (cutscene != null && stage.IsPartOfPrefabContents(cutscene.gameObject)) Stop(true);
            };
#endif
...
#pragma warning restore
        ...
        protected override void OnGUI()
        {
            ...
            if (interactingClip == null && e.type == EventType.Layout)
                foreach (var group in cutscene.groups) {
                    foreach (var track in group.tracks) track.clips = track.clips.OrderBy(a => a.startTime).ToList();
                }
        ...
        private void ShowToolbar()
        {
            ...
            if (GUILayout.Button("Render", EditorStyles.toolbarButton, GUILayout.Width(60))) RenderWindow.Open();
            ...
            Prefs.magnetSnapping = GUILayout.Toggle(Prefs.magnetSnapping
                , new GUIContent(Styles.magnetIcon, "Clips Magnet Snapping"), EditorStyles.toolbarButton);
            Prefs.rippleMode = GUILayout.Toggle(Prefs.rippleMode
                , new GUIContent(Styles.rippleIcon, "Ripple moving of next clips and keyframes (Shift)")
            ...
            if (GUILayout.Button(
                string.Format("SLATE Cinematic Sequencer v{0}", Cutscene.VERSION_NUMBER.ToString("0.00"))
                , EditorStyles.toolbarButton))
        ...
        private void ShowListGroups(Event e, ref float nextYPos)
        {
            ...
            for (var g = 0; g < cutscene.groups.Count; g++) {
                ...
                AddCursorRect(groupRect, pickedGroup == null ? MouseCursor.Link : MouseCursor.MoveArrow);
                ...
                if (group.actor == null) {
                    var oRect = Rect.MinMaxRect(groupRect.xMin + 20, groupRect.yMin + 1, groupRect.xMax - 20
                        , groupRect.yMax - 1);
                    group.actor = (GameObject)EditorGUI.ObjectField(oRect, group.actor, typeof(GameObject), true);
                }
                ///---
                ...
                if ((e.type == EventType.ContextClick && groupRect.Contains(e.mousePosition)) || plusClicked) {
                    ...
                    foreach (var _info in EditorTools.GetTypeMetaDerivedFrom(typeof(CutsceneTrack))) {
                        var info = _info;
                        if (info.attachableTypes == null || !info.attachableTypes.Contains(group.GetType())) continue;
                    ...
                    if (i != 0 && e.type == EventType.ContextClick && sectionsMenu != null) {
                        sectionsMenu.AddItem(new GUIContent("Edit"), false, () => {
                            DoPopup(() => {
                                ...
                                section1.time = EditorGUILayout.Slider("Time", section1.time, previousSectionTime + 0.1f
            ...
            if (multiSelection != null) {
                ...
                if (e.type == EventType.MouseDrag && multiSelectionScaleDirection != 0) {
                    foreach (var clipWrapper in multiSelection) {
                        ...
                        var lerpMin = multiSelectionScaleDirection == -1 ? Mathf.Clamp(pointerTime, 0, preTimeMax)
                            : preTimeMin;
                        var lerpMax = multiSelectionScaleDirection == 1 ? Mathf.Max(pointerTime, preTimeMin)
                            : preTimeMax;
        ...
        private void ShowWelcome()
        {
            ...
            GUI.Label(new Rect(iconRect.x + 68, iconRect.yMax - 42, iconRect.width, 40), "v" + Cutscene.VERSION_NUMBER
                , Styles.leftLabel);
            GUI.color = Color.white;
            ...
            if (!isAboutButtonPressed) {
                ...
                if (GUILayout.Button("New Cutscene", GUILayout.Height(40))) InitializeAll(Commands.CreateCutscene());
                GUI.backgroundColor = Color.white;
            }
                    ...
                    foreach (var board in boards) {
                        ...
                        if (isAll || target.ToLower() == "slate") {
                            if (!isAll) {
                                ...
                                result += uptodate ? "<b>You are up to date on the latest version!</b>"
                                    : string.Format("<b>There is a new version available! ( v{0} )</b>", version);
                            }
        ...
        private class ActionClipWrapper
        {
            ...
            public ActionClip Split(float time)
            {
                ...
                if (copy is ISubClipContainable) (copy as ISubClipContainable).subClipOffset -= delta;
                return copy;
            }
            ...
            public void StretchFit()
            {
                ...
                if (nextClip == null || nextClip.startTime > action.endTime) action.endTime = targetEnd;
                ...
                if (action is ISubClipContainable) (action as ISubClipContainable).subClipOffset += delta;
            }
                ...
                if (hasParameters) {
                    foreach (var param in action.animationData.animatedParameters)
                        if (param.HasAnyKey()) {
                            ...
                            if (param.GetKeyNext(action.length) > action.length) param.TryKeyIdentity(action.length);
                        }
