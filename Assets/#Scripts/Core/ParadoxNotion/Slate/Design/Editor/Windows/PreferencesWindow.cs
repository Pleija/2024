    ...
    public class PreferencesWindow : PopupWindowContent
    {
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            if (Prefs.timeStepMode == Prefs.TimeStepMode.Seconds)
                Prefs.snapInterval = EditorTools.CleanPopup<float>("Working Snap Interval", Prefs.snapInterval
                    , Prefs.snapIntervals.ToList());
            else
                Prefs.frameRate =
                    EditorTools.CleanPopup<int>("Working Frame Rate", Prefs.frameRate, Prefs.frameRates.ToList());
            ...
            Prefs.autoCleanKeysOffRange = EditorGUILayout.Toggle(
                new GUIContent("Auto Clean Keys", "If enabled, will automatically clean keys off clip range")
            ...
            Prefs.defaultTangentMode =
                (TangentMode)EditorGUILayout.EnumPopup("Initial Keyframe Tangent", Prefs.defaultTangentMode);
            Prefs.keyframesStyle =
                (Prefs.KeyframesStyle)EditorGUILayout.EnumPopup("Keyframes Style", Prefs.keyframesStyle);
            GUILayout.EndVertical();
            GUILayout.BeginVertical("box");
            ...
            Prefs.autoCreateDirectorCamera =
                EditorGUILayout.Toggle("Auto Create Director Camera", Prefs.autoCreateDirectorCamera);
            GUILayout.EndVertical();
            GUILayout.BeginVertical("box");
            ...
            var newUseHDRP = EditorGUILayout.ToggleLeft(
                new GUIContent("Use HDRP Define", "Enable this if your project is using the HDRP Rendering Pipeline")
                , useHDRP);
            ...
            var newUseURP = EditorGUILayout.ToggleLeft(
                new GUIContent("Use URP Define", "Enable this if your project is using the URP Rendering Pipeline")
                , useURP);
            ...
            if (GUILayout.Button(new GUIContent(Prefs.autoKey ? "AUTOKEY IS ENABLED" : "AUTOKEY IS DISABLED"
                , Styles.keyIcon)))
