...
namespace Slate
{
    public static class AnimationDataUtility
    {
        ...
        //...
        private static string[] Internal_GetAnimatableMemberPaths(Type type, string path)
        {
            var result = new List<string>();

            foreach (var field in type.RTGetFields()) {
                var current = path + field.Name;
                if (field.RTIsDefined<AnimatableParameterAttribute>(true)) result.Add(current);
