...
{
    [Category("Transform"), System.Obsolete("Use Set Parent")]
    public class SetParentTemporary : ActorActionClip
    {
        ...
        protected override void OnUpdate(float deltaTime)
        {
            if (deltaTime < length) {
                ...
                if (matchPosition) actor.transform.localPosition = Vector3.zero;
                if (matchRotation) actor.transform.localEulerAngles = Vector3.zero;
                if (matchScale) actor.transform.localScale = Vector3.one;
            }
            else {
