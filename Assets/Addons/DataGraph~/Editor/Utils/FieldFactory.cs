...
{
    [Obsolete(
        "Field Factory is not necessary anymore. You can use a SerializedProperty directly instead.")]
    public static class FieldFactory
    {
        private static readonly Dictionary<Type, Type> fieldDrawers = new Dictionary<Type, Type>();

        private static readonly MethodInfo createFieldMethod =
            typeof(FieldFactory).GetMethod("CreateFieldSpecific",
        ...
        static FieldFactory()
        {
            foreach (var type in AppDomain.CurrentDomain.GetAllTypes()) {
                var drawerAttribute =
                    type.GetCustomAttributes(typeof(FieldDrawerAttribute), false).FirstOrDefault()
                        as FieldDrawerAttribute;
        ...
        public static INotifyValueChanged<T> CreateField<T>(T value, string label = null) =>
            CreateField(value != null ? value.GetType() : typeof(T), label) as
        ...
        public static VisualElement CreateField(Type t, string label)
        {
            ...
            if (drawerType == null)
                drawerType = fieldDrawers.FirstOrDefault(kp => kp.Key.IsReallyAssignableFrom(t))
                    .Value;
            ...
            if (drawerType == typeof(EnumField))
                ...
                try {
                    field = Activator.CreateInstance(drawerType,
                        ...
                        BindingFlags.Instance | BindingFlags.OptionalParamBinding, null,
        ...
        public static INotifyValueChanged<T> CreateFieldSpecific<T>(T value,
            Action<object> onValueChanged, string label)
        {
            var fieldDrawer = CreateField<T>(value, label);
            if (fieldDrawer == null) return null;
        ...
        public static VisualElement CreateField(Type fieldType, object value,
            Action<object> onValueChanged, string label)
        {
            if (typeof(Enum).IsAssignableFrom(fieldType)) fieldType = typeof(Enum);
            ...
            if (fieldType == typeof(LayerMask)) {
                ...
                try {
                    ...
                    try {
                        field = createFieldSpecificMethod.Invoke(null,
                            new object[] { value, onValueChanged, label }) as VisualElement;
                    }
                    catch { }

                    // handle the Object field case
                    if (field == null && (value == null || value is UnityEngine.Object)) {
                        createFieldSpecificMethod =
                            createFieldMethod.MakeGenericMethod(typeof(UnityEngine.Object));
                        field = createFieldSpecificMethod.Invoke(null,
                            new object[] { value, onValueChanged, label }) as VisualElement;
