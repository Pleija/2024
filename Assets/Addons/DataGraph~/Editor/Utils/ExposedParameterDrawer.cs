...
namespace GraphProcessor
{
    ...
    public class ExposedParameterDrawer : PropertyDrawer
    {
        ...
        {
            if (displayName == null) displayName = GetNameProperty(property).stringValue;
        ...
        protected SerializedProperty GetValProperty(SerializedProperty property) =>
            property.FindPropertyRelative("val");
        ...
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            ...
            container.Add(new IMGUIContainer(() => {
                ...
                if ((FloatParameter.FloatMode)mode.intValue == FloatParameter.FloatMode.Slider) {
                    newValue = EditorGUILayout.Slider(name.stringValue, val.floatValue,
        ...
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            ...
            container.Add(new IMGUIContainer(() => {
                ...
                if ((IntParameter.IntMode)mode.intValue == IntParameter.IntMode.Slider) {
                    newValue = EditorGUILayout.IntSlider(name.stringValue, val.intValue,
    ...
    public class Vector2ParameterDrawer : ExposedParameterDrawer
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            ...
            container.Add(new IMGUIContainer(() => {
                ...
                if ((Vector2Parameter.Vector2Mode)mode.intValue ==
                    Vector2Parameter.Vector2Mode.MinMaxSlider) {
                    var x = val.vector2Value.x;
                    var y = val.vector2Value.y;
                    EditorGUILayout.MinMaxSlider(name.stringValue, ref x, ref y, min.floatValue,
                        max.floatValue);
                    val.vector2Value = new Vector2(x, y);
                }
                else {
                    val.vector2Value =
                        EditorGUILayout.Vector2Field(name.stringValue, val.vector2Value);
                }
                if (EditorGUI.EndChangeCheck()) ApplyModifiedProperties(property);
                EditorGUIUtility.labelWidth = 0;
            }));
            return container;
        }
    ...
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property) =>
            CreateHideInInspectorField(property);
        ...
        {
            var guid = settingsProperty.FindPropertyRelative(nameof(ExposedParameter.Settings.guid))
                .stringValue;
            return GetGraph(settingsProperty).GetExposedParameterFromGUID(guid);
        }

        protected static PropertyField CreateSettingsField(SerializedProperty settingsProperty,
            string fieldName, string displayName = null)
        {
            var prop = settingsProperty.FindPropertyRelative(fieldName);
            var param = GetParameter(settingsProperty);
            var graph = GetGraph(settingsProperty);
            if (displayName == null) displayName = ObjectNames.NicifyVariableName(fieldName);
    ...
    public class ExposedColorSettingsDrawer : ExposedParameterSettingsDrawer
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty settingsProperty)
        {
            ...
            settings.Add(CreateSettingsField(settingsProperty,
                nameof(ColorParameter.ColorSettings.mode), "Mode"));
            return settings;
        }
    }
    ...
    public class FloatSettingsDrawer : ExposedParameterSettingsDrawer
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty settingsProperty)
        {
            ...
            settings.Add(CreateHideInInspectorField(settingsProperty));
            var mode = CreateSettingsField(settingsProperty,
                nameof(FloatParameter.FloatSettings.mode), "Mode");
            var min = CreateSettingsField(settingsProperty,
                nameof(FloatParameter.FloatSettings.min), "Min");
            var max = CreateSettingsField(settingsProperty,
                nameof(FloatParameter.FloatSettings.max), "Max");
            mode.RegisterValueChangeCallback(e => UpdateVisibility(e.changedProperty));
            UpdateVisibility(
                settingsProperty.FindPropertyRelative(nameof(FloatParameter.FloatSettings.mode)));
    ...
    public class IntSettingsDrawer : ExposedParameterSettingsDrawer
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty settingsProperty)
        {
            ...
            settings.Bind(settingsProperty.serializedObject);
            settings.Add(CreateHideInInspectorField(settingsProperty));
            var mode = CreateSettingsField(settingsProperty, nameof(IntParameter.IntSettings.mode),
                "Mode");
            var min = CreateSettingsField(settingsProperty, nameof(IntParameter.IntSettings.min),
                "Min");
            var max = CreateSettingsField(settingsProperty, nameof(IntParameter.IntSettings.max),
                "Max");
            mode.RegisterValueChangeCallback(e => UpdateVisibility(e.changedProperty));
            UpdateVisibility(
                settingsProperty.FindPropertyRelative(nameof(IntParameter.IntSettings.mode)));
    ...
    public class Vector2SettingsDrawer : ExposedParameterSettingsDrawer
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty settingsProperty)
        {
            ...
            settings.Add(CreateHideInInspectorField(settingsProperty));
            settings.Add(CreateSettingsField(settingsProperty,
                nameof(Vector2Parameter.Vector2Settings.mode), "Mode"));
            settings.Add(CreateSettingsField(settingsProperty,
                nameof(Vector2Parameter.Vector2Settings.min), "Min"));
            settings.Add(CreateSettingsField(settingsProperty,
                nameof(Vector2Parameter.Vector2Settings.max), "Max"));
            return settings;
        }
    }
    ...
    public class GradientSettingsDrawer : ExposedParameterSettingsDrawer
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty settingsProperty)
        {
            ...
            settings.Add(CreateHideInInspectorField(settingsProperty));
            settings.Add(CreateSettingsField(settingsProperty,
                nameof(GradientParameter.GradientSettings.mode), "Mode"));
            return settings;
        }
    }
}
