...
namespace GraphProcessor
{
    ...
    {
        private static Dictionary<Type, Type> stackNodeViewPerType = new Dictionary<Type, Type>();

        static StackNodeViewProvider()
        {
            foreach (var t in TypeCache.GetTypesWithAttribute<CustomStackNodeView>()) {
                var attr = t.GetCustomAttributes(false).Select(a => a as CustomStackNodeView)
                    .FirstOrDefault();
