...
namespace GraphProcessor
{
    public class BaseEdgeDragHelper : EdgeDragHelper
    {
        ...
        protected Dictionary<BaseNodeView, List<PortView>> compatiblePorts =
            new Dictionary<BaseNodeView, List<PortView>>();
        ...
        public override void Reset(bool didConnect = false)
        {
            ...
            if (panSchedule != null) panSchedule.Pause();
        ...
        public override bool HandleMouseDown(MouseDownEvent evt)
        {
            ...
            foreach (PortView port in graphView.GetCompatiblePorts(draggedPort, nodeAdapter)) {
                ...
                if (portList == null) portList = compatiblePorts[port.owner] = new List<PortView>();
                portList.Add(port);
            }
            ...
            if (panSchedule == null) {
                panSchedule = graphView.schedule.Execute(Pan).Every(k_PanInterval)
        ...
        internal Vector2 GetEffectivePanSpeed(Vector2 mousePos)
        {
            ...
            if (mousePos.x <= k_PanAreaWidth)
                effectiveSpeed.x = -((k_PanAreaWidth - mousePos.x) / k_PanAreaWidth + 0.5f) *
                    k_PanSpeed;
            else if (mousePos.x >= graphView.contentContainer.layout.width - k_PanAreaWidth)
                effectiveSpeed.x =
                    ((mousePos.x - (graphView.contentContainer.layout.width - k_PanAreaWidth)) /
                        k_PanAreaWidth + 0.5f) * k_PanSpeed;
            if (mousePos.y <= k_PanAreaWidth)
                effectiveSpeed.y = -((k_PanAreaWidth - mousePos.y) / k_PanAreaWidth + 0.5f) *
                    k_PanSpeed;
            else if (mousePos.y >= graphView.contentContainer.layout.height - k_PanAreaWidth)
                effectiveSpeed.y =
                    ((mousePos.y - (graphView.contentContainer.layout.height - k_PanAreaWidth)) /
                        k_PanAreaWidth + 0.5f) * k_PanSpeed;
        ...
        public override void HandleMouseMove(MouseMoveEvent evt)
        {
            ...
            var gvMousePos =
                ve.ChangeCoordinatesTo(graphView.contentContainer, evt.localMousePosition);
            ...
            if (endPort != null) {
                ...
                if (edgeCandidate.output == null) {
                    ...
                    if (ghostEdge.output != null) ghostEdge.output.portCapLit = false;
                    ghostEdge.output = endPort;
                    ghostEdge.output.portCapLit = true;
                }
                else {
                    if (ghostEdge.input != null) ghostEdge.input.portCapLit = false;
        ...
        protected virtual EdgeView CreateEdgeView() => new EdgeView();
        ...
        private Rect GetPortBounds(BaseNodeView nodeView, int index, List<PortView> portList)
        {
            ...
            if (port.orientation == Orientation.Horizontal) {
                ...
                if (index == 0) bounds.yMin = nodeView.worldBound.yMin;
                if (index == portList.Count - 1) bounds.yMax = nodeView.worldBound.yMax;
