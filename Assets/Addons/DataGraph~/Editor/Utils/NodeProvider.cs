...
namespace GraphProcessor
{
    public static class NodeProvider
    {
        ...
        private static Dictionary<Type, MonoScript> nodeViewScripts =
            new Dictionary<Type, MonoScript>();

        private static Dictionary<Type, MonoScript>
            nodeScripts = new Dictionary<Type, MonoScript>();
        ...
        private static Dictionary<BaseGraph, NodeDescriptions> specificNodeDescriptions =
        ...
        public static void LoadGraph(BaseGraph graph)
        {
            ...
            foreach (var nodeInfo in specificNodes) {
                ...
                if (compatible) BuildCacheForNode(nodeInfo.nodeType, descriptions, graph);
            }
        }
        ...
        private static void BuildGenericNodeCache()
        {
            foreach (var nodeType in TypeCache.GetTypesDerivedFrom<BaseNode>()) {
                if (!IsNodeAccessibleFromMenu(nodeType)) continue;
                if (IsNodeSpecificToGraph(nodeType)) continue;
        ...
        {
            var attrs =
                nodeType.GetCustomAttributes(typeof(NodeMenuItemAttribute), false) as
            ...
            foreach (var field in nodeType.GetFields(BindingFlags.Instance | BindingFlags.Public |
                ...
                if (field.GetCustomAttribute<HideInInspector>() == null && field
                    .GetCustomAttributes().Any(c => c is InputAttribute || c is OutputAttribute))
        ...
        private static bool IsNodeSpecificToGraph(Type nodeType)
        {
            var isCompatibleWithGraphMethods = nodeType
                .GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic |
                    BindingFlags.FlattenHierarchy).Where(m =>
                    m.GetCustomAttribute<IsCompatibleWithGraph>() != null);
            var nodeMenuAttributes = nodeType.GetCustomAttributes<NodeMenuItemAttribute>();
            var compatibleGraphTypes = nodeMenuAttributes
                .Where(n => n.onlyCompatibleWithGraph != null)
            ...
            foreach (var method in isCompatibleWithGraphMethods) {
                ...
                if (method.ReturnType != typeof(bool) || p.Count() != 1 ||
                    p[0].ParameterType != typeof(BaseGraph))
                    Debug.LogError(
            ...
            if (compatibleMethods.Count > 0 || compatibleGraphTypes.Count > 0) {
                ...
                if (compatibleGraphTypes.Count == 0) compatibleGraphTypes.Add(null);
        ...
        private static void ProvideNodePortCreationDescription(Type nodeType,
            NodeDescriptions targetDescription, BaseGraph graph = null)
        {
        ...
        private static void AddNodeScriptAsset(Type type)
        {
            ...
            if (nodeScriptAsset != null) nodeScripts[type] = nodeScriptAsset;
        }

        private static void AddNodeViewScriptAsset(Type type)
        {
            var attrs =
                type.GetCustomAttributes(typeof(NodeCustomEditor), false) as NodeCustomEditor[];
            ...
            if (attrs != null && attrs.Length > 0) {
                ...
                if (nodeViewScriptAsset != null) nodeViewScripts[type] = nodeViewScriptAsset;
            }
        }
        ...
        private static MonoScript FindScriptFromClassName(string className)
        {
            ...
            foreach (var scriptGUID in scriptGUIDs) {
                ...
                if (script != null && string.Equals(className,
        ...
        {
            Type view;
            if (nodeViewPerType.TryGetValue(nodeType, out view)) return view;
            Type baseType = null;
            ...
            foreach (var type in nodeViewPerType)
                ...
                if (nodeType.IsSubclassOf(type.Key) &&
                    (baseType == null || type.Value.IsSubclassOf(baseType)))
                    baseType = type.Value;
        ...
        public static IEnumerable<(string path, Type type)> GetNodeMenuEntries(
            BaseGraph graph = null)
        {
            foreach (var node in genericNodes.nodePerMenuTitle) yield return (node.Key, node.Value);
        ...
        {
            foreach (var type in genericNodes.slotTypes) yield return type;
            if (graph != null && specificNodeDescriptions.TryGetValue(graph, out var specificNodes))
            ...
            BaseGraph graph = null)
        {
            foreach (var description in genericNodes.nodeCreatePortDescription) {
                if (!IsPortCompatible(description)) continue;
