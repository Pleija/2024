...
namespace GraphProcessor
{
    ...
    public class BaseGraphView : GraphView, IDisposable
    {
        ...
        /// <summary>
        ///     Dictionary of the node views accessed view the node instance, faster than a Find in the node
        ///     view list
        /// </summary>
        ...
        public Dictionary<BaseNode, BaseNodeView> nodeViewsPerNode =
            new Dictionary<BaseNode, BaseNodeView>();
        ...
        private Dictionary<Type, PinnedElementView> pinnedElements =
            new Dictionary<Type, PinnedElementView>();
        ...
        private Dictionary<Type, (Type nodeType, MethodInfo initalizeNodeFromObject)>
            nodeTypePerCreateAssetType = new Dictionary<Type, (Type, MethodInfo)>();
...
#region Callbacks
        ...
        private void UnserializeAndPasteCallback(string operationName, string serializedData)
        {
            ...
            var unserializedGroups = data.copiedGroups
                .Select(g => JsonSerializer.Deserialize<Group>(g)).ToList();
            ...
            foreach (var serializedNode in data.copiedNodes) {
                ...
                node.createdWithinGroup =
                    unserializedGroups.Any(g => g.innerNodeGUIDs.Contains(sourceGUID));
                node.OnNodeCreated();
            ...
            foreach (var serializedEdge in data.copiedEdges) {
                ...
                var inputPort =
                    oldInputNode.GetPort(edge.inputPort.fieldName, edge.inputPortIdentifier);
                var outputPort =
                    oldOutputNode.GetPort(edge.outputPort.fieldName, edge.outputPortIdentifier);
                var newEdge = SerializableEdge.CreateNewEdge(graph, inputPort, outputPort);

                if (nodeViewsPerNode.ContainsKey(oldInputNode) &&
                    nodeViewsPerNode.ContainsKey(oldOutputNode)) {
                    var edgeView = CreateEdgeView();
                    edgeView.userData = newEdge;
                    edgeView.input = nodeViewsPerNode[oldInputNode]
                        .GetPortViewFromFieldName(newEdge.inputFieldName,
                            newEdge.inputPortIdentifier);
                    edgeView.output = nodeViewsPerNode[oldOutputNode]
                        .GetPortViewFromFieldName(newEdge.outputFieldName,
                            newEdge.outputPortIdentifier);
                    Connect(edgeView);
                }
            }
        }

        public virtual EdgeView CreateEdgeView() => new EdgeView();
                ...
                changes.elementsToRemove.RemoveAll(e => {
                    switch (e) {
                        ...
                        case BaseNodeView nodeView:
                            ...
                            foreach (var pv in nodeView.inputPortViews.Concat(nodeView
                                .outputPortViews))
        ...
        private void ElementResizedCallback(VisualElement elem)
        {
            ...
            if (groupView != null) groupView.group.size = groupView.GetPosition().size;
        }
        ...
        public override List<Port> GetCompatiblePorts(Port startPort, NodeAdapter nodeAdapter)
        {
            ...
            compatiblePorts.AddRange(ports.ToList().Where(p => {
                ...
                if (portView.owner == (startPort as PortView).owner) return false;
                if (p.direction == startPort.direction) return false;

                //Check for type assignability
                if (!BaseGraph.TypesAreConnectable(startPort.portType, p.portType)) return false;
        ...
        protected virtual void BuildGroupContextualMenu(ContextualMenuPopulateEvent evt,
            int menuPosition = -1)
        {
            if (menuPosition == -1) menuPosition = evt.menu.MenuItems().Count;
            var position =
        ...
        protected void BuildHelpContextualMenu(ContextualMenuPopulateEvent evt)
        {
            evt.menu.AppendAction("Help/Reset Pinned Windows", e => {
                foreach (var kp in pinnedElements) kp.Value.ResetPosition();
            });
        }
        ...
        private bool DoesSelectionContainsInspectorNodes()
        {
            ...
            var selectedNodesNotInInspector =
                selectedNodes.Except(nodeInspector.selectedNodes).ToList();
            var nodeInInspectorWithoutSelectedNodes =
        ...
        {
            var mousePos =
                (e.currentTarget as VisualElement).ChangeCoordinatesTo(contentViewContainer,
                    ...
                    foreach (var kp in nodeTypePerCreateAssetType)
                        if (kp.Key.IsAssignableFrom(objectType))
                            try {
                                ...
                                if ((bool)kp.Value.initalizeNodeFromObject.Invoke(node,
                                    new[] { obj })) {
                                    AddNode(node);
                                    break;
                                }
        ...
        private void DragUpdatedCallback(DragUpdatedEvent e)
        {
            ...
            if (dragData != null)
                ...
                if (dragData.OfType<ExposedParameterFieldView>().Any())
                    dragging = true;
            if (dragObjects.Length > 0) dragging = true;
            if (dragging) DragAndDrop.visualMode = DragAndDropVisualMode.Generic;
            UpdateNodeInspectorSelection();
        }
#endregion
...
#region Initialization
        ...
        public void Initialize(BaseGraph graph)
        {
            ...
            RegisterCallback<KeyDownEvent>(e => {
                if (e.keyCode == KeyCode.S && e.actionKey) SaveGraphToDisk();
            });
            ClearGraphElements();
            InitializeGraphView();
            InitializeNodeViews();
            ...
            foreach (var nodeInfo in NodeProvider.GetNodeMenuEntries(graph)) {
                ...
                var exceptInheritedInterfaces =
                    interfaces.Except(interfaces.SelectMany(t => t.GetInterfaces()));

                foreach (var i in interfaces)
                    if (i.IsGenericType &&
                        i.GetGenericTypeDefinition() == typeof(ICreateNodeFrom<>)) {
                        ...
                        if (initializeFunction.DeclaringType == nodeInfo.type)
                            nodeTypePerCreateAssetType[genericArgumentType] =
                                (nodeInfo.type, initializeFunction);
                    }
            }
        }
        ...
        private void InitializeEdgeViews()
        {
            ...
            foreach (var serializedEdge in graph.edges) {
                ...
                if (inputNodeView == null || outputNodeView == null) continue;
                var edgeView = CreateEdgeView();
                edgeView.userData = serializedEdge;
                edgeView.input = inputNodeView.GetPortViewFromFieldName(
                    serializedEdge.inputFieldName, serializedEdge.inputPortIdentifier);
                edgeView.output = outputNodeView.GetPortViewFromFieldName(
                    serializedEdge.outputFieldName, serializedEdge.outputPortIdentifier);
        ...
        {
            foreach (var group in graph.groups) AddGroupView(group);
        }

        private void InitializeStickyNotes()
        {
#if UNITY_2020_1_OR_NEWER
            foreach (var group in graph.stickyNotes) AddStickyNoteView(group);
#endif
        }

        private void InitializeStackNodes()
        {
            foreach (var stackNode in graph.stackNodes) AddStackNodeView(stackNode);
        }
...
#region Graph content modification
        ...
        public BaseNodeView AddNodeView(BaseNode node)
        {
            ...
            if (viewType == null) viewType = typeof(BaseNodeView);
        ...
        {
            foreach (var stickyNodeView in stickyNoteViews) RemoveElement(stickyNodeView);
        ...
        {
            foreach (var groupView in groupViews) RemoveElement(groupView);
            groupViews.Clear();
        }

        public bool CanConnectEdge(EdgeView e, bool autoDisconnectInputs = true)
        {
            if (e.input == null || e.output == null) return false;
        ...
        {
            if (!CanConnectEdge(e, autoDisconnectInputs)) return false;
            var inputPortView = e.input as PortView;
            var outputPortView = e.output as PortView;
            var inputNodeView = inputPortView.node as BaseNodeView;
            var outputNodeView = outputPortView.node as BaseNodeView;
        ...
        public bool Connect(PortView inputPortView, PortView outputPortView,
            bool autoDisconnectInputs = true)
        {
            var inputPort = inputPortView.owner.nodeTarget.GetPort(inputPortView.fieldName,
                inputPortView.portData.identifier);
            var outputPort = outputPortView.owner.nodeTarget.GetPort(outputPortView.fieldName,
                outputPortView.portData.identifier);
        ...
        public bool Connect(EdgeView e, bool autoDisconnectInputs = true)
        {
            ...
            var inputNodeView = inputPortView.node as BaseNodeView;
            var outputNodeView = outputPortView.node as BaseNodeView;
            var inputPort = inputNodeView.nodeTarget.GetPort(inputPortView.fieldName,
                inputPortView.portData.identifier);
            var outputPort = outputNodeView.nodeTarget.GetPort(outputPortView.fieldName,
                outputPortView.portData.identifier);
        ...
        public void DisconnectView(EdgeView e, bool refreshPorts = true)
        {
            ...
            if (e?.input?.node is BaseNodeView inputNodeView) {
                ...
                if (refreshPorts) inputNodeView.RefreshPorts();
            }

            if (e?.output?.node is BaseNodeView outputNodeView) {
                e.output.Disconnect(e);
                if (refreshPorts) outputNodeView.RefreshPorts();
            }
            edgeViews.Remove(e);
        }
        ...
        {
            foreach (var edge in edgeViews) RemoveElement(edge);
            edgeViews.Clear();
        }
        ...
        public Status GetPinnedElementStatus<T>() where T : PinnedElementView =>
            GetPinnedElementStatus(typeof(T));
        ...
        public RelayNodeView AddRelayNode(PortView inputPort, PortView outputPort, Vector2 position)
        {
            ...
            if (outputPort != null) Connect(view.inputPortViews[0], outputPort);
            if (inputPort != null) Connect(inputPort, view.outputPortViews[0]);
            return view;
        }

        /// <summary>
        ///     Update all the serialized property bindings (in case a node was deleted / added, the property
        ///     pathes needs to be updated)
        /// </summary>
