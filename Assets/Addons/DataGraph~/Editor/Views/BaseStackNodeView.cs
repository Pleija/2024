...
{
    /// <summary>
    ///     Stack node view implementation, can be used to stack multiple node inside a context like VFX
    ///     graph does.
    /// </summary>
    public class BaseStackNodeView : StackNode
    {
        ...
        /// <inheritdoc />
        protected override void OnSeparatorContextualMenuEvent(ContextualMenuPopulateEvent evt,
            int separatorIndex)
        {
        ...
        /// <inheritdoc />
        protected override bool AcceptsElement(GraphElement element, ref int proposedIndex,
            int maxIndex)
        {
            ...
            if (accept && element is BaseNodeView nodeView) {
                ...
                if (oldIndex != -1) {
                    ...
                    if (oldIndex != index) onNodeReordered?.Invoke(nodeView, oldIndex, index);
                }
