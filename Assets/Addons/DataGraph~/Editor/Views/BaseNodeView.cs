...
namespace GraphProcessor
{
    ...
    public class BaseNodeView : NodeView
    {
        ...
        protected Dictionary<string, List<PortView>> portsPerFieldName =
            new Dictionary<string, List<PortView>>();
...
#region Initialization
        public void Initialize(BaseGraphView owner, BaseNode node)
        {
            nodeTarget = node;
            this.owner = owner;
            if (!node.deletable) capabilities &= ~Capabilities.Deletable;
            // Note that the Renamable capability is useless right now as it haven't been implemented in Graphview
            if (node.isRenamable) capabilities |= Capabilities.Renamable;
            ...
            node.onPortsUpdated += a =>
                schedule.Execute(_ => UpdatePortsForField(a)).ExecuteLater(0);
            ...
            if (GetType().GetMethod(nameof(Enable), new Type[] { }).DeclaringType !=
                typeof(BaseNodeView))
        ...
        private void InitializeView()
        {
            ...
            if (nodeTarget.showControlsOnHover) {
                ...
                RegisterCallback<MouseOutEvent>(e => {
                    ...
                    var graphMousePosition =
                        owner.contentViewContainer.WorldToLocal(e.mousePosition);
                    if (rect.Contains(graphMousePosition) || !nodeTarget.showControlsOnHover)
                        return;
                    mouseOverControls = false;
                    schedule.Execute(_ => {
                        if (!mouseOverControls) controlsContainer.style.display = DisplayStyle.None;
                    }).ExecuteLater(500);
                });
            }
            Undo.undoRedoPerformed += UpdateFieldValues;
            debugContainer = new VisualElement { name = "debug" };
            if (nodeTarget.debug) mainContainer.Add(debugContainer);
            initializing = true;
            UpdateTitle();
            ...
            if ((capabilities & Capabilities.Renamable) != 0) SetupRenamableTitle();
        }
        ...
        private void SetupRenamableTitle()
        {
            ...
            titleLabel.RegisterCallback<MouseDownEvent>(e => {
                if (e.clickCount == 2 && e.button == (int)MouseButton.LeftMouse) OpenTitleEditor();
            });
            ...
            titleTextField.RegisterCallback<FocusOutEvent>(e =>
                CloseAndSaveTitleEditor(titleTextField.value));
        ...
        {
            title = nodeTarget.GetCustomName() == null ? nodeTarget.GetType().Name
                : nodeTarget.GetCustomName();
        }
        ...
        private void InitializeSettings()
        {
            ...
            if (hasSettings) {
                ...
                var fields = nodeTarget.GetType().GetFields(BindingFlags.Public |
        ...
        private void OnGeometryChanged(GeometryChangedEvent evt)
        {
            if (settingButton != null) {
                var settingsButtonLayout =
                    settingButton.ChangeCoordinatesTo(settingsContainer.parent,
                        settingButton.layout);
...
#region API
        ...
        public PortView GetFirstPortViewFromFieldName(string fieldName) =>
            GetPortViewsFromFieldName(fieldName)?.First();
        ...
        public PortView AddPort(FieldInfo fieldInfo, Direction direction,
            BaseEdgeConnectorListener listener, PortData portData)
        {
        ...
        protected virtual PortView CreatePortView(Direction direction, FieldInfo fieldInfo,
            PortData portData, BaseEdgeConnectorListener listener) =>
        ...
        public void RemovePort(PortView p)
        {
            ...
            foreach (var e in edgesCopy) owner.Disconnect(e, false);

            if (p.direction == Direction.Input) {
                if (inputPortViews.Remove(p)) p.RemoveFromHierarchy();
            }
            else {
                if (outputPortViews.Remove(p)) p.RemoveFromHierarchy();
            }
            List<PortView> ports;
            portsPerFieldName.TryGetValue(p.fieldName, out ports);
            ports.Remove(p);
        ...
        public static Rect
            GetNodeRect(Node node, float left = int.MaxValue, float top = int.MaxValue) => new Rect(
        ...
        public void AddMessageView(string message, NodeMessageType messageType)
        {
            ...
            switch (messageType) {
                case NodeMessageType.Warning:
                    badge = new NodeBadgeView(message,
                        EditorGUIUtility.IconContent("Collab.Warning").image, Color.yellow);
                    break;
...
#region Callbacks & Overrides
        ...
        private Dictionary<string, List<(object value, VisualElement target)>> visibleConditions =
            new Dictionary<string, List<(object value, VisualElement target)>>();

        private Dictionary<string, VisualElement> hideElementIfConnected =
            new Dictionary<string, VisualElement>();

        private Dictionary<FieldInfo, List<VisualElement>> fieldControlsMap =
            new Dictionary<FieldInfo, List<VisualElement>>();
        ...
        protected virtual void DrawDefaultInspector(bool fromInspector = false)
        {
            ...
            foreach (var field in fields) {
                ...
                var hasInputOrOutputAttribute = hasInputAttribute ||
                    field.GetCustomAttribute(typeof(OutputAttribute)) != null;
                var showAsDrawer = !fromInspector &&
        ...
        private void UpdateOtherFieldValueSpecific<T>(FieldInfo field, object newValue)
        {
            foreach (var inputField in fieldControlsMap[field]) {
                ...
                if (notify != null) notify.SetValueWithoutNotify((T)newValue);
            }
        }

        private static MethodInfo specificUpdateOtherFieldValue =
        ...
        private void UpdateOtherFieldValue(FieldInfo info, object newValue)
        {
            ...
            var fieldType = info.FieldType.IsSubclassOf(typeof(UnityEngine.Object))
                ? typeof(UnityEngine.Object) : info.FieldType;
        ...
        private static MethodInfo specificGetValue =
        ...
        protected VisualElement AddControlField(string fieldName, string label = null,
            bool showInputDrawer = false, Action valueChangedCallback = null) => AddControlField(
            nodeTarget.GetType().GetField(fieldName,
                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance), label,
            showInputDrawer, valueChangedCallback);
        ...
        internal void SyncSerializedPropertyPathes()
        {
            ...
            foreach (var propertyField in this.Query<PropertyField>().ToList()) {
                ...
                propertyField.bindingPath = s_ReplaceNodeIndexPropertyPath.Replace(
                    propertyField.bindingPath,
        ...
        protected VisualElement AddControlField(FieldInfo field, string label = null,
            bool showInputDrawer = false, Action valueChangedCallback = null)
        {
            ...
            if (visibleCondition != null) {
                ...
                if (conditionField == null) {
                    ...
                    UpdateFieldVisibility(visibleCondition.fieldName,
                        conditionField.GetValue(nodeTarget));
                }
            }
            return element;
        }
        ...
        internal void OnPortConnected(PortView port)
        {
            if (port.direction == Direction.Input &&
                inputContainerElement?.Q(port.fieldName) != null)
        ...
        public override void SetPosition(Rect newPos)
        {
            if (initializing || !nodeTarget.isLocked) {
                ...
                if (!initializing) owner.RegisterCompleteObjectUndo("Moved graph node");
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            if (nodeTarget.unlockable)
                evt.menu.AppendAction(nodeTarget.isLocked ? "Unlock" : "Lock",
                    (e) => ChangeLockStatus(), LockStatus);
        }
        ...
        private Status LockStatus(DropdownMenuAction action) => Status.Normal;

        private Status DebugStatus(DropdownMenuAction action)
        {
            if (nodeTarget.debug) return Status.Checked;
            return Status.Normal;
        }

        private Status OpenNodeScriptStatus(DropdownMenuAction action)
        {
            if (NodeProvider.GetNodeScript(nodeTarget.GetType()) != null) return Status.Normal;
            return Status.Disabled;
        }

        private Status OpenNodeViewScriptStatus(DropdownMenuAction action)
        {
            if (NodeProvider.GetNodeViewScript(GetType()) != null) return Status.Normal;
            return Status.Disabled;
        }

        private IEnumerable<PortView> SyncPortCounts(IEnumerable<NodePort> ports,
            IEnumerable<PortView> portViews)
        {
            var listener = owner.connectorListener;
            ...
            foreach (var p in ports)
                ...
                if (!portViews.Any(pv => p.portData.identifier == pv.portData.identifier)) {
                    var portDirection = nodeTarget.IsFieldInput(p.fieldName) ? Direction.Input
                        : Direction.Output;
        ...
        protected virtual VisualElement CreateSettingsView() =>
            new Label("Settings") { name = "header" };
