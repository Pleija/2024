...
public class RelayNodeView : BaseNodeView
{
    ...
    {
        if (relay.GetNonRelayEdges().Count != 1) return DropdownMenuAction.Status.Disabled;
        if (relay.packInput)
    ...
    {
        if (relay.GetNonRelayEdges().Count == 0) return DropdownMenuAction.Status.Disabled;
    ...
    private void UpdateSize()
    {
        if (relay.unpackOutput) {
            ...
            style.width = -1;
            if (input != null) input.style.height = -1;
            if (output != null) output.style.height = -1;
            RemoveFromClassList("hideLabels");
        }
        else {
            style.height = 20;
            style.width = 50;
            if (input != null) input.style.height = 16;
            if (output != null) output.style.height = 16;
    ...
    public override void OnRemoved()
    {
        ...
        schedule.Execute(() => {
            if (!relay.unpackOutput) {
                ...
                if (inputEdges.Count == 0 || outputEdges.Count == 0) return;
