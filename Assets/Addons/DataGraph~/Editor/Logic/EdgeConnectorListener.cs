...
namespace GraphProcessor
{
    ...
    public class BaseEdgeConnectorListener : IEdgeConnectorListener
    {
        ...
        public virtual void OnDropOutsidePort(Edge edge, Vector2 position)
        {
            ...
            if (!edge.isGhostEdge) graphView.Disconnect(edge as EdgeView);
        ...
        public virtual void OnDrop(GraphView graphView, Edge edge)
        {
            ...
            if (edgeView.isConnected) {
                ...
                if (!wasOnTheSamePort) this.graphView.Disconnect(edgeView);
            }
            if (edgeView.input.node == null || edgeView.output.node == null) return;
            edgeInputPorts[edge] = edge.input as PortView;
            edgeOutputPorts[edge] = edge.output as PortView;

            try {
                this.graphView.RegisterCompleteObjectUndo("Connected " + edgeView.input.node.name +
                    " and " + edgeView.output.node.name);
        ...
        private void ShowNodeCreationMenuFromEdge(EdgeView edgeView, Vector2 position)
        {
            ...
            SearchWindow.Open(
                new SearchWindowContext(position + EditorWindow.focusedWindow.position.position),
