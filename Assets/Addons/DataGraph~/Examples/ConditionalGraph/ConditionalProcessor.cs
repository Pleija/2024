...
namespace Nodes.Examples.ConditionalGraph
{
    public class ConditionalProcessor : BaseGraphProcessor
    {
        ...
        private Dictionary<BaseNode, List<BaseNode>> nonConditionalDependenciesCache =
        ...
        {
            // Gather start nodes:
            startNodeList = graph.nodes.Where(n => n is StartNode).Select(n => n as StartNode)
                .ToList();
        ...
        private IEnumerable<BaseNode> GatherNonConditionalDependencies(BaseNode node)
        {
            ...
            while (dependencies.Count > 0) {
                ...
                foreach (var d in dependency.GetInputNodes().Where(n => !(n is IConditionalNode)))
                    dependencies.Push(d);
                if (dependency != node) yield return dependency;
            }
        }
                    ...
                    if (nodeDependenciesGathered.Contains(node)) {
                        ...
                        switch (node) {
                            ...
                            case ForLoopNode forLoopNode:
                                forLoopNode.index =
                                    forLoopNode.start - 1; // Initialize the start index
                            ...
                            case IConditionalNode cNode:
                                foreach (var n in cNode.GetExecutedNodes()) nodeToExecute.Push(n);
                                break;
                            default:
        ...
        public void Step()
        {
            if (currentGraphExecution == null) {
                var nodeToExecute = new Stack<BaseNode>();
                if (startNodeList.Count > 0) startNodeList.ForEach(s => nodeToExecute.Push(s));
                currentGraphExecution = startNodeList.Count == 0 ? RunTheGraph()
