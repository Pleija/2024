...
namespace GraphProcessor
{
    ...
    /// </summary>
    public enum ComputeOrderType { DepthFirst, BreadthFirst }
    ...
    public class BaseGraph : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        /// <summary>
        ///     Json list of serialized nodes only used for copy pasting in the editor. Note that this field
        ///     isn't serialized
        /// </summary>
        ...
        public Dictionary<string, SerializableEdge> edgesPerGUID =
            new Dictionary<string, SerializableEdge>();
        ...
        private List<ExposedParameter> serializedParameterList = new List<ExposedParameter>();

        [SerializeField]
        public List<StickyNote> stickyNotes = new List<StickyNote>();

        [NonSerialized]
        private Dictionary<BaseNode, int> computeOrderDictionary = new Dictionary<BaseNode, int>();
        ...
        {
            isEnabled = false;
            foreach (var node in nodes) node.DisableInternal();
        }
        ...
        public void Disconnect(BaseNode inputNode, string inputFieldName, BaseNode outputNode,
            string outputFieldName)
        {
            edges.RemoveAll(r => {
                var remove = r.inputNode == inputNode && r.outputNode == outputNode &&
                    r.outputFieldName == outputFieldName && r.inputFieldName == inputFieldName;
        ...
        public void Disconnect(string edgeGUID)
        {
            ...
            foreach (var (node, edge) in disconnectEvents) node?.OnEdgeDisconnected(edge);
        }
        ...
        /// <summary>
        ///     Invoke the onGraphChanges event, can be used as trigger to execute the graph when the content
...
#pragma warning restore CS0618
        ...
        public void UpdateComputeOrder(ComputeOrderType type = ComputeOrderType.DepthFirst)
        {
            ...
            foreach (var node in nodes) {
                if (node.GetOutputNodes().Count() == 0) graphOutputs.Add(node);
                node.computeOrder = 0;
            }
            ...
            switch (type) {
                ...
                case ComputeOrderType.BreadthFirst:
                    foreach (var node in nodes) UpdateComputeOrderBreadthFirst(0, node);
                    break;
            }
        }
        ...
        public string AddExposedParameter(string name, Type type, object value = null)
        {
            if (!type.IsSubclassOf(typeof(ExposedParameter)))
                Debug.LogError(
                    $"Can't add parameter of type {type}, the type doesn't inherit from ExposedParameter.");
        ...
        public void UpdateExposedParameter(string guid, object value)
        {
            ...
            if (value != null && !param.GetValueType().IsAssignableFrom(value.GetType()))
                throw new Exception("Type mismatch when updating parameter " + param.name +
                    ": from " + param.GetValueType() + " to " +
        ...
        public object GetParameterValue(string name) =>
            exposedParameters.FirstOrDefault(p => p.name == name)?.value;
        ...
        /// <summary>
        ///     Link the current graph to the scene in parameter, allowing the graph to pick and serialize
        ...
        private HashSet<BaseNode> infiniteLoopTracker = new HashSet<BaseNode>();
        ...
        private int UpdateComputeOrderBreadthFirst(int depth, BaseNode node)
        {
            ...
            if (computeOrderDictionary.ContainsKey(node)) return node.computeOrder;
            if (!infiniteLoopTracker.Add(node)) return -1;
            ...
            if (computeOrder != -1) computeOrder++;
        ...
        private void UpdateComputeOrderDepthFirst()
        {
            ...
            foreach (var node in GraphUtils.DepthFirstSort(this)) {
                if (node.computeOrder == loopComputeOrder) continue;
                if (!node.canProcess)
                    node.computeOrder = -1;
                else
                    node.computeOrder = computeOrder++;
        ...
        private void PropagateComputeOrder(BaseNode node, int computeOrder)
        {
            ...
            while (deps.Count > 0) {
                ...
                foreach (var dep in n.GetOutputNodes()) deps.Push(dep);
            }
        }
        ...
        {
            edges.RemoveAll(e =>
                e.inputNode == null || e.outputNode == null ||
                string.IsNullOrEmpty(e.outputFieldName) || string.IsNullOrEmpty(e.inputFieldName));
            nodes.RemoveAll(n => n == null);
        }
        ...
        public static bool TypesAreConnectable(Type t1, Type t2)
        {
            ...
            if (TypeAdapter.AreIncompatible(t1, t2)) return false;

            //Check if there is custom adapters for this assignation
            if (CustomPortIO.IsAssignable(t1, t2)) return true;

            //Check for type assignability
            if (t2.IsReallyAssignableFrom(t1)) return true;

            // User defined type convertions
            if (TypeAdapter.AreAssignable(t1, t2)) return true;
            return false;
        }
    }
}
