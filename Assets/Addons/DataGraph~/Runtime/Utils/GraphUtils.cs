    ...
    public static class GraphUtils
    {
        ...
        private static TraversalGraph ConvertGraphToTraversalGraph(BaseGraph graph)
        {
            ...
            foreach (var node in graph.nodes) {
                ...
                g.nodes.Add(tn);
                nodeMap[node] = tn;
                if (graph.graphOutputs.Contains(node)) g.outputs.Add(tn);
            }

            foreach (var tn in g.nodes) {
                tn.inputs = tn.node.GetInputNodes().Where(n => nodeMap.ContainsKey(n))
                    .Select(n => nodeMap[n]).ToList();
                tn.outputs = tn.node.GetOutputNodes().Where(n => nodeMap.ContainsKey(n))
                    .Select(n => nodeMap[n]).ToList();
            }
            return g;
        }
        ...
        public static List<BaseNode> DepthFirstSort(BaseGraph g)
        {
            ...
            foreach (var n in graph.nodes) DFS(n);

            void DFS(TarversalNode n)
            {
                if (n.state == State.Black) return;
                n.state = State.Grey;

                if (n.node is ParameterNode parameterNode &&
                    parameterNode.accessor == ParameterAccessor.Get) {
        ...
        public static void FindCyclesInGraph(BaseGraph g, Action<BaseNode> cyclicNode)
        {
            ...
            void DFS(TarversalNode n)
            {
                ...
                foreach (var input in n.inputs)
                    if (input.state == State.White)
                        DFS(input);
                    else if (input.state == State.Grey) cyclicNodes.Add(n);
                n.state = State.Black;
            }
