...
namespace GraphProcessor
{
    ...
    public class PortData : IEquatable<PortData>
    {
        ...
        public bool Equals(PortData other) => identifier == other.identifier &&
            displayName == other.displayName && displayType == other.displayType &&
            acceptMultipleEdges == other.acceptMultipleEdges && sizeInPixel == other.sizeInPixel &&
            tooltip == other.tooltip && vertical == other.vertical;
    ...
    public class NodePort
    {
        ...
        private List<SerializableEdge> edges = new List<SerializableEdge>();

        private Dictionary<SerializableEdge, PushDataDelegate> pushDataDelegates =
            new Dictionary<SerializableEdge, PushDataDelegate>();

        private List<SerializableEdge> edgeWithRemoteCustomIO = new List<SerializableEdge>();
        ...
        ///     This is an optimization compared to dynamically setting values using Reflection (which is
        ///     really slow)
        ///     More info:
        ...
        public NodePort(BaseNode owner, string fieldName, PortData portData) : this(owner, owner,
            fieldName, portData) { }
        ...
        public NodePort(BaseNode owner, object fieldOwner, string fieldName, PortData portData)
        {
            ...
            fieldInfo = fieldOwner.GetType().GetField(fieldName,
        ...
        public void Add(SerializableEdge edge)
        {
            ...
            if (edge.inputNode == owner) {
                if (edge.outputPort.customPortIOMethod != null) edgeWithRemoteCustomIO.Add(edge);
            }
            else {
                if (edge.inputPort.customPortIOMethod != null) edgeWithRemoteCustomIO.Add(edge);
            }

            //if we have a custom io implementation, we don't need to genereate the defaut one
            if (edge.inputPort.customPortIOMethod != null ||
                edge.outputPort.customPortIOMethod != null)
                return;
            var edgeDelegate = CreatePushDataDelegateForEdge(edge);
            if (edgeDelegate != null) pushDataDelegates[edge] = edgeDelegate;
        }
        ...
        private PushDataDelegate CreatePushDataDelegateForEdge(SerializableEdge edge)
        {
            try {
                ...
                if (!BaseGraph.TypesAreConnectable(inputField.FieldType, outputField.FieldType)) {
                    Debug.LogError("Can't convert from " + inputField.FieldType + " to " +
                ...
                Expression inputParamField =
                    Expression.Field(Expression.Constant(edge.inputNode), inputField);
                Expression outputParamField =
                    Expression.Field(Expression.Constant(edge.outputNode), outputField);
                inType = edge.inputPort.portData.displayType ?? inputField.FieldType;
                outType = edge.outputPort.portData.displayType ?? outputField.FieldType;
                ...
                if (TypeAdapter.AreAssignable(outType, inType)) {
                    ...
                    outputParamField =
                        Expression.Call(TypeAdapter.GetConvertionMethod(outType, inType),
                            convertedParam);
        ...
        public void PushData()
        {
            ...
            foreach (var pushDataDelegate in pushDataDelegates) pushDataDelegate.Value();
            if (edgeWithRemoteCustomIO.Count == 0) return;
            ...
            foreach (var edge in edgeWithRemoteCustomIO) edge.passThroughBuffer = ourValue;
        }
            ...
            if (edges.Count > 0) {
                ...
                if (passThroughObject != null)
                    if (TypeAdapter.AreAssignable(fieldInfo.FieldType, passThroughObject.GetType()))
                        passThroughObject =
                            TypeAdapter.Convert(passThroughObject, fieldInfo.FieldType);
    ...
    public abstract class NodePortContainer : List<NodePort>
    {
        ...
        public void Add(SerializableEdge edge)
        {
            ...
            var portIdentifier = edge.inputNode == node ? edge.inputPortIdentifier
                : edge.outputPortIdentifier;

            // Force empty string to null since portIdentifier is a serialized value
            if (string.IsNullOrEmpty(portIdentifier)) portIdentifier = null;
