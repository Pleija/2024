...
namespace GraphProcessor
{
    ...
    public delegate IEnumerable<PortData> CustomPortTypeBehaviorDelegate(string fieldName,
    ...
    public abstract class BaseNode
    {
        ...
        /// <summary>
        ///     Set a custom uss file for the node. We use a Resources.Load to get the stylesheet so be sure to
        ///     put the correct resources path
        ...
        /// <summary>
        ///     Tell wether or not the node can be processed. Do not check anything from inputs because
        ///     this step happens before inputs are sent to the node
        /// </summary>
        ...
        [NonSerialized]
        private bool _needsInspector = false;
        ...
        /// <summary>
        ///     Can the node be renamed in the UI. By default a node can be renamed by double clicking it's
        ///     name.
        /// </summary>
        ...
        /// <summary>
        ///     True only when the node was created from a duplicate operation and is inside a group that was
        ///     also duplicated at the same time.
        /// </summary>
        public bool createdWithinGroup { get; internal set; } = false;

        [NonSerialized]
        internal Dictionary<string, NodeFieldInformation> nodeFields =
            new Dictionary<string, NodeFieldInformation>();

        [NonSerialized]
        internal Dictionary<Type, CustomPortTypeBehaviorDelegate> customPortTypeBehaviorMap =
        ...
        internal class NodeFieldInformation
        {
            ...
            public NodeFieldInformation(FieldInfo info, string name, bool input, bool isMultiple,
                string tooltip, bool vertical, CustomPortBehaviorDelegate behavior)
            {
                this.input = input;
        ...
        private Stack<PortUpdate> fieldsToUpdate = new Stack<PortUpdate>();
        private HashSet<PortUpdate> updatedFields = new HashSet<PortUpdate>();
...
#region Initialization
        ...
        private void InitializeCustomPortTypeMethods()
        {
            ...
            while (true) {
                ...
                foreach (var method in methods) {
                    ...
                    if (typeBehaviors.Length == 0) continue;
                    CustomPortTypeBehaviorDelegate deleg = null;

                    try {
                        deleg = Delegate.CreateDelegate(typeof(CustomPortTypeBehaviorDelegate),
                            this, method) as CustomPortTypeBehaviorDelegate;
                    }
                    catch (Exception e) {
                        Debug.LogError(e);
                        Debug.LogError(
        ...
        ///     This will allow the node creation menu to correctly recognize ports that can be connected
        ///     between nodes
        /// </summary>
            ...
            foreach (var key in OverrideFieldOrder(nodeFields.Values.Select(k => k.info))) {
                ...
                if (HasCustomBehavior(nodeField))
                    ...
                    AddPort(nodeField.input, nodeField.fieldName, new PortData {
                        ...
                        displayName = nodeField.name, tooltip = nodeField.tooltip,
        ...
        /// <summary>
        ///     Override the field order inside the node. It allows to re-order all the ports and field in the
        ///     UI.
        /// </summary>
        ...
        public virtual IEnumerable<FieldInfo> OverrideFieldOrder(IEnumerable<FieldInfo> fields)
        {
            ...
            return fields.OrderByDescending(f =>
                (long)(GetFieldInheritanceLevel(f) << 32 | (long)f.MetadataToken));
        }
        ...
        /// <summary>
        ///     Update all ports of the node without updating the connected ports. Only use this method when
        ///     you need to update all the nodes ports in your graph.
        /// </summary>
        ...
        {
            var changed = false;
            if (!nodeFields.ContainsKey(fieldName)) return false;
            var fieldInfo = nodeFields[fieldName];
            if (!HasCustomBehavior(fieldInfo)) return false;
            ...
            if (fieldInfo.behavior != null) {
                foreach (var portData in fieldInfo.behavior(edges)) AddPortData(portData);
            }
            else {
            ...
            {
                var port =
                    nodePorts.FirstOrDefault(n => n.portData.identifier == portData.identifier);
                ...
                if (port == null) {
                    ...
                    if (!BaseGraph.TypesAreConnectable(port.portData.displayType,
                        portData.displayType))
            ...
            if (sendPortUpdatedEvent) onPortsUpdated?.Invoke(fieldName);
            return changed;
        }

        private bool HasCustomBehavior(NodeFieldInformation info)
        {
            if (info.behavior != null) return true;
            if (customPortTypeBehaviorMap.ContainsKey(info.info.FieldType)) return true;
            return false;
        }
        ...
        public bool UpdatePortsForField(string fieldName, bool sendPortUpdatedEvent = true)
        {
            ...
            fieldsToUpdate.Push(new PortUpdate
                { fieldNames = new List<string>() { fieldName }, node = this });
                ...
                foreach (var field in fields)
                    if (node.UpdatePortsForFieldLocal(field, sendPortUpdatedEvent)) {
                        foreach (var port in node.IsFieldInput(field)
                            ? (NodePortContainer)node.inputPorts : node.outputPorts) {
                            if (port.fieldName != field) continue;

                            foreach (var edge in port.GetEdges()) {
                                var edgeNode = node.IsFieldInput(field) ? edge.outputNode
                                    : edge.inputNode;
                                var fieldsWithBehavior = edgeNode.nodeFields.Values
                                    .Where(f => HasCustomBehavior(f)).Select(f => f.fieldName)
                                    .ToList();
                                fieldsToUpdate.Push(new PortUpdate
        ...
        private HashSet<BaseNode> portUpdateHashSet = new HashSet<BaseNode>();
        ...
        public virtual FieldInfo[] GetNodeFields() => GetType()
        ...
        private void InitializeInOutDatas()
        {
            ...
            foreach (var field in fields) {
                ...
                var name = field.Name;
                string tooltip = null;
                if (showInInspector != null) _needsInspector = true;
                if (inputAttribute == null && outputAttribute == null) continue;

                //check if field is a collection type
                isMultiple = inputAttribute != null ? inputAttribute.allowMultiple
                    : outputAttribute.allowMultiple;
                input = inputAttribute != null;
                tooltip = tooltipAttribute?.tooltip;
                if (!string.IsNullOrEmpty(inputAttribute?.name)) name = inputAttribute.name;
                if (!string.IsNullOrEmpty(outputAttribute?.name)) name = outputAttribute.name;

                // By default we set the behavior to null, if the field have a custom behavior, it will be set in the loop just below
                nodeFields[field.Name] = new NodeFieldInformation(field, name, input, isMultiple,
                    tooltip, vertical != null, null);
            }

            foreach (var method in methods) {
                var customPortBehaviorAttribute =
                    method.GetCustomAttribute<CustomPortBehaviorAttribute>();
                CustomPortBehaviorDelegate behavior = null;
                if (customPortBehaviorAttribute == null) continue;

                // Check if custom port behavior function is valid
                try {
                    var referenceType = typeof(CustomPortBehaviorDelegate);
                    behavior =
                        (CustomPortBehaviorDelegate)Delegate.CreateDelegate(referenceType, this,
                            method, true);
                }
                catch {
                    Debug.LogError("The function " + method +
                        " cannot be converted to the required delegate format: " +
...
#region Events and Processing
        ...
        public void OnEdgeDisconnected(SerializableEdge edge)
        {
            ...
            var haveConnectedEdges = edge.inputNode.inputPorts
                .Where(p => p.fieldName == edge.inputFieldName).Any(p => p.GetEdges().Count != 0);
...
#region API and utils
        ...
        public BaseNode FindInDependencies(Func<BaseNode, bool> condition)
        {
            ...
            while (dependencies.Count > 0) {
                ...
                foreach (var dep in node.GetInputNodes()) dependencies.Push(dep);
            }
            return null;
        }
        ...
        public NodePort GetPort(string fieldName, string identifier)
        {
            return inputPorts.Concat(outputPorts).FirstOrDefault(p => {
                var bothNull = string.IsNullOrEmpty(identifier) &&
                    string.IsNullOrEmpty(p.portData.identifier);
                return p.fieldName == fieldName &&
                    (bothNull || identifier == p.portData.identifier);
            });
        }
        ...
        public IEnumerable<NodePort> GetAllPorts()
        {
            ...
            foreach (var port in outputPorts) yield return port;
        }
        ...
        {
            foreach (var message in messages) onMessageRemoved?.Invoke(message);
            messages.Clear();
        }
        ...
        /// <summary>
        ///     Get the name of the node. If the node have a custom name (set using the UI by double clicking
        ///     on the node title) then it will return this name first, otherwise it returns the value of the
        ///     name field.
        /// </summary>
        /// <returns>The name of the node as written in the title</returns>
        public string GetCustomName() =>
            string.IsNullOrEmpty(nodeCustomName) ? name : nodeCustomName;
#endregion
    }
}
