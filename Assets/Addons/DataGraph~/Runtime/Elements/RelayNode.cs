...
public class RelayNode : BaseNode
{
    ...
    private SerializableType inputType = new SerializableType(typeof(object));
    ...
    public void GetInputs(List<SerializableEdge> edges)
    {
        ...
        if (edges.Count == 1 && edges.First().outputNode.GetType() == typeof(RelayNode)) {
            ...
            input.types = edges.Select(e =>
                e.outputPort.portData.displayType ?? e.outputPort.fieldInfo.FieldType).ToList();
        }
    }
        ...
        if (outputPort.portData.identifier != packIdentifier && outputIndex >= 0 &&
            ...
            foreach (var edge in edges) {
                ...
                edge.passThroughBuffer =
                    inputRelay != null && !inputRelay.packInput ? output : data;
            }
        }
        else {
            foreach (var edge in edges) edge.passThroughBuffer = output;
        }
    }
    ...
    private IEnumerable<PortData> OutputPortBehavior(List<SerializableEdge> edges)
    {
        ...
        if (unpackOutput && inputPortEdges.Count == 1) {
            yield return new PortData {
                ...
                acceptMultipleEdges = true,
                sizeInPixel =
                    Mathf.Min(k_MaxPortSize,
                        Mathf.Max(underlyingPortData.Count, 1) + 7), // TODO: function
            };
    ...
    private static List<(Type, string)> s_empty = new List<(Type, string)>();

    public List<(Type type, string name)> GetUnderlyingPortDataList()
    {
        // get input edges:
        if (inputPorts.Count == 0) return s_empty;
        var inputEdges = GetNonRelayEdges();
        if (inputEdges != null)
            return inputEdges.Select(e => (
                e.outputPort.portData.displayType ?? e.outputPort.fieldInfo.FieldType,
    ...
    public List<SerializableEdge> GetNonRelayEdges()
    {
        ...
        while (inputEdges.Count == 1 &&
            inputEdges.First().outputNode.GetType() == typeof(RelayNode))
