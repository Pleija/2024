...
namespace GraphProcessor
{
    ...
    public static class CustomPortIO
    {
        ...
        private static Dictionary<Type, List<Type>> assignableTypes =
            new Dictionary<Type, List<Type>>();

        private static PortIOPerNode customIOPortMethods = new PortIOPerNode();
        ...
        private static void LoadCustomPortMethods()
        {
            ...
            foreach (var type in AppDomain.CurrentDomain.GetAllTypes()) {
                ...
                if (!type.IsSubclassOf(typeof(BaseNode))) continue;
                ...
                foreach (var method in methods) {
                    ...
                    if (portInputAttr == null && portOutputAttr == null) continue;
                    ...
                    if (deleg == null) {
                        Debug.LogWarning("Can't use custom IO port function " + method +
                            ": The method have to respect this format: " +
                            typeof(CustomPortIODelegate));
                        continue;
                    }
                    var fieldName = portInputAttr == null ? portOutputAttr.fieldName
                        : portInputAttr.fieldName;
                    var customType = portInputAttr == null ? portOutputAttr.outputType
                        : portInputAttr.inputType;
        ...
        public static CustomPortIODelegate GetCustomPortMethod(Type nodeType, string fieldName)
        {
            ...
            if (portIOPerField == null) return null;
            portIOPerField.TryGetValue(fieldName, out deleg);
            return deleg;
        }

        private static void AddCustomIOMethod(Type nodeType, string fieldName,
            CustomPortIODelegate deleg)
        {
        ...
        {
            if (assignableTypes.ContainsKey(input)) return assignableTypes[input].Contains(output);
            return false;
        }
    }
}
