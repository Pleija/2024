...
{
    /// <summary>
    ///     Implement this interface to use the inside your class to define type convertions to use inside
    ///     the graph.
    ///     Example:
    ///     <code>
    ...
    {
        private static Dictionary<(Type from, Type to), Func<object, object>> adapters =
            new Dictionary<(Type, Type), Func<object, object>>();

        private static Dictionary<(Type from, Type to), MethodInfo> adapterMethods =
            new Dictionary<(Type, Type), MethodInfo>();

        private static List<(Type from, Type to)> incompatibleTypes =
            new List<( Type from, Type to)>();

        [NonSerialized]
        private static bool adaptersLoaded = false;

#if !ENABLE_IL2CPP
        private static Func<object, object> ConvertTypeMethodHelper<TParam, TReturn>(
            MethodInfo method)
        {
            // Convert the slow MethodInfo into a fast, strongly typed, open delegate
            var func =
                (Func<TParam, TReturn>)Delegate.CreateDelegate(typeof(Func<TParam, TReturn>),
                    method);
                ...
                if (typeof(ITypeAdapter).IsAssignableFrom(type)) {
                    ...
                    foreach (var method in type.GetMethods(BindingFlags.Static |
                        ...
                        if (method.ReturnType == typeof(void)) {
                            Debug.LogError(
                                $"Ignoring convertion method {method} because it does not returns anything");
                            continue;
                        }
                        ...
                        try {
                            ...
                            Func<object, object> r = (object param) => {
                                return (object)method.Invoke(null, new object[] { param });
                            };
#else
                            var genericHelper = typeof(TypeAdapter).GetMethod(
                                "ConvertTypeMethodHelper",
...
#endif
                            adapters.Add(
                                (method.GetParameters()[0].ParameterType, method.ReturnType), r);
                            adapterMethods.Add(
                            ...
                            Debug.LogError(
                                $"Failed to load the type convertion method: {method}\n{e}");
                        }
                    }
                }
        ...
        {
            if (incompatibleTypes.Any((k) => k.from == from && k.to == to)) return true;
            return false;
        }

        public static bool AreAssignable(Type from, Type to)
        {
            if (!adaptersLoaded) LoadAllAdapters();
            if (AreIncompatible(from, to)) return false;
            return adapters.ContainsKey((from, to));
        }

        public static MethodInfo GetConvertionMethod(Type from, Type to) =>
            adapterMethods[(from, to)];
