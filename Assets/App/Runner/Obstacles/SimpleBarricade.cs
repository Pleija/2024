    ...
    public class SimpleBarricade : Obstacle
    {
        ...
        public override IEnumerator Spawn(TrackSegment segment, float t)
        {
            ...
            var isTutorialFirst = TrackManager.instance.isTutorial
                    && TrackManager.instance.firstObstacle
                    && segment == segment.manager.currentSegment;
            if (isTutorialFirst) TrackManager.instance.firstObstacle = false;
            var count = isTutorialFirst ? 1 : Random.Range(
                k_MinObstacleCount, k_MaxObstacleCount + 1);
            var startLane = isTutorialFirst ? 0 : Random.Range(
                k_LeftMostLaneIndex, k_RightMostLaneIndex + 1);
            Vector3 position;
            Quaternion rotation;
