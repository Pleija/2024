...
public class Sample : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        GUI.changed = false;
        m_TagOn["All"] = GUILayout.Toggle(
            m_TagOn["All"], "All", "button", GUILayout.ExpandWidth(false));
        on |= GUI.changed;
        ...
        foreach (TagName tag in Tag.Object.all) {
            ...
            on |= GUI.changed;
            GUI.changed = false;
            m_TagOff[tag] = GUILayout.Toggle(
                m_TagOff[tag], "X", "button", GUILayout.ExpandWidth(false));
            off |= GUI.changed;
        }
        ...
        if (on)
            foreach (var kv in m_TagOn)
                if (kv.Value)
                    m_TagOff[kv.Key] = false;
        if (off)
            foreach (var kv in m_TagOff)
                if (kv.Value)
                    m_TagOn[kv.Key] = false;
