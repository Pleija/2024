...
namespace MoreTags
{
    ...
    public static class TagHelper
    {
        ...
        private static TagNames ConvertToTagNames(string str)
        {
            ...
            if (!str.StartsWith("*.")) {
                if (str.EndsWith(".*")) return ((TagGroup)str.TrimEnd('.', '*')).all;
                if (str.EndsWith(".?")) return ((TagGroup)str.TrimEnd('.', '?')).children;
                return (TagName)str;
            }
            str = str.Substring(2);
            if (str.EndsWith(".*")) return ((TagChildren)str.TrimEnd('.', '*')).all;
            if (str.EndsWith(".?")) return ((TagChildren)str.TrimEnd('.', '?')).children;
