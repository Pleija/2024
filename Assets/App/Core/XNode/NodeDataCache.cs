...
namespace XNode
{
    ...
    public static class NodeDataCache
    {
            ...
            foreach (var port in ports.Values.ToList()) {
                ...
                // If port doesn't exist anymore, remove it
                    ...
                    if (formerlySerializedAs != null
                        && formerlySerializedAs.TryGetValue(port.fieldName, out newName))
        ...
        /// <summary>
        ///     Extracts the underlying types from arrays and lists, the only collections for dynamic port
        ///     lists
        ...
        private static Type GetBackingValueType(Type portValType)
        {
            ...
            if (portValType.IsGenericType
                && portValType.GetGenericTypeDefinition() == typeof(List<>))
        ...
        {
            var fieldInfo = new List<FieldInfo>(nodeType.GetFields(BindingFlags.Public
            ...
            while ((tempType = tempType.BaseType) != typeof(Node)) {
                var parentFields =
                        tempType.GetFields(BindingFlags.NonPublic | BindingFlags.Instance);
...
#if ODIN_INSPECTOR
        public static List<PropertyInfo> GetNodeProperties(Type nodeType)
        {
            var propertyInfo = new List<PropertyInfo>(nodeType.GetProperties(BindingFlags.Public
                | BindingFlags.NonPublic
                | BindingFlags.Instance));
            ...
            while ((tempType = tempType.BaseType) != typeof(Node)) {
                var parentProperties =
                        tempType.GetProperties(BindingFlags.NonPublic | BindingFlags.Instance);

                for (var i = 0; i < parentProperties.Length; i++) {
                    // Ensure that we do not already have a member with this type and name
                    var parentProperty = parentProperties[i];
        ...
        private static void CachePorts(Type nodeType)
        {
            ...
            for (var i = 0; i < fieldInfo.Count; i++) {
                ...
                var inputAttrib =
                        attribs.FirstOrDefault(x =>
                                x is Node.InputAttribute) as Node.InputAttribute;
                var outputAttrib =
                        attribs.FirstOrDefault(x => x is Node.OutputAttribute) as
                                Node.OutputAttribute;
                var formerlySerializedAsAttribute =
                ...
                if (formerlySerializedAsAttribute != null) {
                    ...
                    if (formerlySerializedAsCache[nodeType]
                        .ContainsKey(formerlySerializedAsAttribute.oldName))
                        Debug.LogError("Another FormerlySerializedAs with value '"
                            + formerlySerializedAsAttribute.oldName
            ...
            var supportsPropertyPorts =
                    nodeType.GetCustomAttribute<ShowOdinSerializedPropertiesInInspectorAttribute>()
                    != null;
            var propertyInfo = GetNodeProperties(nodeType);

            for (var i = 0; i < propertyInfo.Count; i++) {
                //Get InputAttribute and OutputAttribute
                var attribs = propertyInfo[i].GetCustomAttributes(true);
                var inputAttrib =
                        attribs.FirstOrDefault(x =>
                                x is Node.InputAttribute) as Node.InputAttribute;
                var outputAttrib =
                ...
                if (inputAttrib != null && outputAttrib != null) {
                    ...
                    if (!supportsPropertyPorts) {
                        Debug.LogError(
                            "This Node type does not support properties as ports. Is this type serialized by Odin and includes the ShowOdinSerializedPropertiesInInspector attribute?");
                    }
                    else {
                        if (!portDataCache.ContainsKey(nodeType))
                            portDataCache.Add(nodeType, new List<NodePort>());
