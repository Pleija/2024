...
namespace XNode
{
    ...
    public class NodePort
    {
        ...
        /// </summary>
        public NodePort(string fieldName, Type type, IO direction,
            Node.ConnectionType connectionType, Node.TypeConstraint typeConstraint, Node node)
        {
        ...
        /// <summary>
        ///     Return the output value of the first connected port. Returns null if none found or
        ///     invalid.
        /// </summary>
        /// <returns>
        ...
        public void Connect(NodePort port)
        {
            ...
            if (direction == port.direction) {
                Debug.LogWarning("Cannot connect two "
                    + (direction == IO.Input ? "input" : "output")
                    + " connections");
                return;
            }
#if UNITY_EDITOR
        ...
        public bool CanConnectTo(NodePort port)
        {
            ...
            if (input.typeConstraint == Node.TypeConstraint.Inherited
                ...
                return false;
            if (input.typeConstraint == Node.TypeConstraint.Strict
                && input.ValueType != output.ValueType)
                return false;
            ...
            if (output.typeConstraint == Node.TypeConstraint.Inherited
                && !input.ValueType.IsAssignableFrom(output.ValueType))
                return false;
            if (output.typeConstraint == Node.TypeConstraint.Strict
                && input.ValueType != output.ValueType)
                return false;
        ...
        public void Disconnect(int i)
        {
            ...
            if (otherPort != null)
                otherPort.connections.RemoveAll(it => {
                    return it.Port == this;
                });
            // Remove this ports connection to the other
            connections.RemoveAt(i);
        ...
        public void SwapConnections(NodePort targetPort)
        {
            ...
            for (var i = 0; i < aConnectionCount; i++) portConnections.Add(connections[i].Port);
            ...
            for (var i = 0; i < portConnections.Count; i++) targetPort.Connect(portConnections[i]);

            // Add target port connections to this one
            for (var i = 0; i < targetPortConnections.Count; i++) Connect(targetPortConnections[i]);
        }
