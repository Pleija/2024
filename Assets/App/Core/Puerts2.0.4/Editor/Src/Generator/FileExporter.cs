        ...
        public class FileExporter
        {
                ...
                bool csharpModuleWillGen = false)
            {
                if (!Utils.HasFilter) {
                    ...
                    genTypes = configure["Puerts.BindingAttribute"]
                            .Select(kv => kv.Key)
                            .Where(o => o is Type)
                            .Cast<Type>()
                ...
                var tsTypes = configure["Puerts.TypingAttribute"]
                        .Select(kv => kv.Key)
                        .Where(o => o is Type)
                        .Cast<Type>()
                        // 导不出泛型 https://github.com/Tencent/puerts/issues/963
                ...
                using (var jsEnv = new JsEnv(loader)) {
                    ...
                    var typingRender = jsEnv.ExecuteModule<Func<TypingGenInfo, bool, string>>(
                        "puerts/templates/dts.tpl.mjs", "default");

                    using (var textWriter = new StreamWriter(saveTo + "Typing~/csharp/index.d.ts",
                        false, Encoding.UTF8)) {
                        var fileContext = typingRender(TypingGenInfo.FromTypes(tsTypes),
                            csharpModuleWillGen);
            ...
            public static void ExportWrapper(string saveTo, ILoader loader = null)
            {
                ...
                var blittableCopyTypes = new HashSet<Type>(
                    configure["Puerts.BlittableCopyAttribute"]
                            .Select(kv => kv.Key)
                            .Where(o => o is Type)
                            .Cast<Type>()
                ...
                using (var jsEnv = new JsEnv(loader)) {
                    var wrapRender = jsEnv.ExecuteModule<Func<StaticWrapperInfo, string>>(
                    ...
                    foreach (var type in genTypes) {
                        if (type.IsEnum
                            ...
                            continue;
                        var staticWrapperInfo = StaticWrapperInfo.FromType(type, genTypes);
                    ...
                    foreach (var item in wrapperInfoMap) {
                        ...
                        while (makeFileUniqueMap.ContainsKey(filePath.ToLower())) {
                            filePath = saveTo
                                    + staticWrapperInfo.WrapClassName
                                    + "_"
                                    + uniqueId
                                    + ".cs";
                            uniqueId++;
            ...
            public static void GenRegisterInfo(string outDir, ILoader loader = null)
            {
                ...
                using (var jsEnv = new JsEnv(loader)) {
                    var registerInfoRender =
                            jsEnv.ExecuteModule<Func<List<RegisterInfoForGenerate>, string>>(
                                "puerts/templates/registerinfo.tpl.mjs", "default");
                    ...
                    using (var textWriter = new StreamWriter(
                        registerInfoPath, false, Encoding.UTF8)) {
