...
namespace Puerts.Editor
{
    namespace Generator
    {
        namespace Wrapper
        {
            ...
            {
                public Dictionary<string, LazyMemberRegisterInfo> LazyMembers =
                        new Dictionary<string, LazyMemberRegisterInfo>();

                public void Add(MethodInfo m)
                {
                    if (LazyMembers.ContainsKey(m.Name)) return;
                    LazyMembers.Add(m.Name, new LazyMemberRegisterInfo {
                        Name = m.Name, IsStatic = m.IsStatic, Type = LazyMemberType.Method,
                    });
                }
                ...
                public void Add(ConstructorInfo m)
                {
                    if (LazyMembers.ContainsKey(m.Name)) return;
                    LazyMembers.Add(m.Name, new LazyMemberRegisterInfo {
                        Name = m.Name, IsStatic = m.IsStatic, Type = LazyMemberType.Constructor,
                    });
                }
                ...
                public bool Contains(string name) => LazyMembers.ContainsKey(name);
            ...
            public class StaticWrapperInfo
            {
                ...
                public static StaticWrapperInfo FromType(Type type, List<Type> genTypes)
                {
                    ...
                    if (type.IsGenericType) {
                        ...
                        if (genericArguments.Where((t, index) => !t.IsPrimitive && t != typeof(System.String) && t != typeof(DateTime)).Count() > 0) {
                            ...
                            type = definitionType.MakeGenericType(
                                definitionGenericArguments.Select(
                                    (dType, index) => {
                            ...
                            GenericArgumentsInfos = definitionGenericArguments.Select(
                                (ga, index) => {
                                    Type t = genericArguments[index];
                                    ...
                                    GenericParameterAttributes constraints =
                                            ga.GenericParameterAttributes & GenericParameterAttributes.SpecialConstraintMask;
                                    ...
                                    if ((constraints & GenericParameterAttributes.ReferenceTypeConstraint) != 0) constraintsInfo.Add("class");
                                    ...
                                    else if ((constraints & GenericParameterAttributes.DefaultConstructorConstraint) != 0) constraintsInfo.Add("new()");
                    ...
                    var methodLists = Puerts.Utils.GetMethodAndOverrideMethod(type, Utils.Flags)
                            ...
                            .Where(m =>
                                    !m.IsSpecialName && Puerts.Utils.IsNotGenericOrValidGeneric(m))
                            .Where(m => {
                            ...
                            .ToArray();
                    var extensionMethodsList = Utils.GetExtensionMethods(
                        type, new HashSet<Type>(genTypes));
                    ...
                    if (extensionMethodsList != null) {
                        ...
                        if (genTypes != null)
                            extensionMethodsList = extensionMethodsList
                                    .Where(m => genTypes.Contains(m.DeclaringType))
                                    .ToArray();
                    ...
                    return new StaticWrapperInfo {
                        ...
                        Namespaces =
                                (extensionMethodsList != null
                                        ...
                                        : new string[0]).Concat(new[] { "System" })
                                .Distinct()
                                .ToArray(),
                        ...
                        GenericArgumentsInfo = GenericArgumentsInfos, Methods = methodGroups.Select(
                                    kv => {
                                        ...
                                        return MethodGenInfo.FromType(type, false, kv.Value,
                                            exOverloads);
                                    })
                                .Concat(extensionMethodGroup.Select(kv =>
                                        MethodGenInfo.FromType(type, false, null, kv.Value)))
                                .ToArray(),
                        ...
                        Operators = operatorGroups
                                .Select(o => MethodGenInfo.FromType(type, false, o))
                                .ToArray(),
                ...
                public static ParameterGenInfo FromParameterInfo(ParameterInfo parameterInfo)
                {
                    ...
                    if (result.IsParams)
                        result.TypeName = Utils
                                .RemoveRefAndToConstraintType(parameterInfo.ParameterType
                                        .GetElementType())
                                .GetFriendlyName();
                    result.ExpectCsType =
                    ...
                    result.DefaultValue = result.IsParams
                            ...
                            : ConvertDefaultValueToString(parameterInfo.DefaultValue,
                                result.TypeName);
                ...
                private static string ConvertDefaultValueToString(object value, string typeName)
                {
                    if (value != null) {
                        ...
                        if (valueType.IsPrimitive) {
                            if (valueType == typeof(bool)) return value.ToString().ToLower();
                            ...
                            if (valueType == typeof(char)) return "(char)" + (ushort)(char)value;
                            return value.ToString();
                        }
                    }
            ...
            public class OverloadGenInfo : DataTypeInfo
            {
                ...
                private string ParameterInfosMark;
                            ...
                            if (ps[i].IsOptional || mainInfo.ParameterInfos[i].IsParams) {
                                ...
                                optionalInfo = new OverloadGenInfo {
                                    ParameterInfos = pinfos.Select(info =>
                                                    ParameterGenInfo.FromParameterInfo(info))
                                            .ToArray(),
