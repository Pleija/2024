...
namespace Puerts.Editor
{
    namespace Generator
    {
        ...
        {
            public new string Constructor;

            // use string to work well in js
            public new string MemberType;
            public new string Method;
            public new string PropertyGetter;
            public new string PropertySetter;
            public new string UseBindingMode;
        }
        ...
        internal class RegisterInfoGenerator
        {
                ...
                HashSet<Type> blittableCopyTypes)
            {
                var flag = BindingFlags.DeclaredOnly
                        ...
                        | BindingFlags.Static
                        | BindingFlags.Public;
                return genTypes.Where(type => !(type.IsEnum
                            || type.IsArray
                        ...
                        .Select(type => {
                            ...
                            foreach (var m in Utils
                                    ...
                                    .ToArray()
                                    .Where(m => genTypes == null ? true
                                            : genTypes.Contains(m.DeclaringType))
                                    .ToArray())
                                Collector.Add(m.Name, new MemberRegisterInfoForGenerate {
                                    Name = m.Name,
                                    UseBindingMode = Utils.getBindingMode(m).ToString(),
                            ...
                            foreach (var m in type.GetEvents(Utils.Flags)
                                ...
                                if (removeMethod != null && removeMethod.IsPublic)
                                    Collector.Add("remove_" + m.Name,
                                        new MemberRegisterInfoForGenerate {
                                            Name = "remove_" + m.Name,
                                            UseBindingMode = Utils.getBindingMode(m).ToString(),
                            ...
                            foreach (var m in type.GetProperties(flag)
                                ...
                                else if (m.GetIndexParameters().GetLength(0) == 0
                                    && !m.IsSpecialName) {
                                    Collector.Add(m.Name, new MemberRegisterInfoForGenerate {
                                        Name = m.Name,
                                        UseBindingMode = Utils.getBindingMode(m).ToString(),
                                        ...
                                        PropertyGetter = m.GetGetMethod() != null ? "G_" + m.Name
                                                : null,
                                        PropertySetter = m.GetSetMethod() != null ? "S_" + m.Name
                                                : null,
                            ...
                            foreach (var m in type.GetFields(flag)
                                ...
                                Collector.Add(m.Name, new MemberRegisterInfoForGenerate {
                                    ...
                                    PropertySetter = !m.IsInitOnly && !m.IsLiteral ? "S_" + m.Name
                                            : null,
                                }, m.IsStatic);
            ...
            {
                private Dictionary<string, MemberRegisterInfoForGenerate> Collector =
                        new Dictionary<string, MemberRegisterInfoForGenerate>();

                private Dictionary<string, MemberRegisterInfoForGenerate> CollectorStatic =
                        new Dictionary<string, MemberRegisterInfoForGenerate>();

                internal void Add(string Name, MemberRegisterInfoForGenerate newMRI, bool isStatic)
                {
                    var dict = isStatic ? CollectorStatic : Collector;
                    MemberRegisterInfoForGenerate oldMRI;

                    if (dict.TryGetValue(Name, out oldMRI)) {
                        if ((int)Enum.Parse(typeof(BindingMode), newMRI.UseBindingMode)
                            > (int)Enum.Parse(typeof(BindingMode), oldMRI.UseBindingMode))
                ...
                internal List<MemberRegisterInfoForGenerate> GetAllMember()
                {
                    return Collector.Select(kv => kv.Value)
                            .Concat(CollectorStatic.Select(kv => kv.Value))
                            .ToList();
                }
            }
        }
