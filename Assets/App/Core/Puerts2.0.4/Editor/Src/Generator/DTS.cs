            ...
            public class TsParameterGenInfo
            {
                    ...
                    bool isGenericTypeDefinition)
                {
                    ...
                    return new TsParameterGenInfo {
                        ...
                        TypeName = Utils.GetTsTypeName(Utils.ToConstraintType(
                    ...
                    return new TsMethodGenInfo {
                        ...
                        ParameterInfos = methodBase.GetParameters()
                                .Skip(skipExtentionMethodThis
                                    && Utils.isDefined(methodBase, typeof(ExtensionAttribute)) ? 1
                                            : 0)
                                .Select(info =>
                                        TsParameterGenInfo.FromParameterInfo(info,
                                            isGenericTypeDefinition))
                                .ToArray(),
                        TypeName = methodBase.IsConstructor ? "" : Utils.GetTsTypeName(
                            Utils.ToConstraintType((methodBase as MethodInfo).ReturnType,
                                isGenericTypeDefinition)),
                ...
                public static TsMethodGenInfo[] FromType(Type type, HashSet<Type> genTypeSet)
                {
                    var declMethods = type.GetMethods(Utils.Flags)
                            .Where(m =>
                                    ...
                                    || !genTypeSet.Contains(m.DeclaringType))
                            .ToArray();
                    ...
                    var methods = type.GetMethods(Utils.Flags)
                            ...
                            .Concat(declMethods)
                            .Where(m =>
                                    !Utils.IsNotSupportedMember(m, true)
                ...
                public static TsTypeGenInfo FromType(Type type, HashSet<Type> genTypeSet)
                {
                    var result = new TsTypeGenInfo {
                        ...
                        Properties = genTypeSet.Contains(type) ? type.GetFields(Utils.Flags)
                                ...
                                .Concat(type.GetProperties(Utils.Flags)
                                        .Where(m => m.Name != "Item")
                                        .Where(m => !Utils.IsNotSupportedMember(m, true))
                                        .Where(m => Utils.getBindingMode(m)
                        ...
                        ExtensionMethods = Utils.GetExtensionMethods(type, genTypeSet)
                                ...
                                .Where(m => Utils.getBindingMode(m) != BindingMode.DontBinding)
                                .Select(m =>
                                        TsMethodGenInfo.FromMethodBase(m,
                    ...
                    if (type.IsEnum) {
                        ...
                        var KeyValues = type.GetFields(BindingFlags.Static | BindingFlags.Public)
                                ...
                                .Select(f => f.Name
                                        + " = "
                                        + Convert.ChangeType(f.GetValue(null),
            ...
            public class TypingGenInfo
            {
                ...
                public static TypingGenInfo FromTypes(IEnumerable<Type> types)
                {
                    ...
                    return new TypingGenInfo {
                        ...
                        TaskDef = refTypes.Contains(typeof(Task<>))
                    ...
                    TsTypeGenInfo info)
                {
                    if (info.IsCheckOk
                        || (info.BaseType == null
                            && info.interfaces == null) /* || info.IsInterface */
                    ) {
                        info.IsCheckOk = true;
                        return;
                    }
                    ...
                    if (info.BaseType != null
                        ...
                        baseProperties = TsPropertyGenInfo.FromTsTypeGenInfo(
                            tsGenTypeInfos, targetInfo, true);
                    }
                    ...
                    if (info.interfaces != null) {
                        ...
                        foreach (var iface in info.interfaces) {
                            if (!tsGenTypeInfos.TryGetValue(iface.FullName, out targetInfo))
                                continue;
                            methods.AddRange(TsMethodGenInfo.FromTsGenTypeInfos(tsGenTypeInfos,
                                targetInfo,
                                true));
                            properties.AddRange(TsPropertyGenInfo.FromTsTypeGenInfo(tsGenTypeInfos,
                                targetInfo,
                                true));
                        }
                    ...
                    var ownMethods = MethodGenInfosToDict(
                    ...
                    if (ifaceMethods != null) {
                        ...
                        var implProperties = ifaceProperties.Where(prop =>
                                !(ownProperties.FirstOrDefault(p => prop.Name.Equals(p.Name))
                                    != null
                                    || (baseProperties != null
                                        && baseProperties.FirstOrDefault(p =>
                                                prop.Name.Equals(p.Name))
                                        != null)));
                    ...
                    Type type)
                {
                    ...
                    if ((refTypes.Contains(rawType) && type != rawType)
                        || type.IsPointer
                        || rawType.IsPointer)
                        return;
