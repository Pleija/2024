...
namespace Puerts
{
    ...
    {
        private static List<Type> type_def_extention_method = new List<Type>();
        ...
        public static void Test()
        {
            ...
            Debug.Log(extensionMethodMap.Values.SelectMany(x => x)
                    ...
                    .Select(x =>
                            //   x.GetNiceName())
                            @$"{x.ReturnType.GetTsTypeName()} {x.DeclaringType.GetTsTypeName()}.{x.Name}{
                                (x.ContainsGenericParameters ? "<" + x.GetGenericArguments()
                                        .Select(t => t.GetTsTypeName()).JoinStr() + ">" : "")}({
                                        x.GetParameters().Select(p => p.Name + ": " +
                                                p.ParameterType.GetTsTypeName(true)).JoinStr()})")
                    .JoinStr("\n"));
        }
        ...
        {
            IEnumerator<Type> enumerator = Utils.GetAllTypes().GetEnumerator();
