    ...
    internal class TypeRegister
    {
        ...
        private static IntPtr GetWrapperFunc(RegisterInfo registerInfo, MemberInfo member, string signature)
        {
            ...
            BindingMode bindingMode =
                    GetBindingMode(registerInfo, name, isMethod ? ((MethodInfo)member).IsStatic : false);
        ...
        {
            if (ReflectionWrapperFunc == IntPtr.Zero)
                ReflectionWrapperFunc =
                        NativeAPI.FindWrapFunc(null);
            if (ReflectionFieldWrappers == IntPtr.Zero)
                ReflectionFieldWrappers =
                        NativeAPI.FindFieldWrap(null);
            if (RegisterInfoManager == null) RegisterInfoManager = new RegisterInfoManager();
        ...
        private static void Register(Type type, bool includeNonPublic, bool throwIfMemberFail =
                false)
        {
            BindingFlags flag =
        ...
        private static void Register(Type type, MethodBase[] ctors = null, MethodBase[] methods =
                null, PropertyInfo[] properties = null, FieldInfo[] fields =
 null, bool throwIfMemberFail = false)
        {
            IntPtr typeInfo = IntPtr.Zero;

            try {
                bool isDelegate =
                        typeof(MulticastDelegate).IsAssignableFrom(type) && type != typeof(MulticastDelegate);
                var typeId = NativeAPI.GetTypeId(type);
                //UnityEngine.Debug.Log(string.Format("{0} typeId is {1}", type, typeId));
                var superTypeId =
                        (isDelegate || type == typeof(object) || type.BaseType == null) ? IntPtr.Zero : NativeAPI.GetTypeId(type.BaseType);
                Func<RegisterInfo> getRegisterInfoFunc;
                bool hasRegisterInfo =
                        RegisterInfoManager.TryGetValue(type, out getRegisterInfoFunc);
                RegisterInfo registerInfo = null;
                if (hasRegisterInfo) registerInfo = getRegisterInfoFunc();
                typeInfo =
                        NativeAPI.CreateCSharpTypeInfo(type.ToString(), typeId, superTypeId, typeId, type.IsValueType, isDelegate, isDelegate ? TypeUtils.GetMethodSignature(type.GetMethod("Invoke"), true) : "");
                ...
                if (!isDelegate) {
                    if (ctors != null && ctors.Length > 0 && (!type.IsArray || type == typeof(System.Array))) {
                        foreach (var ctor in ctors) {
                            ...
                            var wrapData =
                                    NativeAPI.AddConstructor(typeInfo, signature, wrapper, methodInfoPointer, methodPointer, usedTypes.Count);
                    ...
                    Action<string, MethodInfo, bool, bool, bool> AddMethodToType =
                            (string name, MethodInfo method, bool isGetter, bool isSetter, bool isExtensionMethod) => {
                                method = TypeUtils.HandleMaybeGenericMethod(method);
                                if (method == null) return;
                                List<Type> usedTypes =
 TypeUtils.GetUsedTypes(method, isExtensionMethod);
                                var signature =
                                        TypeUtils.GetMethodSignature(method, false, isExtensionMethod);
                                ...
                                var wrapData =
                                        NativeAPI.AddMethod(typeInfo, signature, wrapper, name, !isExtensionMethod && method.IsStatic, isExtensionMethod, isGetter, isSetter, methodInfoPointer, methodPointer, usedTypes.Count);
                    ...
                    if (fields != null) {
                        foreach (var field in fields) {
                            string signature =
                                    (field.IsStatic ? "" : "t") + TypeUtils.GetTypeSignature(field.FieldType);
                            var name = field.Name;
                            var wrapper =
                                    GetFieldWrapper(registerInfo, name, field.IsStatic, signature);
                            ...
                            if (!NativeAPI.AddField(typeInfo, wrapper, name, field.IsStatic, NativeAPI.GetFieldInfoPointer(field), NativeAPI.GetFieldOffset(field, type.IsValueType), NativeAPI.GetTypeId(field.FieldType))) {
                                if (!throwIfMemberFail) {
#if WARNING_IF_MEMBERFAIL
