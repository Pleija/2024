...
namespace PuertsIl2cpp
{
    public static class ExtensionMethodInfo
    {
        ...
        {
            if (LoadExtensionMethod != null) return LoadExtensionMethod(type);
            return null;
        }
                ...
                ExtensionMethodInfos_Gen =
                        (from assembly in AppDomain.CurrentDomain.GetAssemblies()
                            select assembly.GetType(
                                "PuertsIl2cpp.ExtensionMethodInfos_Gen_Internal"))
                        .FirstOrDefault(x => x != null);
    ...
    public static class TypeUtils
    {
        private static Type GetType(string className, bool isQualifiedName)
        {
            ...
            var p1 = className.IndexOf('[');

            if (p1 > 0 && !isQualifiedName) {
                var qualified_name = className.Substring(0, p1 + 1);
                var generic_params = className.Substring(
                            p1 + 1, className.Length - qualified_name.Length - 1)
                        .Split(',');
        ...
        public static Type GetType(string className) => GetType(className, false);

        public static string GetValueTypeFieldsSignature(Type type)
        {
            if (!type.IsValueType) throw new Exception(type + " is not a valuetype");
            var sb = new StringBuilder();
            if (type.BaseType.IsValueType) sb.Append(GetTypeSignature(type.BaseType));
        ...
        {
            if (type == typeof(void)) return TypeSignatures.Void;
            if (type == typeof(bool)) return TypeSignatures.Bool;
            if (type == typeof(byte)) return TypeSignatures.Byte;
            if (type == typeof(sbyte)) return TypeSignatures.Sbyte;
            if (type == typeof(short)) return TypeSignatures.Short;
            if (type == typeof(ushort)) return TypeSignatures.Ushort;
            if (type == typeof(int)) return TypeSignatures.Int;
            if (type == typeof(uint)) return TypeSignatures.Uint;
            if (type == typeof(long)) return TypeSignatures.Long;
            if (type == typeof(ulong)) return TypeSignatures.Ulong;
            if (type == typeof(char)) return TypeSignatures.Char;
            if (type == typeof(double)) return TypeSignatures.Double;
            if (type == typeof(float)) return TypeSignatures.Float;
            if (type == typeof(ArrayBuffer)) return TypeSignatures.ArrayBuffer;
            if (type == typeof(IntPtr) || type == typeof(UIntPtr)) return TypeSignatures.IntPtr;

            //else if (type == typeof(DateTime)) //是否要支持？
            //{
            //    return "d";
            //}
            if (type == typeof(string)) return TypeSignatures.String;
            if (type == typeof(object)) //object特殊处理，比如check可以不用判断，比如return可以优化
            ...
            if (!type.IsValueType) return TypeSignatures.Object;

            if (type.IsValueType && !type.IsPrimitive) {
                //return "s" + Marshal.SizeOf(type);
                if (Nullable.GetUnderlyingType(type) != null)
                    return TypeSignatures.NullableStructPrefix
                            + GetValueTypeFieldsSignature(type)
                            + "_";
        ...
        {
            if (methodBase is ConstructorInfo) return "t";
            ...
            bool isDelegateInvoke = false, bool isExtensionMethod = false)
        {
            ...
            if (methodBase is ConstructorInfo) {
                ...
                if (!methodInfo.IsStatic && !isDelegateInvoke)
                    signature += methodBase.DeclaringType == typeof(object) ? "T" : "t";
        ...
        public static List<Type> GetUsedTypes(MethodBase methodBase, bool isExtensionMethod = false)
        {
            ...
            types.AddRange(methodBase.GetParameters()
                    .Skip(isExtensionMethod ? 1 : 0)
                    .Select(m =>
                            m.ParameterType.IsByRef ? m.ParameterType.GetElementType()
                                    : m.ParameterType)
        ...
        internal static bool HasValidContraint(Type type, List<Type> validTypes)
        {
            ...
            if (type.IsGenericParameter) {
                ...
                foreach (var parameterConstraint in parameterConstraints)
                    ...
                    if (!IsClass(parameterConstraint)
