            ...
            try {
                if (reflectionWrap == null) {
                    var overload = Utils
                            ...
                            .Where(item => item.IsGenericMethodDefinition
                                    && item.GetGenericArguments().Count()
                                    == genericArguments.Count())
                            .Select(item =>
                                    item.MakeGenericMethod(genericArguments))
                            .ToArray();
                    if (overload.Count() == 0)
                        throw new Exception("no suitable method found to make GenericMethodWrap");
                    reflectionWrap = new MethodReflectionWrap(memberName,
                        overload.Select(m => new OverloadReflectionWrap(m, env)).ToList());
                }
