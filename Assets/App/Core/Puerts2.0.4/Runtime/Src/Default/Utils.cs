    ...
    public static class Utils
    {
        ...
        private static bool IsMatchParameters(IEnumerable<Type[]> typeList, Type[] pTypes)
        {
            foreach (var types in typeList) {
                if (types.Length != pTypes.Length) continue;
                bool exclude = true;

                for (int i = 0; i < pTypes.Length && exclude; i++) {
                    if (pTypes[i] != types[i]) exclude = false;
                }
        ...
        public static MethodInfo[] GetMethodAndOverrideMethodByName(Type type, string name)
        {
            ...
            Dictionary<string, IEnumerable<Type[]>> errorMethods =
                    type.GetMethods().Where(m => m.DeclaringType != type && IsObsoleteError(m)).GroupBy(m => m.Name).ToDictionary(i => i.Key, i => i.Cast<MethodInfo>().Select(m => m.GetParameters().Select(o => o.ParameterType).ToArray()));
            ...
            while (type.BaseType != null && type.BaseType != objType) {
                type = type.BaseType;
                MethodInfo[] methods =
                        type.GetMember(name).Select(m => (MethodInfo)m).Where(m => !IsObsoleteError(m) && !IsVirtualMethod(m)).Where(m => !errorMethods.TryGetValue(m.Name, out matchTypes) || !IsMatchParameters(matchTypes, m.GetParameters().Select(o => o.ParameterType).ToArray())) //filter override method
                                .ToArray();
...
namespace Puerts
{
    public static class Utils
    {
        public static long TwoIntToLong(int b, int a) => ((long)a << 32) | (b & 0xFFFFFFFFL);
        ...
        public static IntPtr GetObjectPtr(int jsEnvIdx, Type type, object obj)
        {
            ...
            return new IntPtr(type.IsValueType() ? jsEnv.objectPool.AddBoxedValueType(obj)
                    : jsEnv.objectPool.FindOrAddObject(obj));
        }
        ...
        private static bool HasValidContraint(Type type, List<Type> validTypes)
        {
            ...
            if (type.IsGenericParameter) {
                if (type.BaseType != null && type.BaseType.IsValueType) return false;
            ...
            ParameterInfo[] pinfos = null)
        {
            ...
            var validGenericParameter = new List<Type>();
        ...
        public static MethodInfo[] GetMethodAndOverrideMethod(Type type, BindingFlags flag)
        {
            ...
            while (type.BaseType != null && type.BaseType != objType) {
                ...
                var methods = type.GetMethods(flag)
                        .Where(m => Array.IndexOf(methodNames, m.Name) != -1)
                        .Where(m => !IsObsoleteError(m) && !IsVirtualMethod(m))
                        .Where(m =>
                                !m.IsSpecialName
                                || (!m.Name.StartsWith("get_")
                                    && !m.Name.StartsWith("set_"))) //filter property
                        .Where(m => !errorMethods.TryGetValue(m.Name, out matchTypes)
                                || !IsMatchParameters(matchTypes,
                                    m.GetParameters()
                                            .Select(o => o.ParameterType)
                                            .ToArray())) //filter override method
        ...
        private static bool IsVirtualMethod(MethodInfo memberInfo) => memberInfo.IsAbstract
                || (memberInfo.Attributes & MethodAttributes.NewSlot) == MethodAttributes.NewSlot;

        private static bool IsObsoleteError(MemberInfo memberInfo)
        {
            var obsolete =
                    memberInfo.GetCustomAttributes(typeof(ObsoleteAttribute), true).FirstOrDefault()
        ...
        private static bool IsMatchParameters(IEnumerable<Type[]> typeList, Type[] pTypes)
        {
            foreach (var types in typeList) {
                if (types.Length != pTypes.Length) continue;
                var exclude = true;
                for (var i = 0; i < pTypes.Length && exclude; i++)
                    if (pTypes[i] != types[i])
                        exclude = false;
                if (exclude) return true;
            }
        ...
        public static IEnumerable<MethodInfo> GetExtensionMethodsOf(Type type_to_be_extend)
        {
            if (Utils_Internal.extensionMethodMap == null) {
                var type_def_extention_method = new List<Type>();
                ...
                while (enumerator.MoveNext()) {
                    ...
                    for (var i = 0; i < fields.Length; i++) {
                        ...
                        if (typeof(IEnumerable<Type>).IsAssignableFrom(field.FieldType)) {
                            ...
                            if (types != null)
                                type_def_extention_method.AddRange(types.Where(t => t != null
        ...
        private static Type GetExtendedType(MethodInfo method)
        {
            ...
            var parameterConstraints = type.GetGenericParameterConstraints();
            if (parameterConstraints.Length == 0) throw new InvalidOperationException();
            ...
            JsValueType csTypeMask, GetValueForCheck valueGetter = null, object value = null)
        {
            if (jsType == JsValueType.NativeObject) {
                ...
                if (csType.IsPrimitive) // TypedValue赋值给基础类型
                {
                    // 可能的优化： 为TypedValue设计一个新的JSValueType，这样这里就不需要先取值
                    // 此处优化后，可以将下面使用valueGetter的地方独立出去，这里只专注typeMask
                    value = valueGetter();
                    if (value.GetType() == csType) return true;
                }
            }
            if ((csTypeMask & jsType) != jsType) return false;

            if (jsType == JsValueType.NativeObject) {
                if (value == null) value = valueGetter();
                if (!csType.IsAssignableFrom(value.GetType())) return false;
            }
            return true;
        }
        ...
        public static IEnumerable<Type> GetAllTypes(bool exclude_generic_definition = true)
        {
            ...
            return from assembly in assemblies where !(assembly.IsDynamic) from type in assembly.GetTypes() where exclude_generic_definition ? !type.GetTypeInfo().IsGenericTypeDefinition : true select type;
        }
#else
        public static List<Type> GetAllTypes(bool exclude_generic_definition = true)
        {
            ...
            for (var i = 0; i < assemblies.Length; i++)
                try {
                    ...
                    if (!(assemblies[i].ManifestModule is ModuleBuilder)) {
#endif
                        allTypes.AddRange(assemblies[i]
                                .GetTypes()
                                .Where(type =>
                                        exclude_generic_definition ? !type.IsGenericTypeDefinition()
                                                : true));
#if (UNITY_EDITOR || PUERTS_GENERAL) && !NET_STANDARD_2_0
                    }
#endif
