...
public class ResLoader : ILoader, IModuleChecker
{
    ...
    public bool FileExists(string filepath)
    {
        ...
        var exist = ((Application.isEditor || Debug.isDebugBuild)
                    && !string.IsNullOrEmpty(Redis.Database.StringGet(assetPath)))
                || Cache.TryGetValue(filepath, out var result)
                || assets.Any(x => x.path == assetPath)
                || Res.Exists<TextAsset>(assetPath) != null
