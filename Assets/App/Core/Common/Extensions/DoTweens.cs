    ...
    public static class DoTweens
    {
        public static Sequence DOJump(this RectTransform target, Vector3 endValue, int numJumps, float duration,
            ...
            return DOTween.Sequence().Append(DOTween.To(() => target.anchoredPosition3D, delegate(Vector3 x) {
                ...
                }, new Vector3(endValue.x, 0f, 0f), duration).SetOptions(AxisConstraint.X, snapping)
                .SetEase(Ease.Linear))
                .Join(DOTween.To(() => target.anchoredPosition3D, delegate(Vector3 x) {
                    target.anchoredPosition3D = x;
                }, new Vector3(0f, 0f, endValue.z), duration).SetOptions(AxisConstraint.Z, snapping)
                .SetEase(Ease.Linear))
                .Join(DOTween.To(() => target.anchoredPosition3D, delegate(Vector3 x) {
                        target.anchoredPosition3D = x;
                    }, new Vector3(0f, endValue.y, 0f), duration / (numJumps * 2))
                    .SetOptions(AxisConstraint.Y, snapping)
                    .SetEase(Ease.OutQuad).SetLoops(numJumps * 2, LoopType.Yoyo)).SetTarget(target)
