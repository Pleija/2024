    ...
    public class CanSeeTargetAny2D : ConditionTask<Transform>
    {
        ...
        protected override bool OnCheck()
        {
            ...
            foreach (var o in targetObjects.value) {
                ...
                if (Vector2.Distance(agent.position, t.position) > maxDistance.value) continue;
            ...
            if (store) {
                var ordered = temp.OrderBy(x =>
                        Vector3.Distance(agent.position, x.transform.position));
        ...
        public override void OnDrawGizmosSelected()
        {
            if (agent != null) {
                ...
                Gizmos.matrix = Matrix4x4.TRS((Vector2)agent.position + offset, agent.rotation,
                    Vector3.one);
