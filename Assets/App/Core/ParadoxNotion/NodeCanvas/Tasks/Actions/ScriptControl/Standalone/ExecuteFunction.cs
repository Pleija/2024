    ...
    public class ExecuteFunction : ActionTask, IReflectedWrapper
    {
        ...
        protected override string OnInit()
        {
            ...
            if (targetMethod == null)
                return string.Format("Missing Method '{0}'", functionWrapper.AsString());
        ...
        protected override void OnExecute()
        {
            ...
            if (targetMethod.ReturnType == typeof(IEnumerator)) {
                StartCoroutine(
                    InternalCoroutine(
                        (IEnumerator)((ReflectedFunctionWrapper)functionWrapper).Call()));
                return;
            }
            if (targetMethod.ReturnType == typeof(void))
        ...
        protected override void OnTaskInspectorGUI()
        {
                ...
                if (agent != null) {
                    foreach (var comp in agent.GetComponents(typeof(Component))
                        ...
                        menu = EditorUtils.GetInstanceMethodSelectionMenu(comp.GetType(),
                            typeof(object), typeof(object), SetMethod, 6, false,
                            false, menu);
                    menu.AddSeparator("/");
                }
            ...
            if (m != null) {
                ...
                EditorGUILayout.LabelField("Method", m.Name);
                EditorGUILayout.LabelField("Returns", m.ReturnType.FriendlyName());
                ...
                if (m.ReturnType == typeof(void)) {
                    for (var i = 0; i < paramNames.Length; i++)
                        BBParameterEditor.ParameterField(paramNames[i], variables[i]);
                }
                else {
                    for (var i = 0; i < paramNames.Length; i++)
                        BBParameterEditor.ParameterField(paramNames[i], variables[i + 1]);
                    if (m.ReturnType != typeof(IEnumerator))
                        BBParameterEditor.ParameterField("Save Return Value", variables[0], true);
                }
            }
        }
