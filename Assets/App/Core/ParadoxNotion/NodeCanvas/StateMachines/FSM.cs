...
namespace NodeCanvas.StateMachines
{
    ...
    public class FSM : Graph
    {
        ...
        public bool isReady => agent != null
                && !agent.name.Contains(".")
                && !agent.name.Contains("(")
                && !agent.name.Contains(" ");
        ...
        protected override void OnGraphUpdate()
        {
            ...
            //if null state, stop.
            if (currentState == null) Stop(false);
        }
        ...
        public void Invoke(string fn, params object[] param)
        {
            ...
            switch (param.Length) {
                ...
                case 3:
                    JsEnv.self
                            .Eval<Action<object, object, object>>(
            ...
            if (JsEnv.self.Eval<JSObject>($"${FsmName}?.{fn}") != null)
                return param.Length switch {
                    ...
                    1 => JsEnv.self.Eval<Func<object, T>>($"${FsmName}.{fn}.bind(${FsmName})")
                            .Invoke(param[0]),
                    2 => JsEnv.self
                            .Eval<Func<object, object, T>>($"${FsmName}.{fn}.bind(${FsmName})")
                            .Invoke(param[0], param[1]),
                    3 => JsEnv.self
                            .Eval<Func<object, object, object, T>>(
                                $"${FsmName}.{fn}.bind(${FsmName})")
                            .Invoke(param[0], param[1], param[2]),
                    4 => JsEnv.self
        ...
        public FSMState PeekStack() =>
                stateStack != null && stateStack.Count > 0 ? stateStack.Peek() : null;

        ///----------------------------------------------------------------------------------------------
        ///---------------------------------------UNITY EDITOR-------------------------------------------
#if UNITY_EDITOR
        [MenuItem("Tools/ParadoxNotion/NodeCanvas/Create/State Machine Asset", false, 1)]
