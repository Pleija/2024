...
namespace NodeCanvas.DialogueTrees
{
    ...
    public class ProbabilitySelector : DTNode
    {
        ...
        protected override Status OnExecute(Component agent, IBlackboard blackboard)
        {
            ...
            for (var i = 0; i < outConnections.Count; i++) {
                if (!successIndeces.Contains(i)) continue;
        ...
        {
            TaskEditor.TaskFieldMulti(childOptions[i].condition, DLGTree, c => {
                childOptions[i].condition = c;
            });
            EditorUtils.Separator();
            GUILayout.BeginHorizontal();
            BBParameterEditor.ParameterField("Weight", childOptions[i].weight);
