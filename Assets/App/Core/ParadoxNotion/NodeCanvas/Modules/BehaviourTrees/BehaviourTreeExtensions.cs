...
namespace NodeCanvas.BehaviourTrees
{
    public static class BehaviourTreeExtensions
    {
        ...
        public static Node ReplaceWith(this Node node, Type t)
        {
            var newNode = node.graph.AddNode(t, node.position);
            foreach (var c in node.inConnections.ToArray()) c.SetTargetNode(newNode);
            foreach (var c in node.outConnections.ToArray()) c.SetSourceNode(newNode);
            if (node.graph.primeNode == node) node.graph.primeNode = newNode;
            ...
            if (node is ITaskAssignable && newNode is ITaskAssignable) {
                ...
                try {
                    assignableNewNode.task = assignableNode.task;
                }
                catch {
                    /* cant assign */
                }
        ...
        {
            if (!EditorUtility.DisplayDialog("Convert to SubTree",
        ...
        public static Node DuplicateBranch(this BTNode root, Graph targetGraph)
        {
            ...
            for (var i = 0; i < root.outConnections.Count; i++)
                dupConnections.Add(root.outConnections[i]
                        .Duplicate(newNode,
                            DuplicateBranch((BTNode)root.outConnections[i].targetNode,
                                targetGraph)));
        ...
        public static Node DecorateWith(this BTNode node, Type t)
        {
            var newNode = node.graph.AddNode(t, node.position + new Vector2(0, -80));
        ...
        public static List<BTNode> GetAllChildNodesRecursively(this BTNode root, bool includeThis)
        {
            ...
            if (includeThis) childList.Add(root);
