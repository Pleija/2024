    ...
    public class Parallel : BTComposite
    {
        ...
        protected override Status OnExecute(Component agent, IBlackboard blackboard)
        {
            ...
            for (var i = 0; i < outConnections.Count; i++) {
                ...
                var isConnectionFinished = finishedConnections[i];
                if (!dynamic && isConnectionFinished) continue;
                if (connection.status != Status.Running && isConnectionFinished) connection.Reset();
                status = connection.Execute(agent, blackboard);

                if (defferedStatus == Status.Resting) {
                    if (status == Status.Failure
                        && (policy == ParallelPolicy.FirstFailure
                            || policy == ParallelPolicy.FirstSuccessOrFailure))
                        defferedStatus = Status.Failure;
                    if (status == Status.Success
                        && (policy == ParallelPolicy.FirstSuccess
                            || policy == ParallelPolicy.FirstSuccessOrFailure))
