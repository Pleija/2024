...
namespace NodeCanvas.BehaviourTrees
{
    ...
    public class Repeater : BTDecorator
    {
        ...
        public enum RepeatUntilStatus { Failure = 0, Success = 1 }
        ...
        protected override Status OnExecute(Component agent, IBlackboard blackboard)
        {
            ...
            switch (repeaterMode) {
                case RepeaterMode.RepeatTimes:
                    if (currentIteration >= repeatTimes.value) return status;
                    currentIteration++;
                    break;
                case RepeaterMode.RepeatUntil:
                    if ((int)status == (int)repeatUntilStatus) return status;
                    break;
            }
        ...
        protected override void OnNodeGUI()
        {
            if (repeaterMode == RepeaterMode.RepeatTimes) {
                ...
                if (Application.isPlaying) GUILayout.Label("Iteration: " + currentIteration);
            }
