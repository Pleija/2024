    ...
    public class DialogueTreeInspector : GraphInspector
    {
        ...
        public static void ShowActorParameters(DialogueTree dialogue)
        {
            ...
            EditorUtils.ReorderableList(dialogue.actorParameters, options, (i, picked) => {
                ...
                if (dialogue.actorParameters.Where(r => r != reference)
                    .Select(r => r.name)
                ...
                var newReference = EditorGUILayout.ObjectField(
                    reference.actor as Object, typeof(Object), true);
                        ...
                        if (newReference is Component)
                            newReference =
                                    (newReference as Component).GetComponent(
                                        typeof(IDialogueActor));
                        if (newReference is GameObject)
                            newReference =
                                    (newReference as GameObject).GetComponent(
                                        typeof(IDialogueActor));
