...
namespace FlowCanvas
{
    ...
    public abstract class FlowNode : Node, ISerializationCallbackReceiver
    {
        ...
        public void AssignSelfInstancePort()
        {
            ...
            var instanceInput = inputPorts.Values.OfType<ValueInput>()
                    .FirstOrDefault(p =>
                            p.IsUnitySceneObject() && !p.skipSelfInstanceAssignment);

            if (instanceInput != null
                && !instanceInput.isConnected
        ...
        private void DeserializeInputPortValues()
        {
            ...
            foreach (var pair in _inputPortValues) {
                ...
                if (!inputPorts.TryGetValue(pair.Key, out inputPort))
                        // update from previous version
                    inputPort =
                            inputPorts.Values.FirstOrDefault(p =>
                                    CheckReverseIDEquality(p, pair.Key));
                if (inputPort is ValueInput
                    && pair.Value != null
        ...
        public ValueInput TryAddFieldDelegateValueInput(FieldInfo field, object instance)
        {
            if (typeof(Delegate).RTIsAssignableFrom(field.FieldType)) {
                ...
                if (invokeMethod.ReturnType != typeof(void) && parameters.Length == 0) {
                    ...
                    var port = (ValueInput)Activator.CreateInstance(portType, instance, name, name);
                    var getterType = typeof(ValueHandler<>).RTMakeGenericType(delType);
                    var getter = port.GetType()
                            .RTGetMethod("get_value")
        ...
        public ValueOutput TryAddPropertyValueOutput(PropertyInfo prop, object instance)
        {
            if (prop.CanRead) {
                ...
                var port = (ValueOutput)Activator.CreateInstance(
                    portType, this, name, name, getter);
        ...
        private void TryHandleWildPortConnection(Type currentType, Type targetType)
        {
            ...
            var content = GetType();
            var newType = TryGetNewGenericTypeForWild(
                wildType, currentType, targetType, content, null);
        ...
        private void OnPortsGatheredInEditor()
        {
            ...
            orderedOutputs = outputPorts.Values
                    .OrderBy(p => p is FlowOutput ? 0 : p.IsDelegate() ? -1 : 1)
                    .ToArray();
            ...
            Vector2 canvasMousePos, float zoomFactor)
        {
            ...
            for (var i = 0; i < outConnections.Count; i++) {
                ...
                if (binder != null) {
                    ...
                    var sourcePos = sourcePort != null ? sourcePort.pos : new Vector2(
                        binder.sourceNode.rect.xMax + 10, binder.sourceNode.rect.y + 10);
                    var targetPos = targetPort != null ? targetPort.pos : new Vector2(
                        binder.targetNode.rect.xMin - 10, binder.targetNode.rect.y + 10);
                ...
                if (binder == null && !(binder.targetNode is MissingNode))
                    outConnections[i]
                            .DrawConnectionGUI(outConnections[i].sourceNode.rect.center,
        ...
        private static bool ShouldDrawPort(Port port, bool pop)
        {
            ...
            if (pop || contextPort != null)
                if (GraphEditorUtility.allowClick
                    && port.parent.rect.ExpandBy(30, 40, 30, 5)
            ...
            bool isSelfInstancePort)
        {
            ...
            if (!port.isConnected && port is ValueInput) {
                ...
                if (!(port as ValueInput).isDefaultValue) {
                    //Not a defaultValue
                    if (value is Object && value as Object != null) {
                ...
                if (!string.IsNullOrEmpty(labelString)) {
                    var size =
                            StyleSheet.labelOnCanvas.CalcSize(
                                EditorUtils.GetTempContent(labelString));
