...
namespace FlowCanvas
{
    ...
    public abstract class Port
    {
        ...
        public enum BindStatus { Valid = 0, Missing = 1, InvalidCast = 2 }
        ...
        {
            if (this is ValueOutput || (this is FlowOutput && !isConnected)) return true;
            if (this is FlowInput || (this is ValueInput && !isConnected)) return true;
            return false;
        }
        ...
        {
            if (IsInputPort())
                return parent.inConnections.OfType<BinderConnection>()
                        .Where(c => c.targetPort == this);
            return parent.outConnections.OfType<BinderConnection>()
                    .Where(c => c.sourcePort == this);
        }
        ...
        {
            return parent.inConnections.OfType<BinderConnection>()
                    .FirstOrDefault(c => c.targetPort == this);
        }
        ...
        {
            return parent.outConnections.OfType<BinderConnection>()
                    .FirstOrDefault(c => c.sourcePort == this);
        }
        ...
        public bool IsUnityObject() => typeof(Object).RTIsAssignableFrom(type);

        public bool IsUnitySceneObject() =>
                typeof(Component).RTIsAssignableFrom(type) || type == typeof(GameObject);

        public bool IsDelegate() => typeof(Delegate).RTIsAssignableFrom(type);
        public bool IsEnumerableCollection() => type.IsEnumerableCollection();
        public bool IsWild() => type == typeof(Wild);
        public override string ToString() => name.SplitCamelCase();
#if UNITY_EDITOR
        ...
        internal void EnsureCachedGUIContent()
        {
            ...
            if (IsFlowPort()) {
                ...
                var hexColor = ColorUtils.ColorToHex(TypePrefs.GetTypeColor(type));
                var text = string.Format("<color={0}>{1}{2}</color>", hexColor,
                    IsEnumerableCollection() ? "#" : string.Empty, displayNameCaped);
                Texture icon = null;
                var wildDefType = parent.GetNodeWildDefinitionType();
                var isWildDef = wildDefType != null
                        && (wildDefType == type || wildDefType == type.GetEnumerableElementType());
                icon = TypePrefs.GetTypeIcon(isWildDef ? typeof(Wild) : type);
                if (IsDelegate())
                    text = string.Format("<b><color={0}>[{1}]</color></b>", hexColor, text);
                content.text = text;
                content.image = icon;
            }
            content.tooltip = displayName.Length > displayNameCaped.Length ? displayName
                    : string.Empty;

            if (bindStatus != BindStatus.Valid) {
                content.text = displayName.FormatError();
                content.image = Icons.errorIcon;
    ...
    public class FlowInput : Port
    {
        ...
        public FlowInput(FlowNode parent, string name, string ID, FlowHandler pointer) : base(
            parent, name, ID) => this.pointer = pointer;
    ...
    public class FlowOutput : Port
    {
        ...
        public void Call(Flow f)
        {
            if (pointer != null && parent.graph.isRunning) {
                ...
                if (parent.isBreakpoint) {
                    MonoManager.current.StartCoroutine(DebugBreakWait(f));
                    return;
                }
                Continue(f);
#else
        ...
        private IEnumerator DebugBreakWait(Flow f)
        {
            Logger.LogWarning(string.Format("On Node '{0}'", parent.name), "Breakpoint", parent);
            Debug.Break();
            parent.SetStatus(Status.Running);
            yield return null;
            parent.SetStatus(Status.Resting);
            Continue(f);
        }

        private void Continue(Flow f)
        {
            try {
                pointer(f);
            }
    ...
    public abstract class ValueInput : Port
    {
        ...
        public static ValueInput<T> CreateInstance<T>(FlowNode parent, string name, string ID) =>
                new ValueInput<T>(parent, name, ID);

        ///<summary>Creates a generic instance of ValueInput</summary>
        public static ValueInput CreateInstance(Type t, FlowNode parent, string name, string ID) =>
                (ValueInput)Activator.CreateInstance(typeof(ValueInput<>).RTMakeGenericType(t),
                    parent, name, ID);

        /// <summary>
        ///     Convenience method to set default port value and serialized port value, which should be the
        ///     same value in any
        ///     case.
        /// </summary>
    ...
    /// <summary>
    ///     Value Input port for a known type. .value refers to either connected binded port if any, or the
    ///     serialized
    ///     exposed value
    /// </summary>
    public class ValueInput<T> : ValueInput
    {
        ...
        public override bool isDefaultValue => ObjectUtils.AnyEquals(serializedValue, defaultValue);
        ...
        /// <summary>
        ///     Convenience method that sets the default and serialized value of the port. Can be used
        ///     when registering ports.
        /// </summary>
        ...
        /// <summary>
        ///     Convenience method to make the port skip self object assigment if it was a valid candidate in
        ///     the first place.
        ///     Can be used when registering ports.
        /// </summary>
        public ValueInput<T> SkipSelfInstanceAssignment(bool skip = true)
        {
            skipSelfInstanceAssignment = skip;
            return this;
        }

        /// <summary>
        ///     Convenience method to flag the value input as required to set which will show an error in UI
        ///     (useful for
        ///     nullable only types). Can be used when registering ports.
        /// </summary>
        ...
        public override void BindTo(ValueOutput source)
        {
            ...
            getter = TypeConverter.GetConverterFuncFromTo<T>(source.type, typeof(T),
                source.GetObjectValue);
        }
    ...
    public abstract class ValueOutput : Port
    {
        ...
        public static ValueOutput<T> CreateInstance<T>(FlowNode parent, string name, string ID,
            ValueHandler<T> getter) => new ValueOutput<T>(parent, name, ID, getter);

        ///<summary>Creates a generic instance of ValueOutput</summary>
        public static ValueOutput CreateInstance(Type t, FlowNode parent, string name, string ID,
            ValueHandlerObject getter) => (ValueOutput)Activator.CreateInstance(
            typeof(ValueOutput<>).RTMakeGenericType(t), parent, name, ID, getter);

        /// <summary>
        ///     Used only in case that a binder required casting cause of different port types (a
        ///     conversion)
        /// </summary>
        public abstract object GetObjectValue();
    }
    ...
    {
        //normal
        public ValueOutput(FlowNode parent, string name, string ID, ValueHandler<T> getter) : base(
            parent, name, ID) => this.getter = getter;
        ...
        public override object GetObjectValue() => getter();
