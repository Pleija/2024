...
namespace FlowCanvas
{
    ...
    public static class TypeConverter
    {
            ...
            ValueHandler<object> func)
        {
            ...
            //convertibles
            if (typeof(IConvertible).RTIsAssignableFrom(targetType)
            ...
            if (targetType == typeof(bool) && typeof(Object).RTIsAssignableFrom(sourceType))
                return () => {
                    return (T)(object)(func() as Object != null);
                };
            ///----------------------------------------------------------------------------------------------
            ...
            if (sourceType == typeof(GameObject) && targetType.RTIsInterface())
                return () => {
                    try {
                        return (func() as GameObject).GetComponent<T>();
                    }
                    catch {
                        return default;
                    }
            ...
            if (typeof(Component).RTIsAssignableFrom(sourceType) && targetType.RTIsInterface())
                return () => {
                    try {
                        return (func() as Component).GetComponent<T>();
                    }
                    catch {
                        return default;
                    }
            ...
            if (ReflectionTools.TryConvert(sourceType, targetType, out var exp))
                return () => {
                    try {
                        return (T)func();
                    }
            ...
            if (typeof(IEnumerable).RTIsAssignableFrom(sourceType)
                && typeof(IEnumerable).RTIsAssignableFrom(targetType))
                try {
                    var elementFrom = sourceType.RTIsArray() ? sourceType.GetElementType()
                            : sourceType.RTGetGenericArguments().Single();
                    var elementTo = targetType.RTIsArray() ? targetType.GetElementType()
                            : targetType.RTGetGenericArguments().Single();
        ...
        public static bool HasConvertion(Type sourceType, Type targetType)
        {
            ...
            if (sourceType == typeof(Flow) && sourceType != targetType) return false;
            return GetConverterFuncFromTo<object>(sourceType, targetType, null) != null;
        }

        ///<summary>Rarely used</summary>
        public static T QuickConvert<T>(object obj) => (T)QuickConvert(obj, typeof(T));

        public static object QuickConvert(object obj, Type type)
        {
            if (obj == null || type == null) return null;
            var func = GetConverterFuncFromTo<object>(obj.GetType(), type, () => {
                return obj;
            });
            return func();
        }
