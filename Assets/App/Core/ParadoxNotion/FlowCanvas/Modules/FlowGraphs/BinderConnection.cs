...
namespace FlowCanvas
{
    public class BinderConnection : Connection
    {
        ...
        /// </summary>
        public Type bindingType => GetType().RTIsGenericType()
        ...
        {
            if (!CanBeBoundVerbosed(source, target, null, out var verbose)) {
                Logger.LogWarning(verbose, LogTag.EDITOR, source.parent);
                return null;
            }
            UndoUtility.RecordObject(source.parent.graph, "Connect Ports");
            ...
            if (source is ValueOutput && target is ValueInput)
                binder = (BinderConnection)Activator.CreateInstance(
                    typeof(BinderConnection<>).RTMakeGenericType(target.type));
            ...
            if (binder != null) {
                ...
                //for live editing
                if (Application.isPlaying) binder.Bind();
            }
            UndoUtility.SetDirty(source.parent.graph);
            return binder;
        }
        ...
        {
            if (newSourcePort == sourcePort) return;
            if (newSourcePort == null || !newSourcePort.IsOutputPort()) return;
            ...
            SetSourceNode(newSourcePort.parent);
        ...
        {
            if (newTargetPort == targetPort) return;
            if (newTargetPort == null || !newTargetPort.IsInputPort()) return;
            ...
            SetTargetNode(newTargetPort.parent);
        ...
        public void GatherAndValidateSourcePort()
        {
            ...
            //the id is missing...
            Logger.LogError(string.Format("Output Port with ID '{0}' is missing on node {1}",
                sourcePortID,
        ...
        public void GatherAndValidateTargetPort()
        {
            ...
            //the id is missing...
            Logger.LogError(string.Format("Input Port with ID '{0}' is missing on node {1}",
                targetPortID,
                targetNode.name), LogTag.VALIDATION, targetNode);
            var target = targetNode as FlowNode;
            Port missingPort = null;
            if (bindingType == typeof(Flow))
                missingPort = target.AddFlowInput(targetPortID, targetPortID, f => {
                    throw new Exception("Port is missing");
                });
            else
        ...
        {
            if (sourcePort is FlowOutput) (sourcePort as FlowOutput).UnBind();
        }
        ...
        protected sealed override string GetConnectionInfo()
        {
            ...
            if (targetPort.willDraw) {
                if (Application.isPlaying) return GetTransferDataLabel();
                if (!targetPort.type.IsAssignableFrom(sourcePort.type)) return "<size=14>➲</size>";
            }
            return null;
        }

        //Data label to show on binder info
        protected virtual string GetTransferDataLabel() => null;
        ...
        protected override void OnConnectionInspectorGUI()
        {
            ...
            if (sourcePort == null || sourcePort.bindStatus != Port.BindStatus.Valid)
                EditorGUILayout.HelpBox(string.Format(
                    ...
                    sourcePortID, sourcePort?.bindStatus), MessageType.Error);
            if (targetPort == null || targetPort.bindStatus != Port.BindStatus.Valid)
                EditorGUILayout.HelpBox(string.Format(
                    "Target Port with ID '{0}' is {1} on target node.\nYou should relink the connection or simply remove it.",
                    targetPortID, targetPort?.bindStatus), MessageType.Error);
        }
