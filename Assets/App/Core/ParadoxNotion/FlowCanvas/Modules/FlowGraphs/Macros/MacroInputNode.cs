...
namespace FlowCanvas.Macros
{
    ...
    public class MacroInputNode : FlowNode
    {
        ...
        protected override void RegisterPorts()
        {
            ...
            for (var i = 0; i < macro.inputDefinitions.Count; i++) {
                ...
                if (def.type == typeof(Flow))
                    ...
                    AddValueOutput(def.name, def.type, () => {
                        return macro.entryFunctionMap[def.ID]();
                    }, def.ID);
            }
        }
        ...
        protected override GenericMenu OnDragAndDropPortContextMenu(GenericMenu menu, Port port)
        {
            ...
            if (port.IsInputPort())
                menu.AddItem(new GUIContent(string.Format("Promote to new Input '{0}'", port.name)),
                    false,
                    () => {
        ...
        protected override void OnNodeInspectorGUI()
        {
            ...
            if (GUILayout.Button("Add Flow Input"))
                macro.AddInputDefinition(
                    new DynamicParameterDefinition("Flow Input", typeof(Flow)));
            if (GUILayout.Button("Add Value Input"))
                EditorUtils.ShowPreferedTypesSelectionMenu(typeof(object), t => {
                    macro.AddInputDefinition(new DynamicParameterDefinition(
                        string.Format("{0} Input", t.FriendlyName()),
                        t));
                });
            ...
            EditorUtils.ReorderableList(macro.inputDefinitions, options, (i, picked) => {
                ...
                def.name = EditorGUILayout.DelayedTextField(def.name, GUILayout.Width(150),
