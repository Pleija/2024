...
namespace FlowCanvas.Nodes
{
    ...
    public abstract class ReflectedMethodBaseNodeWrapper : FlowNode, IReflectedWrapper
    {
        ...
        public int exposedParamsCount {
            get => _exposedParamsCount;
            set {
                if (_exposedParamsCount != value) {
                    _exposedParamsCount = value;
                    if (_exposedParamsCount <= 0) _exposeParams = false;
                    GatherPorts();
                }
            }
        }

        ISerializedReflectedInfo IReflectedWrapper.GetSerializedInfo() => serializedMethodBase;
        public Type GetRuntimeIconType() => method != null ? method.DeclaringType : null;
        ...
        public void SetDefaultParameterValues(MethodBase newMethod)
        {
            ...
            for (var i = 0; i < parameters.Length; i++) {
                ...
                if (parameter.IsOptional && parameter.DefaultValue != null) {
                    ...
                    if (paramPort != null) paramPort.serializedValue = parameter.DefaultValue;
                }
            }
        }
        ...
        public void SetDropInstanceReference(MethodBase newMethod, object instance = null)
        {
            ...
            if (instance != null && !newMethod.IsStatic) {
                ...
                if (port != null) port.serializedValue = instance;
            }
        }
        ...
        protected override void OnNodeInspectorGUI()
        {
            if (method != null) {
                ...
                if (returnType != typeof(void) && !method.Name.StartsWith("get_"))
                    callable = EditorGUILayout.Toggle(EditorUtils.GetTempContent("Callable", null,
                ...
                if (lastParam != null && lastParam.IsParams(parameters)) {
                    exposeParams = EditorGUILayout.Toggle(EditorUtils.GetTempContent(
                            "Expose Parameters", null,
                            "This method accepts a params array. You can chose to expose the array to individual ports."),
                        exposeParams);

                    if (exposeParams) {
                        EditorGUI.indentLevel++;
                        exposedParamsCount = EditorGUILayout.DelayedIntField(
                            "Parameters Count", exposedParamsCount);
                        EditorGUI.indentLevel--;
                    }
                }
            }
