...
namespace FlowCanvas.Nodes
{
    ...
    public class ReflectedExtractorNodeWrapper<T> : FlowNode
    {
        ...
        public override string name => string.Format("Extract ({0})", typeof(T).FriendlyName());
            ...
            for (var i = 0; i < _selectedInstanceMembers.Count; i++) {
                ...
                if (!_memberInfos.TryGetValue(name, out info))
                    Logger.LogError(string.Format(
                        "Field/Property named '{0}', was not found on type '{1}'", name,
                        typeof(T)), "Extractor", this);
                final[i] = info;
            }
            extractorNode = BaseReflectedExtractorNode.GetExtractorNode(typeof(T), false, final);
            if (extractorNode != null) extractorNode.RegisterPorts(this);
        }
        ...
        //...
        private bool DoListCheckout(List<string> options, ref List<string> selected)
        {
            var changed = false;
            ...
            for (var i = 0; i < options.Count; i++) {
                ...
                var newActive = EditorGUILayout.Toggle(active, GUILayout.Width(20));
                GUILayout.EndHorizontal();

                if (newActive != active) {
                    changed = true;
                    UndoUtility.RecordObject(graph, "Extractor");
                    if (newActive) selected.Add(name);
