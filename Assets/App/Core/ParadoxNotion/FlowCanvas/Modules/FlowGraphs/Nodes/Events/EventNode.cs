...
{
    [Category("Events"), Color("ff5c5c"), ContextDefinedOutputs(typeof(Flow)), ExecutionPriority(1)]
    ///<summary>Base class for event nodes.</summary>
    public abstract class EventNode : FlowNode
    {
        public enum TargetMode { SingleTarget = 0, MultipleTargets = 1 }
    ...
    public abstract class RouterEventNode<T> : EventNode where T : Component
    {
        ...
        public override void OnPostGraphStarted()
        {
            ...
            if (targetMode == TargetMode.SingleTarget) {
                ...
                Subscribe(o.gameObject.GetAddComponent<EventRouter>());
            }
            ...
            if (targetMode == TargetMode.MultipleTargets) {
                ...
                foreach (var target in list)
                    if (target != null)
                        Subscribe(target.gameObject.GetAddComponent<EventRouter>());
            }
        }
        ...
        public override void OnGraphStoped()
        {
            ...
            if (targetMode == TargetMode.SingleTarget && target.value != null) {
                var router = target.value.gameObject.GetComponent<EventRouter>();
            ...
            if (targetMode == TargetMode.MultipleTargets && targets.value != null)
                foreach (var target in targets.value)
                    if (target != null) {
                        var router = target.gameObject.GetComponent<EventRouter>();
                        if (router != null) UnSubscribe(router);
                    }
        }
        ...
        protected T ResolveReceiver(GameObject receiver)
        {
            ...
            for (var i = 0; i < listResult.Count; i++) {
                ...
                if (element != null && element.gameObject == receiver) return element;
            }
            return null;
        }
    }
