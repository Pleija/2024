...
namespace FlowCanvas.Nodes
{
    [Category("Flow Controllers/Switchers"),
     ...
     ContextDefinedInputs(typeof(string)), HasRefreshButton]
    public class SwitchString : FlowControlNode
    {
        ...
        protected override void RegisterPorts()
        {
            ...
            for (var i = 0; i < cases.Length; i++)
                cases[i] = AddFlowOutput(string.Format("\"{0}\"", comparisonOutputs[i]),
                    i.ToString());
            ...
            AddFlowInput("In", f => {
                ...
                if (selectorValue != null)
                    for (var i = 0; i < comparisonOutputs.Count; i++)
                        if (selectorValue.Equals(comparisonOutputs[i],
