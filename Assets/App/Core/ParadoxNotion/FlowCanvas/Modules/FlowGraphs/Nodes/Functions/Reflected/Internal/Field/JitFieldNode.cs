...
namespace FlowCanvas.Nodes
{
    ...
    {
        private static readonly Dictionary<string, UniversalDelegate> GetDelegates =
                new Dictionary<string, UniversalDelegate>(StringComparer.Ordinal);

        private static readonly Dictionary<string, UniversalDelegate> SetDelegates =
                new Dictionary<string, UniversalDelegate>(StringComparer.Ordinal);
            ...
            if ((!SetDelegates.TryGetValue(key, out setDelegat) || setDelegat == null)
                ...
                if (valueId >= 0) {
                    ...
                    if (instanceId >= 0) {
                        ...
                        ilGen.Emit(delegateParams[instanceId].GetCurrentType().RTIsValueType()
