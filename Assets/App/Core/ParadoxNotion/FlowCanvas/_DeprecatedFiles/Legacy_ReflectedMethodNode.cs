...
namespace FlowCanvas.Nodes.Legacy
{
    ...
    public abstract class ReflectedMethodNode
    {
        ...
        private static ReflectedMethodNode TryCreateJit(MethodInfo method)
        {
            if (method.ReturnType == typeof(void)) {
                ...
                return (ReflectedMethodNode)Activator.CreateInstance(argTypes.Count > 0
                        ? type.RTMakeGenericType(argTypes.ToArray()) : type);
            }
            else {
                Type type = null;
                var argTypes = new List<Type>();
    ...
    public sealed class PureReflectedMethodNode : ReflectedMethodNode
    {
            ...
            ReflectedMethodRegistrationOptions options)
        {
            ...
            //Return value port
            if (method.ReturnType != typeof(void))
                node.AddValueOutput("Value", method.ReturnType, () => {
                    return options.callable ? returnValue : CallMethod();
                });
            ...
            for (var _i = 0; _i < parameters.Length; _i++) {
                ...
                if (instanceInput != null
                    && paramName
                    == instanceInput
                            .name) //for rare cases where it's same as instance name like for example in Animation class.
                    paramName = paramName + " ";

                if (parameter.IsOut || parameter.ParameterType.IsByRef) {
                    node.AddValueOutput(paramName, parameter.ParameterType.GetElementType(), () => {
        ...
        {
            if (args == null) args = new object[inputs.Count + (paramsInputs != null ? 1 : 0)];
            for (var i = 0; i < inputs.Count; i++) args[i] = inputs[i].value;

            if (paramsInputs != null) {
                var paramsArray = Array.CreateInstance(
                    paramsArrayType.GetElementType(), paramsInputs.Count);
                for (var i = 0; i < paramsInputs.Count; i++)
                    paramsArray.SetValue(paramsInputs[i].value, i);
                args[args.Length - 1] = paramsArray;
            }
            if (method.IsStatic) return returnValue = method.Invoke(null, args);
            instance = instanceInput.value;
            if (instance == null || instance.Equals(null)) return returnValue = null;
    ...
    public sealed class ReflectedActionNode<T1> : ReflectedMethodNode
    {
            ...
            ReflectedMethodRegistrationOptions options)
        {
            ...
            if (!method.IsStatic)
                node.AddValueOutput(GetName(method, 0), () => {
                    return instance;
                });
        }
    }
    ...
    public sealed class ReflectedFunctionNode<TResult> : ReflectedMethodNode
    {
            ...
            ReflectedMethodRegistrationOptions options)
        {
            ...
            node.AddValueOutput("Value", () => {
                return options.callable ? returnValue : Call();
            });
        }
    }
    ...
    public sealed class ReflectedFunctionNode<T1, TResult> : ReflectedMethodNode
    {
            ...
            ReflectedMethodRegistrationOptions options)
        {
            ...
            node.AddValueOutput("Value", () => {
                return options.callable ? returnValue : Call(p1.value);
            });
        }
    }
    ...
    public sealed class ReflectedFunctionNode<T1, T2, TResult> : ReflectedMethodNode
    {
            ...
            ReflectedMethodRegistrationOptions options)
        {
            ...
            node.AddValueOutput("Value", () => {
                return options.callable ? returnValue : Call(p1.value, p2.value);
            });
        }
    }
    ...
    public sealed class ReflectedFunctionNode<T1, T2, T3, TResult> : ReflectedMethodNode
    {
            ...
            ReflectedMethodRegistrationOptions options)
        {
            ...
            node.AddValueOutput("Value", () => {
                return options.callable ? returnValue : Call(p1.value, p2.value, p3.value);
            });
        }
    }
            ...
            ReflectedFunctionNode<T1, T2, T3, T4, T5, T6, T7, T8, TResult> : ReflectedMethodNode
    {
        ...
        public override void RegisterPorts(FlowNode node, MethodInfo method,
            ReflectedMethodRegistrationOptions options)
        {
            call = method.RTCreateDelegate<FunctionCall<T1, T2, T3, T4, T5, T6, T7, T8, TResult>>(
                null);
            var p1 = node.AddValueInput<T1>(GetName(method, 0));
            var p2 = node.AddValueInput<T2>(GetName(method, 1));
            var p3 = node.AddValueInput<T3>(GetName(method, 2));
