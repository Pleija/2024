...
namespace Slate
{
    public enum TangentMode { Editable, Smooth, Linear, Constant, ClampedAuto }
    ...
    public static class CurveUtility
    {
        ...
        public static float GetKeyNext(float time, params AnimationCurve[] curves)
        {
            ...
            return keys.OrderBy(k => k.time)
                    .FirstOrDefault(k => k.time > time + KEY_PROXIMITY_TOLERANCE)
                    .time;
        }
        ...
        /// <summary>
        ///     Remove all keys of negative time (< 0)</summary>
        ...
        public static void RemoveKeysOffRange(this AnimationCurve curve, float min, float max)
        {
            ...
            for (var i = 0; i < curve.length; i++) {
                var key = curve[i];
                if (key.time >= min && key.time <= max) finalKeys.Add(key);
            }
        ...
        //...
        public static bool GetKeyBroken(Keyframe keyframe) => GetKeyBrokenDelegate(keyframe);
