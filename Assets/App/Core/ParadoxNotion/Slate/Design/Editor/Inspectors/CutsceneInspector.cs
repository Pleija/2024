...
namespace Slate
{
    ...
    public class CutsceneInspector : OdinEditor
    {
        ...
        private static Dictionary<Object, Editor> directableEditors =
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorUtility.IsPersistent(cutscene)) {
                EditorGUILayout.HelpBox("To edit a cutscene prefab please open it first.",
                    MessageType.Info);
                return;
            }
            var e = Event.current;
        ...
        private void DoCutsceneInspector()
        {
            ...
            if (optionsFold) {
                ...
                if (explLayersProp.boolValue) EditorGUILayout.PropertyField(activeLayersProp);
        ...
        private void DoActorsInspector()
        {
            ...
            if (actorsFold) {
                ...
                foreach (var group in cutscene.groups.OfType<ActorGroup>()) {
                    var name = string.IsNullOrEmpty(group.name) ? "(No Name Specified)"
                            : group.name;
                    group.actor = EditorGUILayout.ObjectField(
                        name, group.actor, typeof(GameObject), true) as GameObject;
                    exists = true;
                }
        ...
        private static void DoSelectionInspector()
        {
            ...
            if (currentDirectableEditor != null
                && (currentDirectableEditor.target != selection || selection == null)) {
                var disableMethod = currentDirectableEditor.GetType()
                        .GetMethod("OnDisable",
            ...
            if (currentDirectableEditor != newEditor) {
                var enableMethod = newEditor.GetType()
                        .GetMethod("OnEnable",
                            BindingFlags.Instance
                            | BindingFlags.Public
                            | BindingFlags.NonPublic
        ...
        private static void ShowPreliminaryInspector()
        {
            ...
            var nameAtt =
                    type.GetCustomAttributes(typeof(NameAttribute), false).FirstOrDefault() as
                            NameAttribute;
