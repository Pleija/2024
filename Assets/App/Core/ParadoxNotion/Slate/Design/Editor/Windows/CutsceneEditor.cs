...
namespace Slate
{
    public class CutsceneEditor : OdinEditorWindow
    {
        ...
        private void SafeDoAction(Action call)
        {
        ...
        private void DoPopup(Action call)
        {
            onDoPopup = call;
        }
...
#pragma warning restore
        ...
        private void StepBackward()
        {
            ...
            if (keyable != null) {
                var time =
                        keyable.animationData.GetKeyPrevious(
                            keyable.RootTimeToLocalTimeUnclamped());
                cutscene.currentTime = time + keyable.startTime;
                return;
            }
                    ...
                    if (o is GameObject) {
                        ...
                        if (cutscene.GetAffectedActors().Contains(go)) {
                            ShowNotification(new GUIContent(string.Format(
                                "GameObject '{0}' is already in the cutscene",
                                o.name)));
                            continue;
                        }
        ...
        private void ShowToolbar()
        {
            ...
            if (GUILayout.Button(string.Format("[{0}]", cutscene.name),
                ...
                GUILayout.Width(100))) {
                GenericMenu.MenuFunction2 SelectCutscene = cut => {
            ...
            if (GUILayout.Button("Snap: " + Prefs.snapInterval, EditorStyles.toolbarDropDown,
                GUILayout.Width(90))) {
                var menu = new GenericMenu();
            ...
            if (GUILayout.Button(string.Format("SLATE Cinematic Sequencer v{0}",
        ...
        private void DoScrubControls()
        {
            ...
            if (isMovingScrubCarret) {
                ...
                cutscene.currentTime = Mathf.Clamp(cutscene.currentTime,
                    Mathf.Max(viewTimeMin, 0) + float.Epsilon,
        ...
        private void ShowPlaybackControls(Rect topLeftRect)
        {
            ...
            if (GUI.Button(autoKeyRect, Styles.keyIcon, "box")) {
            ...
            if (GUILayout.Button(Styles.stepReverseIcon, "box", GUILayout.Width(20),
                GUILayout.Height(20))) {
                StepBackward();
                Event.current.Use();
            }
            ...
            if (isStoped) {
                if (GUILayout.Button(Styles.playReverseIcon, "box", GUILayout.Width(20),
                    GUILayout.Height(20))) {
                    PlayReverse();
                    Event.current.Use();
                }
                ...
                if (GUILayout.Button(Styles.playIcon, "box", GUILayout.Width(20),
                    GUILayout.Height(20))) {
                    Play();
                    Event.current.Use();
                }
                ...
                if (GUILayout.Button(Styles.pauseIcon, "box", GUILayout.Width(44),
                    GUILayout.Height(20))) {
                    Pause();
                    Event.current.Use();
                }
            ...
            if (GUILayout.Button(Styles.stopIcon, "box", GUILayout.Width(20),
                GUILayout.Height(20))) {
                Stop(false);
                Event.current.Use();
            }
            ...
            if (GUILayout.Button(Styles.stepIcon, "box", GUILayout.Width(20),
                GUILayout.Height(20))) {
                StepForward();
                Event.current.Use();
            }
        ...
        private void ShowTimeInfo(Rect topMiddleRect)
        {
            ...
            var lowMod = 0.01f;
            var modulos = new[] {
                0.1f, 0.5f, 1, 5, 10, 50, 100, 500, 1000, 5000, 10000, 50000, 100000, 250000,
                500000,
            }; //... O.o
            ...
            timeInfoStart = Mathf.FloorToInt(viewTimeMin / timeInfoInterval) * timeInfoInterval;
            timeInfoEnd = Mathf.CeilToInt(viewTimeMax / timeInfoInterval) * timeInfoInterval;
            ...
            GUI.BeginGroup(topMiddleRect);
            {
                ...
                for (var i = timeInfoStart; i <= timeInfoEnd; i += timeInfoInterval) {
                    ...
                    GUI.color = isProSkin ? Color.white : Color.black;
                    var markRect = Rect.MinMaxRect(
                        posX - 2, TOP_MARGIN - 3, posX + 2, TOP_MARGIN - 1);
        ...
        private void ShowGroupsAndTracksList(Rect leftRect)
        {
            ...
            var wasEnabled = GUI.enabled;
            GUI.enabled = true;
            var collapseAllRect = Rect.MinMaxRect(
                leftRect.x + 5, leftRect.y + 4, 20, leftRect.y + 20 - 1);
            ...
            if (GUI.Button(addRect, "Add Actor Group")) {
                var newGroup = cutscene.AddGroup<ActorGroup>().AddTrack<ActorActionTrack>();
                CutsceneUtility.selectedObject = newGroup;
            }
            ...
            for (var g = 0; g < cutscene.groups.Count; g++) {
                ...
                if (group.isCollapsed) {
                    ...
                    foreach (var track in group.tracks) {
                        foreach (var clip in track.clips) {
                            ...
                            GUI.DrawTexture(Rect.MinMaxRect(start + 0.5f, collapseRect.y + 2,
                                end - 0.5f,
                ...
                for (var t = 0; t < group.tracks.Count; t++) {
                    ...
                    if (viewTimeMin < 0) //just visual clarity
                        GUI.Box(Rect.MinMaxRect(TimeToPos(viewTimeMin), trackPosRect.yMin,
                            TimeToPos(0),
                    ...
                    for (var a = 0; a < track.clips.Count; a++) {
                            ...
                            if (multiSelection == null || multiSelection.Count < 1) {
                                ...
                                if (Prefs.magnetSnapping && !e.control) {
                                    ...
                                    if (snapStart != null && snapEnd != null) {
                                        ...
                                        var distEnd = Mathf.Abs(
                                            snapEnd.Value - (xTime + action.length));
                        ...
                        var isVisible = Rect.MinMaxRect(
                                    0, scrollPos.y, centerRect.width, centerRect.height)
                                .Overlaps(clipRect);
                        ...
                        var nextPosX = TimeToPos(
                            nextClip != null ? nextClip.startTime : viewTimeMax);
        ...
        private void ShowGroupSections(CutsceneGroup group, Rect rect)
        {
            ...
            for (var i = 0; i < sections.Count - 1; i++) {
                ...
                if (section1.colorizeBackground)
                    GUI.DrawTexture(Rect.MinMaxRect(sectionRect.xMin, sectionRect.yMax + 1,
                        sectionRect.xMax,
                ...
                GUI.DrawTexture(markRect, whiteTexture);
                GUI.color =
                        (section1.color.grayscale >= 0.5 ? Color.black : Color.white).WithAlpha(
                            0.5f);
                        ...
                        sectionsMenu.AddItem(new GUIContent("Edit"), false, () => {
                            DoPopup(() => {
                                ...
                                var previousSectionTime = sections.Last(s =>
                                                s.time < section1.time && s != section1)
                                        .time;
                                var nextSectionTime = sections.First(s =>
                                                s.time > section1.time && s != section1)
                                        .time;
                                section1.time = EditorGUILayout.Slider("Time", section1.time,
                                    previousSectionTime + 0.1f,
                                    nextSectionTime - 0.1f);
                                section1.exitMode =
                                        (Section.ExitMode)EditorGUILayout.EnumPopup("Exit Mode",
                                            section1.exitMode);
                                if (section1.exitMode == Section.ExitMode.Loop)
                                    section1.loopCount = EditorGUILayout.IntField(
                                        "Loops", section1.loopCount);
                                section1.color = EditorGUILayout.ColorField(
                                    "Color", section1.color);
                                section1.colorizeBackground = EditorGUILayout.Toggle(
                                    "Colorize Background", section1.colorizeBackground);
                            });
                        });
        ...
        private void DoMultiSelection()
        {
            ...
            if (e.rawType == EventType.MouseUp) {
                if (bigEnough) {
                    multiSelection = clipWrappers.Values
                            .Where(b => r.Overlaps(b.rect) && !b.action.isLocked)
                            .ToList();
            ...
            if (multiSelection != null) {
                var boundRect = RectUtility
                        .GetBoundRect(multiSelection.Select(b => b.rect).ToArray())
                        .ExpandBy(4);
                var leftDragRect = new Rect(
                    boundRect.xMin - 6, boundRect.yMin, 4, boundRect.height);
                var rightDragRect = new Rect(
                    boundRect.xMax + 2, boundRect.yMin, 4, boundRect.height);
                ...
                if (e.type == EventType.MouseDown
                    && (leftDragRect.Contains(e.mousePosition)
                        || rightDragRect.Contains(e.mousePosition))) {
                    multiSelectionScaleDirection = leftDragRect.Contains(e.mousePosition) ? -1 : 1;
                    var minTime = Mathf.Min(
                        multiSelection.Select(b => b.action.startTime).ToArray());
        ...
        private class ActionClipWrapper
        {
            ...
            public void OnClipGUI(int windowID)
            {
                ...
                e = Event.current;
                overlapIn = previousClip != null ? Mathf.Max(
                    previousClip.endTime - action.startTime, 0) : 0;
                ...
                if (editor.interactingClip == null || !editor.interactingClip.isDragging) {
                    var overlap = previousClip != null ? Mathf.Max(
                        previousClip.endTime - action.startTime, 0) : 0;
                ...
                if (e.type == EventType.MouseDown) {
                    ...
                    if (e.clickCount == 2)
                        ...
                        Selection.activeObject =
                                action.GetType()
                                        .GetProperty("actor")
                                        .GetValue(action, null) as Object;
                }
            ...
            private void DoClipContextMenu()
            {
                ...
                if (multiSelection != null && multiSelection.Contains(this)) {
                    menu.AddItem(new GUIContent("Delete Clips"), false, () => {
                        editor.SafeDoAction(() => {
                            foreach (var act in multiSelection.Select(b => b.action)
                                    .ToArray())
                                (act.parent as CutsceneTrack).DeleteAction(act);
