...
namespace Slate
{
    ...
    public static class EditorTools
    {
        ...
        public static void BeginBody(string text)
        {
            BeginBody(new GUIContent(text));
        }
            ...
            AnimationDataCollection.AddParameterDelegate callback)
        {
            ...
            foreach (var _comp in root.GetComponentsInChildren<Component>(true)) {
                ...
                var transformPath = root.transform.CalculateTransformPath(comp.transform);
                ...
                Predicate<Type> shouldContinue = t => !typeof(Object).IsAssignableFrom(t)
                        && !t.IsValueType
                        && !t.IsEnum
        ...
        public static T Popup<T>(string prefix, T selected, List<T> options,
            params GUILayoutOption[] GUIOptions) =>
                Popup(null, prefix, selected, options, GUIOptions);
        ...
        public static List<TypeMetaInfo> GetTypeMetaDerivedFrom(Type baseType)
        {
            ...
            foreach (var type in ReflectionTools.GetImplementationsOf(baseType)) {
                ...
                var catAtt =
                        type.GetCustomAttributes(typeof(CategoryAttribute), true).FirstOrDefault()
                                as CategoryAttribute;
                if (catAtt != null) info.category = catAtt.category;
                var attachAtt =
                        type.GetCustomAttributes(typeof(AttachableAttribute), true).FirstOrDefault()
                                as AttachableAttribute;
        ...
        {
            AnimBool fold = null;
            return foldOutStates.TryGetValue(o, out fold) ? fold.target
                    : (foldOutStates[o] = new AnimBool(false)).target;
        }
        ...
        public static Texture2D GetAudioClipTexture(AudioClip clip, int width, int height)
        {
            ...
            var step = Mathf.CeilToInt(clip.samples / width);
            ...
            while (i < width) {
                var barHeight = Mathf.CeilToInt(
                    Mathf.Clamp(Mathf.Abs(samples[i * step]) * height, 0, height));
                var add = samples[i * step] > 0 ? 1 : -1;
                for (var j = 0; j < barHeight; j++)
                    texture.SetPixel(i,
                        Mathf.FloorToInt(height / 2)
                        - Mathf.FloorToInt(barHeight / 2) * add
                        + j * add, Color.white);
                ++i;
            }
            texture.Apply();
