    ...
    public static class CurveEditor
    {
        ...
        public class CurveRenderer
        {
            ...
            */
            ...
            public int axisLock {
                set => cEditorType.GetField(
                            "m_AxisLock", BindingFlags.Instance | BindingFlags.NonPublic)
            ...
            public void Init()
            {
                ...
                cEditorCTR = cEditorType.GetConstructor(new[]
            ...
            private object GetCurveEditorSettings()
            {
                ...
                settingsType.GetField("rectangleToolFlags",
                            BindingFlags.Instance | BindingFlags.NonPublic)
                        .SetValue(settings, 1);
                settingsType.GetProperty("hRangeLocked",
                            BindingFlags.Instance | BindingFlags.NonPublic)
                        .SetValue(settings, true, null);
                settingsType.GetProperty("vRangeLocked",
                            BindingFlags.Instance | BindingFlags.NonPublic)
            ...
            private void CreateDelegates()
            {
                ...
                rectSetter = cEditorType.GetProperty("rect")
                        .GetSetMethod()
                        .RTCreateDelegate<Action<Rect>>(cEditor);
                rectGetter = cEditorType.GetProperty("rect")
                        .GetGetMethod()
                        .RTCreateDelegate<Func<Rect>>(cEditor);
                shownAreaSetter = cEditorType.GetProperty("shownArea")
                        .GetSetMethod()
                        .RTCreateDelegate<Action<Rect>>(cEditor);
                shownAreaGetter = cEditorType.GetProperty("shownArea")
                        .GetGetMethod()
                        .RTCreateDelegate<Func<Rect>>(cEditor);
                hRangeMaxSetter = cEditorType.GetProperty("hRangeMax")
                        .GetSetMethod()
                        .RTCreateDelegate<Action<float>>(cEditor);
                hRangeMaxGetter = cEditorType.GetProperty("hRangeMax")
                        .GetGetMethod()
