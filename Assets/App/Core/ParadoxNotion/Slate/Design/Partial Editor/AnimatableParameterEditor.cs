    ...
    public static class AnimatableParameterEditor
    {
            ...
            if (EditorTools.GetObjectFoldOut(animParam)) {
                ...
                if (hasExpression) {
                    if (GUILayout.Button(Styles.expressionIcon, GUI.skin.label, GUILayout.Width(18))) {
                        var menu =
                                Slate.Expressions.ExpressionsMenuGenerator.GetExpressionEnvironmentMenu(
                                    animParam.GetExpressionEnvironment(), (template) => {
                                        animParam.scriptExpression = template;
                                    }
                                );
                        menu.ShowAsContext();
                    }
                    ...
                    animParam.scriptExpression =
                            EditorGUILayout.DelayedTextField(animParam.scriptExpression);
                }
                GUILayout.EndHorizontal();
        ...
        public static void DoParamGearContextMenu(AnimatedParameter animParam, IKeyable keyable)
        {
            ...
            if (animParam.enabled) {
                if (hasKeyNow) {
                    ...
                    menu.AddItem(new GUIContent("Remove Key"), false, () => {
                        animParam.RemoveKey(keyableTime);
                    });
                }
                else {
                ...
                if (hasAnyKey) {
                    menu.AddItem(new GUIContent("Pre Wrap Mode/Clamp"), false, () => {
                        animParam.SetPreWrapMode(WrapMode.ClampForever);
                    });
                    menu.AddItem(new GUIContent("Pre Wrap Mode/Loop"), false, () => {
                        animParam.SetPreWrapMode(WrapMode.Loop);
                    });
                    menu.AddItem(new GUIContent("Pre Wrap Mode/PingPong"), false, () => {
                        animParam.SetPreWrapMode(WrapMode.PingPong);
                    });
                    menu.AddItem(new GUIContent("Post Wrap Mode/Clamp"), false, () => {
                        animParam.SetPostWrapMode(WrapMode.ClampForever);
                    });
                    menu.AddItem(new GUIContent("Post Wrap Mode/Loop"), false, () => {
                        animParam.SetPostWrapMode(WrapMode.Loop);
                    });
                    menu.AddItem(new GUIContent("Post Wrap Mode/PingPong"), false, () => {
                        animParam.SetPostWrapMode(WrapMode.PingPong);
                    });
                }
                else {
                ...
                if (!animParam.hasActiveExpression) {
                    menu.AddItem(
                        new GUIContent("Set Expression"), false, () => {
                            animParam.scriptExpression = "value";
                        }
                    );
                    ...
                    menu.AddItem(
                        new GUIContent("Remove Expression"), false, () => {
                            animParam.scriptExpression = null;
                        }
                    );
                }
#endif
            }
            menu.AddItem(new GUIContent(animParam.enabled ? "Disable" : "Enable"), false, () => {
                animParam.SetEnabled(!animParam.enabled, keyableTime);
            });
            menu.AddSeparator("/");
            if (hasAnyKey)
        ...
        public static void DoParameterField(string name, AnimatedParameter animParam, float time)
        {
            ...
            try {
                ...
                if (type == typeof(int)) {
                    if (animParamAtt != null
                        && animParamAtt.min != null
...
#else
                    newValue =
                            EditorGUILayout.ColorField(new GUIContent(name), (Color)value, false, true, true, new ColorPickerHDRConfig(0f, float.MaxValue, 0f, float.MaxValue));
#endif
                }
            ...
            bool isRecording)
        {
            ...
            if (e.rawType == EventType.MouseUp) isDraggingTime = false;
            ...
            if (isRecording) {
                ...
                Handles.color = EditorGUIUtility.isProSkin ? Styles.recordingColor : Color.red;
