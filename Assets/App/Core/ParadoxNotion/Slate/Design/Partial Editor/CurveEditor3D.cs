...
namespace Slate
{
    ...
    public static class CurveEditor3D
    {
        ...
        {
            private const float DRAW_RESOLUTION = 0.1f;
            private const float DRAW_THRESHOLD = 0.01f;
            private const float HANDLE_DISTANCE_COMPENSATION = 2f;
                ...
                Transform transformContext, float time, float timeSpan = 50f)
            {
                ...
                if (curves == null || curves.Length != 3) return;
                var curveX = curves[0];
                var curveY = curves[1];
                var curveZ = curves[2];
                if (curveX.length < 2 || curveY.length < 2 || curveZ.length < 2) return;
                if (curveX.length != curveY.length || curveY.length != curveZ.length) return;
                ...
                for (var k = 0; k < curveX.length; k++) {
                    ...
                    var isBroken = CurveUtility.GetKeyBroken(keyX)
                            && CurveUtility.GetKeyBroken(keyY)
                    ...
                    if ((screenPos - e.mousePosition).magnitude < 10)
                ...
                for (var t = start; t <= end; t += DRAW_RESOLUTION) {
                    ...
                    var nextPos = new Vector3(curveX.Evaluate(t + DRAW_RESOLUTION),
                        curveY.Evaluate(t + DRAW_RESOLUTION),
            ...
            private enum ContextAction { None, SetBrokenMode, SetTangentMode, Delete }
        }
    }
}
