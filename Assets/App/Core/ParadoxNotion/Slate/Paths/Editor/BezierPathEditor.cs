...
namespace Slate
{
    ...
    public class BezierPathEditor : OdinEditor
    {
        ...
        {
            for (var i = 0; i < path.pointCount; i++) DrawPointSceneGUI(path[i], i);
        }
        ...
        private void DrawPointSceneGUI(BezierPoint point, int index)
        {
            Handles.BeginGUI();
            var e = Event.current;
            ...
            if (e.type == EventType.MouseDown) {
                ...
                if ((screenPos - e.mousePosition).magnitude < 13) {
                    if (e.button == 0) {
                        selectedPoint = point;
                        SceneView.RepaintAll();
                    }
                    ...
                    if (e.button == 1) {
                        ...
                        menu.AddItem(new GUIContent("Tangent/Broken"), false, () => {
