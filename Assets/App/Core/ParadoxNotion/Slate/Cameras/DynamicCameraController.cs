...
namespace Slate
{
    ...
    public class DynamicCameraController
    {
        ...
        public Transposer transposer => _transposer;
            ...
            bool isHard)
        {
            ...
            if (transposer.target != null
                ...
                if (transposer.trackingMode == Transposer.TrackingMode.RailTracking) {
                    ...
                    //clamp it
                    if ((projectT - transposer.railStart).normalized
                        != (transposer.railEnd - transposer.railStart).normalized)
            ...
            if (composer.target != null && composer.trackingMode != Composer.TrackingMode.None)
                if (composer.trackingMode == Composer.TrackingMode.FrameComposition) {
                    ...
                    var left = Mathf.Clamp01(
                        composer.frameCenter.x + 0.5f - composer.frameExtends.x / 2);
                    var right = Mathf.Clamp01(
                        composer.frameCenter.x + 0.5f + composer.frameExtends.x / 2);
                    var top = Mathf.Clamp01(composer.frameCenter.y
                        + 0.5f
                        - composer.frameExtends.y / 2);
                    var bottom = Mathf.Clamp01(
                        composer.frameCenter.y + 0.5f + composer.frameExtends.y / 2);
                    var viewFrame = Rect.MinMaxRect(left, top, right, bottom);
                    var worldFrameCenter = cam.ViewportToWorldPoint(new Vector3(
                        1 - viewFrame.center.x, viewFrame.center.y,
                    ...
                    if (isHard || composer.smoothDamping == MIN_DAMP) {
                        ...
                        directableCamera.rotation = Quaternion.Lerp(
                            wasRotation, rotationToFrame, normDamp);
                    }
        ...
        {
            public enum OffsetMode { LocalSpace, WorldSpace }
            public enum TrackingMode { None, OffsetTracking, RailTracking }
        ...
        {
            public enum TrackingMode { None, FrameComposition }
            public TrackingMode trackingMode;
            ...
            [Tooltip("The smoothness to be applied."), Range(MIN_DAMP, MAX_DAMP)]
            public float smoothDamping = 3f;
        }
        ...
        public void DoSceneGUI(IDirectableCamera directableCamera)
        {
            if (transposer.target != null
                ...
                if (transposer.trackingMode == Transposer.TrackingMode.RailTracking) {
                    ...
                    var rStart = Handles.PositionHandle(transposer.railStart, Quaternion.identity);
                    var rEnd = Handles.PositionHandle(transposer.railEnd, Quaternion.identity);

                    if (EditorGUI.EndChangeCheck()) {
                        Undo.RecordObject(directableCamera as Object, "Rail Change");
