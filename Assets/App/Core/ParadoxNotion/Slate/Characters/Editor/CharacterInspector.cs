...
namespace Slate
{
    ...
    public class CharacterInspector : OdinEditor
    {
        ...
        private Character character => (Character)target;
        ...
        protected override void OnEnable()
        {
            ...
            if (!Application.isPlaying && character != null) character.ResetExpressions();
        }
        ...
        private void SetWireframeHidden(bool active)
        {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var skins = character.GetComponentsInChildren<Component>()
                    .Where(c => c is SkinnedMeshRenderer
            ...
            foreach (var expression in character.expressions.ToArray()) {
                ...
                foldStates[expression] = EditorGUILayout.Foldout(
                    foldStates[expression], expression.name);
                ...
                if (foldStates[expression]) {
                    ...
                    foreach (var shape in expression.blendShapes.ToArray()) {
                        ...
                        EditorGUI.BeginChangeCheck();
                        skin = EditorTools.Popup("Skin", skin, skins);

                        if (skin != null) {
                            name = EditorTools.Popup("Shape", name,
                                skin.GetBlendShapeNames().ToList());
                            weight = EditorGUILayout.Slider("Weight", weight, -1, 2);
                        }
                        GUILayout.EndVertical();
                        if (skin != shape.skin || name != shape.name) shape.SetRealWeight(0);
