...
namespace Slate
{
    ...
    public class Cutscene : MonoBehaviour, IDirector
    {
        ...
        public enum WrapMode { Once, Loop, PingPong }

        ///<summary>What happens when cutscene stops</summary>
        public enum StopMode { Skip, Rewind, Hold, SkipRewindNoUndo }

        ///<summary>Update modes for cutscene</summary>
        public enum UpdateMode { Normal, AnimatePhysics, UnscaledTime, Manual }

        ///<summary>The direction the cutscene can play. An enum for clarity.</summary>
        public enum PlayingDirection { Forwards, Backwards }
        ...
        public DirectorGroup directorGroup {
            get {
                ...
                if (groups.Count > 0 && groups[0] is DirectorGroup) return (DirectorGroup)groups[0];
        ...
        GameObject IDirector.context => this != null ? gameObject : null;

        ///<summary>The groups</summary>
        IEnumerable<IDirectable> IDirector.children => groups;
        ...
        {
            Validate();
            allSceneCutscenes[name] = this;
            if (directorGroup != null) directorGroup.OnSectionReached += DirectorSectionReached;
        }
        ...
        protected void OnDestroy()
        {
            ...
            StopAllCoroutines();
            isActive = false;
            allSceneCutscenes.Remove(name);
            for (var i = 0; i < directables.Count; i++) directables[i].RootDestroyed();
        }
        ...
        {
            if (isActive
                && (updateMode == UpdateMode.Normal || updateMode == UpdateMode.UnscaledTime)) {
                if (isPaused) {
                    Sample();
                    return;
                }
        ...
        private void DirectorSectionReached(Section section)
        {
            if (!isActive) return;
            ...
            if (OnSectionReached != null) OnSectionReached(section);
        }
        ...
        {
            if (timePointers == null) InitializeTimePointers();
        ...
        public void Play(float startTime, float endTime, WrapMode wrapMode = WrapMode.Once,
            Action callback = null, PlayingDirection playDirection = PlayingDirection.Forwards)
        {
            ...
            UpdateCutscene(Time
                    .deltaTime); //immediately update once now instead of waiting LaterUpdate, FixedUpdate etc.
        ...
        public void Stop(StopMode stopMode)
        {
            ...
            isActive = false;
            isPaused = false;
            if (stopMode == StopMode.Skip) Sample(playTimeMax);
            if (stopMode == StopMode.Rewind) Sample(playTimeMin);
            if (stopMode == StopMode.Hold) Sample();
            ...
            if (OnCutsceneStopped != null) OnCutsceneStopped(this);
        ...
        {
            if (isActive) Stop(StopMode.Hold);
            currentTime = 0;
        ...
        public void SkipCurrentSection()
        {
            ...
            var section = forward ? directorGroup.GetSectionAfter(currentTime)
        ...
        public bool JumpToSection(string name) => JumpToSection(GetSectionByName(name));
        ...
        public bool PlayFromSection(string name) => PlayFromSection(name, defaultWrapMode);

        public bool PlayFromSection(string name, WrapMode wrap, Action callback = null) =>
                PlayFromSection(GetSectionByName(name), wrap, callback);

        public bool PlayFromSection(Section section) => PlayFromSection(section, defaultWrapMode);
        ...
        public bool PlaySection(Section section) => PlaySection(section, defaultWrapMode);
        ...
        */
        ...
        {
            if (groupsRoot.transform.parent != transform) groupsRoot.transform.parent = transform;
        ...
        public static Cutscene Play(string name, Action callback)
        {
            ...
            if (cutscene != null) {
                var instance = Instantiate(cutscene);
        ...
        public static Cutscene FindFromResources(string name)
        {
            ...
            Debug.LogWarning(string.Format(
                "Cutscene of name '{0}' does not exists in the Resources folder",
                name));
            return null;
        }
...
#if UNITY_EDITOR
            Debug.Log(string.Format("<b>({0}) Global Message Send:</b> '{1}' ({2})", name, message,
                value), gameObject);
#endif
        }
        ...
        public void SetGroupActorOfName(string groupName, GameObject newActor)
        {
            ...
            if (group == null) {
                Debug.LogError(string.Format(
                    "Actor Group with name '{0}' doesn't exist in cutscene",
        ...
        /// <summary>
        ///     Providing a path to the element in the order of Group->Track->Clip, like for example
        ///     ("MyGroup/MyTrack/MyClip"), returns that element.
        /// </summary>
        public T FindElement<T>(string path) where T : IDirectable => (T)FindElement(path);
        ...
        public IDirectable FindElement(string path)
        {
            ...
            var result =
                    groups.FirstOrDefault(g => g.name.ToLower() == split[0].ToLower()) as
                            IDirectable;
        ...
        //...
        public override string ToString() => string.Format("'{0}' Cutscene", name);

        ///<summary>Get a section by name</summary>
        public Section GetSectionByName(string name) => directorGroup.GetSectionByName(name);

        ///<summary>Get a section by UID</summary>
        public Section GetSectionByUID(string UID) => directorGroup.GetSectionByUID(UID);

        ///<summary>All section names of the DirectorGroup</summary>
        public Section[] GetSections() => directorGroup.sections.ToArray();
        ...
        {
            if (currentTime > 0) Stop(StopMode.Rewind);
        }
        ...
        public static Cutscene Create(Transform parent = null)
        {
            ...
            if (parent != null) cutscene.transform.SetParent(parent, false);
        ...
        ///<summary>Add a group to the cutscene.</summary>
        public T AddGroup<T>(GameObject targetActor = null) where T : CutsceneGroup =>
                (T)AddGroup(typeof(T), targetActor);

        public CutsceneGroup AddGroup(Type type, GameObject targetActor = null)
        {
            ...
            var newGroup = new GameObject(type.Name).AddComponent(type) as CutsceneGroup;
            Undo.RegisterCreatedObjectUndo(newGroup.gameObject, "Add Group");
            Undo.SetTransformParent(newGroup.transform, groupsRoot, "Add Group");
        ...
        public void DeleteGroup(CutsceneGroup group)
        {
            if (!group.gameObject.IsSafePrefabDelete()) {
                EditorUtility.DisplayDialog("Delete Group",
        ...
        {
            if (group == null || group is DirectorGroup) return null;
            var newGroup = Instantiate(group);
            Undo.RegisterCreatedObjectUndo(newGroup.gameObject, "Duplicate Group");
            Undo.SetTransformParent(newGroup.transform, groupsRoot, "Duplicate Group");
            Undo.RegisterCompleteObjectUndo(this, "Duplicate Group");
            newGroup.actor = targetActor;
            newGroup.transform.localPosition = Vector3.zero;
            groups.Add(newGroup);
            Validate();
