...
namespace Slate
{
    ...
    public class AnimatedParameter : IAnimatableData, ISerializationCallbackReceiver
    {
        ...
        public bool enabled => !_isDisabled;
        ...
        public bool hasActiveExpression => !string.IsNullOrEmpty(scriptExpression);
        ...
        public object targetObject => keyable != null ? keyable.animatedParametersTarget : null;
        ...
        [NonSerialized]
        private object
                _animatableAttribute; //Cache of possible [AnimatableParameter] attribute used for creating this parameter
        ...
        public bool isValid {
            get {
                if (string.IsNullOrEmpty(serializedData) || data == null) return false;
        ...
        {
            if (isValid) return isProperty ? property : (MemberInfo)field;
            return null;
        }
        ...
        {
            if (hasActiveExpression) return false;
            if (!isValid || !enabled) return false;
            if (!HasAnyKey() && !isExternal) return false;
        ...
        {
            if (!enabled || targetObject == null || targetObject.Equals(null)) return;
        ...
        private void Evaluate_1_Curves(float time, float previousTime, float weight = 1f)
        {
            if (!HasAnyKey()) return;
#if UNITY_EDITOR
            {
                if (!Application.isPlaying) {
                    if (time == previousTime && HasChanged()) {
                        if (!Prefs.autoKey) {
                            //in case of no Auto-Key, store changed params
                    ...
                    if (!Prefs.autoKey) CutsceneUtility.changedParameterCallbacks.Remove(this);
                }
            }
#endif
        ...
        private bool Evaluate_2_Expression(float time, float previousTime, float weight = 1f)
        {
#if SLATE_USE_EXPRESSIONS
            {
        ...
        public void RestoreSnapshot()
        {
            ...
            if (snapshot != null && isExternal) SetCurrentValue(snapshot);
            lastEval = null;
            snapshot = null;
        }
        ...
        public object GetEvalValue(float time)
        {
            ...
            for (var i = 0; i < curves.Length; i++) floats[i] = curves[i].Evaluate(time);
        ...
        [NonSerialized]
        private object _resolvedMemberObject;
        ...
        public object ResolvedMemberObject()
        {
            ...
            if (_resolvedMemberObject != null
                && !_resolvedMemberObject.Equals(null)
                && snapshot != null)
            ...
            if (targetObject is GameObject) {
                ...
                if (!string.IsNullOrEmpty(transformHierarchyPath)) {
                    var leafTransform =
                            leafGameObject.transform.ResolveTransformPath(transformHierarchyPath);
        ...
        private float[] GetCurrentValueAsFloats()
        {
            ...
            if (obj is Transform && animatedType == typeof(Vector3)) {
                //special treat
                ...
                if (parameterName == "localScale") vector = transform.localScale;
        ...
        {
            SetCurrentValue(parameterModel.ConvertToFloats(value));
        }
        ...
        public bool HasAnyKey() => CurveUtility.HasAnyKey(curves);

        ///<summary>Has any key at time?</summary>
        public bool HasKey(float time) => CurveUtility.HasKey(time, curves);

        ///<summary>Returns the key time after time, or first key if time is last key time.</summary>
        public float GetKeyNext(float time) => CurveUtility.GetKeyNext(time, curves);

        ///<summary>Returns the key time before time, or last key if time is first key time.</summary>
        public float GetKeyPrevious(float time) => CurveUtility.GetKeyPrevious(time, curves);
        ...
        {
            RecordUndo();
            foreach (var curve in curves) curve.preWrapMode = mode;
            NotifyChange();
        }
        ...
        {
            RecordUndo();
            foreach (var curve in curves) curve.postWrapMode = mode;
            NotifyChange();
        }
        ...
        public void OffsetValue(object deltaValue)
        {
            ...
            for (var i = 0; i < curves.Length; i++) curves[i].OffsetCurveValue(decomposed[i]);
            NotifyChange();
        }
        ...
        private void RecordUndo()
        {
            ...
            {
                Object obj = null;
                obj = keyable as Object;
                if (obj != null) Undo.RecordObject(obj, "Parameter Change");
                obj = ResolvedMemberObject() as Object;
                if (obj != null) Undo.RecordObject(obj, "Parameter Change");
            }
#endif
        }
        ...
        {
            if (onParameterChanged != null) onParameterChanged(this);
        }
    }
}
