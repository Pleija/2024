...
namespace Slate
{
    ...
    {
        public delegate bool AddParameterDelegate(Type type, string memberPath,
        ...
        public AnimationDataCollection(IKeyable keyable, Type type, string[] memberPaths,
            string transformPath)
        {
        ...
        public List<AnimatedParameter> animatedParameters => _animatedParameters;
        ...
        public AnimationCurve[] GetCurves() => Internal_GetCurves(true);
        ...
        {
            if (animatedParameters != null)
                return animatedParameters.Select(p => p.GetKeyNext(time))
                        .OrderBy(t => t)
        ...
        {
            if (animatedParameters != null)
                return animatedParameters.Select(p => p.GetKeyPrevious(time))
                        .OrderBy(t => t)
                        .LastOrDefault(t => t < time);
            return 0;
        }
        ...
        ///     of root transform.
        /// </summary>
        public bool TryAddParameter(IKeyable keyable, Type type, string memberPath,
            string transformPath)
        {
            ...
            if (!newParam.isValid) return false;
        ...
        public void ReOrderParameters()
        {
            _animatedParameters = animatedParameters.OrderBy(p => p.ToString())
                    .OrderBy(p => p.transformHierarchyPath)
                    .ToList();
        }
        ...
        {
            if (animatedParameters == null) return null;
            return _animatedParameters.Find(d => d.parameterName.ToLower() == name.ToLower());
        }

        public AnimationCurve[] GetCurvesAll() => Internal_GetCurves(false);

        private AnimationCurve[] Internal_GetCurves(bool enabledParamsOnly)
        {
            if (animatedParameters == null) return new AnimationCurve[0];
            ...
            for (var i = 0; i < animatedParameters.Count; i++)
                if (!enabledParamsOnly || animatedParameters[i].enabled) {
                    ...
                    if (curves != null) result.AddRange(curves);
                }
