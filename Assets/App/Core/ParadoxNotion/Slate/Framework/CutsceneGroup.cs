...
namespace Slate
{
    ...
    public abstract class CutsceneGroup : MonoBehaviour, IDirectable
    {
        ...
        bool IDirectable.Initialize()
        {
            ...
            if (EditorUtility.IsPersistent(actor))
        ...
        private void TryInstantiateLocalActor()
        {
            if (refDuplicateActor == null) {
                var original = actor;
                refDuplicateActor = Instantiate(original);
        ...
        {
            actor.transform.position = this.TransformPosition(
                initialLocalPosition, TransformSpace.CutsceneSpace);
            actor.transform.rotation = this.TransformRotation(
                initialLocalRotation, TransformSpace.CutsceneSpace);
        }
        ...
        public CutsceneTrack AddTrack(Type type, string name = null)
        {
            ...
            var newTrack = Undo.AddComponent(go, type) as CutsceneTrack;
            Undo.SetTransformParent(newTrack.transform, transform, "New Track");
            Undo.RegisterCompleteObjectUndo(this, "New Track");
            newTrack.transform.localPosition = Vector3.zero;
        ...
        public void DeleteTrack(CutsceneTrack track)
        {
            if (!track.gameObject.IsSafePrefabDelete()) {
                EditorUtility.DisplayDialog("Delete Track",
        ...
        public CutsceneTrack DuplicateTrack(CutsceneTrack track)
        {
            ...
            Undo.RegisterCreatedObjectUndo(newTrack.gameObject, "Duplicate Track");
            Undo.SetTransformParent(newTrack.transform, transform, "Duplicate Track");
            Undo.RegisterCompleteObjectUndo(this, "Duplicate Track");
            newTrack.transform.localPosition = Vector3.zero;
            tracks.Add(newTrack);
            root.Validate();
            CutsceneUtility.selectedObject = newTrack;
