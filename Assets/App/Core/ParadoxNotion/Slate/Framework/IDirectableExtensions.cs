...
namespace Slate
{
    public static class IDirectableExtensions
    {
        ...
        public static float RootTimeToLocalTime(this IDirectable directable) =>
                directable.ToLocalTime(directable.root.currentTime);

        ///<summary>The local time of the directable based on root current time</summary>
        public static float RootTimeToLocalTimeUnclamped(this IDirectable directable) =>
                directable.ToLocalTimeUnclamped(directable.root.currentTime);
        ...
        public static bool IsRootTimeWithinClip(this IDirectable directable) =>
                IsTimeWithinClip(directable, directable.root.currentTime);
        ...
        public static float ToLocalTime(this IDirectable directable, float time) =>
                Mathf.Clamp(time - directable.startTime, 0, directable.GetLength());

        ///<summary>The local time of the directable based on provided time</summary>
        public static float ToLocalTimeUnclamped(this IDirectable directable, float time) =>
                time - directable.startTime;
        ...
        public static bool IsTimeWithinClip(this IDirectable directable, float time) =>
                time >= directable.startTime && time <= directable.endTime && time > 0;
        ...
        public static bool CanBlendIn(this IDirectable directable)
        {
            ...
            return blendInProp != null
                    && blendInProp.CanWrite
                    && directable.blendIn != -1
        ...
        public static bool CanBlendOut(this IDirectable directable)
        {
            ...
            return blendOutProp != null
                    && blendOutProp.CanWrite
                    && directable.blendOut != -1
        ...
        public static T GetNextSibling<T>(this IDirectable directable) where T : IDirectable =>
                (T)GetNextSibling(directable);

        public static IDirectable GetNextSibling(this IDirectable directable)
        {
            if (directable.parent != null)
                return directable.parent.children.FirstOrDefault(d =>
                        d != directable && d.startTime > directable.startTime);
            return null;
        }
        ...
        public static float GetWeight(this IDirectable directable) =>
                GetWeight(directable, RootTimeToLocalTime(directable));
        ...
        public static float GetWeight(this IDirectable directable, float time) =>
                GetWeight(directable, time, directable.blendIn, directable.blendOut);
        ...
        public static Vector3 ActorPositionInSpace(this IDirectable directable,
            TransformSpace space) => directable.actor != null
        ...
        {
            if (Application.isPlaying || keyable.root.isReSampleFrame) return false;
            if (!Prefs.autoKey || GUIUtility.hotControl != 0) return false;
            var case1 = ReferenceEquals(CutsceneUtility.selectedObject, keyable);
            var activeTransform = Selection.activeTransform;
            var actor = keyable.actor;
            var case2 = actor != null
