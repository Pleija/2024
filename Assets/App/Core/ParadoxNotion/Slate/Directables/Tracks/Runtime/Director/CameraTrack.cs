...
namespace Slate
{
    ...
    public class CameraTrack : CutsceneTrack
    {
        ...
        public override float endTime {
            get => parent.endTime - _endTimeOffset;
        ...
        public override float blendIn {
            get {
                ...
                return firstShot.startTime - startTime + _blendIn;
            }
            set => _blendIn = value;
        }
        ...
        public override float blendOut {
            get {
                ...
                return endTime - lastShot.endTime + _blendOut;
            }
            set => _blendOut = value;
        }
        ...
        {
            if (activeCameraTrack != null) return;
            activeCameraTrack = this;
            firstShot = (CameraShot)clips.FirstOrDefault(s => s.startTime >= startTime);
            lastShot = (CameraShot)clips.LastOrDefault(s => s.endTime <= endTime);
            currentShot = firstShot;
            if (exitCameraOverride != null) exitCameraOverride.gameObject.SetActive(false);
            DirectorCamera.Enable();
        }

        protected override void OnUpdate(float time, float previousTime)
        {
            if (activeCameraTrack != this) return;
            ...
            Func<float, float> TimeToPos)
        {
            ...
            if (blendIn > 0) {
                ...
                if (first != null) {
                    var a = new Vector2(TimeToPos(startTime), posRect.y + defaultHeight / 2);
                    var b = new Vector2(TimeToPos(first.startTime), a.y);
                    b.x -= 1;
                    Handles.DrawLine(a, b);
                    Handles.DrawLine(b, new Vector2(b.x - 5, b.y - 5));
                    Handles.DrawLine(b, new Vector2(b.x - 5, b.y + 5));
                }
            }

            if (blendOut > 0) {
                var last = clips.LastOrDefault(s => s.endTime <= endTime);

                if (last != null) {
                    var a = new Vector2(TimeToPos(endTime), posRect.y + defaultHeight / 2);
                    var b = new Vector2(TimeToPos(last.endTime), a.y);
                    Handles.DrawLine(a, b);
                    Handles.DrawLine(a, new Vector2(a.x - 5, a.y - 5));
                    Handles.DrawLine(a, new Vector2(a.x - 5, a.y + 5));
                }
            }
