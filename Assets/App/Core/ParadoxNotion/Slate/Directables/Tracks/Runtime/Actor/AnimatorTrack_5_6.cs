...
namespace Slate
{
    ...
    partial class AnimatorTrack : CutsceneTrack
    {
        ...
        {
            animator =
                    actor.GetComponentInChildren<Animator>(); //re-get to fetch from virtual actor ref instance if any
        ...
        void CreateAndPlayTree()
        {
            ...
            foreach (var playAnimClip in clipActions) {
                var clipPlayableHandle =
                        graph.CreateAnimationClipPlayable(playAnimClip.animationClip);
