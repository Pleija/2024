...
namespace Slate
{
    ...
    partial class AnimatorTrack
    {
        ...
        protected override bool OnInitialize()
        {
            ...
            siblingTracks = parent.children.OfType<AnimatorTrack>()
                    .Where(t => t.isActive)
                    .Reverse()
                    .ToList();
            return true;
        ...
        protected override void OnEnter()
        {
            ...
            if (isMasterTrack) {
                ...
                CreateAndPlayTree();
                if (useRootMotion) BakeRootMotion();
        ...
        private void PostUpdateMasterTrack(float time, float previousTime)
        {
            ...
            if (useRootMotion && useBakedRootMotion) ApplyBakedRootMotion(time);
        }
        ...
        private void PostExitMasterTrack()
        {
            animator.applyRootMotion = wasRootMotion;
            animator.cullingMode = wasCullingMode;
            if (graph.IsValid()) graph.Destroy();
            if (useRootMotion) ApplyBakedRootMotion(endTime - startTime);
        }
        ...
        protected override void OnReverse()
        {
            if (isMasterTrack) {
                ...
                animator.cullingMode = wasCullingMode;
                if (graph.IsValid()) graph.Destroy();
                if (useRootMotion) ApplyBakedRootMotion(0);
            }
        }
        ...
        private void CreateAndPlayTree()
        {
            ...
            animatorPlayable = AnimatorControllerPlayable.Create(
                graph, animator.runtimeAnimatorController);
        ...
        private Playable CreateClipsMixer(PlayableGraph graph)
        {
        ...
        private void EvaluateTrackClips(float time, float previousTime)
        {
            foreach (var clip in (this as IDirectable).children) {
                if (time >= clip.startTime && previousTime < clip.startTime) clip.Enter();
                if (time >= clip.startTime && time <= clip.endTime)
                    clip.Update(time - clip.startTime, previousTime - clip.startTime);
                if (time >= clip.endTime && previousTime < clip.endTime) clip.Exit();
            }
        }
        ...
        private void ApplyBakedRootMotion(float time)
        {
            ...
            animator.transform.localPosition = snap ? posNext : Vector3.Lerp(
                posNow, posNext, Mathf.InverseLerp(tNow, tNext, time));
            var rotNow = rmRotations[frame];
            var rotNext = rmRotations[nextFrame];
            animator.transform.localRotation = snap ? rotNext : Quaternion.Lerp(
                rotNow, rotNext, Mathf.InverseLerp(tNow, tNext, time));
        }
    }
}
