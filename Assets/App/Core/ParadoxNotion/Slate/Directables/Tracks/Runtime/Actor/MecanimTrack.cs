...
namespace Slate
{
    ...
    public class MecanimTrack : CutsceneTrack
    {
        ...
        private const int RECORDING_FRAMERATE = 10;
        ...
        protected override void OnEnter()
        {
            ...
            if (Application.isPlaying || layerOrder != 0) {
                //only 0 MecanimTrack layer does the recording
                animator = null;
                return;
            }
            ...
            animator.recorderStartTime = startTime;
            ...
            foreach (var track in (parent as CutsceneGroup).tracks.OfType<MecanimTrack>()
                .Where(t => t.isActive)
                .Reverse())
                clips.AddRange(track.clips.OfType<MecanimBaseClip>().Where(a => a.isValid));
