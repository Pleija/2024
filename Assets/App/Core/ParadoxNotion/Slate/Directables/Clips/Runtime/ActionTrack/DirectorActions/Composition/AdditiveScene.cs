...
namespace Slate.ActionClips
{
    ...
    public class AdditiveScene : DirectorActionClip, ISubClipContainable
    {
        ...
        [ShowIf("updateRootCutscenes", 1), Tooltip("Update offset of root cutscenes")]
        ...
        float ISubClipContainable.subClipSpeed => 0;
        ...
        {
            if (temporary) {
                DenitializeSubSceneCutscenes(true);
                Deactivate();
            }
        }
        ...
        {
            DenitializeSubSceneCutscenes(false);
            Deactivate();
        }
        ...
        private string CleanPath(string path) => path.Replace("Assets/", "").Replace(".unity", "");
            ...
            if (subScene.isLoaded && subScene.IsValid())
                foreach (var go in subScene.GetRootGameObjects()) {
                    go.transform.position += TransformPosition(
                        scenePosition, (TransformSpace)space);
                    go.transform.rotation *= TransformRotation(
                        sceneRotation, (TransformSpace)space);
                    //clean up cameras
                    ...
                    if (cutscene != null) rootCutscenes.Add(cutscene);
                }
        }
        ...
        {
            GUI.DrawTexture(new Rect(0, 0, rect.height, rect.height), Styles.sceneIcon);
        }
