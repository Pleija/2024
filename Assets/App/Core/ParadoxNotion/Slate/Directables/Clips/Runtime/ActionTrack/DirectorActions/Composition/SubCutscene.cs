...
namespace Slate.ActionClips
{
    ...
    public class SubCutscene : DirectorActionClip, ISubClipContainable
    {
        ...
        [Required, FormerlySerializedAs("cutscene")]
        ...
        public override string info {
            get {
                ...
                return subCutscene != null ? string.Format(
                            "            SubCutscene\n            '{0}'", subCutscene.name)
                        : "No Cutscene Selected";
            }
        }

        public override bool isValid => subCutscene != null && !ReferenceEquals(subCutscene, root);
        ...
        public new GameObject
                actor => //this is not really needed but makes double clicking the clip, select the target cutscene
                isValid ? subCutscene.gameObject : base.actor;

        float ISubClipContainable.subClipOffset {
            get => subCutsceneTimeOffset;
            set => subCutsceneTimeOffset = value;
        }

        float ISubClipContainable.subClipLength => isValid ? subCutscene.length : 0;
        float ISubClipContainable.subClipSpeed => isValid ? subCutscene.playbackSpeed : 0;
