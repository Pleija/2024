...
namespace ParadoxNotion.Services
{
    ...
    public static class Threader
    {
        ...
        {
            applicationIsPlaying = EditorApplication.isPlayingOrWillChangePlaymode;
        ...
        {
            if (state == PlayModeStateChange.EnteredPlayMode) applicationIsPlaying = true;
            if (state == PlayModeStateChange.ExitingPlayMode) applicationIsPlaying = false;
        }
        ...
        private static void OnEditorUpdate()
        {
            if (threadMonitors.Count > 0)
                for (var i = 0; i < threadMonitors.Count; i++) {
                    ...
                    if (!e.MoveNext()) threadMonitors.RemoveAt(i);
                }
        }
#endif
