...
namespace ParadoxNotion
{
    ...
    public static class RectUtils
    {
        ...
        public static bool Encapsulates(this Rect a, Rect b) =>
                a.x < b.x && a.xMax > b.xMax && a.y < b.y && a.yMax > b.yMax;
        ...
        public static void DrawDebugRect(Rect rect, string label = "", Color color = default)
        {
            ...
            GUI.Label(rect, rect.x.ToString(), Styles.leftLabel);
            GUI.Label(rect, rect.y.ToString(), Styles.topCenterLabel);
            GUI.Label(rect,
                string.Format("{0}\nWidth:{1}", rect.xMax.ToString(), rect.width.ToString()),
                Styles.rightLabel);
            GUI.Label(rect,
                string.Format("{0}\nHeight:{1}", rect.yMax.ToString(), rect.height.ToString()),
                Styles.bottomCenterLabel);
            GUI.Label(rect, label, Styles.centerLabel);
            GUI.color = Color.white;
        }
#endif
    }
