...
namespace ParadoxNotion
{
    ...
    public static class StringUtils
    {
        ...
        public static string SplitCamelCase(this string s)
        {
            ...
            string result;
            if (splitCaseCache.TryGetValue(s, out result)) return result;
            result = s;
            var underscoreIndex = result.IndexOf('_');
            if (underscoreIndex <= 1) result = result.Substring(underscoreIndex + 1);
        ...
        public static string GetAlphabetLetter(int index)
        {
            ...
            if (index >= ALPHABET.Length) return index.ToString();
            ...
            string categoryName = "")
        {
            ...
            var firstLeafWord = leafName.Split(
                CHAR_SPACE_ARRAY, StringSplitOptions.RemoveEmptyEntries)[0];
            ...
            var score = 1f;
            if (categoryName.Contains(inputWords[0])) score *= 0.9f;
            if (firstLeafWord == inputWords[inputWords.Length - 1]) score *= 0.5f;
            if (leafName.StartsWith(inputWords[0])) score *= 0.5f;
            if (leafName.StartsWith(inputWords[inputWords.Length - 1])) score *= 0.5f;
            return score;
        }
        ...
        public static bool SearchMatch(string input, string leafName, string categoryName = "")
        {
            ...
            if (input.LastOrDefault() == '.') return categoryName.Contains(words[0]);
            ...
            for (var i = 0; i < words.Length; i++) {
                var word = words[i];
                if (!leftover.Contains(word)) return false;
        ...
        public static string ToStringAdvanced(this object o)
        {
            ...
            if (o is string) return string.Format("\"{0}\"", (string)o);
            if (o is Object) return (o as Object).name;
            var t = o.GetType();

            if (t.RTIsSubclassOf(typeof(Enum)))
                if (t.RTIsDefined<FlagsAttribute>(true)) {
