...
namespace ParadoxNotion.Serialization.FullSerializer.Internal
{
    public class fsPrimitiveConverter : fsConverter
    {
        ...
        private static bool UseInt64(Type type) => type == typeof(sbyte)
                || type == typeof(byte)
                || type == typeof(short)
                || type == typeof(ushort)
                || type == typeof(int)
            ...
            Type storageType)
        {
            ...
            if (UseDouble(storageType) || UseInt64(storageType)) {
                ...
                else if (fsGlobalConfig.Serialize64BitIntegerAsString
                    && storage.IsString
            ...
            return fsResult.Fail(GetType().Name
                + ": Bad data; expected bool, number, string, but got "
                + storage);
        }
    }
}
