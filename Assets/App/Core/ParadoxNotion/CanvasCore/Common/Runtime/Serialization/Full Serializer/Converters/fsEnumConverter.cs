...
namespace ParadoxNotion.Serialization.FullSerializer.Internal
{
    ...
    {
        public override bool CanProcess(Type type) => type.IsEnum;
            ...
            Type storageType)
        {
            if (fsGlobalConfig.SerializeEnumsAsInteger) {
                ...
                foreach (var value in Enum.GetValues(storageType)) {
                    ...
                    var isSet = (instanceValue & integralValue) == integralValue;
        ...
        {
            if (data.IsString) {
                var enumValues = data.AsString.Split(
                ...
                for (var i = 0; i < enumValues.Length; ++i) {
                    ...
                    if (ArrayContains(Enum.GetNames(storageType), enumValue) == false)
                        return fsResult.Fail("Cannot find enum name "
                            + enumValue
                            + " on type "
                            + storageType);
