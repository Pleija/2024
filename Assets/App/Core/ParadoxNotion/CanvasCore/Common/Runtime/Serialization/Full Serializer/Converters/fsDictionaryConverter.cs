            ...
            Type storageType)
        {
            ...
            while (enumerator.MoveNext()) {
                ...
                if ((result += Serializer.TrySerialize(keyStorageType, enumerator.Key, out keyData))
                    .Failed)
                    return result;
                if ((result += Serializer.TrySerialize(
                    valueStorageType, enumerator.Value, out valueData)).Failed)
                    return result;
        ...
        public override fsResult TryDeserialize(fsData data, ref object instance_, Type storageType)
        {
            ...
            if (data.IsDictionary) {
                foreach (var entry in data.AsDictionary) {
                    ...
                    var keyData = new fsData(entry.Key);
                    var valueData = entry.Value;
                    object keyInstance = null;
                    object valueInstance = null;
                    if ((result += Serializer.TryDeserialize(
                        keyData, keyStorageType, ref keyInstance)).Failed)
                        return result;
                    if ((result += Serializer.TryDeserialize(
                        valueData, valueStorageType, ref valueInstance)).Failed)
                        return result;
