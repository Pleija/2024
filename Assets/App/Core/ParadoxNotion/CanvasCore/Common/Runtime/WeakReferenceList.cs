...
namespace ParadoxNotion
{
    ...
    public class WeakReferenceList<T> where T : class
    {
        ...
        public WeakReferenceList() => list = new List<WeakReference<T>>();
        public WeakReferenceList(int capacity) => list = new List<WeakReference<T>>(capacity);
        public int Count => list.Count;

        public T this[int i] {
            get {
                list[i].TryGetTarget(out var reference);
                return reference;
            }
            set => list[i].SetTarget(value);
        }
        ...
        public bool Contains(T item, out int index)
        {
            for (var i = 0; i < list.Count; i++)
                if (list[i].TryGetTarget(out var target) && ReferenceEquals(target, item)) {
                    index = i;
                    return true;
                }
            index = -1;
        ...
        public List<T> ToReferenceList()
        {
            ...
            for (var i = 0; i < list.Count; i++) {
                var element = list[i];
                if (element.TryGetTarget(out var reference)) result.Add(reference);
            }
            return result;
        }
        ...
        public static implicit operator WeakReferenceList<T>(List<T> value)
        {
            ...
            for (var i = 0; i < value.Count; i++) result.Add(value[i]);
            return result;
        }
    }
}
