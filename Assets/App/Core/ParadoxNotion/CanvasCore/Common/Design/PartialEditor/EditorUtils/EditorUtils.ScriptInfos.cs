...
namespace ParadoxNotion.Design
{
    ...
    partial class EditorUtils
    {
        ...
        public static List<ScriptInfo> GetScriptInfosOfType(Type baseType)
        {
            ...
            var subTypes = baseType.IsGenericTypeDefinition ? new[] { baseType }
                    : ReflectionTools.GetImplementationsOf(baseType);

            foreach (var subType in subTypes) {
                if (subType.IsAbstract
                    || subType.RTIsDefined(typeof(DoNotListAttribute), true)
                ...
                if (isGeneric) {
                    var exposeAsBaseDefinition = Prefs.collapseGenericTypes
            ...
            infosResult = infosResult.Where(s => s.isValid)
