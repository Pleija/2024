...
namespace ParadoxNotion.Design
{
    ...
    public class GenericMenuBrowser : PopupWindowContent
    {
        ...
        private static Thread menuGenerationThread;
        private static Thread treeGenerationThread;
        private static Thread searchGenerationThread;
        ...
        private Type wasFocusedWindowType;
        private bool willRepaint;

        ///<summary>constructor</summary>
        private GenericMenuBrowser(GenericMenu newMenu, string title, Type keyType)
        {
            current = this;
            headerTitle = title;
        ...
        private bool filterFavorites {
            get => _filterFavorites && currentKeyType != null;
        ...
        public static void ShowAsync(Vector2 pos, string title, Type keyType,
        ...
        private void SetMenu(GenericMenu newMenu)
        {
        ...
        public override void OnClose()
        {
            ...
            if (treeGenerationThread != null && treeGenerationThread.IsAlive) {
                treeGenerationThread.Abort();
                treeGenerationThread = null;
            }

            if (searchGenerationThread != null && searchGenerationThread.IsAlive) {
                searchGenerationThread.Abort();
        ...
        private Node GenerateSearchResults()
        {
            ...
            searchRootNode.children = leafNodes
                    .Where(x => (filterFavorites ? x.isFavorite : true)
                            && StringUtils.SearchMatch(search, x.name, x.category))
                    .OrderBy(x =>
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            if (EditorGUIUtility.isProSkin) Styles.Draw(rect, Styles.shadowedBackground);
            ...
            if ((treeGenerationThread != null
                    && treeGenerationThread.ThreadState != ThreadState.Stopped)
                || items == null
                || items.Length == 0) {
        ...
        private void DoHeader(Rect headerRect, Event e)
        {
            //HEADER
            GUILayout.Space(5);
            GUILayout.Label(string.Format("<color=#{0}><size=14><b>{1}</b></size></color>",
            ...
            if (currentKeyType != null)
                filterFavorites = EditorGUILayout.ToggleLeft(
                    "FavOnly", filterFavorites, GUILayout.Width(70));
            ...
            if (currentNode.parent != null && string.IsNullOrEmpty(search)) {
                ...
                if (GUILayout.Button(string.Format("<b><size=14>◄ {0}/{1}</size></b>",
        ...
        {
            if (treeGenerationThread != null) return;
            ...
            foreach (var childPair in currentNode.children) {
                ...
                if (icon == null && memberInfo != null) icon = TypePrefs.GetTypeIcon(memberInfo);
                ...
                //
                if (filterFavorites && !node.isFavorite && !node.HasAnyFavoriteChild()) continue;

                if (node.isLeaf && node.item.separator) {
                    if (itemAdded) EditorUtils.Separator();
                    continue;
                }
                itemAdded = true;
        ...
        private void DoFooter(Rect helpRect, Event e)
        {
            ...
            var hoveringNode = hoveringIndex >= 0 && currentNode.children.Count > 0
                    ? currentNode.children.Values.ToList()[hoveringIndex] : null;
        ...
        private class Node
        {
            ...
            public bool isLeaf => item.isValid;
            public string category => parent != null ? parent.fullPath : string.Empty;

            //Is node favorite?
            public bool isFavorite {
                get {
                    if (current.favorites.Contains(fullPath)) return true;
                    var p = parent;
            ...
            {
                if (fav && !isFavorite) current.AddFavorite(fullPath);
                if (fav == false && isFavorite) current.RemoveFavorite(fullPath);
            }
        }
        ...
        private class SerializationData
        {
            ...
            {
                allFavorites = new Dictionary<string, List<string>>(StringComparer.Ordinal);
