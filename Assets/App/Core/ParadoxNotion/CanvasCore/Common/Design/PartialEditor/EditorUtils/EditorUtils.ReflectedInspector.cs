    ...
    partial class EditorUtils
    {
        ...
        public static void ReflectedObjectInspector(object target, UnityObject unityObjectContext)
        {
            ...
            for (var i = 0; i < fields.Length; i++) {
                ...
                if (field.IsPublic || field.RTIsDefined(typeof(ExposeFieldAttribute), true)) {
                    ...
                    var serializationInfo = new InspectedFieldInfo(
                        unityObjectContext, field, target, attributes);
            ...
            InspectedFieldInfo info)
        {
            ...
            if (info.attributes != null) {
                ...
                var tooltipAtt =
                        info.attributes.FirstOrDefault(a => a is TooltipAttribute) as
                                TooltipAttribute;
        ...
        public static object DirectFieldControl(GUIContent content, object value, Type t,
            UnityObject unityObjectContext, object[] attributes, out bool handled, object context,
            params GUILayoutOption[] options)
        {
            handled = true;

            if (typeof(AssetReference).IsAssignableFrom(t) && context is Variable variable) {
                var target = variable.assetObject;

                if (target == null) {
                    Debug.Log($"{variable.name} target is null");
                    target = variable.assetObject = ScriptableObject.CreateInstance<AssetObject>();
                    target.Reference = value as AssetReference;
                }
                target.serializedObject ??= new SerializedObject(target);
                target.property ??= target.serializedObject.FindProperty("value");
                //target.Reference ??= value as AssetReference;
                target.serializedObject.Update();
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(target.property, content, false);
                target.serializedObject.ApplyModifiedProperties();
                var path = AssetDatabase.GetAssetPath(target.Reference?.editorAsset);

                if (path != target.path) {
                    target.path = path;
                    Debug.Log($"changed: {path} => {target.path}");
                    UndoUtility.RecordObject(unityObjectContext, "Variable Value Change");
                    //variable.SetValueBoxed(target.Reference);
                    UndoUtility.SetDirty(unityObjectContext);
                    //EditorUtility.SetDirty(unityObjectContext);
                    UndoUtility.SetDirty(target);
                ...
                if (value == null || value is UnityObject) {
                    ...
                    if (unityObjectContext != null && newValue != null)
                        if (!Application.isPlaying
                            ...
                            && !EditorUtility.IsPersistent(newValue)) {
                            Logger.LogWarning("Assets can not have scene object references",
                                "Editor",
                                unityObjectContext);
            ...
            //Check Type second
            if (t == typeof(Type))
                return Popup(content, (Type)value, TypePrefs.GetPreferedTypesList(true), options);
            ...
            if (t.IsAbstract
                ...
                || typeof(UnityEventBase).IsAssignableFrom(t)) {
                EditorGUILayout.LabelField(content,
            ...
            if (value == null && t.RTIsValueType()) value = Activator.CreateInstance(t);
            ...
            if (t == typeof(Color)) {
                var att =
                        attributes?.FirstOrDefault(a => a is ColorUsageAttribute) as
                                ColorUsageAttribute;
