    ...
    partial class EditorUtils
    {
        ...
        public static GenericMenu GetMenu<T>(List<T> options, T current, Action<T> callback)
        {
            ...
            foreach (var _option in options) {
                ...
                menu.AddItem(new GUIContent(label), Equals(current, option), () => {
                    callback(option);
                });
            }
            return menu;
            ...
            bool showAddTypeOption = false)
        {
            ...
            GenericMenu.MenuFunction2 Selected = t => {
                callback((Type)t);
            };
            ...
            foreach (var t in TypePrefs.GetPreferedTypesList(constrainType, true)) {
                ...
                var finalType =
                        isGenericDefinition
                        && genericDefinitionType.TryMakeGeneric(t, out var genericType)
                                ? genericType : t;
                ...
                var finalDictListType =
                        isGenericDefinition
                        && genericDefinitionType.TryMakeGeneric(dictListType,
                            out var genericDictListType) ? genericDictListType : dictListType;
                if (constrainType.IsAssignableFrom(finalDictListType))
                    dictTypes[finalDictListType] = nsString;
            }
            foreach (var pair in listTypes)
                menu.AddItem(new GUIContent(subCategory
                    + TypePrefs.LIST_MENU_STRING
                    + pair.Value
                    + pair.Key.FriendlyName()), false, Selected, pair.Key);
            foreach (var pair in dictTypes)
                menu.AddItem(new GUIContent(subCategory
                    + TypePrefs.DICT_MENU_STRING
                    + pair.Value
                    + pair.Key.FriendlyName()), false, Selected, pair.Key);
            ...
            if (menu.GetItemCount() == 0)
                menu.AddDisabledItem(new GUIContent(string.Format(
                    "No {0} derived types found in Preferred Types List",
                    baseType.Name)));
            return menu;
        }
            ...
            string subMenu = null)
        {
            ...
            foreach (var field in type.GetFields(flags)
                ...
                menu.AddItem(new GUIContent(string.Format("{0}/{1} : {2}", category, field.Name,
            ...
            bool mustWrite = true, GenericMenu menu = null, string subMenu = null)
        {
            ...
            foreach (var prop in type.GetProperties(flags)) {
                ...
                var category = inherited ? subMenu + type.FriendlyName() + "/Inherited"
                        : subMenu + type.FriendlyName();
                menu.AddItem(new GUIContent(string.Format("{0}/{1} : {2}", category, prop.Name,
            ...
            GenericMenu menu = null, string subMenu = null)
        {
            ...
            foreach (var method in type.GetMethods(flags)) {
                if (propertiesOnly != method.IsPropertyAccessor()) continue;
                if (method.IsGenericMethod) continue;
                if (!returnType.IsAssignableFrom(method.ReturnType)) continue;
                if (method.ReturnType == typeof(void) && excludeVoid) continue;
                var parameters = method.GetParameters();
                if (parameters.Length > maxParameters && maxParameters != -1) continue;
                if (parameters.Length > 0)
                    if (acceptedParamsType != typeof(object)
                        && parameters.Any(param =>
                                !acceptedParamsType.IsAssignableFrom(param.ParameterType)))
                        continue;
        ...
        public static MenuItemInfo[] GetMenuItems(GenericMenu menu)
        {
            ...
            var contentGetter =
                    ReflectionTools.GetFieldGetter<object, GUIContent>(
                        itemType.GetField("content"));
            var sepGetter =
