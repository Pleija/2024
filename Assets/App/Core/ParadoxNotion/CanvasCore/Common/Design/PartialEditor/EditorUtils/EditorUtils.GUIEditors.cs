    ...
    partial class EditorUtils
    {
            ...
            params GUILayoutOption[] layoutOptions)
        {
            LayerMask tempMask = EditorGUILayout.MaskField(content,
                ...
                InternalEditorUtility.layers, layoutOptions);
            ...
            if (GUILayout.Button("Add Element")) {
                if (!typeof(UnityObject).IsAssignableFrom(keyType)) {
                    ...
                    if (dict.Contains(newKey)) {
                        Logger.LogWarning(
                            string.Format("Key '{0}' already exists in Dictionary", newKey),
                            "Editor");
                        return dict;
                    }
                    keys.Add(newKey);
            ...
            params GUILayoutOption[] GUIOptions)
        {
            ...
            var componentNames = components.Where(c => c != null)
                    .Select(c =>
                            c.GetType().FriendlyName() + " (" + c.gameObject.name + ")")
                    .ToList();
        ...
        public static T Popup<T>(T selected, IEnumerable<T> options,
            params GUILayoutOption[] GUIOptions) =>
                Popup(GUIContent.none, selected, options, GUIOptions);

        ///<summary>Generic Popup for selection of any element within a list</summary>
        public static T Popup<T>(string prefix, T selected, IEnumerable<T> options,
            params GUILayoutOption[] GUIOptions) => Popup(
            ...
            params GUILayoutOption[] GUIOptions)
        {
            ...
            var stringedOptions = new List<string>(
                listOptions.Select(o => o != null ? o.ToString() : "[NONE]"));
        ...
        public static void ButtonPopup<T>(string prefix, T selected, List<T> options,
            Action<T> Callback)
        {
            ButtonPopup(string.IsNullOrEmpty(prefix) ? GUIContent.none : new GUIContent(prefix),
                selected,
                options, Callback);
        }
            ...
            Action<T> Callback)
        {
            ...
            if (GUILayout.Button(buttonText, "MiniPopup", GUILayout.Width(0),
