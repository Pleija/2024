...
namespace ParadoxNotion.Design
{
    ...
    public class DimIfDefaultDrawer : AttributeDrawer<DimIfDefaultAttribute>
    {
        ...
        {
            var dim = false;
            if (fieldInfo.FieldType.IsClass) dim = instance == null;
            if (fieldInfo.FieldType == typeof(bool)) dim = (bool)instance == false;
            if (fieldInfo.FieldType == typeof(int)) dim = (int)instance == 0;
            if (fieldInfo.FieldType == typeof(float)) dim = (float)instance == 0;
            if (fieldInfo.FieldType == typeof(string)) dim = string.IsNullOrEmpty((string)instance);
        ...
        public override object OnGUI(GUIContent content, object instance)
        {
            ...
            if (member != null) {
                ...
                if (memberType == null || !memberType.IsValueType)
                    ...
                    intValue = (int)Convert.ChangeType(memberValue, typeof(int));
    ...
    public class RequiredFieldDrawer : AttributeDrawer<RequiredFieldAttribute>
    {
        public override object OnGUI(GUIContent content, object instance)
        {
            var isNull = instance == null
                    || instance.Equals(null)
                    || (instance is string && string.IsNullOrEmpty((string)instance));
            instance = MoveNextDrawer();
        ...
        public override object OnGUI(GUIContent content, object instance)
        {
            if (attribute.options != null && attribute.options.Length > 0) {
                ...
                if (fieldInfo.FieldType.IsAssignableFrom(optType))
                    return EditorUtils.Popup(content, instance, attribute.options);
            }
            return MoveNextDrawer();
        }
    }
    ...
    public class SliderFieldDrawer : AttributeDrawer<SliderFieldAttribute>
    {
        ...
        {
            if (fieldInfo.FieldType == typeof(float))
                return EditorGUILayout.Slider(content, (float)instance, attribute.min,
                    attribute.max);
            if (fieldInfo.FieldType == typeof(int))
    ...
    /// <summary>
    ///     Can be used on an interface to popup select a concrete implementation.
    ///     <summary>
    public class ReferenceFieldDrawer : AttributeDrawer<ReferenceFieldAttribute>
    {
        public override object OnGUI(GUIContent content, object instance)
        {
            ...
            var selection = EditorUtils.Popup(content,
            ...
            if (instance == null || instance.GetType() != selection)
                if (!typeof(Object).IsAssignableFrom(selection))
                    return Activator.CreateInstance(selection);
