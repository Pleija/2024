...
namespace NodeCanvas.Editor
{
    public class TaskEditor : EditorObjectWrapper<Task>
    {
        ...
        public static void TaskFieldMulti<T>(T task, ITaskSystem ownerSystem, Action<T> callback)
                where T : Task
        {
            TaskFieldMulti(task, ownerSystem, typeof(T), t => {
                callback((T)t);
            });
        }
        ...
        {
            ShowCreateTaskSelectionButton(ownerSystem, typeof(T), t => {
                callback((T)t);
            });
        }
        ...
        private void ShowInspector(Action<Task> callback, bool showTitlebar = true)
        {
            if (task.ownerSystem == null) {
                GUILayout.Label(
                    "<b>Owner System is null! This should really not happen but it did!\nPlease report a bug. Thank you :)</b>");
                return;
            }
            ...
            if (!showTitlebar || ShowTitlebar(callback)) {
        ...
        private bool ShowTitlebar(Action<Task> callback)
        {
            ...
            GUILayout.BeginHorizontal(GUI.skin.box);
            {
                ...
                if (taskType.IsGenericType) {
                    ...
                    if (isBase
                        && GUILayout.Button(StyleSheet.genericType, GUI.skin.label,
                            GUILayout.Width(20),
                            GUILayout.Height(20))) {
        ...
        private GenericMenu GetMenu(Action<Task> callback)
        {
            ...
            menu.AddItem(new GUIContent("Open Script"), false, () => {
                EditorUtils.OpenScriptOfType(taskType);
            });
            menu.AddItem(new GUIContent("Copy"), false, () => {
                CopyBuffer.SetCache(task);
            });
        ...
        {
            if (task.agentType == null) return;
            var agentParam = agentParameterProp.value;
            ...
            if (task.agentIsOverride) {
                ...
                var compInfo = task.agent == null ? task.agentType.FriendlyName().FormatError()
                        : task.agentType.FriendlyName();
