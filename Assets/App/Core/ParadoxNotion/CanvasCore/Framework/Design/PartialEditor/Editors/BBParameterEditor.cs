...
namespace NodeCanvas.Editor
{
    public static class BBParameterEditor
    {
            ...
            bool blackboardOnly = false, bool required = false, InspectedFieldInfo info = default)
        {
            ...
            if (blackboardOnly && !bbParam.useBlackboard) bbParam.useBlackboard = true;
                    ...
                    if (bbParam.useBlackboard) {
                        ...
                        if (info.attributes != null)
                            info.attributes = info.attributes.Where(a =>
                                            !(a is DrawerAttribute)
                                            || !((DrawerAttribute)a).isDecorator)
                                    .ToArray();
                        GUILayout.BeginVertical();
                    ...
                    if (bbParam.varRef == null && !string.IsNullOrEmpty(bbParam.name)) {
                        if (!bbParam.isPresumedDynamic)
                            EditorUtils.MarkLastFieldError(
                                $"Missing Variable. Type of '{bbParam.varType.FriendlyName()}'");
                        if (bbParam.isPresumedDynamic)
                        ...
                        if (required
                            && !string.IsNullOrEmpty(bbParam.name)
                            && !bbParam.isDefined
                            && bbParam.isNull)
                            EditorUtils.MarkLastFieldWarning(
            ...
            if (pop) {
                ...
                if (bbParam.bb != null)
                    foreach (var actualBB in bbParam.bb.GetAllParents(true).Reverse()) {
                        var variables =
                                actualBB.variables.Values.Where(v =>
                                        v.CanConvertTo(bbParam.varType));
                        ...
                        menu.AddItem(new GUIContent(actualBB.identifier + "/(Create New #2)"),
                            false, () => {
                                UndoUtility.RecordObject(actualBB.unityContextObject,
                                    "New Variable");
                                if (actualBB.AddVariable(cachedContentTextForNew, bbParam.varType)
                                    != null)
