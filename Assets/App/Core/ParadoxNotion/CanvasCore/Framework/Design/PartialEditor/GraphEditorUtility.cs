    ...
    public static class GraphEditorUtility
    {
        ...
        public static IGraphElement activeElement {
            get {
                ...
                _activeElement.TryGetTarget(out var reference);
                return reference;
            }
            set {
                GUIUtility.keyboardControl = 0;
                if (!activeElements.Contains(value) || value == null) _activeElements.Clear();
                _activeElement.TryGetTarget(out var reference);
                ...
                if (reference != value && activeElements.Count == 0) {
                    ...
                    lastNodeID = -1;
                    lastConnectionID = -1;
                    if (value is Node) lastNodeID = (value as Node).ID;
        ...
        public static List<IGraphElement> activeElements {
            get => _activeElements.ToReferenceList();
            set {
                GUIUtility.keyboardControl = 0;
        ...
        public static IEnumerable<Type> GetDropedReferenceNodeTypes<T>(Object obj)
                where T : IGraphElement
        {
            ...
            foreach (var type in ReflectionTools.GetImplementationsOf(typeof(T))) {
                ...
                if (att != null && att.type == targetType) yield return type;
            }
        }
        ...
        {
            if (activeNode != null) return new[] { activeNode };
        ...
        public static void ScanForStructTypesAndAppendThem(Graph graph)
        {
            ...
            serializedTypes = serializedTypes.Concat(serializedTypes.Where(t => t.IsGenericType)
                            .Select(t => t.RTGetGenericArguments().First()))
                    .ToList();
            ...
            var userInfo = string.Join("\n",
                resultTypes.OrderBy(t => t.Namespace)
                        .ThenBy(t => t.Name)
