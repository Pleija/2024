...
namespace NodeCanvas.Framework
{
    ...
    {
        //Verbose level
        public enum VerboseLevel { Compact = 0, Partial = 1, Full = 2 }
        private const string DEFAULT_HEX_COLOR_LIGHT = "eed9a7";
        private const string DEFAULT_HEX_COLOR_DARK = "333333";

        ///----------------------------------------------------------------------------------------------
        private static readonly Vector2 MIN_SIZE = new Vector2(80, 20);
        ...
        public bool isSelected => GraphEditorUtility.activeElement == this
                || GraphEditorUtility.activeElements.Contains(this);
        ...
        public Color nodeColor {
            get {
                if (!colorLoaded) {
                    ...
                    var cAtt = GetType().RTGetAttribute<ColorAttribute>(true);
                ...
                if (!hasColorAttribute && customColor != default) return customColor;
...
";
                ...
                if (bindComponent && !extends.GetText().Contains(bindComponent.GetType().Name)) {
                    ...
                    if (root.OfKind(SyntaxKind.ImportEqualsDeclaration)
                        .All(x =>
        ...
        private static void ShowHeader(Node node)
        {
            ...
            //icon
            if (node.hasIcon
                && (node.iconAlignment == Alignment2x2.Default
        ...
        private static void HandleEvents(Node node, Event e)
        {
            ...
            if (e.type == EventType.MouseDown && GraphEditorUtility.allowClick && e.button != 2) {
                ...
                if (!e.control) GraphEditorUtility.activeElement = node;
            ...
            if (e.type == EventType.MouseUp) {
                if (node.nodeIsPressed) node.TrySortConnectionsByRelativePosition();
        ...
        private static GenericMenu GetNodeMenu_Multi(Graph graph)
        {
            ...
            menu.AddItem(new GUIContent("Duplicate Selected Nodes"), false, () => {
                var newNodes = Graph.CloneNodes(
                    GraphEditorUtility.activeElements.OfType<Node>().ToList(), graph);
        ...
        public static GenericMenu GetNodeMenu_Single(Node node)
        {
            ...
            if (node is ITaskAssignable) {
                ...
                if (assignable.task != null)
                    menu.AddItem(new GUIContent("Copy Assigned Task"), false, () => {
                        CopyBuffer.SetCache(assignable.task);
                    });
                else
                ...
                if (CopyBuffer.TryGetCache<Task>(out var copy))
                    menu.AddItem(new GUIContent("Paste Assigned Task"), false, () => {
                        ...
                        try {
                            ...
                            Logger.LogWarning("Can't paste Task here. Incombatible Types",
                                LogTag.EDITOR, node);
                        }
                    });
                else
            ...
            if (menu != null) {
                ...
                foreach (var _m in node.GetType().RTGetMethods()) {
                    ...
                    if (att != null)
                        menu.AddItem(new GUIContent(att.menuItem), false, () => {
                            m.Invoke(node, null);
                        });
                }
                menu.AddSeparator("/");
        ...
        {
            if (!Prefs.showComments || string.IsNullOrEmpty(node.comments)) return;
            ...
            if (node.commentsAlignment == Alignment2x2.Right)
                commentsRect = new Rect(node.rect.xMax + 5, node.rect.yMin,
                    Mathf.Min(size.x, node.rect.width * 2),
                    node.rect.height);
        ...
        private static void DrawBreakpoint(Node node)
        {
        ...
        private static void RecursivePanNode(Node node, Vector2 delta)
        {
            ...
            for (var i = 0; i < node.outConnections.Count; i++) {
                ...
                if (child.ID > node.ID) RecursivePanNode(child, delta);
            }
        }
        ...
        public static void ShowNodeInspectorGUI(Node node)
        {
            ...
            if (GUILayout.Button("Edit...", style, GUILayout.Width(40))) {
                ...
                InternalEditorUtility.OpenFileAtLineExternal(file, 1);
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                EditorUtils.CommentLastTextField(node.comments);
                GUILayout.EndHorizontal();
            }
            GUI.color = Color.white;
            ...
            if (node is IGraphAssignable) {
                ...
                if (assignable.subGraphParameter != null)
                    if (ReferenceEquals(assignable.subGraphParameter.value, assignable.graph)) {
                        Logger.LogWarning("SubGraph can't be itself.", LogTag.EDITOR);
        ...
        private void SetActive(bool active)
        {
            ...
            if (!active) GraphEditorUtility.activeElement = null;
            UndoUtility.RecordObject(graph, "SetActive");

            //disable all incomming
            foreach (var cIn in inConnections) cIn.isActive = active;

            //disable all outgoing
            foreach (var cOut in outConnections) cOut.isActive = active;
            ...
            foreach (var child in outConnections.Select(c => c.targetNode)) {
                if (child.isActive == !active) continue;
                ...
                foreach (var connection in inConnections.ToArray()) {
                    ...
                    if (graph.flowDirection == PlanarDirection.Horizontal)
                        node.outConnections = node.outConnections
                                .OrderBy(c => c.targetNode.rect.center.y)
                                .ToList();
                    if (graph.flowDirection == PlanarDirection.Vertical)
                        node.outConnections = node.outConnections
                                .OrderBy(c => c.targetNode.rect.center.x)
                                .ToList();
            ...
            Vector2 canvasMousePos, float zoomFactor)
        {
            ...
            if (clickedPort != null && e.type == EventType.MouseUp && e.button == 0) {
                if (rect.Contains(e.mousePosition)) {
                    ...
                    if (dragDropMisses == graph.allNodes.Count && clickedPort != null) {
                        ...
                        clickedPort = null;
                        Action<Type> Selected = delegate(Type type) {
            ...
            for (var i = 0; i < outConnections.Count; i++) {
                ...
                if (connection == null || connection.targetNode is MissingNode) continue;
                var sourcePos = rect.center;
                var targetPos = rect.center;
                ...
                if (fullDrawPass || drawCanvas.Overlaps(boundRect)) {
                    if (collapsed || connection.targetNode.isHidden) continue;
        ...
        protected virtual GenericMenu OnContextMenu(GenericMenu menu) => menu;

        ///<summary>Get connection information node wise, to show on top of the connection</summary>
        public virtual string GetConnectionInfo(int index) => null;
