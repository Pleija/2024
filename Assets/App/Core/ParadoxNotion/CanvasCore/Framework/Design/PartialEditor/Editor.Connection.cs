...
namespace NodeCanvas.Framework
{
    ...
    {
        public enum RelinkState { None, Source, Target }
        public enum TipConnectionStyle { None, Circle, Arrow }
        ...
        public void DrawConnectionGUI(Vector2 fromPos, Vector2 toPos)
        {
            ...
            centerRect.center = CurveUtils.GetPosAlongCurve(
                fromPos, toPos, fromTangent, toTangent, 0.55f);
        ...
        private void DrawConnection(Vector2 fromPos, Vector2 toPos)
        {
            ...
            if (isRelinkingActive) {
                ...
                CurveUtils.ResolveTangents(fromPos, toPos, Prefs.connectionsMLT,
                    graph.flowDirection, out fromTangent,
                    out toTangent);
                size = defaultSize;
            }
            ...
            if (assignable != null) {
                if (assignable.task != null)
                    menu.AddItem(new GUIContent("Copy Assigned Condition"), false, () => {
                        CopyBuffer.Set(assignable.task);
                    });
                else
