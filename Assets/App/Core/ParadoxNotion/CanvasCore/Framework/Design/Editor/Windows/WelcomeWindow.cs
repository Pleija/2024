    ...
    public class WelcomeWindow : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            if (assetType == null)
                assetType = ReflectionTools.GetImplementationsOf(typeof(Graph))
                        .Where(x => x.IsDefined(typeof(GraphInfoAttribute), true))
                        .LastOrDefault();
            att = assetType != null
                    ? assetType.RTGetAttributesRecursive<GraphInfoAttribute>().LastOrDefault()
                    : null;
        ...
        private void OnGUI()
        {
            ...
            GUI.Label(new Rect(headerRect.x + 333, headerRect.yMax - 56, 150, 20),
                $"<color=#d2d2d2><size=10><b>v{GraphSource.FRAMEWORK_VERSION}</b></size></color>");
            ///----------------------------------------------------------------------------------------------
            ...
            Prefs.hideWelcomeWindow = EditorGUILayout.ToggleLeft(
                "Don't show again.", Prefs.hideWelcomeWindow);
                    ...
                    foreach (var board in boards) {
                        ...
                        if (isAll || target.ToLower() == packageName.ToLower()) {
                            if (!isAll) {
                                ...
                                var uptodate = GraphSource.FRAMEWORK_VERSION
                                        == float.Parse(version);
                                result += uptodate
