...
namespace NodeCanvas.Editor
{
    ...
    partial class GraphEditor
    {
        ...
        private static AnimFloat inspectorPanelAnimFloat;
        private static WeakReference<IGraphElement> _inspectorPanelPreviousSelectionRef;

        private static IGraphElement inspectorPanelPreviousSelection {
            get {
                _inspectorPanelPreviousSelectionRef.TryGetTarget(out var element);
                return element;
            }
        ...
        {
            inspectorPanelAnimFloat = new AnimFloat(0);
            _inspectorPanelPreviousSelectionRef = new WeakReference<IGraphElement>(null);
        ...
        private static void ShowPanels(Graph graph, Vector2 canvasMousePos)
        {
            ...
            GraphEditorUtility.allowClick = !panel1.Contains(e.mousePosition)
                    && !panel2.Contains(e.mousePosition);
        }
        ...
        private static void ShowGraphCommentsGUI(Graph graph, Vector2 canvasMousePos)
        {
            if (Prefs.showComments && !string.IsNullOrEmpty(graph.comments)) {
                ...
                var calcHeight = StyleSheet.commentsBox.CalcHeight(
                    content, Prefs.inspectorPanelWidth);
                var rect = new Rect(canvasRect.xMin + 2, canvasRect.yMax - calcHeight - 2,
                    Prefs.inspectorPanelWidth, calcHeight);
        ...
        private static Rect ShowInspectorGUIPanel(Graph graph, Vector2 canvasMousePos)
        {
            ...
            if (Prefs.useExternalInspector) return inspectorPanel;
            ...
            var inspectedElement = Prefs.animatePanels ? inspectorPanelPreviousSelection
            ...
            var posY = PANELS_Y;
            var headerHeight = 30;
            inspectorPanel.x = Mathf.Lerp(-Prefs.inspectorPanelWidth - scrollWidth, posX,
                inspectorPanelAnimFloat.value);
            ...
            var displayName = !string.IsNullOrEmpty(inspectedElement.name)
                    ? inspectedElement.name.Replace("\n", "") : inspectedElement.name;
            ...
            var resizeRect = Rect.MinMaxRect(inspectorPanel.xMax - 2, inspectorPanel.yMin,
                inspectorPanel.xMax + 2,
                inspectorPanel.yMax);
            ...
            if (GUI.Button(popRect,
                EditorUtils.GetTempContent(Icons.minMaxIcon, "Open External Inspector"),
                GUIStyle.none))
                ExternalInspectorWindow.ShowWindow();
            GUI.color = Color.white;
            if (inspectedElement is Node)
                if (GUI.Button(gearRect,
                    EditorUtils.GetTempContent(Icons.gearPopupIcon, "Context Menu"),
                    GUIStyle.none))
            ...
            if (Prefs.showNodePanel) {
                var contentRect = Rect.MinMaxRect(2, headerHeight, inspectorPanel.width - 2,
                    inspectorPanel.height);
                ...
                var viewRect = Rect.MinMaxRect(0, headerHeight, inspectorPanel.width,
                    inspectorPanel.height);
                inspectorPanelNeedsScroller = position.height < viewRect.height;
                inspectorPanelScrollPos = GUI.BeginScrollView(position, inspectorPanelScrollPos,
                    viewRect, false, false);
                ...
                var contentRect = Rect.MinMaxRect(0, headerHeight, inspectorPanel.width,
                    inspectorPanel.height);
        ...
        private static Rect ShowBlackboardGUIPanel(Graph graph, Vector2 canvasMousePos)
        {
            ...
            var resizeRect = Rect.MinMaxRect(blackboardPanel.xMin - 2, blackboardPanel.yMin,
                blackboardPanel.xMin + 2,
                blackboardPanel.yMax);
            EditorGUIUtility.AddCursorRect(resizeRect, MouseCursor.ResizeHorizontal);

            if (e.type == EventType.MouseDown && resizeRect.Contains(e.mousePosition)) {
                isResizingBlackboardPanel = true;
                e.Use();
            }
            ...
            var headerRect = new Rect(blackboardPanel.x, blackboardPanel.y, blackboardPanel.width,
                30);
            EditorGUIUtility.AddCursorRect(headerRect, MouseCursor.Link);
            if (GUI.Button(headerRect, string.Empty, GUIStyle.none))
            ...
            if (Prefs.showBlackboard) {
                var contentRect = Rect.MinMaxRect(2, headerHeight, blackboardPanel.width - 2,
                    blackboardPanel.height);
                ...
                var viewRect = Rect.MinMaxRect(0, headerHeight, blackboardPanel.width,
                    blackboardPanel.height);
                blackboardPanelNeedsScroller = position.height < viewRect.height;
                blackboardPanelScrollPos = GUI.BeginScrollView(position, blackboardPanelScrollPos,
                    viewRect, false, false);
                GUILayout.BeginArea(contentRect);
                ...
                var contentRect = Rect.MinMaxRect(0, headerHeight, blackboardPanel.width,
                    blackboardPanel.height);
                EditorGUIUtility.AddCursorRect(contentRect, MouseCursor.Link);
                if (GUI.Button(contentRect, "<b>...</b>", Styles.centerLabel))
            ...
            if (graph.canAcceptVariableDrops
                ...
                if (e.type == EventType.MouseUp && !blackboardPanel.Contains(e.mousePosition)) {
                    graph.CallbackOnVariableDropInGraph(BlackboardEditor.pickedVariableBlackboard,
                        BlackboardEditor.pickedVariable,
                        canvasMousePos);
