...
namespace NodeCanvas.Editor
{
    ...
    public partial class GraphEditor
    {
        ...
        public static void ShowToolbar(Graph graph)
        {
            ...
            if (GUILayout.Button(string.Format("{0} @ {1} v{2}", graph.GetType().Name,
                graphInfoAtt != null ? graphInfoAtt.packageName : "NodeCanvas",
                GraphSource.FRAMEWORK_VERSION.ToString("0.00")), EditorStyles.toolbarButton))
            ...
            Prefs.isEditorLocked = GUILayout.Toggle(
                Prefs.isEditorLocked, "Lock", EditorStyles.toolbarButton);
        ...
        private static GenericMenu GetToolbarMenu_File(Graph graph, GraphOwner owner)
        {
            ...
            menu.AddItem(new GUIContent("Import JSON"), false, () => {
                if (graph.allNodes.Count > 0
                    && !EditorUtility.DisplayDialog("Import Graph",
                ...
                if (!string.IsNullOrEmpty(path))
                    if (graph.Deserialize(File.ReadAllText(path), null, true)
                        == false) //true: validate, null: graph._objectReferences
                        EditorUtility.DisplayDialog("Import Failure",
                            "Please read the logs for more information", "OK",
                            string.Empty);
            });
            menu.AddItem(new GUIContent("Export JSON"), false, () => {
                ...
                if (!string.IsNullOrEmpty(path)) {
                    ...
                    json = JSONSerializer.PrettifyJson(json);
            ...
            menu.AddItem(new GUIContent("Export JSON (Include SubGraphs)"), false, () => {
                foreach (var subgraph in graph.GetAllNestedGraphs<Graph>(true).Prepend(graph)) {
                    var subpath = EditorUtility.SaveFilePanelInProject(
                        string.Format("Export '{0}' Graph", subgraph.GetType().Name),
                        subgraph.name, subgraph.GetGraphJSONFileExtension(), string.Empty);

                    if (!string.IsNullOrEmpty(subpath)) {
                        var subjson = subgraph.Serialize(null);
                        subjson = JSONSerializer.PrettifyJson(subjson);
            ...
            menu.AddItem(new GUIContent("Show JSON"), false, () => {
                graph.SelfSerialize();
                JSONSerializer.ShowData(graph.GetSerializedJsonData(), graph.name);
            });
            return menu;
        }
        ...
        {
            var menu = new GenericMenu();
            menu.AddItem(new GUIContent("Show Comments"), Prefs.showComments, () => {
                Prefs.showComments = !Prefs.showComments;
            });
            menu.AddItem(new GUIContent("Show Summary Info"), Prefs.showTaskSummary, () => {
                Prefs.showTaskSummary = !Prefs.showTaskSummary;
            });
            ...
            menu.AddItem(new GUIContent("Grid Snap"), Prefs.snapToGrid, () => {
                Prefs.snapToGrid = !Prefs.snapToGrid;
            });
            menu.AddItem(new GUIContent("Log Events Info"), Prefs.logEventsInfo, () => {
                Prefs.logEventsInfo = !Prefs.logEventsInfo;
            });
            ...
            menu.AddItem(new GUIContent("Connection Style/Soft"), false, () => {
                Prefs.connectionsMLT = 0.75f;
            });
            menu.AddItem(new GUIContent("Connection Style/Softer"), false, () => {
                Prefs.connectionsMLT = 0.5f;
            });
            menu.AddItem(new GUIContent("Connection Style/Direct"), false, () => {
