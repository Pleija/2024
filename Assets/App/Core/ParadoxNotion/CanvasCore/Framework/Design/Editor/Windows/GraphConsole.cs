    ...
    public class GraphConsole : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            Prefs.consoleLogInfo = GUILayout.Toggle(Prefs.consoleLogInfo,
                new GUIContent(Icons.infoIcon), logTypeFilterStyle,
                GUILayout.Width(30));
            GUILayout.Space(2);
            Prefs.consoleLogWarning = GUILayout.Toggle(Prefs.consoleLogWarning,
                new GUIContent(Icons.warningIcon), logTypeFilterStyle,
                GUILayout.Width(30));
            GUILayout.Space(2);
            Prefs.consoleLogError = GUILayout.Toggle(Prefs.consoleLogError,
                new GUIContent(Icons.errorIcon), logTypeFilterStyle,
                GUILayout.Width(30));
            GUILayout.FlexibleSpace();
            Prefs.consoleClearOnPlay = GUILayout.Toggle(Prefs.consoleClearOnPlay,
                new GUIContent("Clear On Play"), logTypeFilterStyle,
                GUILayout.Width(100));
        ...
        {
            if (msg.tag == LogTag.EDITOR) return false;

            if (Threader.isMainThread) {
                if (!Prefs.logEventsInfo && msg.tag == LogTag.EVENT) return true;
                if (!Prefs.logVariablesInfo && msg.tag == LogTag.VARIABLE) return true;
            }
            var graph = Graph.GetElementGraph(msg.context);
            if (graph == null) return false;
            if (!graphsMap.ContainsKey(graph)) graphsMap[graph] = new List<Logger.Message>();
        ...
        public static Logger.Message GetLastMessageForGraph(Graph graph)
        {
            ...
            if (graphsMap.TryGetValue(graph, out list)) return list.LastOrDefault();
            return default;
        }
