...
namespace NodeCanvas.Framework
{
    ...
    public class Variable<T> : Variable
    {
        ...
        public override void BindProperty(MemberInfo prop, GameObject target = null)
        {
            if (prop is PropertyInfo || prop is FieldInfo) {
                _propertyPath = string.Format("{0}.{1}", prop.RTReflectedOrDeclaredType().FullName,
                    prop.Name);
                if (target != null) InitializePropertyBinding(target);
            }
        }
        ...
        {
            if (!isPropertyBound || !Threader.applicationIsPlaying) return;
            getter = null;
            setter = null;
            ...
            if (type == null) {
                Logger.LogError(string.Format(
                    "Type '{0}' not found for Blackboard Variable '{1}' Binding.", typeString,
                    name), LogTag.VARIABLE, go);
                return;
            }
