...
namespace NodeCanvas.Framework
{
    ...
    {
        public enum DisableAction { DisableBehaviour, PauseBehaviour, DoNothing }

        ///----------------------------------------------------------------------------------------------
        public enum EnableAction { EnableBehaviour, DoNothing }

        public enum FirstActivation { OnEnable, OnStart, Async }
        ...
        {
            var param =
                    exposedParameters.Find(x =>
                            x.varRefBoxed != null && x.varRefBoxed.name == name);
        ...
        public ExposedParameter MakeNewExposedParameter<T>(string name)
        {
            ...
            Logger.LogWarning(string.Format("Exposed Graph Variable named '{0}' was not found",
                name));
            return null;
        }
        ...
        public void Initialize()
        {
            ...
            if (firstActivation == FirstActivation.Async) {
                graph.LoadOverwriteAsync(loadData, () => {
                    ...
                    if (!isRunning
                        && enableAction == EnableAction.EnableBehaviour
        ...
        public void OnAfterGraphSerialized(Graph serializedGraph)
        {
            ...
            if (graphIsBound && boundGraphInstance == serializedGraph) {
                ...
                if (PrefabUtility.IsPartOfPrefabInstance(this)) {
                    var boundProp =
                            new SerializedObject(this).FindProperty(
                    ...
                    if (!boundProp.prefabOverride
                        ...
                        Logger.LogWarning("Prefab Bound Graph just got overridden!", LogTag.EDITOR,
                            this);
                    }
                }
                //---
                UndoUtility.RecordObject(this,
                    UndoUtility.GetLastOperationNameOr("Bound Graph Change"));
    ...
    public abstract class GraphOwner<T> : GraphOwner where T : Graph
    {
        ...
        public sealed override Graph graph {
            get {
                ...
                if (graphIsBound && !Application.isPlaying) return boundGraphInstance;
#endif
