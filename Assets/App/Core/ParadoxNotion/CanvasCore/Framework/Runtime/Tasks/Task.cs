...
namespace NodeCanvas.Framework
{
    ...
    public abstract class Task : ISerializationCollectable, ISerializationCallbackReceiver
    {
        ...
        public string summaryInfo {
            get {
#if UNITY_EDITOR
                if (!Prefs.showTaskSummary && !(this is ActionList) && !(this is ConditionList))
                ...
                if (this is ConditionTask)
                    return (agentIsOverride ? "* " : "")
                            + ((this as ConditionTask).invert ? "If <b>!</b> " : "If ")
                            + info;
                return info;
            }
        }
        ...
        {
            var newTask = JSONSerializer.Clone(this);
        ...
        private bool Initialize(Component newAgent)
        {
            ...
            if (_currentAgent == null && agentType != null)
                return Error("Failed to resolve Agent to requested type '"
                    + agentType
        ...
        private bool InitializeFieldAttributes(Component newAgent)
        {
            ...
            for (var i = 0; i < fields.Length; i++) {
                ...
                if (newAgent != null
                    && (typeof(Component).RTIsAssignableFrom(field.FieldType)
                    ...
                    if (typeof(BBParameter).RTIsAssignableFrom(field.FieldType)) {
                        ...
                        if (bbParam == null)
                            return string.Format("* BBParameter '{0}' is null",
                                field.Name.SplitCamelCase());
                        if (!bbParam.isDefined && bbParam.isNoneOrNull)
