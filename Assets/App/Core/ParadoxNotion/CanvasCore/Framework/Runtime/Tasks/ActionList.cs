...
namespace NodeCanvas.Framework
{
    ...
    {
        public enum ActionsExecutionMode { ActionsRunInSequence, ActionsRunInParallel }
        ...
        protected override void OnExecute()
        {
            ...
            for (var i = 0; i < actions.Count; i++) finishedIndeces[i] = false;
        }
            ...
            switch (executionMode) {
                ...
                case ActionsExecutionMode.ActionsRunInParallel: {
                    for (var i = 0; i < actions.Count; i++) {
                        ...
                        if (status == Status.Success) finishedIndeces[i] = true;
                    }
                    var finished = true;
                    for (var i = 0; i < actions.Count; i++) finished &= finishedIndeces[i];
                    if (finished) EndAction(true);
                }
                    break;
                ...
                case ActionsExecutionMode.ActionsRunInSequence: {
                    for (var i = currentActionIndex; i < actions.Count; i++) {
                        if (!actions[i].isUserEnabled) continue;
                        var status = actions[i].Execute(agent, blackboard);
        ...
        public void AddAction(ActionTask action)
        {
            if (action is ActionList) {
                foreach (var subAction in (action as ActionList).actions) AddAction(subAction);
                return;
            }
#if UNITY_EDITOR
        ...
        public void ShowListGUI()
        {
            ...
            EditorUtils.ReorderableList(actions, (i, picked) => {
                ...
                action.isUserEnabled = EditorGUILayout.Toggle(
                    action.isUserEnabled, GUILayout.Width(18));
                GUI.enabled = true;
                GUILayout.Label((action.isPaused ? "<b>||</b> " : action.isRunning ? "► " : "")
        ...
        {
            if (actions.Count == 1) currentViewAction = actions[0];
        ...
        private void DoLoadPreset()
        {
            ...
            if (!string.IsNullOrEmpty(path)) {
                ...
                var list = JSONSerializer.TryDeserializeOverwrite(this, json);
                actions = list.actions;
                executionMode = list.executionMode;
                currentViewAction = null;
                foreach (var a in actions) a.SetOwnerSystem(ownerSystem);
            }
        }
