...
namespace NodeCanvas.Framework
{
    ...
    {
        public enum ConditionsCheckMode { AllTrueRequired, AnyTrueSuffice }
        ...
        protected override bool OnCheck()
        {
            ...
            for (var i = 0; i < conditions.Count; i++) {
                ...
                if (conditions[i].Check(agent, blackboard)) {
                    if (!allTrueRequired) return true;
                    succeedChecks++;
                }
                else {
                    if (allTrueRequired) return false;
                }
            }
        ...
        public void ShowListGUI()
        {
            ...
            EditorUtils.ReorderableList(conditions, (i, picked) => {
                ...
                GUI.color = Color.white.WithAlpha(
                    condition == currentViewCondition ? 0.75f : 0.25f);
                GUILayout.BeginHorizontal("box");
                GUI.color = Color.white.WithAlpha(condition.isUserEnabled ? 0.8f : 0.25f);
                GUI.enabled = !Application.isPlaying;
                condition.isUserEnabled = EditorGUILayout.Toggle(
                    condition.isUserEnabled, GUILayout.Width(18));
                GUI.enabled = true;
        ...
        {
            if (conditions.Count == 1) currentViewCondition = conditions[0];
        ...
        private void DoLoadPreset()
        {
            ...
            if (!string.IsNullOrEmpty(path)) {
                ...
                foreach (var a in conditions) a.SetOwnerSystem(ownerSystem);
            }
        }
