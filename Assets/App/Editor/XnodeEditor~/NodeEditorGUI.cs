...
namespace XNodeEditor
{
    ...
    public partial class NodeEditorWindow
    {
        ...
        {
#if ODIN_INSPECTOR
            if (!OdinInspectorHelper.IsReady) return;
#endif
            var e = Event.current;
            var m = GUI.matrix;
        ...
        protected void UpdateOrderedNodeIndices()
        {
            ...
            while (orderedNodeIndices.Count > graph.nodes.Count) {
                var removeIndex = orderedNodeIndices.IndexOf(orderedNodeIndices.Count - 1);
        ...
        {
            GUIUtility.ScaleAroundPivot(Vector2.one * zoom, rect.size * 0.5f);
            var offset = new Vector3((rect.width * zoom - rect.width) * 0.5f,
                (rect.height * zoom - rect.height) * 0.5f + -topPadding * zoom + topPadding, 0);
        ...
        public void DrawSelectionBox()
        {
            if (currentActivity == NodeActivity.DragGrid) {
                ...
                r.size /= zoom;
                Handles.DrawSolidRectangleWithOutline(r, new Color(0, 0, 0, 0.1f),
        ...
        private static Vector2 CalculateBezierPoint(Vector2 p0, Vector2 p1, Vector2 p2, Vector2 p3,
            float t)
        {
            var u = 1 - t;
            float tt = t * t, uu = u * u;
            float uuu = uu * u, ttt = tt * t;
            return new Vector2(uuu * p0.x + 3 * uu * t * p1.x + 3 * u * tt * p2.x + ttt * p3.x,
        ...
        public void DrawNoodle(Gradient gradient, NoodlePath path, NoodleStroke stroke,
            float thickness, List<Vector2> gridPoints)
        {
                ...
                case NoodlePath.Curvy:
                    ...
                    for (var i = 0; i < length - 1; i++) {
                        ...
                        if (i < length - 2) {
                            ...
                            var p = (ab + cb) * 0.5f;
                            var tangentLength = (dist_ab + Vector2.Distance(point_b, point_c))
                                    * 0.005f
                                    * zoom;
                            var side = ac.x * (point_b.y - point_a.y)
                                    - ac.y * (point_b.x - point_a.x);
                        ...
                        for (var j = 1; j <= division; ++j) {
                            if (stroke == NoodleStroke.Dashed) {
                                ...
                                if (draw < 0) continue;
                                if (draw == 0)
                                    bezierPrevious = CalculateBezierPoint(point_a, tangent_a,
                                        tangent_b, point_b, (j - 1f) / division);
                            }
                            if (i == length - 2)
                                Handles.color = gradient.Evaluate((j + 1f) / division);
                            var bezierNext = CalculateBezierPoint(point_a, tangent_a, tangent_b,
                                point_b, j / (float)division);
            ...
            foreach (var node in graph.nodes) {
                ...
                foreach (var output in node.Outputs) {
                    ...
                    for (var k = 0; k < output.ConnectionCount; k++) {
                        ...
                        var selected = selectionCache != null
                                && (selectionCache.Contains(node)
                                    || IsHighlighted(node)
                                    || (input != null
                                        && (selectionCache.Contains(input.node)
                                            || IsHighlighted(input.node))));
                        var noodleGradient = graphEditor.GetNoodleGradient(selected, output, input);
                        var noodleThickness =
                                graphEditor.GetNoodleThickness(selected, output, input);
                        ...
                        DrawNoodle(noodleGradient, noodlePath, noodleStroke, noodleThickness,
                            gridPoints);
        ...
        private void DrawNodes()
        {
            ...
            var preSelection = preBoxSelection != null ? new List<Object>(preBoxSelection)
                    : new List<Object>();
            ...
            foreach (var n in orderedNodeIndices) {
                ...
                if (foldable || node.folded) {
                    ...
                    var folded = !EditorGUI.Foldout(
                        new Rect(nodePos + new Vector2(-12, 10), new Vector2(18, 18)), !node.folded,
                        GUIContent.none, NodeFoldoutStyle);
                ...
                var selected = (selectionCache != null && selectionCache.Contains(graph.nodes[n]))
                        || IsHighlighted(graph.nodes[n]);
        ...
        {
            if (!NodeEditorPreferences.GetSettings().portTooltips || graphEditor == null) return;
            string tooltip = null;
            if (hoveredPort != null)
