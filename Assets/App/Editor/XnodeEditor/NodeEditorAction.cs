...
namespace XNodeEditor
{
    public partial class NodeEditorWindow
    {
            ...
            switch (e.type) {
                ...
                case EventType.ScrollWheel:
                    ...
                    if (NodeEditorPreferences.GetSettings().zoomToMouse)
                        panOffset += (1 - oldZoom / zoom)
                                * (WindowToGridPosition(e.mousePosition) + panOffset);
                    break;
                case EventType.MouseDrag:
                    if (e.button == 0) {
                        if (IsDraggingPort) {
                            ...
                            if (IsHoveringPort
                                && hoveredPort.IsInput
                                && !draggedOutput.IsConnectedTo(hoveredPort))
                        ...
                        if (IsHoveringPort) {
                            if (hoveredPort.IsOutput) {
                                ...
                                if (hoveredPort.IsConnected) {
                                    ...
                                    var outputConnectionIndex =
                                            output.GetConnectionIndex(hoveredPort);
                                    draggedOutputReroutes =
                                            output.GetReroutePoints(outputConnectionIndex);
                    ...
                    if (e.button == 0) {
                        ...
                        if (IsDraggingPort) {
                            ...
                            if (draggedOutputTarget != null
                                ...
                                var connectionIndex =
                                        draggedOutput.GetConnectionIndex(draggedOutputTarget);

                                if (connectionIndex != -1) {
                                    draggedOutput.GetReroutePoints(connectionIndex)
                                            .AddRange(draggedOutputReroutes);
                                    if (NodeEditor.onUpdateNode != null)
                                        NodeEditor.onUpdateNode(node);
                        ...
                        if (currentActivity == NodeActivity.HoldNode && !(e.control || e.shift)) {
                            ...
                            if (isDoubleClick) {
                                var nodeDimension = nodeSizes.ContainsKey(hoveredNode)
                                        ? nodeSizes[hoveredNode] / 2 : Vector2.zero;
        ...
        private void RecalculateDragOffsets(Event current)
        {
            ...
            // Selected reroutes
            for (var i = 0; i < selectedReroutes.Count; i++)
                dragOffset[Selection.objects.Length + i] = selectedReroutes[i].GetPoint()
                        - WindowToGridPosition(current.mousePosition);
        }

        /// <summary>
        ///     Puts all selected nodes in focus. If no nodes are present, resets view and zoom to to
        ///     origin
        /// </summary>
        public void Home()
        {
            ...
            if (nodes.Count > 0) {
                ...
                var maxPos = nodes
                        .Select(x => x.position
                                + (nodeSizes.ContainsKey(x) ? nodeSizes[x] : Vector2.zero))
        ...
        public void DuplicateSelectedNodes()
        {
            ...
            var selectedNodes = Selection.objects.Select(x => x as Node)
                    .Where(x => x != null && x.graph == graph)
                    .ToArray();
        ...
        {
            copyBuffer = Selection.objects.Select(x => x as Node)
                    .Where(x => x != null && x.graph == graph)
                    .ToArray();
        }
            ...
            for (var i = 0; i < nodes.Length; i++) {
                ...
                foreach (var port in srcNode.Ports)
                    for (var c = 0; c < port.ConnectionCount; c++) {
                        var inputPort = port.direction == NodePort.IO.Input ? port
                                : port.GetConnection(c);
                        var outputPort = port.direction == NodePort.IO.Output ? port
                                : port.GetConnection(c);
        ...
        {
            if (IsDraggingPort) {
                var gradient = graphEditor.GetNoodleGradient(true, draggedOutput, null);
                var thickness = graphEditor.GetNoodleThickness(true, draggedOutput, null);
                ...
                var portStyle = current.graphEditor.GetPortStyle(draggedOutput);
                ...
                for (var i = 0; i < draggedOutputReroutes.Count; i++) {
                    ...
                    NodeEditorGUILayout.DrawPortHandle(rect, bgcol, frcol,
                        portStyle.normal.background, portStyle.active.background);
                }
            }
        }
        ...
        public void AutoConnect(Node node)
        {
            ...
            var inputPort = node.Ports.FirstOrDefault(x =>
                    x.IsInput && graphEditor.CanConnect(autoConnectOutput, x));
