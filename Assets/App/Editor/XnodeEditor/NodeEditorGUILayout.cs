...
namespace XNodeEditor
{
    ...
    public static class NodeEditorGUILayout
    {
        ...
        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, GUIContent label,
            NodePort port, bool includeChildren = true, params GUILayoutOption[] options)
        {
            if (property == null) throw new NullReferenceException();
            ...
            if (port == null) {
                var node = property.serializedObject.targetObject as Node;
                var hiddenByFold = node.folded;
                DontFoldAttribute dontFoldAttribute;
                if (NodeEditorUtilities.GetCachedAttrib(node.GetType(), property.name,
                    out dontFoldAttribute))
                    hiddenByFold = false;
                if (!hiddenByFold)
                ...
                var rect = new Rect();
                var hiddenByFold = port.node.folded && !port.IsConnected;
                DontFoldAttribute dontFoldAttribute;
                if (NodeEditorUtilities.GetCachedAttrib(port.node.GetType(), property.name,
                    out dontFoldAttribute))
                    hiddenByFold = false;
                var propertyAttributes =
                        NodeEditorUtilities.GetCachedPropertyAttribs(port.node.GetType(),
                            property.name);
                ...
                if (port.direction == NodePort.IO.Input) {
                    ...
                    foreach (var attr in propertyAttributes)
                        if (attr is SpaceAttribute) {
                            ...
                            if (usePropertyAttributes) {
                                ...
                                GUI.Label(position, (attr as HeaderAttribute).header,
                    ...
                    if (dynamicPortList) {
                        ...
                        var connectionType = inputAttribute != null ? inputAttribute.connectionType
                                : Node.ConnectionType.Multiple;
                        DynamicPortList(hiddenByFold, property.name, type,
                            property.serializedObject,
                            port.direction, connectionType);
                        return;
                    }
                    ...
                    switch (showBacking) {
                        case Node.ShowBackingValue.Unconnected:
                            ...
                            if (port.IsConnected)
                                EditorGUILayout.LabelField(label != null ? label
                    ...
                    float paddingLeft = NodeEditorWindow.current.graphEditor.GetPortStyle(port)
                            .padding.left;
                    ...
                    if (dynamicPortList) {
                        ...
                        var connectionType = outputAttribute != null
                                ? outputAttribute.connectionType : Node.ConnectionType.Multiple;
                        DynamicPortList(hiddenByFold, property.name, type,
                            property.serializedObject,
                            port.direction, connectionType);
                        return;
                    ...
                    switch (showBacking) {
                        case Node.ShowBackingValue.Unconnected:
                            ...
                            if (port.IsConnected)
                                EditorGUILayout.LabelField(
                                    label != null ? label : new GUIContent(
                                        property.displayName, tooltip),
                        ...
                        case Node.ShowBackingValue.Never:
                            ...
                            EditorGUILayout.LabelField(
                                label != null ? label : new GUIContent(
                                    property.displayName, tooltip), NodeEditorResources.OutputPort,
                                GUILayout.MinWidth(30));
                            break;
                        case Node.ShowBackingValue.Always:
                            // Display an editable property field
                    ...
                    rect = GUILayoutUtility.GetLastRect();
                    rect.width += NodeEditorWindow.current.graphEditor.GetPortStyle(port)
                            .padding
                            .right;
                ...
                var backgroundColor =
                        NodeEditorWindow.current.graphEditor.GetPortBackgroundColor(port);
                ...
                DrawPortHandle(rect, backgroundColor, col, portStyle.normal.background,
            ...
            params GUILayoutOption[] options)
        {
            ...
            var content = label != null ? label
                    : new GUIContent(ObjectNames.NicifyVariableName(port.fieldName));
        ...
        public static void DrawPortHandle(Rect rect, Color backgroundColor, Color typeColor,
            Texture2D border, Texture2D dot)
        {
            var col = GUI.color;
        ...
        ///     Called on the list on creation. Use this if you want to customize the
        ///     created ReorderableList
        /// </param>
        public static void DynamicPortList(bool hiddenByFold, string fieldName, Type type,
            SerializedObject serializedObject, NodePort.IO io,
            ...
            Action<ReorderableList> onCreation = null)
        {
            ...
            if (hiddenByFold && dynamicPorts.All(x => !x.IsConnected)) return;
            ...
            if (list == null) {
                ...
                list = CreateReorderableList(fieldName, dynamicPorts, arrayData, type,
                    serializedObject, io, connectionType,
                    typeConstraint, onCreation);
                if (reorderableListCache.TryGetValue(serializedObject.targetObject, out rlc))
        ...
        private static ReorderableList CreateReorderableList(string fieldName,
            List<NodePort> dynamicPorts, SerializedProperty arrayData, Type type,
            SerializedObject serializedObject, NodePort.IO io, Node.ConnectionType connectionType,
            Node.TypeConstraint typeConstraint, Action<ReorderableList> onCreation)
        {
            ...
            var label = arrayData != null ? arrayData.displayName
            ...
            list.drawElementCallback = (rect, index, isActive, isFocused) => {
                ...
                if (port != null) {
                    var pos = rect.position
                            + (port.IsOutput ? new Vector2(rect.width + 6, 0)
                                    : new Vector2(-36, 0));
                    PortField(pos, port);
                }
            };
            ...
            list.onReorderCallback = rl => {
                ...
                if (rl.index > reorderableListIndex)
                    for (var i = reorderableListIndex; i < rl.index; ++i) {
                        ...
                        hasRect = NodeEditorWindow.current.portConnectionPoints.TryGetValue(port,
                            out rect);
                        hasNewRect =
                                NodeEditorWindow.current.portConnectionPoints.TryGetValue(nextPort,
                                    out newRect);
                        NodeEditorWindow.current.portConnectionPoints[port] =
                                hasNewRect ? newRect : rect;
                        NodeEditorWindow.current.portConnectionPoints[nextPort] =
                                hasRect ? rect : newRect;
                    }
                // Move down
                else
                    for (var i = reorderableListIndex; i > rl.index; --i) {
            ...
            list.onRemoveCallback = rl => {
                ...
                if (dynamicPorts[index] == null) {
                    ...
                    Debug.LogWarning("DynamicPorts["
                        + index
                        + "] out of range. Length was "
                        + dynamicPorts.Count
                        + " - Skipped");
                }
                else {
                ...
                if (hasArrayData && arrayData.propertyType != SerializedPropertyType.String) {
                    if (arrayData.arraySize <= index) {
                        Debug.LogWarning("Attempted to remove array index "
                            + index
                            + " where only "
                            + arrayData.arraySize
                            + " exist - Skipped");
...
#region Obsolete
        ...
        public static bool IsInstancePortListPort(NodePort port) => IsDynamicPortListPort(port);

        [Obsolete("Use DynamicPortList instead")]
        public static void InstancePortList(string fieldName, Type type,
            SerializedObject serializedObject, NodePort.IO io,
            Node.ConnectionType connectionType = Node.ConnectionType.Multiple,
            Node.TypeConstraint typeConstraint = Node.TypeConstraint.None,
            Action<ReorderableList> onCreation = null)
        {
            DynamicPortList(false, fieldName, type, serializedObject, io, connectionType,
                typeConstraint,
                onCreation);
        }
#endregion
