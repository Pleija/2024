...
namespace XNodeEditor
{
    public class AdvancedGenericMenu : AdvancedDropdown
    {
        ...
        public AdvancedGenericMenu() : base(new AdvancedDropdownState()) => Name = "";
        ...
        private AdvancedGenericMenuItem FindOrCreateItem(string name,
            AdvancedGenericMenuItem currentRoot = null)
        {
            ...
            if (currentRoot == null) {
                ...
                if (item == null) items.Add(item = new AdvancedGenericMenuItem(paths[0]));
            }
            else {
                item = currentRoot.children.OfType<AdvancedGenericMenuItem>()
        ...
        {
            position.width = Mathf.Clamp(position.width,
                DefaultMinWidth.HasValue ? DefaultMinWidth.Value : 1f,
        ...
        {
            if (item is AdvancedGenericMenuItem gmItem) gmItem.Run();
        }
        ...
        private class AdvancedGenericMenuItem : AdvancedDropdownItem
        {
            ...
            public AdvancedGenericMenuItem(string name, bool enabled, Texture2D icon,
                MenuFunction func) : base(name)
            {
                Set(enabled, icon, func);
            }

            public AdvancedGenericMenuItem(string name, bool enabled, Texture2D icon,
                MenuFunction2 func, object userData) : base(name)
            {
