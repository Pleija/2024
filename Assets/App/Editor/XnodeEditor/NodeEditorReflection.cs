...
namespace XNodeEditor
{
    ...
    public static class NodeEditorReflection
    {
        ...
        [NonSerialized]
        private static Dictionary<Type, bool> nodeNotFoldable;
        ...
        public static Type[] nodeTypes =>
                _nodeTypes != null ? _nodeTypes : _nodeTypes = GetNodeTypes();

        /// <summary>
        ///     Return a delegate used to determine whether window is docked or not. It is faster to
        ///     cache this delegate than run the reflection required each time.
        /// </summary>
        public static Func<bool> GetIsDockedDelegate(this EditorWindow window)
        {
            var fullBinding = BindingFlags.Public
                    | BindingFlags.NonPublic
                    | BindingFlags.Instance
                    | BindingFlags.Static;
            var isDockedMethod = typeof(EditorWindow).GetProperty("docked", fullBinding)
                    .GetGetMethod(true);
            ...
            if (items.Length != 0) {
                ...
                for (var i = 0; i < items.Length; i++) {
                    ...
                    if (invalidatedEntries.Contains(kvp.Key.menuItem))
                        ...
                        contextMenu.AddItem(new GUIContent(kvp.Key.menuItem), false,
                            () => kvp.Value.Invoke(obj, null));
                }
            }
        }
        ...
        public static KeyValuePair<ContextMenu, MethodInfo>[] GetContextMenuMethods(object obj)
        {
            ...
            for (var i = 0; i < methods.Length; i++) {
                var attribs = methods[i]
                        .GetCustomAttributes(typeof(ContextMenu), true)
                        .Select(x => x as ContextMenu)
                        .ToArray();
                if (attribs == null || attribs.Length == 0) continue;
        ...
        public static void OpenPreferences()
        {
            try {
                ...
                //Get sections
                FieldInfo sectionsField =
                        type.GetField("m_Sections", BindingFlags.Instance | BindingFlags.NonPublic);
                ...
                for (int i = 0; i < sections.Count; i++) {
                    GUIContent sectionContent =
                            sectionContentField.GetValue(sections[i]) as GUIContent;
