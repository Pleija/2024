    ...
    public class Client : Singleton<Client>, IAutoCreate, IDontDestroyOnLoad
    {
            ...
            Action<TRequest, TResult> result)
        {
            ...
            self.Hub.Invoke<(ResultStatus status, byte[] data)>("Q", id,
                        XJson.Encrypt(MessagePackSerializer.Serialize(key), data.MD5()), data)
                    .OnSuccess(t => {
        ...
        {
            Call<long, long>(ApiFunc.GetTimestamp, r => DateTimeOffset.UtcNow.ToUnixTimeSeconds(),
                (r, t) => { });
        }
        ...
        public void Connect()
        {
            ...
            GetType()
                    ...
                    .Where(type => typeof(INetService).IsAssignableFrom(type))
                    .SelectMany(type =>
                            type.GetMethods(BindingFlags.Public | BindingFlags.Instance))
                    .ForEach(info => {
                        var instance = instances.TryGetValue(info.DeclaringType!, out var ret)
                                ? ret : instances[info.DeclaringType] =
                                        Activator.CreateInstance(info.DeclaringType);
                        var paramTypes = info.GetParameters().Length > 0
        ...
        private void Hub_OnConnected(HubConnection hub)
        {
            ...
            hub.Send("SendMetadata",
                new Metadata { intData = 123, strData = "meta data", myEnum = MyEnum.One });
            ...
            hub.Invoke<string>("NoParam")
                    .OnSuccess(ret => AddText(string.Format(
                        "'<color=green>NoParam</color>' returned: '<color=yellow>{0}</color>'",
                        ret)))
                    .OnError(error => AddText(string.Format(
                        "'<color=green>NoParam</color>' error: '<color=red>{0}</color>'",
                        error)));
            ...
            hub.GetDownStreamController<int>("ObservableCounter", 10, 1000)
                    ...
                    .OnSuccess(result =>
                            AddText(
                                "'<color=green>ObservableCounter(10, 1000)</color>' OnSuccess."))
                    .OnError(error => AddText(string.Format(
            ...
            controller
                    .OnItem(result =>
                            AddText(string.Format(
                                ...
                                result)))
                    .OnSuccess(result =>
                            AddText("'<color=green>ChannelCounter(10, 1000)</color>' OnSuccess."))
                    .OnError(error => AddText(string.Format(
            ...
            hub.GetDownStreamController<Person>("GetRandomPersons", 20, 2000)
                    ...
                    .OnSuccess(result =>
                            AddText(
                                "'<color=green>GetRandomPersons(20, 1000)</color>' OnSuccess."));
        }
    }
}
