...
namespace UnityRoyale
{
    public class CardManager : MonoBehaviour
    {
        ...
        {
            var newDeckLoaderComp = gameObject.AddComponent<DeckLoader>();
        ...
        private IEnumerator PromoteCardFromDeck(int position, float delay = 0f)
        {
            ...
            backupCardTransform.DOAnchorPos(new Vector2(210f * (position + 1) + 20f, 0f), .2f + .05f * position)
                .SetEase(Ease.OutQuad);
            backupCardTransform.localScale = Vector3.one;

            //store a reference to the Card component in the array
            var cardScript = backupCardTransform.GetComponent<Card>();
        ...
        private void CardDragged(int cardId, Vector2 dragAmount)
        {
            ...
            var planeHit = Physics.Raycast(ray, out hit, Mathf.Infinity, playingFieldMask);
            ...
            if (planeHit) {
                if (!cardIsActive) {
                    ...
                    var dataToSpawn = cards[cardId].cardData.placeablesData;
                    var offsets = cards[cardId].cardData.relativeOffsets;

                    //spawn all the preview Placeables and parent them to the cardPreview
                    for (var i = 0; i < dataToSpawn.Length; i++) {
                        var newPlaceable = Instantiate<GameObject>(dataToSpawn[i].associatedPrefab,
        ...
        private void CardReleased(int cardId)
        {
            ...
            if (Physics.Raycast(ray, out hit, Mathf.Infinity, playingFieldMask)) {
                ...
                cards[cardId].GetComponent<RectTransform>().DOAnchorPos(new Vector2(220f * (cardId + 1), 0f), .2f)
                    .SetEase(Ease.OutQuad);
            }
        ...
        private void ClearPreviewObjects()
        {
            ...
            for (var i = 0; i < previewHolder.transform.childCount; i++)
