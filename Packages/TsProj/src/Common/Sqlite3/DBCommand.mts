...
import SQLiteReader = CS.SqlCipher4Unity3D.SQLiteReader;

/**读取SqliteDataReader中的值 */
export function readValue(reader: SQLiteReader, index: string | number, type: DBType) {
...
/**绑定SqliteCommand参数 */
export function bindParameter(command: SQLiteCommand, value: any): number {
...
export class DBCommand {
    ...
    private finally(command: SQLiteCommand, reader?: SQLiteReader) {
        //command.Cancel();
        //command.Dispose();
        reader?.Close();
        reader?.Dispose();
