...
import { SQL_DEFAULT_SCHEMA, SqlDatabase } from '../core/index.mjs';
...
export class DbCatalogDAO {
    ...
    try {
      ...
      if (
        ...
        const res = await this.sqldb.all(
          `select * from ${schema}.sqlite_master where type='table' and name=:tableName and UPPER(sql) like '%AUTOINCREMENT%'`,
          { ':tableName': tableName },
        );
        if (res && res.length === 1) {
        ...
        if (idx.origin !== 'pk') {
          promises.push(
            new Promise((resolve, reject) => {
              ...
              this.callSchemaQueryPragma(
                'index_xinfo',
                quoteSimpleIdentifier(idx.name),
                quotedSchema,
              )
                .then((xinfo) => {
                  xinfo
        ...
        .forEach((fk) => {
          if (lastId === fk.id) {
            ...
            if (lastFk) {
              ...
              info.foreignKeys[
                FKDefinition.genericForeignKeyId(fromCols, lastFk.table, toCols)
              ] = fkInfo;
            }
            // new fk
            lastId = fk.id;
  ...
  protected callSchemaQueryPragma(
    pragmaName: string,
