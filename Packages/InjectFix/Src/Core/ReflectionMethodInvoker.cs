...
namespace Hotfix.Core
{
    internal class ReflectionMethodInvoker
    {
        ...
        // #lizard forgives
        public unsafe void Invoke(VirtualMachine virtualMachine, ref CallInfo callInfo, bool isInstantiate)
        {
            var managedStack = callInfo.managedStack;
            var pushResult = false;
            var args = new object[paramCount];
            try
            {
                //virtualMachine._Info("method: " + method);
                Value* pArg = callInfo.argumentBase;
                ...
                for (int i = 0; i < paramCount; i++)
                {
                    if (!outFlags[i])
                    {
                        args[i] = EvaluationStackOperation.ToObject(callInfo.evaluationStackBase, pArg, managedStack,
                ...
                else
                {
                    ...
                    if (hasThis)
                    {
                        instance = EvaluationStackOperation.ToObject(callInfo.evaluationStackBase, callInfo.argumentBase,
                ...
                for (int i = 0; i < paramCount; i++)
                {
                    if (refFlags[i])
                    {
                        callInfo.UpdateReference(i + paramStart, args[i], virtualMachine, rawTypes[i]);
                    }
                }
                ...
                if (hasReturn || isInstantiate)
                {
                    ...
                    {
                        callInfo.UpdateReference(0, ret, virtualMachine, method.DeclaringType);
                    }
                    else
                    {
                        callInfo.PushObjectAsResult(ret, returnType);
                        pushResult = true;
                    }
                }
            }
            ...
            finally
            {
                ...
                {
                    managedStack[pArg - callInfo.evaluationStackBase] = null;
                    pArg++;
                }
            }
        }
