...
namespace Hotfix.Core
{
    ...
    public static class PatchManager
    {
        ...
        {
            var redirectTypeName = "Hotfix.RedirectTo." + method.DeclaringType.AssemblyQualifiedName;
        ...
        unsafe static public VirtualMachine Load(Stream stream, bool checkNew = true)
        {
            ...
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                for(int i = 0; i < 100; i++)
                {
                    ...
                    var idMapType = Type.GetType("Hotfix.IDMAP" + i + assemblyStr, false);
                ...
                if (checkNew)
                {
                    ...
                    for (int i = 0; i < newClassCount; i++)
                    {
                        ...
                        {
                            throw new Exception(
                                $"{newClassName} class is expected to be a new class , but it already exists ");
                        }
                    }
                }
