    ...
    public class PerfTest
    {
        ...
        static void Base()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                virtualMachine.Execute(1, ref callInfo, 0);
                CallInfo.End(ref callInfo);
                sw.Stop();
        ...
        static void SafeCall()
        {
            ...
            {
                CallInfo callInfo = CallInfo.Begin();
                callInfo.PushInt32(4);
                callInfo.PushInt32(6);
                virtualMachine.Execute(0, ref callInfo, 2);
                CallInfo.End(ref callInfo);
                callInfo.GetInt32();
            }
        ...
        unsafe static void UnsafeCall()
        {
            ...
            for (int i = 0; i < LOOPS; i++)
            {
                ...
                evaluationStackPointer->Type = Hotfix.Core.ValueType.Integer;

                (evaluationStackPointer + 1)->Value1 = 20;
                (evaluationStackPointer + 1)->Type = Hotfix.Core.ValueType.Integer;
