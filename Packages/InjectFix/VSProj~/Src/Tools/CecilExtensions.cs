    ...
    static internal class CecilExtensions
    {
            ...
            out int firstInstruction)
        {
            ...
            if (instructions.Count > 2 && instructions[1].OpCode.Code == Code.Call)
            {
                ...
                if (callMethod.DeclaringType.FullName == "Hotfix.Core.SwitchFlag" && callMethod.Name == "Get")
                {
                    injectType = InjectType.Switch;
                }
                else if (callMethod.DeclaringType.FullName == "Hotfix.WrappersManagerImpl" && callMethod.Name == "Get")
                {
