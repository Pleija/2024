    ...
    class CodeTranslator
    {
        ...
        void fieldAccessInject(InjectType injectType, MethodDefinition method, int methodId)
        {
            ...
            if (injectType == InjectType.Redirect  
                // todo: mo -> [Patch]的方法全部替换内容
                || injectType == InjectType.Switch && method.CustomAttributes.Any(x => x.AttributeType.Name.Contains("Patch")))
            {
                Console.WriteLine($"Redirect Field: {method.Name}");
                msIls.Clear();
        ...
        void idAccessInject(InjectType injectType, MethodDefinition method, int methodId)
        {
            ...
            if (injectType == InjectType.Redirect
                ...
                || injectType == InjectType.Switch && method.CustomAttributes.Any(x => x.AttributeType.Name.Contains("Patch")))
            {
                Console.WriteLine($"Redirect Method: {method.Name}");
                msIls.Clear();
                body.ExceptionHandlers.Clear();
                body.Variables.Clear();
        ...
        void init(AssemblyDefinition assembly, AssemblyDefinition ilfixAassembly)
        {
            ...
            wrapperType = new TypeDefinition("Hotfix", DYNAMICWRAPPER, Mono.Cecil.TypeAttributes.Class
            ...
            itfBridgeType = new TypeDefinition("Hotfix", INTERFACEBRIDGE, TypeAttributes.Class | TypeAttributes.Public,
            ...
            Utils_TryAdapterToDelegate_Ref = assembly.MainModule.ImportReference(
                ilfixAassembly.MainModule.Types.Single(t => t.FullName == "Hotfix.Core.Utils")
        ...
        void idMapTypeCheck()
        {
            if (idMapType == null || idMapType.Fields.Count >= MAX_ID_MAP_FIELD_COUNT)
            {
                ...
                idMapType = new TypeDefinition("Hotfix", "IDMAP" + idMapList.Count, TypeAttributes.Public | TypeAttributes.Sealed,
                        enumType);
        ...
        {
            wrapperMgrImpl = new TypeDefinition("Hotfix", "WrappersManagerImpl", Mono.Cecil.TypeAttributes.Class
        ...
        const string REDIRECT_NAMESPACE = "Hotfix.RedirectTo";
            ...
            GenerateConfigure configure, ProcessMode mode)
        {
            ...
            var allTypes = (from type in assembly.GetAllType()
                            where type.Namespace != "Hotfix" && !type.IsGeneric() && !(isCompilerGenerated(type) || isNewClass(type))
                            select type).ToList();
                ...
                select method))
            {
               ...
               // Console.WriteLine($"method: {method.Name}");
                if ( configure.TryGetConfigure("Hotfix.InterpretAttribute", method, out flag) || configure.TryGetConfigure("Hotfix.PatchAttribute", method, out flag))
                {
                    Console.WriteLine($"Interpret Redirect: {method.Name}");
                    methodToInjectType[method] = InjectType.Redirect;
                    hasRedirect = true;
                }
                else if(configure.TryGetConfigure("Hotfix.HotfixAttribute", method, out flag))
                {
                    //Console.WriteLine($"switch: {method.Name}");
        ...
        void genCodeForCustomBridge()
        {
            var customBirdgeTypes = (
                ...
                where type.CustomAttributes.Any(ca => ca.AttributeType.FullName == "Hotfix.CustomBridgeAttribute")
                from method in type.Methods
        ...
        public void Serialize(Stream output)
        {
            ...
            {
                writer.Write(Hotfix.Core.Instruction.INSTRUCTION_FORMAT_MAGIC);
                ...
                writer.Write(idMap0Name.Substring("Hotfix.IDMAP0".Length));
                ...
                var newClassTypes = (from type in assembly.GetAllType()
                                where type.Namespace != "Hotfix" && !type.IsGeneric() && isNewClass(type)
                                select type);
