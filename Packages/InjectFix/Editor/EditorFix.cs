...
namespace Hotfix
{
    ...
    public partial class EditorFix
    {
        ...
        public static void CallIFix(List<string> args)
        {
            ...
            var inject_tool_path = "Packages/InjectFix/Tools~/Hotfix.exe";
            ...
            hotfix_injection.StartInfo.Arguments = "--debug --runtime=v4.0.30319 \"" +
                                                   inject_tool_path +
                                                   "\" \""
#else
...
//            target = BuildPipeline.GetBuildTargetGroup(EditorUserBuildSettings.activeBuildTarget);
//            rawSymbols = EditorPrefs.GetString("patching");
//            PlayerSettings.SetScriptingDefineSymbolsForGroup(target, rawSymbols);
            ...
            try {
                IsInjected = true;
                EditorPrefs.SetBool(IS_INJECTING, true);
                InjectAllAssemblys();
            }
            catch (Exception e) {
                ...
                IsInjected = false;
                EditorPrefs.SetBool(IS_INJECTING, false);
            }
...
// #endif
            ...
            Debug.Log($"========== Inject Finished, NEED LoadPatch ==========");
            // EditorApplication.delayCall += LoadPatch;
            AssetDatabase.Refresh();

            //Helpers.TestLoadPatch();
        }
        ...
        public static void GetBackupInfo(out string[] backups, out string[] timestamps)
        {
            ...
            var allBackup = Directory.GetFiles(InjectConfigure.BACKUP_PATH)
                .Where(path => r.Match(path).Success)
            ...
            timestamps = allBackup.Select(path => DateTime.ParseExact(r.Match(path).Groups[1].Captures[0].Value,
                        InjectConfigure.TIMESTAMP_FORMAT, System.Globalization.CultureInfo.InvariantCulture)
        ...
        //注入的目标文件夹
        public static string targetAssembliesFolder = "";
        ...
        public static void InjectAssembly(string assembly)
        {
            var configure = Configure.GetConfigureByTags(new List<string>() {
                "Hotfix.HotfixAttribute",
                "Hotfix.PatchAttribute",
                "Hotfix.InterpretAttribute",
                "Hotfix.ReverseWrapperAttribute",
            });
            ...
            using (BinaryWriter writer = new BinaryWriter(new FileStream(processCfgPath, FileMode.Create,
                ...
                foreach (var kv in configure) {
                    ...
                    foreach (var cfgItem in typeList) {
                        ...
                        if (filters.Count > 0 && kv.Key == "Hotfix.HotfixAttribute") {
            ...
            if (hasSomethingToDo) {
                var core_path = InjectConfigure.CoreDllPath;
                ...
                var patch_path = string.Format(InjectConfigure.ResourcePath + "/{0}.ill.bytes", assembly);
...
//            var ts = DateTime.Now.ToString(TIMESTAMP_FORMAT);
//            EditorPrefs.SetString("last_patch", "");
//            doBackup("");
//            Debug.Log($"Backup: {ts}");

            foreach (var assembly in injectAssemblys) {
                InjectAssembly(assembly);
                var assemblyPath = $"./Library/{GetScriptAssembliesFolder()}/{assembly}.dll";
                if (File.Exists(assemblyPath)) {
                    EditorPrefs.SetString(Application.productName + Path.GetFileName(assemblyPath),
                        CheckMD5(assemblyPath));
                }

                else if (EditorPrefs.HasKey(Application.productName + Path.GetFileName(assemblyPath))) {
                    EditorPrefs.DeleteKey(Application.productName + Path.GetFileName(assemblyPath));
                }
            }

            doBackup("injected", true);

            AssetDatabase.Refresh();
        }

        static bool usePlayerAssembly => false;

        private static string GetScriptAssembliesFolder()
        {
            var assembliesFolder = "PlayerScriptAssemblies";
            if (!usePlayerAssembly || !Directory.Exists(string.Format("./Library/{0}/", assembliesFolder))) {
       ...
       public static string[] injectAssemblys {
            get {
                return InjectConfigure.injectTypes.Select(x => x.Assembly.GetName().Name).Distinct().ToArray();
        ...
        static void doBackup(string ts, bool force = false)
        {
            ts = ts.TrimStart('/').TrimEnd('/') + (string.IsNullOrEmpty(ts) ? "" : "/");

            if (!Directory.Exists(InjectConfigure.BACKUP_PATH + "/" + ts)) {
                Directory.CreateDirectory(InjectConfigure.BACKUP_PATH + "/" + ts);
            }

            Directory.GetFiles(InjectConfigure.BACKUP_PATH + "/" + ts).ToList().ForEach(File.Delete);

            var coreDll = InjectConfigure.CoreDllPath;

            //AssetDatabase.FindAssets("IFix.Core.dll").Select(AssetDatabase.GUIDToAssetPath).FirstOrDefault()+"";
            if (File.Exists(coreDll)) {
                File.Copy(coreDll, $"{InjectConfigure.BACKUP_PATH}/{ts}Hotfix.Core.dll", true);
            }
            ...
            foreach (var assembly in injectAssemblys) {
                ...
                if (!File.Exists(string.Format("{0}/{1}{2}.dll", InjectConfigure.BACKUP_PATH, ts, assembly)) || force)
                    File.Copy(dllFile, string.Format("{0}/{1}{2}.dll", InjectConfigure.BACKUP_PATH, ts, assembly),
                        true);

                var mdbFile = string.Format("{0}{1}.mdb", scriptAssembliesDir, assembly);
                if (File.Exists(mdbFile)) {
                    if (!File.Exists(string.Format("{0}/{1}{2}.mdb", InjectConfigure.BACKUP_PATH, ts, assembly)) ||
                        force)
                        File.Copy(mdbFile, string.Format("{0}/{1}{2}.mdb", InjectConfigure.BACKUP_PATH, ts, assembly),
                            true);
                }

                var pdbFile = string.Format("{0}{1}.pdb", scriptAssembliesDir, assembly);
                if (File.Exists(pdbFile)) {
                    if (!File.Exists(string.Format("{0}/{1}{2}.pdb", InjectConfigure.BACKUP_PATH, ts, assembly)) ||
                        force)
                        File.Copy(pdbFile, string.Format("{0}/{1}{2}.pdb", InjectConfigure.BACKUP_PATH, ts, assembly),
                            true);
                }
            }
        }
        ...
        static void doRestore(string ts)
        {
            ...
            foreach (var assembly in injectAssemblys) {
                var dllFile = string.Format("{0}/{1}{2}.dll", InjectConfigure.BACKUP_PATH, ts, assembly);
                if (!File.Exists(dllFile)) {
                    continue;
                }

                string tTarget;
                File.Copy(dllFile, tTarget = string.Format("{0}{1}.dll", scriptAssembliesDir, assembly), true);
                UnityEngine.Debug.Log("Revert to: " + dllFile + " => " + tTarget);

                var mdbFile = string.Format("{0}/{1}{2}.mdb", InjectConfigure.BACKUP_PATH, ts, assembly);
                if (File.Exists(mdbFile)) {
                    File.Copy(mdbFile, tTarget = string.Format("{0}{1}.mdb", scriptAssembliesDir, assembly), true);
                    UnityEngine.Debug.Log("Revert to: " + mdbFile + " => " + tTarget);
                }

                var pdbFile = string.Format("{0}/{1}{2}.pdb", InjectConfigure.BACKUP_PATH, ts, assembly);
                if (File.Exists(pdbFile)) {
                    File.Copy(pdbFile, tTarget = string.Format("{0}{1}.pdb", scriptAssembliesDir, assembly), true);
                    UnityEngine.Debug.Log("Revert to: " + pdbFile + " => " + tTarget);
                }
            ...
            if (type.IsByRef && type.GetElementType().IsGenericType) {
                ...
                if (type.IsGenericTypeDefinition) {
                    ...
                    return Regex.Replace(type.ToString().Replace('+', '/'), @"(`\d).+", "$1") +
                           "<" +
                           string.Join(",", type.GetGenericArguments().Select(t => GetCecilTypeName(t)).ToArray()) +
                           ">";
                }
            }
            else {
        ...
        static BuildTargetGroup target;
        static string rawSymbols;
        ...
        private static string getCompileArguments(Platform platform, string output)
        {
            ...
            if (!compileTemplates.TryGetValue(platform, out compileTemplate)) {
                ...
                if (!File.Exists(path)) {
                    ...
                    if (!File.Exists(path)) {
                        throw new InvalidOperationException("please put template file for " +
                                                            platform +
...
#endif
                        var assemblyPath =
                            assembly.ManifestModule.FullyQualifiedName.Replace(Path.DirectorySeparatorChar, '/');
        ...
        //生成特定平台的patch
        public static void GenPlatformPatch(Platform platform, string patchOutputDir, string corePath = "")
        {
            if (string.IsNullOrEmpty(corePath)) corePath = InjectConfigure.CoreDllPath;
            ...
            foreach (var assembly in injectAssemblys) {
                GenPatch(assembly, string.Format("{0}/{1}.dll", outputDir, assembly),
                    ...
                    //"./Assets/Plugins/IFix.Core.dll"
                    InjectConfigure.CoreDllPath, string.Format("{0}{1}.patch.bytes", patchOutputDir, assembly));
            }
#else
        ...
        public static void GenPatch(string assembly,
            string assemblyCSharpPath = "./Library/ScriptAssemblies/Assembly-CSharp.dll", string corePath =
                ...
                "", string patchPath = "Assembly-CSharp.patch.bytes")
        {
            ...
            Debug.Log(
                $"{assembly} patches: {string.Join(", ", patchMethods.Select(x => x.DeclaringType.Name + "." + x.Name))}");
            ...
            var processCfgPath = "./Logs/process_cfg.log";
            if (!Directory.Exists(Path.GetDirectoryName(processCfgPath))) {
                Directory.CreateDirectory(Path.GetDirectoryName(processCfgPath) + "");
            }

            if (File.Exists(processCfgPath)) {
                File.Delete(processCfgPath);
            }
            ...
            Debug.Log($"args({args.Count}):\n" + Encoding.UTF8.GetString(File.ReadAllBytes(processCfgPath)));
            CallIFix(args);
...
//            AssetDatabase.Refresh();
        ...
        public static void RecompileUnityEditor()
        {
            ...
            // Utility.GetBuildTargetGroup(EditorUserBuildSettings.activeBuildTarget);
        ...
        public static void Patch_Old()
        {
//            target = BuildPipeline.GetBuildTargetGroup(EditorUserBuildSettings.activeBuildTarget);
//
//            // Utility.GetBuildTargetGroup(EditorUserBuildSettings.activeBuildTarget);
//            rawSymbols = PlayerSettings.GetScriptingDefineSymbolsForGroup(target);
//
//            var list = rawSymbols.Split(';').ToList();
//            if (!list.Contains("PATCHING")) {
//                EditorPrefs.SetString("patching", rawSymbols);
//                PlayerSettings.SetScriptingDefineSymbolsForGroup(target, rawSymbols + ";PATCHING");
//            }
//            else {
//                list.Remove("PATCHING");
//                rawSymbols = string.Join(";", list);
//                EditorPrefs.SetString("patching", rawSymbols);
//                PlayerSettings.SetScriptingDefineSymbolsForGroup(target, rawSymbols);
//                UnityEngine.Debug.Log("Clear Pathing");
//            }
            ...
            var ts = EditorPrefs.GetString("last_patch", "");
...
// todo: OnAssemblyReload
        static void OnAssemblyReload()
        {
            EditorApplication.delayCall -= OnAssemblyReload;
            Debug.Log("Assembly Reloaded");

            InjectAssemblys();

//            if (EditorPrefs.GetBool(PATCHED, false)) {
//                Helpers.TestLoadPatch();
//            }
        }
        ...
        const string PATCHED = "IFIX_PATCHED";
        const string IS_INJECTING = "IS_INJECTING";
...
//        [MenuItem(k_Main + "/Patch", false, 1)]
        ...
        {
            // #if PATCHING
            if (EditorPrefs.GetBool(IS_INJECTING, false)) return;

//             if (false &&
//                 injectAssemblys.All(assembly => {
//                     var assemblyPath = $"./Library/{GetScriptAssembliesFolder()}/{assembly}.dll";
//                     return EditorPrefs.GetString(Application.productName + Path.GetFileName(assemblyPath)) ==
//                            CheckMD5(assemblyPath);
//                 })) {
//                 Debug.Log("Injected, cannot patch");
//
...
//             }
            ...
            EditorPrefs.SetBool(PATCHED, false);

            //AssemblyReloadEvents.afterAssemblyReload -= OnAfterAssemblyReload;
            // EditorApplication.delayCall -= OnAfterAssemblyReload;
            IsPatchOK = true;
            Debug.Log("++++++++++++++ PATHING +++++++++++++++");
            Directory.GetFiles(InjectConfigure.ResourcePath).ToList().ForEach(File.Delete);
            AssetDatabase.Refresh();

            EditorUtility.DisplayProgressBar("Generate Patch for Editor", "patching...", 0);
            try {
                ...
                foreach (var assembly in injectAssemblys) {
                    ...
                    // if (EditorPrefs.GetString(Application.productName + Path.GetFileName(assembly_path)) ==
                    //     CheckMD5(assembly_path)) {
                    //     continue;
                    // }

                    UnityEngine.Debug.Log($"patching {assembly} => File Exists: {File.Exists(assembly_path)} ...");
                    GenPatch(assembly, assembly_path,

                        //"./Assets/Plugins/IFix.Core.dll",
                        InjectConfigure.CoreDllPath,
                        string.Format(InjectConfigure.ResourcePath + "/{0}.patch.bytes", assembly));


                }
#if UNITY_EDITOR
                var patches = Directory.GetFiles(InjectConfigure.ResourcePath, "*.patch.bytes");
                ...
                patches.ForEach(find => {
                    var asset = AssetDatabase.LoadAssetAtPath<TextAsset>(find);
                    var guid = AssetDatabase.AssetPathToGUID(find);
                    var entry = settings.CreateOrMoveEntry(guid, group);

                    entry.labels.Add(label);
                    entry.address = Path.GetFileName(find);
            ...
            if (IsPatchOK) {
                doBackup("origin", true);
                EditorPrefs.SetBool(PATCHED, true);
                Debug.Log($"=========== Patch Finished, NEED INJECT & LOAD ============");
                //EditorApplication.delayCall += OnAssemblyReload;
                // AssemblyReloadEvents.afterAssemblyReload -= OnAssemblyReload;
                // AssemblyReloadEvents.afterAssemblyReload += OnAssemblyReload;
                // AssetDatabase.Refresh();
