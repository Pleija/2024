...
namespace Hotfix.LiveDotNet
{
    //输入三个信息：
    //  1、平台（ios、android）
    //  2、手机的ip地址
    //  3、端口信息，端口信息要和PatchReceiver配置对应上
    public class LiveHotfix : EditorWindow
    {
        ...
        //[MenuItem(IFixEditor.k_Menu+ "/LiveDotNet")]
        public static void OpenWindow()
        {
            LiveHotfix window = GetWindow<LiveHotfix>();
        ...
        void doPatch()
        {
            EditorFix.Platform platform = platformIndex == 0 ? EditorFix.Platform.ios : EditorFix.Platform.android;
            string patchPath = "Temp/Assembly-CSharp.patch.bytes";
            EditorFix.GenPlatformPatch(platform, "Temp/");
