...
namespace SlnMerge.Unity
{
    ...
    public class SolutionFileProcessor : AssetPostprocessor
    {
        ...
        static SolutionFileProcessor()
        {
            ...
            var typeProjectFilesGenerator = Type.GetType(
                "SyntaxTree.VisualStudio.Unity.Bridge.ProjectFilesGenerator, SyntaxTree.VisualStudio.Unity.Bridge");

            if(typeProjectFilesGenerator != null) {
                _hasVsForUnity = true;
                var typeFileGenerationHandler = Type.GetType(
                    "SyntaxTree.VisualStudio.Unity.Bridge.FileGenerationHandler, SyntaxTree.VisualStudio.Unity.Bridge");
                var fieldSolutionFileGeneration =
                    typeProjectFilesGenerator.GetField("SolutionFileGeneration");
                var fieldSolutionFileGenerationDelegate =
                    (Delegate)fieldSolutionFileGeneration.GetValue(null);
                var d = Delegate.CreateDelegate(typeFileGenerationHandler,
                    typeof(SolutionFileProcessor), "Merge");

                if(fieldSolutionFileGenerationDelegate == null) {
                    fieldSolutionFileGeneration.SetValue(null, d);
                }
                else {
                    fieldSolutionFileGeneration.SetValue(null,
                        Delegate.Combine(fieldSolutionFileGenerationDelegate, d));
                }
            }
        }
        ...
        private static bool IsUnityVsIntegrationEnabled {
            get {
                ...
                var t = typeof(EditorApplication).Assembly.GetType(
                    "UnityEditor.VisualStudioIntegration.UnityVSSupport");
                if(t == null) return false;
                var methodShouldUnityVSBeActive = t.GetMethod("ShouldUnityVSBeActive",
        ...
        {
            return IsUnityVsIntegrationEnabled ? content /* Visual Studio with VSTU */
                : Merge(path, content);                  /* other editors (Rider, VSCode ...) */
        }

        private static string Merge(string path, string content)
        {
            if(SlnMerge.TryMerge(path, content, SlnMergeUnityLogger.Instance,
                   out var solutionContent)) {
...
namespace SlnMerge
{
    ...
    public class SlnMergeSettings
    {
        ...
        public static SlnMergeSettings FromFile(string path)
        {
            using(var stream = File.OpenRead(path)) {
                return (SlnMergeSettings)new XmlSerializer(typeof(SlnMergeSettings)).Deserialize(
                    stream);
            }
        }
    }
    ...
    public static class SlnMerge
    {
        ...
        public static bool TryMerge(string solutionFilePath, ISlnMergeLogger logger,
            out string resultSolutionContent)
        {
            return TryMerge(solutionFilePath, File.ReadAllText(solutionFilePath), logger,
                out resultSolutionContent);
        }

        public static bool TryMerge(string solutionFilePath, string solutionFileContent,
            ISlnMergeLogger logger, out string resultSolutionContent)
        {
            try {
                // Load SlnMerge settings from .mergesttings
                var slnFileDirectory = Path.GetDirectoryName(solutionFilePath) + "/Packages";
                var slnMergeSettings = new SlnMergeSettings();
                var slnMergeSettingsPath = Path.Combine(slnFileDirectory,
                    Path.GetFileName(solutionFilePath) + ".mergesettings");

                if(!File.Exists(slnMergeSettingsPath)) {
                    var tmp = Directory.GetFiles(slnFileDirectory, "*.mergesettings")
                        .FirstOrDefault();

                    if(!string.IsNullOrEmpty(tmp)) {
                        new FileInfo(tmp).MoveTo(slnMergeSettingsPath);
                    }
                }
                ...
                var overlaySolutionFilePath = Path.Combine(slnFileDirectory,
                    Path.GetFileNameWithoutExtension(solutionFilePath) + ".Merge.sln");

                if(!string.IsNullOrEmpty(slnMergeSettings.MergeTargetSolution)) {
                    overlaySolutionFilePath = NormalizePath(Path.Combine(slnFileDirectory,
                        slnMergeSettings.MergeTargetSolution));
                }

                if(!File.Exists(overlaySolutionFilePath)) {
                    logger.Warn(
                        $"Cannot load the solution file to merge. skipped: {overlaySolutionFilePath}");
                ...
                var mergedSolutionFile = Merge(solutionFile, overlaySolutionFile, slnMergeSettings,
                    logger);
        ...
        private class SlnMergeMergeContext
        {
            ...
            public SlnMergeMergeContext(SolutionFile solutionFile, SolutionFile overlaySolutionFile,
                SolutionFile mergedSolutionFile, SlnMergeSettings settings, ISlnMergeLogger logger)
            {
        ...
        public static SolutionFile Merge(SolutionFile solutionFile,
            SolutionFile overlaySolutionFile, SlnMergeSettings settings, ISlnMergeLogger logger)
        {
            logger.Debug(
                $"Merge solution: Base={solutionFile.Path}; Overlay={overlaySolutionFile.Path}");
            var ctx = new SlnMergeMergeContext(solutionFile, overlaySolutionFile,
                solutionFile.Clone(), settings, logger);
            MergeProjects(ctx);
        ...
        private static void MergeProjects(SlnMergeMergeContext ctx)
        {
            foreach(var project in ctx.OverlaySolutionFile.Projects) {
                if(!ctx.MergedSolutionFile.Projects.ContainsKey(project.Key)) {
                    ...
                    if(ctx.MergedSolutionFile.Projects.Any(x =>
                           x.Value.Name == project.Value.Name)) {
                        ctx.Logger.Debug(
                            $"Project Conflict: Overlay={project.Value.Name} [{project.Key}; ProjectConflictResolution={ctx.Settings.ProjectConflictResolution}\nMergedSolutionFile.Projects:\n{string.Join("\n", ctx.MergedSolutionFile.Projects.Select(x => $"{x.Value.Name} [{x.Key}]"))}");

                        if(ctx.Settings.ProjectConflictResolution ==
                           ProjectConflictResolution.PreserveUnity) {
                            // Ignore Overlay's project
                            continue;
                        }
                        else if(ctx.Settings.ProjectConflictResolution ==
                                ProjectConflictResolution.PreserveOverlay) {
                            // Remove Unity generated project
                            var duplicatedProject =
                                ctx.MergedSolutionFile.Projects.First(x =>
                                    x.Value.Name == project.Value.Name);
                            ctx.MergedSolutionFile.Projects.Remove(duplicatedProject.Key);
                            ctx.Logger.Debug(
                                $"Remove duplicated project '{duplicatedProject.Value.Name} [{duplicatedProject.Key}; Path={duplicatedProject.Value.Path}]' (PreserveOverlay)");
                        }
                        else {
                    ...
                    if(project.Value.IsFolder) {
                        ...
                        if(project.Value.Sections.TryGetValue(("SolutionItems", "preProject"),
                               out var solutionItems)) {
                            foreach(var solutionItem in solutionItems.Values.ToArray()) {
                                solutionItems.Values.Remove(solutionItem.Key);
                                var solutionItemPathAbsolute =
                                    NormalizePath(Path.Combine(
                                        Path.GetDirectoryName(ctx.OverlaySolutionFile.Path),
                                        solutionItem.Key));
                                var solutionItemPathRelative =
                                    MakeRelative(ctx.MergedSolutionFile.Path,
                                        solutionItemPathAbsolute);
                                solutionItems.Values[solutionItemPathRelative] =
                                    solutionItemPathRelative;
                            }
                        }
                    }
                    else {
                        var overlayProjectPathAbsolute = NormalizePath(
                            Path.Combine(Path.GetDirectoryName(ctx.OverlaySolutionFile.Path),
                                project.Value.Path));
                        project.Value.Path = MakeRelative(ctx.MergedSolutionFile.Path,
                            overlayProjectPathAbsolute);
                    }
                    ctx.MergedSolutionFile.Projects.Add(project.Key, project.Value);
                }
                else {
                    // A overlay project already exists. Deduplication if needed.
                    var sameNameProjects = ctx.MergedSolutionFile.Projects
                        .Where(x => x.Value.Name == project.Value.Name).ToArray();

                    if(sameNameProjects.Length > 1) {
                        ctx.Logger.Debug(
                            $"Duplicated project was detected: Overlay={project.Value.Name} [{project.Key}; ProjectConflictResolution={ctx.Settings.ProjectConflictResolution}\nProjects:\n{string.Join("\n", sameNameProjects.Select(x => $"{x.Value.Name} [{x.Key}]"))}");
                        ...
                        if(removeKey != null) {
                            ...
                            ctx.Logger.Debug(
                                $"Remove duplicated project '{duplicatedProject.Name} [{duplicatedProject.Guid}; Path={duplicatedProject.Path}]'");
                        }
                    }
                }
            }
        ...
        private static void MergeGlobalSections(SlnMergeMergeContext ctx)
        {
            foreach(var sectionKeyValue in ctx.OverlaySolutionFile.Global.Sections) {
                if(ctx.MergedSolutionFile.Global.Sections.TryGetValue(sectionKeyValue.Key,
                    ...
                    ctx.MergedSolutionFile.Global.Sections.Add(sectionKeyValue.Key,
                        sectionKeyValue.Value);
                }
            }
        }
        ...
        {
            if(ctx.Settings.NestedProjects == null ||
               ctx.Settings.NestedProjects.Length == 0) return;
            ...
            if(!ctx.MergedSolutionFile.Global.Sections.TryGetValue(
                   ("NestedProjects", "preSolution"), out var section)) {
                section = new SolutionGlobalSection(ctx.MergedSolutionFile.Global, "NestedProjects",
                    "preSolution");
                ctx.MergedSolutionFile.Global.Sections.Add((section.Category, section.Value),
                    section);
            }

            // Prepare to add nested projects.
            var definedSolutionFolders =
            ...
            foreach(var nestedProject in ctx.Settings.NestedProjects) {
                ...
                if(string.IsNullOrEmpty(nestedProject.ProjectName)) {
                    ...
                    var nestedProjectNamePattern = new Regex("^" +
                        Regex.Escape(nestedProject.ProjectName).Replace(@"\*", ".*")
                            .Replace(@"\?", ".") + "$");
                    nestedProjectGuids = ctx.MergedSolutionFile.Projects.Values
                        .Where(x => nestedProjectNamePattern.IsMatch(x.Name)).Select(x => x.Guid);
                }
                ...
                if(string.IsNullOrEmpty(nestedProject.FolderPath)) {
                    ...
                    if(solutionTree.TryGetValue(nestedProject.FolderPath, out var folderNode)) {
                        if(!folderNode.IsFolder) {
                            throw new Exception(
                                $"Path '{nestedProject.FolderPath}' is not a Solution Folder.");
                        }
                        ...
                        for(var i = 0; i < pathParts.Length; i++) {
                            ...
                            if(solutionTree.TryGetValue(path, out var folderNode2)) {
                                ...
                                if(!definedSolutionFolders.TryGetValue(path, out var guid)) {
                                    throw new Exception(
                                        $"Path '{path}' doesn't exist in the Solution or Solution Folder definitions. To create a new folder, you need to define SolutionFolder in .mergesettings.");
                                }
                                var newFolder = new SolutionProject(ctx.MergedSolutionFile,
                                    typeGuid: GuidProjectTypeFolder, guid: guid.ToUpper(),
                                    name: pathParts[i], path: pathParts[i]);
                                ...
                                if(!string.IsNullOrEmpty(parentPath)) {
                                    section.Values[newFolder.Guid] =
                                        solutionTree[parentPath].Project.Guid;
                                }
                ...
                if(!nestedProjectGuids.Any()) {
                    if(ctx.Settings.ProjectMergeBehavior.HasFlag(ProjectMergeBehavior
                           .ErrorIfProjectOrFolderDoesNotExist)) {
                        throw new Exception(
                            $"Project '{nestedProject.ProjectName}' does not exists in the solution.");
                    }
                ...
                if(nestedProjectFolderGuid == null) {
                    if(ctx.Settings.ProjectMergeBehavior.HasFlag(ProjectMergeBehavior
                           .ErrorIfProjectOrFolderDoesNotExist)) {
                        throw new Exception(
                            $"Solution Folder '{nestedProject.FolderGuid}' (GUID) does not exists in the solution.");
                    }
                    ctx.Logger.Warn(
                        $"Solution Folder '{nestedProject.FolderGuid}' (GUID) does not exists in the solution.");
                    continue;
                ...
                foreach(var nestedProjectGuid in nestedProjectGuids) {
                    if(!ctx.MergedSolutionFile.Projects.ContainsKey(nestedProjectGuid)) {
                        if(ctx.Settings.ProjectMergeBehavior.HasFlag(ProjectMergeBehavior
                               .ErrorIfProjectOrFolderDoesNotExist)) {
                            throw new Exception(
                                $"Project '{nestedProject.FolderGuid}' (GUID) does not exists in the solution.");
                        }
                        ctx.Logger.Warn(
                            $"Project '{nestedProject.FolderGuid}' (GUID) does not exists in the solution.");
                        continue;
        ...
        private static Dictionary<string, SolutionTreeNode> BuildSolutionFlatTree(
            SolutionFile solutionFile)
        {
            var projectByPath =
                new Dictionary<string, SolutionTreeNode>(StringComparer.OrdinalIgnoreCase);
            var projectsByGuid =
                new Dictionary<string, SolutionTreeNode>(StringComparer.OrdinalIgnoreCase);
            ...
            if(solutionFile.Global.Sections.TryGetValue(("NestedProjects", "preSolution"),
                   out var section)) {
                foreach(var keyValue in section.Values) {
                    ...
                    if(!projectsByGuid.ContainsKey(projectGuid)) {
                        ...
                        projectsByGuid[projectGuid] =
                            new SolutionTreeNode(solutionFile.Projects[projectGuid]);
                    }

                    if(!projectsByGuid.ContainsKey(parentProjectGuid)) {
                        projectsByGuid[parentProjectGuid] =
                            new SolutionTreeNode(solutionFile.Projects[parentProjectGuid]);
                    }
        ...
        [DebuggerDisplay(
            "{nameof(SolutionTreeNode)}: {Path,nq}; IsFolder={IsFolder}; Children={Children.Count}")]
    ...
    {
        public Dictionary<string, SolutionProject> Projects { get; } =
                ...
                switch(parsedLine.Type) {
                    case SlnDocLineType.ProjectBegin: {
                        if(!(current is SolutionFile))
                            throw new InvalidOperationException(
                                "Project must be located under Solution");
                    ...
                    case SlnDocLineType.ProjectSectionBegin: {
                        if(!(current is SolutionProject))
                            throw new InvalidOperationException(
                                "ProjectSection must be located under Project");
                    ...
                    case SlnDocLineType.GlobalBegin: {
                        if(!(current is SolutionFile))
                            throw new InvalidOperationException(
                                "Global must be located under Solution");
                        var sln = current as SolutionFile;
                    ...
                    case SlnDocLineType.GlobalSectionBegin: {
                        if(!(current is SolutionGlobal))
                            throw new InvalidOperationException(
                                "GlobalSection must be located under Global");
                        var global = current as SolutionGlobal;
                        var globalSection = new SolutionGlobalSection(current, parsedLine);

                        if(global.Sections.ContainsKey(
                               (globalSection.Category, globalSection.Value))) {
                            // already exists
                            continue;
                        }
                        global.Sections.Add((globalSection.Category, globalSection.Value),
                            globalSection);
        ...
        where TSection : SolutionSection
    {
        ...
        public Dictionary<(string Category, string Value), TSection> Sections { get; } =
            new Dictionary<(string Category, string Value), TSection>();
    ...
    public abstract class SolutionSection : SolutionDocumentNode
    {
        ...
        public SolutionSection(SolutionDocumentNode parent, string category, string value) :
            base(parent)
        {
            Category = category;
            Value = value;
    ...
    public class SolutionDocumentTrivialNode : SolutionDocumentNode
    {
        ...
        public SolutionDocumentTrivialNode(SolutionDocumentNode parent, SolutionDocLine line) :
            base(parent)
        {
            Line = line;
        }
    ...
    [DebuggerDisplay(
        "{nameof(SolutionProject),nq}: TypeGuid={TypeGuid,nq}; Name={Name,nq}; Path={Path,nq}; Guid={Guid,nq}")]
    public class SolutionProject : SolutionSectionContainer<SolutionProjectSection>
    {
        ...
        public bool IsFolder => string.Compare(TypeGuid, SlnMerge.GuidProjectTypeFolder,
        ...
        public SolutionProject(SolutionDocumentNode parent, string typeGuid, string name,
            string path, string guid) : base(parent)
        {
            TypeGuid = typeGuid;
            Name = name;
            Path = path;
    ...
    [DebuggerDisplay(
        "{nameof(SolutionProjectSection),nq}: Category={Category,nq}; Value={Value,nq}; Values={Values.Count,nq}")]
    public class SolutionProjectSection : SolutionSection
    {
        public SolutionProjectSection(SolutionDocumentNode parent, SolutionDocLine line) : base(
            parent, line) { }

        public SolutionProjectSection(SolutionDocumentNode parent, string category, string value) :
            base(parent, category, value) { }
    ...
    public class SolutionGlobal : SolutionSectionContainer<SolutionGlobalSection>
    {
        ...
        public override SolutionDocumentNode Clone(SolutionDocumentNode newParent)
        {
            ...
            foreach(var keyValue in Sections) {
                newNode.Sections.Add(keyValue.Key,
                    (SolutionGlobalSection)keyValue.Value.Clone(newNode));
            }
            return newNode;
        }
    }
    ...
    [DebuggerDisplay(
        "{nameof(SolutionGlobalSection),nq}: Category={Category,nq}; Value={Value,nq}; Values={Values.Count,nq}")]
    public class SolutionGlobalSection : SolutionSection
    {
        public SolutionGlobalSection(SolutionDocumentNode parent, SolutionDocLine line) : base(
            parent, line) { }

        public SolutionGlobalSection(SolutionDocumentNode parent, string category, string value) :
            base(parent, category, value) { }
