...
#region Json
    static readonly Byte[] bytes = {
        0x54, 0x86, 0x5B, 0x40, 0xF2, 0x0B, 0x62, 0x44, 0x93, 0xE6, 0xF7, 0x7B, 0x29, 0x0F, 0x24,
        0x9D,
    };
    ...
    {
        var ret = JsonUtility.FromJson<T>(json.FromBase64());
        return ret;
    }

    public static string ToJson(object value, bool pretty = false)
    {
        var ret = JsonUtility.ToJson(value, pretty).ToBase64();
        return ret;
    }

    public static object FromJson(string json, System.Type type)
    {
        var ret = JsonUtility.FromJson(json.FromBase64(), type);
        return ret;
    }
#endregion
    ...
    public static byte[] DecryptBase64String(object data, byte[] key = null) =>
        Decrypt($"{data}".IsBase64() ? Convert.FromBase64String( $"{data}") : utf8.GetBytes($"{data}") , key ?? bytes);

    public static byte[] DecryptBase64String(object data, string key = null) => Decrypt(
        $"{data}".IsBase64() ? Convert.FromBase64String( $"{data}") : utf8.GetBytes($"{data}"), !string.IsNullOrEmpty(key) ? utf8.GetBytes(key) : bytes);
