        ...
        public class FileExporter
        {
                ...
                bool csharpModuleWillGen = false)
            {
                ...
                using(var jsEnv = new JsEnv(loader)) {
                    ...
                    using(StreamWriter textWriter =
                          new StreamWriter(saveTo + "Typing/csharp/index.d.ts", false,
                              Encoding.UTF8)) {
            ...
            public static void ExportWrapper(string saveTo, ILoader loader = null)
            {
                ...
                using(var jsEnv = new JsEnv(loader)) {
                    ...
                    Dictionary<Type, Wrapper.StaticWrapperInfo> wrapperInfoMap =
                        new Dictionary<Type, Wrapper.StaticWrapperInfo>();

                    foreach(var type in genTypes) {
                        if(type.IsEnum || type.IsArray || (Generator.Utils.IsDelegate(type) &&
                               type != typeof(Delegate))) continue;
                        Wrapper.StaticWrapperInfo staticWrapperInfo =
                            Wrapper.StaticWrapperInfo.FromType(type, genTypes);
